...
test("fifty independent writers form one parent chain", async () => {
  ...
  for (let index = 0; index < branches.length; index += 1) {
    const result = await branches[index].publish();
    assert.equal(result.outcome, "merged");
    assert.equal(result.parentRevision, previous);
    previous = result.revision;
    await branches[index].close();
  }
  for (let index = 0; index < 50; index += 1)
    assert.equal(
...
test("fifty same-inode writers yield one merge and 49 explicit conflicts", async () => {
  ...
  const outcomes = [];
  for (let index = 0; index < branches.length; index += 1)
    outcomes.push(await branches[index].publish({ operationId: `same-op-${index}` }));
  ...
  assert.equal(await filesystem.readFile("/shared", { encoding: "utf8" }), "writer-0");
  for (const branch of branches) await branch.close();
  await filesystem.close();
  database.close();
});
