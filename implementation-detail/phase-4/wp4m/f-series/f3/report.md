...
## F3-D1 prospective causal diagnostic — frozen before source edits or Cargo

Date: 2026-08-20. This additive diagnostic reopening does not relabel, replace,
or modify any F3-v1/v2/v3/v3-r1 artifact or their retained `FAIL / revert`
dispositions. It withdraws only the unsupported universal inference that the
three R64/1-MiB classification experiments exhausted every in-scope SQLite
transport/cap design. Accepted F2-v3 remains the live implementation and F4
remains ineligible in every D1 outcome.

### Entry custody and diagnostic-only authority
...
starting HEAD / tree
  244851dfe60d247646546f9d811c42b4368b9e6f
  f2a98f2a87322a72dbbc1310c3fb7834d2e55d32

accepted live F2-v3 source
  c8ac86be3a97bbcc6b980e93bc7539532e2093c0e6fe741429ef4a26cb3cc158

accepted F2-v3 control executable
  68b599b819da9f05c76d35efd807c5d5f03266dfb7d4ed0cc78da269c4b891c0

corrected immutable F3-v3 source / executable
  69ee5a615b143cb81c1227b747cf601370e096d482cfe0eb907e7f5903fd65ad
  1828b55df0e9778cf3c9e40af6c13a4f6afc8ed59827600974ba20653a01bc18
...
F3-v3-r1 raw / manifest / final audit
  e840a62e68c34c879f25c810bd262f9157a1c9a02fe6f6472324a3f660fe8ce1
  0c71375e2daee6e4ac5bc8b44d9b583b88a9bdbc48e38be8e4eaf12da1d95c54
  49b40eb1ab31eb85251fda6dc75b23e3df4da0d7f30617a2fc6fbbedec06bca9
```

D1 is evidence gathering under the systematic-debugging root-cause phase. It
cannot retain an implementation, qualify F4, amend statement atomicity, select
a cap/policy, or serve as an acceptance campaign. Its terminal result is one
of:

```text
GO       directly measured budget permits one separately preregistered F3-v4
NO-GO    insufficient budget; retain F2-v3 and keep F3/F4 closed
REVISE   diagnostic/custody defect; preserve root and create a new D1-vN
```

### Hypothesis and program-level objective

The historical F3 hypothesis optimized the wrong proxy. It reduced INSERT
statement step/reset executions `5,372 -> 103`, but retained 5,372 logical
row/index insertions, 21,488 INSERT parameter binds, 10,744 INSERT BLOB binds,
and 105,463,458 requested BLOB bytes. D1 tests whether directly removable
native-prepare, VDBE, transient-bind, or statement-subjournal cost is large
enough to overcome the measured batching overhead.

Statement count is a mechanism counter, never the performance objective. The
retained program objectives are:

```text
accepted F2-v3 mapping / durable       492.777 / 659.593 ms
formal -5% floors                      468.138 / 626.613 ms
preferred strategic targets            <=443 / <=610 ms
program stretch                         <=500 ms durable = >=200 MiB/s
```

Full create remains `Theta(B+N)`. With fixed group cap `G`, `Theta(N/G)` is
still `Theta(N)`; classifications, binds, B-tree rows, canonical bytes, and
durable storage work stay linear. Fixed caps retain asymptotic `O(1)` insertion
memory while their constants remain independently measured.

### New immutable D1 root and neutral base

The only D1-v1 artifact root is:

```text
target/wp4m-f3-causal-diagnostic-k64-r64-b1048576-20260820-v1
```

Neither D1 arm prepares a base. Before any child invocation, verify the
accepted F2-v3 artifact manifest and physical-byte-copy its sealed pair-0
empty triple into `neutral-master/`, then copy that master into every pair
base. No hardlink or symlink is allowed.

```text
source master
  target/wp4m-f2-construction-proof-k64-20260819-v3/bases/pair-0.sqlite

database       20,480 bytes
  2ad4a91935ab2d392706bda18e82cad74e27d874039d31c93b4517139afc3db5
authority          32 bytes
  a9bc50612cf653ad964a7b9e579e538834f03a58be66e1ce860db1125cee53c5
expectations       800 bytes
  9776a76cdbba70206e38ba5697da5b12b92f408b26baaddcbbd3a5311f6458dc
```

The master must have 0 object rows, 1 meta row, 0 visible-head rows, page
size/count `4096/5`, schema version 5, DELETE, synchronous 2, temp_store 1,
and mmap_size 0. All six normal pair bases are completed and made read-only
before the first timed child. Each arm receives a distinct writable physical
byte copy with exact SHA-256, `cmp`, apparent-byte, and allocated-byte
preflight.

### Four frozen diagnostic builds

A same-binary selector would require reintroducing the F2 builder/evidence
path into the corrected F3 source and would add a larger causal variable than
matched instrumentation. D1 therefore freezes four separately hashed release
binaries, all using the same Cargo dependency graph and system SQLite:

```text
A-light   accepted F2 per-object transport + light observations
B-light   corrected F3-v3 R64/1-MiB transport + light observations
A-detail  accepted F2 per-object transport + detailed observations
B-detail  corrected F3-v3 R64/1-MiB transport + detailed observations
```

The light build preserves the original high-level rusqlite execute/query
transport. It adds only two mapping-boundary `proc_pid_rusage` snapshots and
checked aggregate bind/byte counters. Only light phase wall, instructions,
cycles, RSS, and footprint are comparable between A/B.

The detail build uses public `raw_bind_parameter` plus `raw_execute`/
`raw_query` so bind and step/reset are separable; direct equivalence tests are
mandatory. It adds hot-path clocks, statement enumeration/status, sampled
statement memory, and optional VFS observation. Detail top-level wall/CPU is
diagnostic-only because observer count is 5,372 for F2 versus 103 for F3.

No dependency, schema/table/sidecar, production module, public framework,
worker, async path, pool, durable metadata, static binding, cap change, or
statement-policy change is permitted. The diagnostic source copies, exact
diffs, build environment, and all four binaries are frozen into D1 custody.
The live source is restored to accepted F2 bytes before terminal disposition.

### Runtime SQLite and statement observations

Capture from each diagnostic executable—not `/usr/bin/sqlite3`—the runtime
`sqlite3_libversion`, `sqlite3_sourceid`, ordered compile options and digest,
runtime page/journal/synchronous/temp/mmap values, plus `otool -L`. A direct
dylib SHA is `Unavailable` when the install name is supplied through the dyld
shared cache.

Detail mode records fixed-cap arrays, never an event log or map:

```text
optimistic/retry/prequery/postquery arity histograms [u64;65]
prepare_cached acquisitions
cache hits / misses / initial native prepares
authorizer object-INSERT compile events
automatic reprepares / statement runs / VM steps
statement MEMUSED before-bind / after-bind / after-step / after-clear maxima
DBSTATUS_STMT_USED mapping start / sampled maximum / mapping end
parameter attempts/successes and integer/BLOB counts
ObjectId / canonical / incumbent-query BLOB bytes
```

Cache classification is the checked live-statement-count delta surrounding
`prepare_cached`: `+1` is a miss/native prepare and `0` is a hit on this
synchronous connection. `Run` is a cross-check, not the classifier, because
a cached never-stepped VM can still have `Run==0`. Install the existing
authorizer hook immediately after connection creation and never toggle it;
object-table INSERT compile events must equal cold acquisitions plus automatic
reprepares on successful rows.

Detail-mode fresh exact predictions are:

```text
F2 histogram {1:5372}; acquisitions/misses/hits = 5372/1/5371
F3 histogram {2:1,27:1,49:3,50:9,51:15,52:16,53:15,
               54:21,55:14,56:6,57:2}; acquisitions/misses/hits = 103/11/92
both: initial rows 5372; INSERT binds 21488; BLOB binds 10744;
      ObjectId bytes 171904; canonical bytes 105291554;
      requested BLOB bytes 105463458; automatic reprepares expected 0
```

Light mode deliberately emits empty arity histograms and zero cache/native-
prepare/status/VM/MEMUSED/DB-STMT/timer observations. Its aggregate bind
attempt/success/type/byte counters remain exact, and its crossing equation is
derived from the existing core SQL/group counters. An empty light histogram
must not make `bind_equation_matches` false: the light equation derives
incumbent-query rows from incumbent ObjectId bytes divided by 32 and insert
rows from the remaining BLOB attempts divided by two, then checks every
parameter/type/success/ObjectId-byte total.

### Non-overlapping timer and process-resource contract

The detail builds share this timer vocabulary:

```text
canonical/descriptor validation
bounded duplicate scan
SQL construction
cache probe + prepare
parameter binding
step + reset (inseparable through public rusqlite)
incumbent authentication
result classification
evidence construction
evidence consumption
clear/drop/cache return
observer-only instrumentation
unattributed residual
```

Parent timers are store group flush, complete group pipeline, and mapping.
Parents are never added to children. The two source shapes have different
pre-existing authority boundaries and therefore prove two explicit equations:

```text
A-detail per-object insertion:
  validation + cache/prepare + bind + step/reset + incumbent authentication
  + cleanup/cache-return + observer + residual = insertion total
  evidence construction is separately timed after that insertion parent

B-detail grouped insertion:
  canonical validation + duplicate scan + SQL construction + cache/prepare
  + bind + step/reset + incumbent authentication + result classification
  + evidence construction + evidence consumption + cleanup/cache-return
  + observer + residual = complete group pipeline

both residuals >= 0
```

The B-detail store-group-flush parent is reported but nested inside the complete
pipeline and is never added as a leaf. The final workspace/transition
two-object group is included explicitly. VFS callback wall is nested in
step/reset and cannot be added again.

`proc_pid_rusage(RUSAGE_INFO_V4)` snapshots bracket the exact mapping timer and
report checked user/system-time, instructions, cycles, logical writes, pageins,
and disk-byte deltas plus resident/footprint endpoints and lifetime/interval
maxima. `/usr/bin/time -l` remains independent whole-child evidence; endpoint
differences are never called high-water values.

When MEMSTATUS is enabled, both arms reset the three SQLite global-memory
high-water epochs immediately before the final pre-mapping rusage snapshot and
emit `high_water_epoch="mapping_reset_at_start"`. Mapping-end values are
therefore comparable interval highs, not process-lifetime highs. Normal light,
detail, VFS, and M4.5 rows must report MEMSTATUS disabled with zero memory
counters; only the separate memory rows may report them observed.

## F3-D1-v1 terminal REVISE and F3-D1-v2 serialization repair preregistration

This section was written after D1-v1 stopped and before any D1-v2 source edit,
Cargo command, executable build, or row.

D1-v1 is terminal `REVISE` and remains immutable. Two runner preflight defects
occurred before any row: a zsh special-parameter collision and confusion of the
SQLite schema cookie (`3`) with the LayerFS metadata schema value (`5`). Both
failed invocations and both pre-row freeze chains are retained. After those
repairs, exactly one row started:

```text
root                  target/wp4m-f3-causal-diagnostic-k64-r64-b1048576-20260820-v1
row                   pair0 warmup A-light
child exit            0
child semantic status PASS (not admitted because serialization was invalid)
wrapper parse status  invalid
started/returned/raw  1 / 1 / 1
stdout SHA-256        6d269dee8258c1977a3e4f9ee47aec3f248a60ab347e8458f5bbffb85c57a834
starts SHA-256        00a34638159761b5e8dca627a2090adad35e0009305e2eaa780a932686a1fb77
returns SHA-256       cabc13e441b120441a106d3fca142b5e5a6073908e62c27f276598e7685e38f5
raw SHA-256           0e64446df971ad2ec0aa7b3ee74fd8e5a039c6903438c232493f48a950bf7047
database SHA-256      f3a6bdf2b013fa768748b5e90f8bdc1892e21af3d4ebf769c0c386eba2afc011
```

`jq` and Python independently reject the 35,779-byte child stdout with one
unmatched final `}` at byte/column 35,778. The source appends two root-closing
braces after the nested `f3_d1` object where exactly one is required. The
private `JsonObject::parse` helper also accepted trailing object bytes, which
is why the existing row test did not catch the malformed serialization.

D1-v2 changes exactly this diagnostic publication boundary:

1. remove the one surplus final brace in both F2-D1 and F3-D1 row renderers;
2. make the private structural JSON parser reject any byte after the one root
   object; and
3. strengthen the existing complete-row test so it proves strict full-input
   parsing and rejects an appended `}`.

No SQL, bind, object, grouping, cap, timer, counter, rusage, memory, VFS,
authority, proof, transaction, COMMIT, storage, fixture, schedule, gate, or
Amdahl rule changes. D1-v2 uses a fresh artifact root:

```text
target/wp4m-f3-causal-diagnostic-k64-r64-b1048576-20260820-v2
```

All four executables are rebuilt once because both source variants change.
The exact 34-row AB/BA diagnostic and four-row M4.5 schedules remain frozen.
Any D1-v2 row failure again closes that campaign root; no selective resume or
replacement is allowed.

Pre-row runner audit also found that the copied M4.5-only script still used
zsh's reserved associative `commands` parameter, although the full-create
runner had already renamed it. D1-v2 preregisters the same mechanical rename to
`commands_file` in the M4.5 runner before any v2 row. It changes no command,
schedule, input, measurement, counter, or gate.

### D1-v2 post-row analyzer repair registration

All 34 full-create and four M4.5 rows completed before analysis. The two frozen
analyzers were then run unchanged and their `REVISE` outputs retained. This
section precedes analyzer repair; raw rows, sources, executables, schedules,
thresholds, gates, and Amdahl bands are immutable.

The Python analyzer had two report implementation defects:

1. it labeled its report `F3-D1-v1` while resolving the v2 artifact root; and
2. it expected `construction_proof_consumptions=1` in the mapping phase even
   though the frozen phase boundary correctly records `0` there and `1` in the
   immediately following pre-COMMIT construction-proof phase. Overall rows and
   all 34 raw phase arrays agree on that exact attribution.

The independent Ruby analyzer defined `median` and `percent_change` as module
functions but called them as unqualified `Analyzer` instance methods. Its
synthetic test exercised module scope and therefore missed the production-scope
`NoMethodError`.

Create, without overwriting the frozen analyzers:

```text
analyze_f3_d1_v2.py
analyze_f3_d1_independent_v2.rb
```

The Python revision changes only the milestone label and the phase-local proof-
consumption expectation, adding an explicit pre-COMMIT-phase expectation of
one. The Ruby revision adds private instance delegators to the already-tested
module functions. No predicate, threshold, operand, measurement, or decision
rule changes. Both revisions must pass synthetic tests, analyze the immutable
raw rows independently, and then receive a read-only diff audit.

### Named diagnostic VFS and global-memory probes

There is no supported status/PRAGMA/trace substitute for anonymous statement-
subjournal opens and I/O. B-detail/A-detail may therefore open their diagnostic
connections through one private named proxy around the actual runtime default
VFS. It is registered nondefault and process-lifetime-owned. Only
`SUBJOURNAL`, `TEMP_DB`, `TEMP_JOURNAL`, and `TRANSIENT_DB` file handles are
proxied; main DB/journal/WAL handles retain their real method tables.

Fixed atomic counters record attempted/successful/error opens, reads/writes/
syncs/truncates/closes, requested bytes, callback wall, maximum requested
extent, delete-on-close flags, and overflow. Bytes are VFS logical requests,
not physical-media bytes. Absence of `SQLITE_OPEN_SUBJOURNAL` means only no
VFS-backed spill; it does not prove no in-memory statement journal.

Before a retained 100-MiB VFS row, a separate small smoke process must prove
proxy forwarding, first/middle/last rollback, integrity/no residue, at least
one subjournal open/write under prospective spill `0`, and no VFS subjournal
open under spill `-1`. Spill configuration occurs before the first SQLite
operation in separate children. No VFS row is acceptance wall evidence.

The normal build reports SQLite global memory as
`U_MEMSTATUS_DISABLED` because `DEFAULT_MEMSTATUS=0`. Separate nonperformance
children enable `SQLITE_CONFIG_MEMSTATUS` before SQLite initialization and
emit `sqlite3_status64` current/high-water values. Their wall/CPU/RSS/pager
values never enter the performance or Amdahl decision.

### Frozen schedules and append-only row custody

Normal light comparison:
...
Detail comparison uses the same complete 12-row schedule. VFS observation is
a separate four-pair diagnostic sequence `AB/AB/BA/AB` (pair0 warmup, pairs
1-3 measured). Global-memory probe is one A/B pair and is untimed for
performance interpretation. The smallest release M4.5 regression is run only
after full-create D1 rows and is independently parsed from the corrected F3
detail executable.

Before every child, append its immutable identity to `f3-d1.starts.jsonl`.
After every return, append a wrapper row to the mode-specific raw JSONL even
for nonzero exit or malformed stdout; retain stdout/stderr/time records. Prove
`planned == started == returned == raw`. No resume, row replacement, selective
rerun, or output deletion exists. A failed child/parser closes that root
`REVISE`.

### Raw-derived analyzers, storage, and sealing

The Python primary and independent Ruby analyzer read raw/preflight/M4.5
directly; neither reads the other's summary. Every gate emits its predicate,
actual operands, named source rows/files, and `PASS|FAIL|UNAVAILABLE`. Literal
semantic/Q/storage/M4.5 PASS strings are forbidden. Both recompute schedule,
start/return/raw reconciliation, hashes, identities, roots, proof/counter/Q/
timer/statement/VFS equations, pager/storage/residue, M4.5, resources, and the
Amdahl budget. Analysis is staged outside the root and copied once before
terminal manifesting.

After all payloads exist: set data/source/report `0444`, executable scripts and
binaries `0555`, subdirectories `0555`; generate a manifest last that excludes
only itself and records every path/byte/SHA/mode while rejecting symlinks and
unlisted files; set manifest/root `0444`/`0555`; then run a verifier that writes
only an external attestation. Record manifest and attestation hashes in tracked
documents. Owner permissions are recoverable; the tracked hashes are the
custody anchor.

### Frozen Amdahl decision and failure rules

For every pair, compute formal recovery needed from candidate B as the maximum
of `B_mapping - 0.95*A_mapping` and `B_durable - 0.95*A_durable`. For each
possible F3-v4 mechanism, subtract unavoidable replacement work, observer-tax
upper bounds, and safety/cleanup cost from one non-overlapping directly
measured candidate component. Do not add parent/child timers, step/VFS nested
time, bind/memcpy overlap, or process/phase CPU.

```text
median credible removable cost <60 ms
  NO-GO; retain F2 and close F3

60-70 ms
  insufficient margin; NO-GO for an acceptance candidate

70-80 ms
  conditional GO only with direct causality, >=4/5 threshold headroom,
  and clear memory/resource margin

>=80-85 ms
  preferred GO threshold for one strategically useful F3-v4

>=100 ms
  supports the upper 15%-mapping / 10%-durable target
```

Statement-journal policy budget must subtract the measured cost of 103 bounded
prequeries and 5,372 additional ID binds. Static-binding budget cannot exceed
measured binding time minus unavoidable FFI/binding/cleanup cost. VFS events
provide causal attribution but their nested wall is never double-counted.

D1-v1 may be repaired freely before the first release binary or started row.
After a binary is frozen, any source or instrumentation change creates a new
binary with explicit custody. After any D1 row starts, an orchestration/source/
binary defect terminally closes this root `REVISE`; a new D1-vN is required.
If D1 returns GO, seal and close it before writing a separate F3-v4
preregistration or source change. Only one policy or one cap may enter F3-v4;
policy, cap, static binding, fixed SQL, and scan structure cannot be stacked.

## F3-D1-v2 terminal result — VALID / NO-GO

The complete v2 campaign and both independently repaired analyzers are closed.
D1-v1 remains sealed `REVISE`; its historical artifacts were not changed.

```text
full-create started / returned / raw   34 / 34 / 34
M4.5 started / returned / raw           4 / 4 / 4
child/JSON failures                      0 / 0
primary analysis                         valid / NO-GO / exit 0
independent analysis                     VALID / NO-GO / exit 0
```

Five measured light pairs, excluding pair 0 warmup:

| Metric | F2 A | grouped F3 B | Arm | Paired | B wins |
|---|---:|---:|---:|---:|---:|
| mapping/CAS | 530.074166 ms | 528.057708 ms | -0.380411% | -1.004541% | 3/5 |
| durable capture | 712.157750 ms | 717.935708 ms | +0.811331% | +1.407266% | 2/5 |
| throughput | 140.418327 MiB/s | 139.288238 MiB/s | regression | — | — |
| RSS | 93,552,640 B | 98,369,536 B | +5.148862% | +5.370889% | 0/5 |
| peak footprint | 92,324,296 B | 97,092,040 B | +5.164127% | +5.371336% | 0/5 |

The exact 5,372-to-103 insertion-execution reduction is proven, but parameter
binds, BLOB bytes, and logical B-tree insertions remain unchanged. Detail
evidence records `1/11` inferred cold prepares, `118,184/103,098` VM steps,
post-bind statement MEMUSED `52,176/1,256,000` bytes, bind wall
`3.032563/4.388413` ms, and step/reset wall `92.198066/102.612911` ms.

The diagnostic VFS directly observes one candidate statement-subjournal spill
per measured B row: 3,946 writes, 8,216,400 requested bytes, and 16.127082 ms
median nested callback wall; A records zero. SQLite global-memory high-water
rises 1,457,584 bytes. Logical Q is 55,325 versus 1,147,173 bytes and terminates
at zero in every row.

Both arms retain exact identities, 5,372 created objects, 105,291,554 canonical
new bytes, 365,262 mapping bytes, one transaction/COMMIT, 26,676 dirty writes,
109,264,896 pager-write bytes, 6,675 spills, 109,273,088 allocated-store bytes,
and no final journal/WAL/SHM. All four M4.5 rows pass with grouping counters
zero. The read-only audit covers 38 databases: one schema hash, all integrity
`ok`, and no residual journal/WAL/SHM.

The primary's largest non-overlapping zero-cost gross observation is the
16.127082 ms VFS callback wall, below the frozen 60 ms no-go threshold before
mandatory replacement work. The independent analyzer declines to call hidden
SQLite CPU mathematically bounded by that value but still finds zero credible
net removable budget because replacement prequeries/binds/safety work is not
measured. Both reach the same `NO-GO`.

Terminal F3 disposition:

```text
FAIL / revert
retain accepted F2-v3 source and executable
do not build F3-v4
F4 ineligible
```

The supported limitation is scoped: the three immutable F3 campaigns at
R64/B1MiB failed, and the subsequent causal diagnostic did not justify any of
the prospectively listed policy/cap/static-bind/native-prepare revisions. It is
not a universal claim about SQLite, other caps, other hosts, or future storage
engines.

Primary evidence:
`target/wp4m-f3-causal-diagnostic-k64-r64-b1048576-20260820-v2/analysis/primary-analysis-v2.json`.
Independent evidence:
`target/wp4m-f3-causal-diagnostic-k64-r64-b1048576-20260820-v2/analysis/independent-analysis-v2.json`.
Full terminal report:
`target/wp4m-f3-causal-diagnostic-k64-r64-b1048576-20260820-v2/D1-V2-FINAL-REPORT.md`.

### Terminal manifest, sealing, and attestation

The first terminal inventory failed closed because its generator emitted
literal `\t` text instead of tab delimiters. It is preserved byte-for-byte as
`d1-v2-terminal-manifest.tsv`, SHA-256
`c928298449bc8b956cae98d3bdade0bb376ba898a05454cc2e456b464d240b5e`,
and is not accepted as terminal custody.

The separately versioned corrected inventory is
`d1-v2-terminal-manifest-r2.tsv`, SHA-256
`f70dd3c87fcecab22fa2af8e5d6bc48cad06bf478581733ca25cfe9c66a9b905`.
It records mode, byte count, SHA-256, and relative path for 405 payloads. A
fresh external walk verifies 405/405 with zero missing/mismatched and zero
listed/actual differences. The sealed root contains 406 files including the
manifest and 18 directories; files are `0444` or `0555`, directories/root are
`0555`, and no owner-writable entry or symlink remains.

The external attestation is
`target/wp4m-f3-causal-diagnostic-k64-r64-b1048576-20260820-v2.attestation.txt`,
SHA-256
`84dc10435fdeefcc6ec4823c86f6d604a412e7c5db3036d364bcd36298fb3a61`.
The sealed final read-only audit SHA-256 is
`4404e052f5d34c454832aa98d0d64077d20d93d03a5c3b0303efe8f92a6fb561`.
Accepted F2-v3 remains live at source hash
`c8ac86be3a97bbcc6b980e93bc7539532e2093c0e6fe741429ef4a26cb3cc158`;
there is no live benchmark-source diff, no F3-v4, no F4 work, and no commit.
