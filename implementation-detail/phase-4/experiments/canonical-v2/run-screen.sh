...
repo=${0:A:h}/../../../..
runner_path=${0:A}
artifact=${repo}/target/phase4-canonical-v2-exploration-20260821-v1
out=${artifact}/screen-results-v1
control=${artifact}/control/phase4_create_edit_benchmark-cp0009
candidate=${artifact}/candidate/phase4_create_edit_benchmark-canonical-v2
control_source=${artifact}/control/phase4_create_edit_benchmark-cp0009.rs
candidate_source=${artifact}/candidate/phase4_create_edit_benchmark-canonical-v2.rs
candidate_codec=${artifact}/candidate/canonical_v2.rs
fixture=${repo}/target/wp4m-f2-construction-proof-k64-20260819-v3/S1-100.source
manifest=${artifact}/PRE-SCREEN-MANIFEST.tsv
lock=${artifact}/CANONICAL_V2_SCREEN.lock
size=104857600
...
row.update({
    ...
    "screen_schema": "canonical-v2-private-screen-row-v1", "screen_arm": arm,
...
export BENCHMARK_LOCK=CANONICAL_V2_SCREEN
...
fields = lambda lines: dict(line.split("=", 1) for line in lines[1:-1]
                            if "=" in line and not line.startswith("range="))
af, bf = fields(a), fields(b)
assert af["source_length"] == bf["source_length"]
assert af["source_fingerprint"] == bf["source_fingerprint"]
assert af["file"].split(",")[:2] == bf["file"].split(",")[:2]
assert af["result"] != bf["result"]
PY
