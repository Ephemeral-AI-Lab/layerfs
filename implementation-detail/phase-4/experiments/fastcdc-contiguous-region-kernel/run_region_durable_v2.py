...
CONTROL = TARGET_ROOT / "operands-v1/phase4_create_edit_benchmark-canonical-v2-control"
...
CORRECTION = HERE / "DURABLE-ORCHESTRATION-CORRECTION-v2.md"
LOCK = TARGET_ROOT / "durable-v2.lock"
ORDERS = ["AB", "BA", "AB", "BA"]
...
def verify_methodology():
    ...
    correction = os.environ.get("FASTCDC_REGION_V2_DURABLE_CORRECTION_SHA256")
    if not correction or sha256(CORRECTION) != correction:
        raise RuntimeError("durable orchestration correction custody mismatch")
...
def main():
    ...
    if (RESULTS / "durable-v1/DURABLE-RAW-v1.jsonl").exists():
        raise RuntimeError("first orchestration attempt unexpectedly contains measured rows")
