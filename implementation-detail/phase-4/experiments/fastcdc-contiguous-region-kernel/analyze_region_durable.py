...
def semantic_row_ok(row):
    ...
    return (
        ...
        and row.get("source_bytes_read") == 104_857_600
        and row.get("source_cdc_bytes_read") == 104_857_600
        and row.get("canonical_stage_source_bytes_read") == 104_857_600
...
def analyze(root):
    ...
    temporal_center_control = average(controls, "sequence") if controls else float("nan")
    temporal_center_candidate = average(candidates, "sequence") if candidates else float("nan")
    temporal_ok = temporal_center_control == temporal_center_candidate
    ...
    disposition = "FASTCDC CONTIGUOUS REGION KERNEL PASS / RETAIN" if performance_ok else "FASTCDC CONTIGUOUS REGION KERNEL NO-GO / REVERT"
    return {
        ...
        "temporal_center_control": temporal_center_control,
        "temporal_center_candidate": temporal_center_candidate,
        "temporal_balance_pass": temporal_ok,
...
def write(root, result):
    ...
    lines = ["# Conditional FastCDC contiguous-region durable A/B v2", "", f"Disposition: **{result['disposition']}**", "",
