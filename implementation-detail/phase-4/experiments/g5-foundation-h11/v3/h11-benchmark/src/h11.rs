...
fn h11_sample(
    ...
    let mut store = Store::open_measured_with_phases(
        &database,
        SELECTED_PROFILE,
        &mut reopen_metrics,
        &mut open_phases,
    ...
    macro_rules! write_report {
        ($output:expr, $whole_q:expr) => {{
            ...
            write!(
                output,
                "{{\"status\":\"PASS\",\"schema\":\"phase4-g5-h11-sample-v3\",\"history_revisions\":{history_count},\"sample\":{sample},\"source_bytes\":{SOURCE_1},\"source_blake3\":\"{source_hash}\",\"profile\":\"{profile}\",\"cache_size_pages\":{H11_CACHE_PAGES},\"history_edit_samples_ns\":["
            )?;
    ...
    println!(
        "{{\"schema\":\"phase4-g5-h11-q-terminal-v3\",\"status\":\"PASS\",\"q_current\":0,\"q_high_water\":{reported_q}}}"
    );
    Ok(())
}
