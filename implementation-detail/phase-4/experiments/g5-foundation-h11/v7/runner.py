...
BINARY_SAMPLE_SCHEMA = "phase4-g5-h11-sample-v7"
BINARY_Q_SCHEMA = "phase4-g5-h11-q-terminal-v7"
...
METHOD_MANIFEST = HERE / "method/METHOD-MANIFEST-v7.tsv"
SOURCE_FREEZE = HERE / "method/SOURCE-FREEZE-v7.json"
PRIMARY = HERE / "analyzers/primary.py"
INDEPENDENT = HERE / "analyzers/independent.py"
GATE_RESULT = REPO / "target/phase4-g5-foundation-h11-20260823-v7"
SCREEN_RESULT = REPO / "target/phase4-g5-foundation-h11-20260823-v7-screen"
...
def preflight(result):
    ...
    return {
        ...
        "operation_log_treatment": "v1/v2 custody-only diagnostic; omitted from v7 method authority",
...
def release_lock(lock, result, terminal_verification=None, state="release"):
    ...
    try:
        ...
        release = {
            "schema": "phase4-g5-h11-lock-release-v7",
        ...
        write_json(result / "LOCK-RELEASE-v7.json", release)
        return release
    finally:
        os.close(lock["fd"])
        lock["fd"] = None
...
def run_child(result, history, sample, ordinal):
    ...
    if (
        len(lines) != 2
        or lines[0].get("schema") != BINARY_SAMPLE_SCHEMA
        or lines[1].get("schema") != BINARY_Q_SCHEMA
    ):
        raise RuntimeError(f"sample/terminal schema mismatch: h{history}s{sample}")
    row, marker = lines
...
def terminal_cleanup(result):
    ...
    return {
        "schema": "phase4-g5-h11-cleanup-v7",
...
def run(mode):
    ...
    if mode == "--dry-run":
        print(compact({"schema": "phase4-g5-h11-dry-run-v7", "status": "PASS", "measured_rows": 0, **frozen}))
        return 0
    ...
    try:
        ...
        if mode == "--gate":
            outputs = []
            for analyzer, name in ((PRIMARY, "PRIMARY-ANALYSIS-v7.json"), (INDEPENDENT, "INDEPENDENT-RECOMPUTATION-v7.json")):
                output = result / name
                completed = subprocess.run([sys.executable, str(analyzer), str(result / "RAW-v7.jsonl"), str(EXPECTED), str(output)], cwd=REPO, text=True, capture_output=True)
                if output.exists():
            ...
            write_json(result / "ANALYZER-AGREEMENT-v7.json", {"schema": "phase4-g5-h11-analyzer-agreement-v7", "status": "PASS" if agreement else "REVISE", "exact_normalized_agreement": agreement})
        ...
        write_json(result / "PREFLIGHT-v7.json", frozen)
        write_json(result / "COMMANDS-v7.json", {"schema": "phase4-g5-h11-commands-v7", "commands": commands})
        write_json(result / "ENVIRONMENT-v7.json", {"schema": "phase4-g5-h11-environment-v7", "python": platform.python_version(), "platform": platform.platform(), "machine": platform.machine(), "rustc": subprocess.check_output(["rustc", "--version"], text=True).strip(), "sqlite": subprocess.check_output(["sqlite3", "--version"], text=True).strip(), "physical_io_bytes": "Unavailable", "controlled_cold": "Unavailable"})
        cleanup = terminal_cleanup(result)
        write_json(result / "CLEANUP-v7.json", cleanup)
        if cleanup["status"] != "PASS":
            raise RuntimeError("cleanup failed")
        payload_count, payload_hash = manifest(
            result,
            "PAYLOAD-MANIFEST-v7.tsv",
            {"PAYLOAD-MANIFEST-v7.tsv", "MEASURED-TERMINAL-v7.json", "TERMINAL-VERIFICATION-v7.json", "FINAL-ARTIFACT-HASHES-v7.tsv", "FINAL-READONLY-VERIFICATION-v7.json", "LOCK-RELEASE-v7.json", "BENCHMARK-LOCK-RELEASE-ATTESTATION-v7.json"},
        )
        status = "PASS"
        terminal = result / "MEASURED-TERMINAL-v7.json"
        write_json(terminal, {"schema": "phase4-g5-h11-terminal-v7", "status": status, "disposition": "H11_PASS_G5_C_GATE_READY" if mode == "--gate" else "H11_SCREEN_PASS", "mode": mode[2:], "rows": len(raw_rows), "payload_files": payload_count, "payload_manifest_sha256": payload_hash, "elapsed_before_terminal_verification_ns": time.monotonic_ns() - started})
        if verify_manifest(result, result / "PAYLOAD-MANIFEST-v7.tsv") != payload_count:
            raise RuntimeError("payload verification count mismatch")
        terminal_verification = result / "TERMINAL-VERIFICATION-v7.json"
        write_json(terminal_verification, {"schema": "phase4-g5-h11-terminal-verification-v7", "status": "PASS", "terminal_sha256": sha256(terminal), "payload_manifest_sha256": payload_hash, "payload_files_verified": payload_count, "lock_owned_through_terminal_verification": verify_owned_lock(lock), "complete_wall_limit_ns": LIMIT_NS})
        complete_ns = time.monotonic_ns() - started
        if complete_ns >= LIMIT_NS:
            raise RuntimeError("complete wall exceeded 20 seconds")
        write_json(result / "COMPLETE-WALL-v7.json", {"schema": "phase4-g5-h11-complete-wall-v7", "status": "PASS", "from": "fail-fast lock acquisition", "through": "terminal verification fsync", "complete_wall_ns": complete_ns, "limit_ns": LIMIT_NS})
        ...
        final_count, final_hash = manifest(result, "FINAL-ARTIFACT-HASHES-v7.tsv", {"FINAL-ARTIFACT-HASHES-v7.tsv", "FINAL-READONLY-VERIFICATION-v7.json"})
        verified = verify_manifest(result, result / "FINAL-ARTIFACT-HASHES-v7.tsv")
        write_json(result / "FINAL-READONLY-VERIFICATION-v7.json", {"schema": "phase4-g5-h11-final-readonly-verification-v7", "status": "PASS", "files_verified": verified, "manifest_files": final_count, "final_artifact_hashes_sha256": final_hash, "lock_absent": not LOCK.exists(), "result_directory_fsynced": True})
        fsync_dir(result)
        print(compact({"status": "PASS", "mode": mode[2:], "result": str(result), "complete_wall_ns": complete_ns, "final_files": final_count}))
        return 0
    except Exception as error:
        if result.exists():
            failed = result / "FAILED-v7.json"
            if not failed.exists():
                write_json(failed, {"schema": "phase4-g5-h11-failure-v7", "status": "REVISE", "error": str(error), "elapsed_ns": time.monotonic_ns() - started})
        raise
    finally:
