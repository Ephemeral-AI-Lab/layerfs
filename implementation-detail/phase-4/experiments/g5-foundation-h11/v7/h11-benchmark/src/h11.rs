...
fn h11_sample(
    ...
    macro_rules! write_report {
        ($output:expr, $whole_q:expr) => {{
            ...
            write!(
                output,
                "{{\"status\":\"PASS\",\"schema\":\"phase4-g5-h11-sample-v7\",\"history_revisions\":{history_count},\"sample\":{sample},\"source_bytes\":{SOURCE_1},\"source_blake3\":\"{source_hash}\",\"profile\":\"{profile}\",\"cache_size_pages\":{H11_CACHE_PAGES},\"history_edit_samples_ns\":["
            )?;
    ...
    println!(
        "{{\"schema\":\"phase4-g5-h11-q-terminal-v7\",\"status\":\"PASS\",\"q_current\":0,\"q_high_water\":{reported_q}}}"
    );
    Ok(())
}
...
mod h11_v7_tests {
    ...
    fn preparation_and_historical_operation_are_charged() {
        let source = test_path("h11-v7-edit-point.source");
