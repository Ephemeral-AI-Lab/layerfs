...
fn h11_sample(
    ...
    macro_rules! write_report {
        ($output:expr, $whole_q:expr) => {{
            ...
            write!(
                output,
                "{{\"status\":\"PASS\",\"schema\":\"phase4-g5-h11-sample-v8\",\"history_revisions\":{history_count},\"sample\":{sample},\"source_bytes\":{SOURCE_1},\"source_blake3\":\"{source_hash}\",\"profile\":\"{profile}\",\"cache_size_pages\":{H11_CACHE_PAGES},\"history_edit_samples_ns\":["
            )?;
    ...
    println!(
        "{{\"schema\":\"phase4-g5-h11-q-terminal-v8\",\"status\":\"PASS\",\"q_current\":0,\"q_high_water\":{reported_q}}}"
    );
    Ok(())
}
...
mod h11_v8_tests {
    ...
    fn test_source() -> (TestRoot, PathBuf) {
        ...
        let nonce = SystemTime::now()
            ...
            .expect("clock")
            .as_nanos();
        root.push(format!("layerfs-h11-v8-{}-{nonce}", std::process::id()));
