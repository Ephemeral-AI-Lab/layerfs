...
impl H11History {
    fn new(capacity: usize) -> CoreResult<Self> {
        Ok(Self {
            edit_ns: H11ChargedVec::with_capacity(capacity)?,
            objects_created: 0,
            objects_reused: 0,
...
fn h11_sample(
    ...
    let target = h11_edit_point(source)?;
    let range = h11_range(target.byte_offset)?;
    let source_hash_charge = h11_charge(64)?;
    let source_hash = source_hash(source)?.1.parse::<ObjectId>()?;
    drop(source_hash_charge);
    ...
    let (base, _base_operation) = h11_create_base(&mut store, source, range.clone())?;
    ...
    let reopen_ns = open_phases
        .preflight_ns
        .checked_add(open_phases.sqlite_open_and_profile_ns)
        .and_then(|value| value.checked_add(cache_profile_ns))
        .and_then(|value| value.checked_add(head_lookup_ns))
        .ok_or(CoreError::LengthOverflow)?;
    ...
    let (historical_tuples, historical_q) =
        h11_verify_historical(&store, &expected, history_count)?;
    ...
    let profile = ObjectId::from_bytes(&profile_id())?;
    let final_output_digest = ObjectId::from_bytes(&after_edit.output_digest)?;
    macro_rules! write_report {
        ($output:expr, $whole_q:expr) => {{
            let output = &mut $output;
            write!(
                output,
                "{{\"status\":\"PASS\",\"schema\":\"phase4-g5-h11-sample-v9\",\"history_revisions\":{history_count},\"sample\":{sample},\"source_bytes\":{SOURCE_1},\"source_blake3\":\"{source_hash}\",\"profile\":\"{profile}\",\"cache_size_pages\":{H11_CACHE_PAGES},\"history_edit_samples_ns\":["
            )?;
            ...
            write!(
                output,
                ",\"final_root\":\"{}\",\"final_transition\":\"{}\",\"final_output_digest\":\"{final_output_digest}\",\"historical_tuples\":[",
                after_edit.root,
    ...
    println!(
        "{{\"schema\":\"phase4-g5-h11-q-terminal-v9\",\"status\":\"PASS\",\"q_current\":0,\"q_high_water\":{reported_q}}}"
    );
    Ok(())
}
...
mod h11_v9_tests {
    ...
    fn test_source() -> (TestRoot, PathBuf) {
        ...
        let nonce = SystemTime::now()
            ...
            .expect("clock")
            .as_nanos();
        root.push(format!("layerfs-h11-v9-{}-{nonce}", std::process::id()));
    ...
    #[test]
    fn whole_harness_q_charges_high_water_and_drops_to_zero() {
        h11_reset_q().expect("reset");
        assert_eq!(std::mem::size_of::<H11Expected>(), 232);
        let values = H11ChargedVec::<H11Expected>::with_capacity(1_001).expect("vector");
        let report = H11ChargedString::with_capacity(4_096).expect("report");
        let expected = 1_001 * 232 + 4_096;
        assert_eq!(h11_q_current(), expected);
        h11_observe_product_q(73_033).expect("operation overlap");
        assert_eq!(h11_q_high_water(), expected + 73_033);
        drop(report);
        drop(values);
        assert_eq!(h11_q_current(), 0);
    }

    #[test]
    fn preparation_and_historical_operation_are_charged() {
        ...
        assert_eq!(h11_q_current(), 0);
        assert_eq!(h11_q_high_water(), 32 * 1024);

        let mut metrics = Metrics::default();
        let (operations, charge) = charged_replace_operation(
            b"file",
            ObjectId::for_bytes(b"prior"),
            ObjectId::for_bytes(b"next"),
            &mut metrics,
        )
        ...
        drop(operations);
        drop(charge);
        finish_q(&mut metrics).expect("product Q");
    }

    #[test]
    fn process_arguments_are_borrowed_and_bounds_checked() {
        let argc = h11_arg_count().expect("argc");
        assert!(argc > 0);
        assert!(!h11_arg(0).expect("argv0").is_empty());
        assert!(h11_arg(argc).is_err());
    }
}
...
pub(super) fn h11_main() -> AnyResult<()> {
    match h11_arg(1)? {
        "--fixture" => {
            h11_require_arg_count(3)?;
            let source = Path::new(h11_arg(2)?);
            if source.exists() {
