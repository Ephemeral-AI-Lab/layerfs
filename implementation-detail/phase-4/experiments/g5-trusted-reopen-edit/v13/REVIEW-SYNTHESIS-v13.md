...
## V12 terminal and accepted v13 method repair

V12 honestly closed as `PREMEASUREMENT_REVISE` after its post-run frozen
verifier rejected `DRY-RUN-INTENT-v12.json`. The producer omitted the required
zero-valued `wrapper_initialization_samples_completed` and
`wrapper_recurring_samples_completed` fields while the verifier retained them.
The dry process reported `106.344946549 s` against `120 s`, but that arithmetic
cannot authorize screen because intent custody failed. V12 has zero product
rows, screen, gate, or lock, and its full chain is bound by
`V12-SUPERSESSION-v13.json`.

V13 changes only the runner intent contract and method version. One shared
initial-progress object now supplies the complete intent producer and verifier
contract, including both split counters exactly once, and the runner self-check
requires the exact object. It retains v12's top-level wrapper semantics derived
from the nested rooted state. It measures one process-first wrapper
...
external operands; no v12 product row or timing is promoted to v13 authority.

V13 keeps the 200-arm schedule, 56 checkpoints, half-slower calibration floor,
...
- use native APFS clone receipts, schema `g5-v13-native-clone-receipt-v1`, to
...
- emit constant-size rooted state, schema `g5-v13-rooted-logical-state-v1`,
...
`FREEZE-VERIFICATION-v13.json` is classified
...
`PRODUCT-CHILD-LIFECYCLE-v13.json` under schema
`phase4-g5-1-product-child-lifecycle-v13`. Per-arm cleanup receipts use schema
`phase4-g5-1-arm-cleanup-receipt-v13`; their ordered terminal record is likewise
duplicated as `WORK-ROOT-LIFECYCLE-v13.json` under schema
`phase4-g5-1-work-root-lifecycle-v13`.
...
`wrapper-calibration-intent-v13`, `wrapper-initialization-sample-v13`,
`wrapper-calibration-sample-v13`, and `wrapper-calibration-result-v13`. The
...
intent embeds `phase4-g5-1-wrapper-calibration-plan-v13`, retains the complete
...
`phase4-g5-1-prearm-wrapper-initialization-v13` record after lock/custody and
...
closes v13 as `PREMEASUREMENT_REVISE` before screen; no schedule or threshold
is reduced.

`DRY-RUN-INTENT-v13.json` is durably written before either calibration begins.
A single outer failure funnel then covers wrapper calibration, hash calibration,
forecasting, and sealing: it always writes `DRY-RUN-FAILED-v13.json` before a
REVISE `DRY-RUN-DISPOSITION-v13.json`, even when the wrapper result is absent.
...
V13 nevertheless voluntarily freezes the stricter v1-lineage design: each
