...
REPO = HERE.parents[4]
METHOD = HERE / "method"
SCHEDULE = METHOD / "SCHEDULE-v13.tsv"
EXPECTED = METHOD / "EXPECTED-OUTCOMES-v13.tsv"
INPUT_MANIFEST = METHOD / "INPUT-MANIFEST-v13.tsv"
METHOD_MANIFEST = METHOD / "METHOD-MANIFEST-v13.tsv"
SOURCE_FREEZE = METHOD / "SOURCE-FREEZE-v13.json"
FREEZE_VERIFICATION = HERE / "FREEZE-VERIFICATION-v13.json"
LIMITATIONS = HERE / "LIMITATIONS-v13.json"
STATIC_CLOSURE = HERE / "STATIC-CLOSURE-v13.json"
DRY_RUN = HERE / "DRY-RUN-v13.json"
DRY_RUN_INTENT = HERE / "DRY-RUN-INTENT-v13.json"
DRY_RUN_CALIBRATION_STDOUT = HERE / "DRY-RUN-CALIBRATION-v13.stdout"
DRY_RUN_CALIBRATION_STDERR = HERE / "DRY-RUN-CALIBRATION-v13.stderr"
DRY_RUN_CALIBRATION_TERMINAL = HERE / "DRY-RUN-CALIBRATION-TERMINAL-v13.json"
DRY_RUN_DISPOSITION = HERE / "DRY-RUN-DISPOSITION-v13.json"
DRY_RUN_FAILED = HERE / "DRY-RUN-FAILED-v13.json"
PREMEASUREMENT_REVISE = HERE / "PREMEASUREMENT-REVISE-v13.json"
WRAPPER_CALIBRATION_INTENT = HERE / "WRAPPER-CALIBRATION-INTENT-v13.json"
WRAPPER_CALIBRATION_RAW = HERE / "WRAPPER-CALIBRATION-RAW-v13.jsonl"
WRAPPER_CALIBRATION_RESULT = HERE / "WRAPPER-CALIBRATION-RESULT-v13.json"
...
SCREEN_RESULT = REPO / f"target/phase4-g5-trusted-reopen-edit-{DATE}-v13-screen"
GATE_RESULT = REPO / f"target/phase4-g5-trusted-reopen-edit-{DATE}-v13"
WRAPPER_CALIBRATION_ROOT = (
    REPO / f"target/phase4-g5-trusted-reopen-edit-wrapper-calibration-{DATE}-v13"
)

# V13 is a method-only revision and reuses the exact hash-verified v10 operands.
...
SENTINEL_SCHEMA = "phase4-g5-1-protected-sentinel-v13"
REQUEST_FIELDS = (
...
V12_SUPERSESSION = HERE / "V12-SUPERSESSION-v13.json"
...
FORECAST_MODEL_VERSION = "phase4-g5-1-v13-fast-law-forecast-v5"
...
PHYSICAL_ALLOCATION_SCHEMA = "g5-v13-physical-allocation-observation-v1"
...
CHILD_LIFECYCLE_SCHEMA = "phase4-g5-1-product-child-lifecycle-v13"
RSS_CLASSIFICATION = "PerProductChildRetainedPeak"
SYNCHRONOUS_RSS_KIND = "synchronous-one-shot"
ARM_CLEANUP_SCHEMA = "phase4-g5-1-arm-cleanup-receipt-v13"
WORK_ROOT_LIFECYCLE_SCHEMA = "phase4-g5-1-work-root-lifecycle-v13"
...
def method_source_names():
    fixed = {
        ...
        for path in (
            HERE / "PREREGISTRATION-v13.md",
            HERE / "REVIEW-SYNTHESIS-v13.md",
            HERE / "SAMPLE-COUNT-INTERPRETATION-ADDENDUM-v13.md",
            HERE / "FOCUSED-TEST-ATTEMPTS-v13.json",
            HERE / "PREMEASUREMENT-READINESS-AUDIT-v13.json",
            LIMITATIONS,
            HERE / "runner.py",
            PRIMARY,
            INDEPENDENT,
...
def freeze_interface_contract():
    return {
        ...
        "wrapper_calibration": {
            ...
            "intent_schema": "phase4-g5-1-wrapper-calibration-intent-v13",
            "initialization_schema": "phase4-g5-1-wrapper-initialization-sample-v13",
            "sample_schema": "phase4-g5-1-wrapper-calibration-sample-v13",
            "result_schema": "phase4-g5-1-wrapper-calibration-result-v13",
...
def write_input_and_method_manifests():
    ...
    freeze = {
        "schema": "phase4-g5-1-source-freeze-v13",
        ...
        "post_freeze_verification": {
            ...
            "schema": "phase4-g5-1-freeze-verification-v13",
...
def run_preparation_command(
    ...
    chronology = evidence_root / "CHRONOLOGY-v13.jsonl"
    ...
    write_json(
        intent,
        {
            "schema": "phase4-g5-1-preparation-command-intent-v13",
            "ordinal": ordinal,
            "label": label,
            "argv": command,
    ...
    terminal = {
        "schema": "phase4-g5-1-preparation-command-terminal-v13",
        "ordinal": ordinal,
        "label": label,
...
def prepare_inputs():
    ...
    if any(
        ...
        raise RuntimeError("v13 method/freeze evidence already exists")
    ...
    if future_input not in sources or missing:
        raise RuntimeError(f"v13 method source preflight mismatch: {missing}")
    write_input_and_method_manifests()
    if verify_sealed_input_manifest() != 93:
        raise RuntimeError("v13 reused input manifest cardinality mismatch")
...
def verify_dry_run(freeze):
    ...
    required = {
        "schema": "phase4-g5-1-dry-run-v13",
        "status": "PASS",
        "measured_rows": 0,
        "benchmark_child_processes_started": 0,
        "stores_opened": 0,
    ...
    intent_required = {
        "schema": "phase4-g5-1-dry-run-intent-v13",
        "status": "STARTED",
        "branch": BRANCH,
        "head": CHECKPOINT,
    ...
    if (
        ...
        or disposition.get("schema") != "phase4-g5-1-dry-run-disposition-v13"
...
def verify_freeze(require_static=False, require_dry=False):
    ...
    if require_static:
        ...
        screen_terminal = SCREEN_RESULT / "TERMINAL-VERIFICATION-v13.json"
        screen_final_manifest = SCREEN_RESULT / "FINAL-ARTIFACT-HASHES-v13.tsv"
        screen_final_verification = SCREEN_RESULT / "FINAL-READONLY-VERIFICATION-v13.json"
        screen_complete_wall = SCREEN_RESULT / "COMPLETE-WALL-v13.json"
        required = {
            "schema": "phase4-g5-1-static-closure-v13",
            "status": "PASS",
            "source_freeze_sha256": sha256(SOURCE_FREEZE),
            "tracked_diff_sha256": freeze["tracked_diff_sha256"],
...
def gate_hash_bytes():
    ...
    prior_method_bytes = sum(
        int(row["bytes"])
        for manifest in (V10_METHOD_MANIFEST, V11_METHOD_MANIFEST, V12_METHOD_MANIFEST)
    ...
    components = {
        ...
        "preserved_v7_v8_v9_v10_v11_v12_failure_evidence": preserved_history_bytes,
...
def gate_workload_enumeration():
    ...
    return {
        "schema": "phase4-g5-1-prospective-workload-v13",
...
def hash_calibration():
    ...
    write_json(
        DRY_RUN_CALIBRATION_TERMINAL,
        {
            "schema": "phase4-g5-1-dry-run-calibration-terminal-v13",
            "status": "RETURNED",
            "return_code": completed.returncode,
    ...
    return {
        "schema": "phase4-g5-1-hash-calibration-v13",
...
def wrapper_calibration_plan():
    ...
    plan = {
        "schema": "phase4-g5-1-wrapper-calibration-plan-v13",
...
def run_wrapper_calibration(freeze, plan):
    ...
    intent = {
        "schema": "phase4-g5-1-wrapper-calibration-intent-v13",
        "status": "STARTED",
    ...
    try:
        ...
        initialization_evidence = {
            "schema": "phase4-g5-1-wrapper-initialization-evidence-v13",
        ...
        initialization = {
            "schema": "phase4-g5-1-wrapper-initialization-sample-v13",
            "status": "PASS",
        ...
        for sample_index in range(1, WRAPPER_CALIBRATION_SAMPLES + 1):
            ...
            representative = {
                "schema": "phase4-g5-1-wrapper-calibration-representative-arm-v13",
                "sample": sample_index,
                "master": master["relative_path"],
            ...
            sample = {
                "schema": "phase4-g5-1-wrapper-calibration-sample-v13",
                "status": "PASS",
                "sample": sample_index,
                "master": master["relative_path"],
                "phases_ns": phases,
        ...
        try:
            ...
            if WRAPPER_CALIBRATION_ROOT.exists():
                if (
                    ...
                    or not WRAPPER_CALIBRATION_ROOT.name.endswith(f"-{DATE}-v13")
                ):
    ...
    result = {
        "schema": "phase4-g5-1-wrapper-calibration-result-v13",
        "status": "PASS",
        "classification": "ZeroProductWrapperCalibration",
...
def verify_wrapper_calibration(freeze):
    ...
    if (
        ...
        != "phase4-g5-1-wrapper-initialization-sample-v13"
    ...
    for sample_index, sample in enumerate(samples, start=1):
        ...
        if (
            sample.get("schema") != "phase4-g5-1-wrapper-calibration-sample-v13"
...
def dry_failure_documents(error_type, error_message, intent_sha256, collected, artifacts):
    failed = {
        "schema": "phase4-g5-1-dry-run-failed-v13",
        "status": "REVISE",
    ...
    disposition = {
        "schema": "phase4-g5-1-dry-run-disposition-v13",
        "status": "REVISE",
        "classification": "UNEXPECTED_DRY_RUN_FAILURE",
        "intent_sha256": intent_sha256,
...
def dry_run():
    ...
    if any(path.exists() for path in evidence_paths):
        raise RuntimeError("v13 dry-run evidence already exists")
    if not FREEZE_VERIFICATION.is_file():
        raise RuntimeError("v13 dry-run requires durable freeze verification")
    ...
    try:
        ...
        value = {
            ...
            "sample_count_interpretation": "deliberately-stricter-v13-choice-not-unambiguous-user-minimum",
        ...
        if status == "REVISE":
            write_json(
                PREMEASUREMENT_REVISE,
                {
                    "schema": "phase4-g5-1-premeasurement-revise-v13",
                    "status": "REVISE",
...
def release_lock(lock, result, terminal_verification=None, state="release"):
    ...
    try:
        ...
        attestation = result / "BENCHMARK-LOCK-RELEASE-ATTESTATION-v13.json"
        ...
        value = {
            "schema": "phase4-g5-1-lock-release-v13",
        ...
        write_json(result / "LOCK-RELEASE-v13.json", value)
        return value
    finally:
        os.close(lock["fd"])
        lock["fd"] = None
...
def run_semantic(executable, case, root, result, label, lifecycle=None):
    stdout = result / f"children-v13/{label}.stdout"
    stderr = result / f"children-v13/{label}.stderr"
    sidecar = result / f"time-v13/{label}.txt"
...
def validate_child_row(envelope):
    ...
    return {
        "schema": "phase4-g5-1-operation-v13",
...
class PersistentChild:
    def __init__(
        ...
        self.stdout_path = result / f"children-v13/{self.label}.stdout"
        self.stderr_path = result / f"children-v13/{self.label}.stderr"
        self.time_path = result / f"time-v13/{self.label}.txt"
...
def analyze(result):
    ...
    for analyzer, name in (
        (PRIMARY, "PRIMARY-ANALYSIS-v13.json"),
        (INDEPENDENT, "INDEPENDENT-RECOMPUTATION-v13.json"),
    ):
        output = result / name
        completed = subprocess.run(
            [
                sys.executable, str(analyzer), str(result / "RAW-v13.jsonl"),
                str(result / "TIMINGS-v13.tsv"), str(SCHEDULE), str(EXPECTED), str(output),
            ],
            cwd=REPO,
            text=True,
            capture_output=True,
    ...
    write_json(
        result / "ANALYZER-AGREEMENT-v13.json",
        {
            "schema": "phase4-g5-1-analyzer-agreement-v13",
...
def run_prearm_wrapper_initialization(result, freeze, dry, lock):
    ...
    evidence = {
        "schema": "phase4-g5-1-prearm-wrapper-initialization-evidence-v13",
    ...
    evidence_path = result / "PREARM-WRAPPER-INITIALIZATION-EVIDENCE-v13.json"
    write_json(evidence_path, evidence)
    total_ns = max(1, time.monotonic_ns() - started)
    value = {
        **evidence,
        "schema": "phase4-g5-1-prearm-wrapper-initialization-v13",
    ...
    write_json(result / "PREARM-WRAPPER-INITIALIZATION-v13.json", value)
    if value["status"] != "PASS":
...
def ladder_prelock(campaign):
    ...
    if campaign == "gate":
        paths.extend(
            (
                SCREEN_RESULT / "TERMINAL-VERIFICATION-v13.json",
                SCREEN_RESULT / "FINAL-ARTIFACT-HASHES-v13.tsv",
                SCREEN_RESULT / "FINAL-READONLY-VERIFICATION-v13.json",
                SCREEN_RESULT / "COMPLETE-WALL-v13.json",
                STATIC_CLOSURE,
            )
        )
...
def run_campaign(campaign):
    ...
    try:
        ...
        work = result.parent / f"{result.name}-work-v13"
        work.mkdir(mode=0o700)
        work_roots = ArmWorkRootLifecycle(work)
        for name in ("operands-v13", "children-v13", "time-v13"):
            (result / name).mkdir()
        fsync_dir(result)
        fsync_dir(result.parent)

        g4_copy = result / "operands-v13/frozen-g4-verified"
        g5_copy = result / "operands-v13/g5-verified-trusted"
        ...
        write_json(
            result / "OPERAND-CUSTODY-v13.json",
            {
                "schema": "phase4-g5-1-operand-custody-v13",
        ...
        write_json(
            result / "PREFLIGHT-v13.json",
            {
                "schema": "phase4-g5-1-preflight-v13",
                "status": "PASS",
                "campaign": campaign,
                "branch": BRANCH,
                "checkpoint": CHECKPOINT,
        ...
        write_json(
            result / "ENVIRONMENT-v13.json",
            {
                "schema": "phase4-g5-1-environment-v13",
        ...
        shutil.copyfile(INPUT_MANIFEST, result / "INPUT-CUSTODY-v13.tsv")
        with (result / "INPUT-CUSTODY-v13.tsv").open("rb") as handle:
            os.fsync(handle.fileno())
                    ...
                    semantics.extend(
                        {
                            ...
                            "expectation_id": "native-semantic-v13",
                            "status": value["status"],
                            "error": value["error"],
                        }
                        for value in values
        ...
        write_json(result / "PRODUCT-CHILD-LIFECYCLE-v13.json", child_lifecycle)
        write_json(result / "WORK-ROOT-LIFECYCLE-v13.json", work_root_lifecycle)

        write_text(result / "RAW-v13.jsonl", "".join(compact(row) + "\n" for row in raw))
        timing_fields = (
        ...
        write_text(
            result / "TIMINGS-v13.tsv",
            "\t".join(timing_fields) + "\n"
            + "".join("\t".join(str(row[name]) for name in timing_fields) + "\n" for row in timings),
        )
        write_text(
            result / "SEMANTIC-FAULT-RESULTS-v13.tsv",
        ...
        write_json(result / "COMMANDS-v13.json", {"schema": "phase4-g5-1-commands-v13", "commands": commands})
        analyze(result)

        if work.parent != result.parent or not work.name.endswith("-work-v13"):
            raise RuntimeError("refusing unsafe work cleanup")
        shutil.rmtree(work)
        fsync_dir(work.parent)
        residue = [str(path) for path in result.parent.glob(f"{result.name}-work-v13")]
        write_json(
            result / "CLEANUP-v13.json",
            {
                "schema": "phase4-g5-1-cleanup-v13",
        ...
        payload_excluded = {
            "PAYLOAD-MANIFEST-v13.tsv", "MEASURED-TERMINAL-v13.json",
            "TERMINAL-VERIFICATION-v13.json", "COMPLETE-WALL-v13.json",
            "BENCHMARK-LOCK-RELEASE-ATTESTATION-v13.json", "LOCK-RELEASE-v13.json",
            "FINAL-ARTIFACT-HASHES-v13.tsv", "FINAL-READONLY-VERIFICATION-v13.json",
        }
        payload = result / "PAYLOAD-MANIFEST-v13.tsv"
        ...
        terminal = result / "MEASURED-TERMINAL-v13.json"
        write_json(
            terminal,
            {
                "schema": "phase4-g5-1-measured-terminal-v13",
                "status": "PASS",
                "campaign": campaign,
                "rows": len(timings),
                ...
                "product_child_lifecycle_sha256": sha256(
                    result / "PRODUCT-CHILD-LIFECYCLE-v13.json"
                ),
                "work_root_lifecycle_sha256": sha256(
                    result / "WORK-ROOT-LIFECYCLE-v13.json"
                ),
                "prearm_wrapper_initialization_sha256": sha256(
                    result / "PREARM-WRAPPER-INITIALIZATION-v13.json"
                ),
        ...
        verification = result / "TERMINAL-VERIFICATION-v13.json"
        write_json(
            verification,
            {
                "schema": "phase4-g5-1-terminal-verification-v13",
                "status": "PASS",
        ...
        final = result / "FINAL-ARTIFACT-HASHES-v13.tsv"
        write_text(
            final,
            manifest_text(
                result,
                excluded={final.name, "FINAL-READONLY-VERIFICATION-v13.json", "COMPLETE-WALL-v13.json"},
            ),
        )
        final_count = verify_manifest(result, final, "result_relative_path")
        write_json(
            result / "FINAL-READONLY-VERIFICATION-v13.json",
            {
                "schema": "phase4-g5-1-final-readonly-verification-v13",
                "status": "PASS",
        ...
        write_json(
            result / "COMPLETE-WALL-v13.json",
            {
                "schema": "phase4-g5-1-complete-wall-v13",
                "status": "PASS",
                "campaign": campaign,
                ...
                "terminal_self_exclusion": "COMPLETE-WALL-v13.json follows the verified final manifest",
            },
        )
        fsync_dir(result)
        ...
        if result.exists() and not (result / "FAILED-v13.json").exists():
            write_json(
                result / "FAILED-v13.json",
                {
                    "schema": "phase4-g5-1-failure-v13",
                    "status": "REVISE",
                    "error": str(error),
...
def run_self_checks():
    ...
    with tempfile.TemporaryDirectory(prefix="layerfs-g5-v13-self-check-") as temporary:
        work = pathlib.Path(temporary) / "work-v13"
        work.mkdir()
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="layerfs-g5-v13-forecast-check-") as temporary:
    ...
    assert FORECAST_MODEL_VERSION == "phase4-g5-1-v13-fast-law-forecast-v5"
    ...
    print(compact({"status": "PASS", "checks": 14, "gate_arms": 200, "checkpoints": 56}))
    return 0
