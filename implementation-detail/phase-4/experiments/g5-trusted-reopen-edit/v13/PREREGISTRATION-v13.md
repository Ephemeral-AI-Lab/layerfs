# Phase 4 G5-1 v13 preregistration
...
The candidate remains CAS + CDC + COW + canonical K64/F64 + SQLite. V13 changes
...
Any failure preserves v13. No observed row may be deleted, replaced, reordered,
or selectively rerun. A source or method repair creates v14.
...
`phase4-g5-1-wrapper-calibration-intent-v13`,
`phase4-g5-1-wrapper-initialization-sample-v13`,
`phase4-g5-1-wrapper-calibration-sample-v13`, and
`phase4-g5-1-wrapper-calibration-result-v13`.
...
`phase4-g5-1-wrapper-calibration-plan-v13` and records the complete candidate
...
must write and fsync the top-level `DRY-RUN-INTENT-v13.json` before invoking
...
write and fsync `DRY-RUN-FAILED-v13.json` first, then write and fsync a
`DRY-RUN-DISPOSITION-v13.json` with `status=REVISE` that binds the FAILED hash
and explicitly records absent optional artifacts. A success or calibrated
overrun writes and fsyncs complete `DRY-RUN-v13.json`, including both
...
`PREMEASUREMENT-REVISE-v13.json` and stops before screen. A missing dry-run or
...
V12 is also byte-preserved and superseded by `V12-SUPERSESSION-v13.json`.
...
V13 is a runner-intent-contract/method-only revision. It adds those two fields
exactly once through the shared initial-progress object consumed by both the
producer and verifier, and a focused exact-object self-check prevents another
producer/verifier drift. It reuses the unchanged v10
...
are not copied, renamed, or represented as v13-generated. V13 reuses no v10
or v12 product row or timing observation. The new v13 input manifest rebinds the same sealed
files byte-for-byte before the v13 method manifest and source freeze are
generated.
...
The final `PREARM-WRAPPER-INITIALIZATION-v13.json` write is outside that bound
...
schedule, expectation, limitation, and supersession byte has settled, v13
...
child. It writes `FREEZE-VERIFICATION-v13.json` with classification
...
V13 voluntarily freezes a deliberately stricter v1-lineage design: each
...
`phase4-g5-1-product-child-lifecycle-v13`. The two copies must be byte-value
...
child, v13 verifies the complete sealed input manifest exactly once: path,
...
`g5-v13-native-clone-receipt-v1`. Its ordered `entries` bind each source master
...
inspection for a future separate API. V13 therefore removes the external
...
`g5-v13-rooted-logical-state-v1` from one query-only, autocommit, constant-row
...
V13 does not claim physical SQLite file-byte parity. The sibling
`physical_allocation_observation` remains schema
`g5-v13-physical-allocation-observation-v1`, classification
...
Each wrapper receipt uses schema `phase4-g5-1-arm-cleanup-receipt-v13` and is
repeated in order by the raw and standalone
`phase4-g5-1-work-root-lifecycle-v13` record. Started and cleaned roots equal
