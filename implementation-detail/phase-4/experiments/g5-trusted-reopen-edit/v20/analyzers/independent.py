...
LIFECYCLE_SCHEMA = "phase4-g5-1-product-child-lifecycle-v20"
...
CLONE_SCHEMA = "g5-v20-native-clone-receipt-v1"
CLONE_TOP = frozenset((
...
PHYSICAL_SCHEMA = "g5-v20-physical-allocation-observation-v1"
...
WORK_ROOT_SCHEMA = "phase4-g5-1-work-root-lifecycle-v20"
...
INPUT_MANIFEST = pathlib.Path(__file__).resolve().parents[1] / "method/INPUT-MANIFEST-v20.tsv"
DRY_RUN = pathlib.Path(__file__).resolve().parents[1] / "DRY-RUN-v20.json"
...
def audit_prearm(
    ...
    facts = (
        ...
        type(evidence) is dict
        and evidence.get("schema") == "phase4-g5-1-prearm-wrapper-initialization-evidence-v20",
        type(evidence) is dict
...
def recompute(raw_file, timing_file, schedule_file, expected_file):
    ...
    rows = [value for value in documents if value.get("schema") == "phase4-g5-1-operation-v20"]
    ...
    sentinel = [value for value in documents if value.get("schema") == "phase4-g5-1-protected-sentinel-v20"]
    ...
    initialization_rows = [
        ...
        if value.get("schema") == "phase4-g5-1-prearm-wrapper-initialization-v20"
    ]
    ...
    artifact = pathlib.Path(raw_file).parent / "PREARM-WRAPPER-INITIALIZATION-v20.json"
    try:
        artifact_bytes = artifact.read_bytes()
    except OSError:
        artifact_bytes = b""
    evidence_path = pathlib.Path(raw_file).parent / (
        "PREARM-WRAPPER-INITIALIZATION-EVIDENCE-v20.json"
    )
    try:
    ...
    audit_lifecycle(
        ...
        sentinel,
        campaign,
        pathlib.Path(raw_file).parent / "PRODUCT-CHILD-LIFECYCLE-v20.json",
        problems,
    )
    audit_work_roots(
        work_root_rows,
        rows,
        pathlib.Path(raw_file).parent / "WORK-ROOT-LIFECYCLE-v20.json",
        problems,
    )
    if campaign == "screen":
        counts = {}
    ...
    for comparison in ("g4-verified-vs-g5-verified", "g5-verified-vs-g5-trusted"):
        ...
        for operation in operations:
            ...
            for pair in sorted(left):
                ...
                if comparison == "g4-verified-vs-g5-verified":
                    reference_count = right_product.get("leaf_batch_references")
                    query_count = right_product.get("leaf_batch_queries")
                    batch_max = right_product.get("leaf_batch_references_max")
                    if not (
                        type(reference_count) is int and reference_count > 0
                        and type(query_count) is int and query_count == -(-reference_count // 32)
                        and type(batch_max) is int and 0 < batch_max <= 32
                    ):
                        problems.add(f"{comparison}:{operation}:pair-{pair}:leaf-batch-equation")
                    else:
                        saved_queries = reference_count - query_count
                        authority_expected = dict(
                            ...
                            sql_calls=left_product.get("sql_calls", 0) + 1 - saved_queries,
                            sql_query_calls=left_product.get("sql_query_calls", 0) + 1 - saved_queries,
    ...
    return {
        "schema": "phase4-g5-1-independent-recomputation-v20",
