...
OP_SCHEMA = "phase4-g5-1-operation-v20"
...
def validate_prearm_initialization(
    ...
    if (
        ...
        != "phase4-g5-1-prearm-wrapper-initialization-evidence-v20"
...
def analyze(raw_path, timing_path, schedule_path, expected_path):
    ...
    sentinel_rows = [row for row in raw if row.get("schema") == "phase4-g5-1-protected-sentinel-v20"]
    ...
    initialization_rows = [
        row
        for row in raw
        if row.get("schema") == "phase4-g5-1-prearm-wrapper-initialization-v20"
    ]
    ...
    initialization_artifact = pathlib.Path(raw_path).parent / (
        "PREARM-WRAPPER-INITIALIZATION-v20.json"
    )
    try:
    ...
    validate_child_lifecycle(
        ...
        sentinel_rows,
        campaign,
        pathlib.Path(raw_path).parent / "PRODUCT-CHILD-LIFECYCLE-v20.json",
        failures,
    )
    validate_work_root_lifecycle(
        work_root_rows,
        operations,
        pathlib.Path(raw_path).parent / "WORK-ROOT-LIFECYCLE-v20.json",
        failures,
    )
    if campaign == "screen":
        counts = {}
        ...
        for pair in sorted(control_by_pair):
            ...
            if comparison == "g4-verified-vs-g5-verified":
                ...
                if (
                    ...
                    or type(batches) is not int or batches != (references + 31) // 32
                    or type(maximum) is not int or not 0 < maximum <= 32
                ):
    ...
    return {"schema": "phase4-g5-1-primary-analysis-v20", "status": "PASS" if not failures else "REVISE", "normalized": normalized}
