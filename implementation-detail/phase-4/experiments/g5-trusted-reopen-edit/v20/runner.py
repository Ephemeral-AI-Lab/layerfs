...
HISTORY = HERE.parent
METHOD = HERE / "method"
SCHEDULE = METHOD / "SCHEDULE-v20.tsv"
EXPECTED = METHOD / "EXPECTED-OUTCOMES-v20.tsv"
INPUT_MANIFEST = METHOD / "INPUT-MANIFEST-v20.tsv"
METHOD_MANIFEST = METHOD / "METHOD-MANIFEST-v20.tsv"
SOURCE_FREEZE = METHOD / "SOURCE-FREEZE-v20.json"
FREEZE_VERIFICATION = HERE / "FREEZE-VERIFICATION-v20.json"
LIMITATIONS = HERE / "LIMITATIONS-v20.json"
STATIC_CLOSURE = HERE / "STATIC-CLOSURE-v20.json"
DRY_RUN = HERE / "DRY-RUN-v20.json"
DRY_RUN_INTENT = HERE / "DRY-RUN-INTENT-v20.json"
DRY_RUN_CALIBRATION_STDOUT = HERE / "DRY-RUN-CALIBRATION-v20.stdout"
DRY_RUN_CALIBRATION_STDERR = HERE / "DRY-RUN-CALIBRATION-v20.stderr"
DRY_RUN_CALIBRATION_TERMINAL = HERE / "DRY-RUN-CALIBRATION-TERMINAL-v20.json"
DRY_RUN_DISPOSITION = HERE / "DRY-RUN-DISPOSITION-v20.json"
DRY_RUN_FAILED = HERE / "DRY-RUN-FAILED-v20.json"
PREMEASUREMENT_REVISE = HERE / "PREMEASUREMENT-REVISE-v20.json"
WRAPPER_CALIBRATION_INTENT = HERE / "WRAPPER-CALIBRATION-INTENT-v20.json"
WRAPPER_CALIBRATION_RAW = HERE / "WRAPPER-CALIBRATION-RAW-v20.jsonl"
WRAPPER_CALIBRATION_RESULT = HERE / "WRAPPER-CALIBRATION-RESULT-v20.json"
...
SCREEN_RESULT = REPO / f"target/phase4-g5-trusted-reopen-edit-{DATE}-v20-screen"
GATE_RESULT = REPO / f"target/phase4-g5-trusted-reopen-edit-{DATE}-v20"
WRAPPER_CALIBRATION_ROOT = (
    REPO / f"target/phase4-g5-trusted-reopen-edit-wrapper-calibration-{DATE}-v20"
)

# V20 changes the product timer equation and uses a fresh hash-bound release.
G5_CHILD_BINARY = HERE / "g5-benchmark/target/release/layerfs-g5-trusted-child-v20"
...
SENTINEL_SCHEMA = "phase4-g5-1-protected-sentinel-v20"
REQUEST_FIELDS = (
...
V20_RELEASE_SHA256 = "50d61cd15d0084945b9a473394f7aad60a0fc04cb252ffa39ab0ff58fe736f52"
...
FORECAST_MODEL_VERSION = "phase4-g5-1-v20-fast-law-forecast-v5"
...
PHYSICAL_ALLOCATION_SCHEMA = "g5-v20-physical-allocation-observation-v1"
...
CHILD_LIFECYCLE_SCHEMA = "phase4-g5-1-product-child-lifecycle-v20"
RSS_CLASSIFICATION = "PerProductChildRetainedPeak"
SYNCHRONOUS_RSS_KIND = "synchronous-one-shot"
ARM_CLEANUP_SCHEMA = "phase4-g5-1-arm-cleanup-receipt-v20"
WORK_ROOT_LIFECYCLE_SCHEMA = "phase4-g5-1-work-root-lifecycle-v20"
...
def verify_repository_identity():
    ...
    verify_file(G5_CHILD_BINARY, V20_RELEASE_SHA256)
...
def method_source_names():
    fixed = {
        ...
        for path in (
            HERE / "PREREGISTRATION-v20.md",
            HERE / "REVIEW-SYNTHESIS-v20.md",
            HERE / "SAMPLE-COUNT-INTERPRETATION-ADDENDUM-v20.md",
            HERE / "FOCUSED-TEST-ATTEMPTS-v20.json",
            HERE / "PREMEASUREMENT-READINESS-AUDIT-v20.json",
            LIMITATIONS,
            HERE / "runner.py",
            PRIMARY,
            INDEPENDENT,
...
def freeze_interface_contract():
    return {
        "product_operand_version": "v20",
        "product_release_reuse": False,
        "product_release_sha256": V20_RELEASE_SHA256,
        ...
        "wrapper_calibration": {
            ...
            "intent_schema": "phase4-g5-1-wrapper-calibration-intent-v20",
            "initialization_schema": "phase4-g5-1-wrapper-initialization-sample-v20",
            "sample_schema": "phase4-g5-1-wrapper-calibration-sample-v20",
            "result_schema": "phase4-g5-1-wrapper-calibration-result-v20",
...
def write_input_and_method_manifests():
    ...
    freeze = {
        "schema": "phase4-g5-1-source-freeze-v20",
        ...
        "post_freeze_verification": {
            ...
            "schema": "phase4-g5-1-freeze-verification-v20",
...
def run_preparation_command(
    ...
    chronology = evidence_root / "CHRONOLOGY-v20.jsonl"
    ...
    write_json(
        intent,
        {
            "schema": "phase4-g5-1-preparation-command-intent-v20",
            "ordinal": ordinal,
            "label": label,
            "argv": command,
    ...
    terminal = {
        "schema": "phase4-g5-1-preparation-command-terminal-v20",
        "ordinal": ordinal,
        "label": label,
...
def prepare_inputs():
    ...
    if any(
        ...
        raise RuntimeError("v20 method/freeze evidence already exists")
    if not os.access(G5_CHILD_BINARY, os.X_OK):
        raise RuntimeError(f"hash-bound v20 child is not executable: {G5_CHILD_BINARY}")
    ...
    if future_input not in sources or missing:
        raise RuntimeError(f"v20 method source preflight mismatch: {missing}")
    write_input_and_method_manifests()
    if verify_sealed_input_manifest() != 93:
        raise RuntimeError("v20 reused input manifest cardinality mismatch")
...
def verify_dry_run(freeze):
    ...
    required = {
        "schema": "phase4-g5-1-dry-run-v20",
        "status": "PASS",
        "measured_rows": 0,
        "benchmark_child_processes_started": 0,
        "stores_opened": 0,
    ...
    intent_required = {
        "schema": "phase4-g5-1-dry-run-intent-v20",
        "status": "STARTED",
        "branch": BRANCH,
        "head": CHECKPOINT,
    ...
    if (
        ...
        or disposition.get("schema") != "phase4-g5-1-dry-run-disposition-v20"
...
def verify_freeze(require_static=False, require_dry=False):
    ...
    if require_static:
        ...
        screen_terminal = SCREEN_RESULT / "TERMINAL-VERIFICATION-v20.json"
        screen_final_manifest = SCREEN_RESULT / "FINAL-ARTIFACT-HASHES-v20.tsv"
        screen_final_verification = SCREEN_RESULT / "FINAL-READONLY-VERIFICATION-v20.json"
        screen_complete_wall = SCREEN_RESULT / "COMPLETE-WALL-v20.json"
        required = {
            "schema": "phase4-g5-1-static-closure-v20",
            "status": "PASS",
            "source_freeze_sha256": sha256(SOURCE_FREEZE),
            "tracked_diff_sha256": freeze["tracked_diff_sha256"],
...
def gate_workload_enumeration():
    ...
    return {
        "schema": "phase4-g5-1-prospective-workload-v20",
...
def hash_calibration():
    ...
    write_json(
        DRY_RUN_CALIBRATION_TERMINAL,
        {
            "schema": "phase4-g5-1-dry-run-calibration-terminal-v20",
            "status": "RETURNED",
            "return_code": completed.returncode,
    ...
    return {
        "schema": "phase4-g5-1-hash-calibration-v20",
...
def wrapper_calibration_plan():
    ...
    plan = {
        "schema": "phase4-g5-1-wrapper-calibration-plan-v20",
...
def run_wrapper_calibration(freeze, plan):
    ...
    intent = {
        "schema": "phase4-g5-1-wrapper-calibration-intent-v20",
        "status": "STARTED",
    ...
    try:
        ...
        initialization_evidence = {
            "schema": "phase4-g5-1-wrapper-initialization-evidence-v20",
        ...
        initialization = {
            "schema": "phase4-g5-1-wrapper-initialization-sample-v20",
            "status": "PASS",
        ...
        for sample_index in range(1, WRAPPER_CALIBRATION_SAMPLES + 1):
            ...
            representative = {
                "schema": "phase4-g5-1-wrapper-calibration-representative-arm-v20",
                "sample": sample_index,
                "master": master["relative_path"],
            ...
            sample = {
                "schema": "phase4-g5-1-wrapper-calibration-sample-v20",
                "status": "PASS",
                "sample": sample_index,
                "master": master["relative_path"],
                "phases_ns": phases,
        ...
        try:
            ...
            if WRAPPER_CALIBRATION_ROOT.exists():
                if (
                    ...
                    != f"phase4-g5-trusted-reopen-edit-wrapper-calibration-{DATE}-v20"
                ):
    ...
    result = {
        "schema": "phase4-g5-1-wrapper-calibration-result-v20",
        "status": "PASS",
        "classification": "ZeroProductWrapperCalibration",
...
def verify_wrapper_calibration(freeze):
    ...
    for sample_index, sample in enumerate(samples, start=1):
        ...
        if (
            sample.get("schema") != "phase4-g5-1-wrapper-calibration-sample-v20"
...
def dry_failure_documents(error_type, error_message, intent_sha256, collected, artifacts):
    failed = {
        "schema": "phase4-g5-1-dry-run-failed-v20",
        "status": "REVISE",
    ...
    disposition = {
        "schema": "phase4-g5-1-dry-run-disposition-v20",
        "status": "REVISE",
        "classification": "UNEXPECTED_DRY_RUN_FAILURE",
        "intent_sha256": intent_sha256,
...
def dry_run():
    ...
    if any(path.exists() for path in evidence_paths):
        raise RuntimeError("v20 dry-run evidence already exists")
    if not FREEZE_VERIFICATION.is_file():
        raise RuntimeError("v20 dry-run requires durable freeze verification")
    ...
    try:
        ...
        value = {
            ...
            "sample_count_interpretation": "deliberately-stricter-v20-choice-not-unambiguous-user-minimum",
        ...
        if status == "REVISE":
            write_json(
                PREMEASUREMENT_REVISE,
                {
                    "schema": "phase4-g5-1-premeasurement-revise-v20",
                    "status": "REVISE",
...
def release_lock(lock, result, terminal_verification=None, state="release"):
    ...
    try:
        ...
        attestation = result / "BENCHMARK-LOCK-RELEASE-ATTESTATION-v20.json"
        ...
        value = {
            "schema": "phase4-g5-1-lock-release-v20",
        ...
        write_json(result / "LOCK-RELEASE-v20.json", value)
        return value
    finally:
        os.close(lock["fd"])
        lock["fd"] = None
...
def run_semantic(executable, case, root, result, label, lifecycle=None):
    stdout = result / f"children-v20/{label}.stdout"
    stderr = result / f"children-v20/{label}.stderr"
    sidecar = result / f"time-v20/{label}.txt"
...
def validate_child_row(envelope):
    ...
    return {
        "schema": "phase4-g5-1-operation-v20",
...
class PersistentChild:
    def __init__(
        ...
        self.stdout_path = result / f"children-v20/{self.label}.stdout"
        self.stderr_path = result / f"children-v20/{self.label}.stderr"
        self.time_path = result / f"time-v20/{self.label}.txt"
...
def analyze(result):
    ...
    for analyzer, name in (
        (PRIMARY, "PRIMARY-ANALYSIS-v20.json"),
        (INDEPENDENT, "INDEPENDENT-RECOMPUTATION-v20.json"),
    ):
        output = result / name
        completed = subprocess.run(
            [
                sys.executable, str(analyzer), str(result / "RAW-v20.jsonl"),
                str(result / "TIMINGS-v20.tsv"), str(SCHEDULE), str(EXPECTED), str(output),
            ],
            cwd=REPO,
            text=True,
            capture_output=True,
    ...
    write_json(
        result / "ANALYZER-AGREEMENT-v20.json",
        {
            "schema": "phase4-g5-1-analyzer-agreement-v20",
...
def run_prearm_wrapper_initialization(result, freeze, dry, lock):
    ...
    evidence = {
        "schema": "phase4-g5-1-prearm-wrapper-initialization-evidence-v20",
    ...
    evidence_path = result / "PREARM-WRAPPER-INITIALIZATION-EVIDENCE-v20.json"
    write_json(evidence_path, evidence)
    total_ns = max(1, time.monotonic_ns() - started)
    value = {
        **evidence,
        "schema": "phase4-g5-1-prearm-wrapper-initialization-v20",
    ...
    write_json(result / "PREARM-WRAPPER-INITIALIZATION-v20.json", value)
    if value["status"] != "PASS":
...
def ladder_prelock(campaign):
    ...
    if campaign == "gate":
        paths.extend(
            (
                SCREEN_RESULT / "TERMINAL-VERIFICATION-v20.json",
                SCREEN_RESULT / "FINAL-ARTIFACT-HASHES-v20.tsv",
                SCREEN_RESULT / "FINAL-READONLY-VERIFICATION-v20.json",
                SCREEN_RESULT / "COMPLETE-WALL-v20.json",
                STATIC_CLOSURE,
            )
        )
...
def run_campaign(campaign):
    ...
    try:
        ...
        work = result.parent / f"{result.name}-work-v20"
        work.mkdir(mode=0o700)
        work_roots = ArmWorkRootLifecycle(work)
        for name in ("operands-v20", "children-v20", "time-v20"):
            (result / name).mkdir()
        fsync_dir(result)
        fsync_dir(result.parent)

        g4_copy = result / "operands-v20/frozen-g4-verified"
        g5_copy = result / "operands-v20/g5-verified-trusted"
        ...
        write_json(
            result / "OPERAND-CUSTODY-v20.json",
            {
                "schema": "phase4-g5-1-operand-custody-v20",
        ...
        write_json(
            result / "PREFLIGHT-v20.json",
            {
                "schema": "phase4-g5-1-preflight-v20",
                "status": "PASS",
                "campaign": campaign,
                "branch": BRANCH,
                "checkpoint": CHECKPOINT,
        ...
        write_json(
            result / "ENVIRONMENT-v20.json",
            {
                "schema": "phase4-g5-1-environment-v20",
        ...
        shutil.copyfile(INPUT_MANIFEST, result / "INPUT-CUSTODY-v20.tsv")
        with (result / "INPUT-CUSTODY-v20.tsv").open("rb") as handle:
            os.fsync(handle.fileno())
                    ...
                    semantics.extend(
                        {
                            ...
                            "expectation_id": "native-semantic-v20",
                            "status": value["status"],
                            "error": value["error"],
                        }
                        for value in values
        ...
        write_json(result / "PRODUCT-CHILD-LIFECYCLE-v20.json", child_lifecycle)
        write_json(result / "WORK-ROOT-LIFECYCLE-v20.json", work_root_lifecycle)

        write_text(result / "RAW-v20.jsonl", "".join(compact(row) + "\n" for row in raw))
        timing_fields = (
        ...
        write_text(
            result / "TIMINGS-v20.tsv",
            "\t".join(timing_fields) + "\n"
            + "".join("\t".join(str(row[name]) for name in timing_fields) + "\n" for row in timings),
        )
        write_text(
            result / "SEMANTIC-FAULT-RESULTS-v20.tsv",
        ...
        write_json(result / "COMMANDS-v20.json", {"schema": "phase4-g5-1-commands-v20", "commands": commands})
        analyze(result)

        if work.parent != result.parent or not work.name.endswith("-work-v20"):
            raise RuntimeError("refusing unsafe work cleanup")
        shutil.rmtree(work)
        fsync_dir(work.parent)
        residue = [str(path) for path in result.parent.glob(f"{result.name}-work-v20")]
        write_json(
            result / "CLEANUP-v20.json",
            {
                "schema": "phase4-g5-1-cleanup-v20",
        ...
        payload_excluded = {
            "PAYLOAD-MANIFEST-v20.tsv", "MEASURED-TERMINAL-v20.json",
            "TERMINAL-VERIFICATION-v20.json", "COMPLETE-WALL-v20.json",
            "BENCHMARK-LOCK-RELEASE-ATTESTATION-v20.json", "LOCK-RELEASE-v20.json",
            "FINAL-ARTIFACT-HASHES-v20.tsv", "FINAL-READONLY-VERIFICATION-v20.json",
        }
        payload = result / "PAYLOAD-MANIFEST-v20.tsv"
        ...
        terminal = result / "MEASURED-TERMINAL-v20.json"
        write_json(
            terminal,
            {
                "schema": "phase4-g5-1-measured-terminal-v20",
                "status": "PASS",
                "campaign": campaign,
                "rows": len(timings),
                ...
                "product_child_lifecycle_sha256": sha256(
                    result / "PRODUCT-CHILD-LIFECYCLE-v20.json"
                ),
                "work_root_lifecycle_sha256": sha256(
                    result / "WORK-ROOT-LIFECYCLE-v20.json"
                ),
                "prearm_wrapper_initialization_sha256": sha256(
                    result / "PREARM-WRAPPER-INITIALIZATION-v20.json"
                ),
        ...
        verification = result / "TERMINAL-VERIFICATION-v20.json"
        write_json(
            verification,
            {
                "schema": "phase4-g5-1-terminal-verification-v20",
                "status": "PASS",
        ...
        final = result / "FINAL-ARTIFACT-HASHES-v20.tsv"
        write_text(
            final,
            manifest_text(
                result,
                excluded={final.name, "FINAL-READONLY-VERIFICATION-v20.json", "COMPLETE-WALL-v20.json"},
            ),
        )
        final_count = verify_manifest(result, final, "result_relative_path")
        write_json(
            result / "FINAL-READONLY-VERIFICATION-v20.json",
            {
                "schema": "phase4-g5-1-final-readonly-verification-v20",
                "status": "PASS",
        ...
        write_json(
            result / "COMPLETE-WALL-v20.json",
            {
                "schema": "phase4-g5-1-complete-wall-v20",
                "status": "PASS",
                "campaign": campaign,
                ...
                "terminal_self_exclusion": "COMPLETE-WALL-v20.json follows the verified final manifest",
            },
        )
        fsync_dir(result)
        ...
        if result.exists() and not (result / "FAILED-v20.json").exists():
            write_json(
                result / "FAILED-v20.json",
                {
                    "schema": "phase4-g5-1-failure-v20",
                    "status": "REVISE",
                    "error": str(error),
...
def run_self_checks():
    ...
    with tempfile.TemporaryDirectory(prefix="layerfs-g5-v20-self-check-") as temporary:
        work = pathlib.Path(temporary) / "work-v20"
        work.mkdir()
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="layerfs-g5-v20-forecast-check-") as temporary:
    ...
    assert FORECAST_MODEL_VERSION == "phase4-g5-1-v20-fast-law-forecast-v5"
