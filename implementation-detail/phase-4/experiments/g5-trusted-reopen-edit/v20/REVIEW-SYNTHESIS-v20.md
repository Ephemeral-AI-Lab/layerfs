# G5-1 v20 review synthesis

V19 completed all 200 rows below 150 seconds but two Verified children exceeded
the 20-MiB cap. V20 keeps the authenticated batch mechanism and uses width 32
...
G5 processes exceeded the 20-MiB per-child RSS cap. V20 changes only the common
...
20,971,520-byte cap. V20 releases free allocator pages only between rows after
...
## V16 screen and accepted v20 analyzer repair
...
V20 changes only those two analyzer predicates and the versioned child/report
...
product row or timing is promoted to v20 authority.

V20 keeps the 200-arm schedule, 56 checkpoints, half-slower calibration floor,
...
- use native APFS clone receipts, schema `g5-v20-native-clone-receipt-v1`, to
...
- emit constant-size rooted state, schema `g5-v20-rooted-logical-state-v1`,
...
`FREEZE-VERIFICATION-v20.json` is classified
...
`PRODUCT-CHILD-LIFECYCLE-v20.json` under schema
`phase4-g5-1-product-child-lifecycle-v20`. Per-arm cleanup receipts use schema
`phase4-g5-1-arm-cleanup-receipt-v20`; their ordered terminal record is likewise
duplicated as `WORK-ROOT-LIFECYCLE-v20.json` under schema
`phase4-g5-1-work-root-lifecycle-v20`.
...
`wrapper-calibration-intent-v20`, `wrapper-initialization-sample-v20`,
`wrapper-calibration-sample-v20`, and `wrapper-calibration-result-v20`. The
...
intent embeds `phase4-g5-1-wrapper-calibration-plan-v20`, retains the complete
...
`phase4-g5-1-prearm-wrapper-initialization-v20` record after lock/custody and
...
closes v20 as `PREMEASUREMENT_REVISE` before screen; no schedule or threshold
is reduced.

`DRY-RUN-INTENT-v20.json` is durably written before either calibration begins.
A single outer failure funnel then covers wrapper calibration, hash calibration,
forecasting, and sealing: it always writes `DRY-RUN-FAILED-v20.json` before a
REVISE `DRY-RUN-DISPOSITION-v20.json`, even when the wrapper result is absent.
...
V20 nevertheless voluntarily freezes the stricter v1-lineage design: each
