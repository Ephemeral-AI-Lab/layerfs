...
fn main() {
    ...
    let git_show = |path: &str| {
        ...
        assert!(
            ...
            "git-show source authority unavailable"
        );
        String::from_utf8(output.stdout).unwrap()
    };
    let retained = git_show("crates/layerfs-engine/src/bin/phase4_create_edit_benchmark.rs");
...
fn prepare_fixed_radix_acceptance_fixtures"#,
    ...
    let retained = replace_once(
        retained,
        "fn main() -> AnyResult<()> {",
        "pub(super) fn main_entry() -> AnyResult<()> {",
    );
    let retained = replace_once(
        retained,
        "PRAGMA synchronous=FULL; PRAGMA temp_store=FILE; PRAGMA mmap_size=0; PRAGMA cache_spill=2000;",
        "PRAGMA synchronous=FULL; PRAGMA temp_store=FILE; PRAGMA mmap_size=0; PRAGMA cache_spill=2000; PRAGMA cache_size=1280;",
    );
    let retained = replace_once(
        retained,
        r#"        assert_eq!(pragma("cache_size"), 2_000);"#,
        r#"        assert_eq!(pragma("cache_size"), 1_280);"#,
    );
    assert_eq!(
        retained.matches("PRAGMA cache_size=1280;").count(),
        1,
        "retained source must contain exactly one 1,280-page Store cache setting",
    );
    assert_eq!(
        retained
            .matches(r#"assert_eq!(pragma("cache_size"), 1_280);"#)
            .count(),
        1,
        "retained Store-profile self-test must require the 1,280-page cache",
    );
    assert_eq!(
        retained.matches("sqlite3_db_release_memory").count(),
        0,
        "retained source must not contain experimental live-Store releases",
    );
    assert_eq!(
        retained.matches("sqlite3_release_memory(i32::MAX)").count(),
        0,
        "retained source must not contain an uncharged post-Store global release",
    );
    fs::write(output.join("retained_control.rs"), retained).unwrap();
    fs::write(
        output.join("phase4_g3_materialization.rs"),
        git_show("crates/layerfs-engine/src/bin/phase4_g3_materialization.rs"),
    )
    .unwrap();
