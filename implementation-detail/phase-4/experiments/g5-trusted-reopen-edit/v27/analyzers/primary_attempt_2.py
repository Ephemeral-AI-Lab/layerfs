...
OP_SCHEMA = "phase4-g5-1-operation-v27"
...
def expected_child_lifecycle(operations, campaign):
    ...
    for (sequence_id, pair), rows in pairs.items():
        ...
        required = [
            f"{int(row.get('wrapper', {}).get('ordinal')):03d}-{sequence_id}-{role}"
        ...
        scopes.append(
            {
                ...
                "row_q_zero": True,
                "pair_status": "PASS",
                "active_product_children_after_pair": 0,
...
def validate_prearm_initialization(
    ...
    if (
        ...
        != "phase4-g5-1-prearm-wrapper-initialization-evidence-v27"
...
def analyze(raw_path, timing_path, schedule_path, expected_path):
    ...
    sentinel_rows = [row for row in raw if row.get("schema") == "phase4-g5-1-protected-sentinel-v27"]
    ...
    initialization_rows = [
        row
        for row in raw
        if row.get("schema") == "phase4-g5-1-prearm-wrapper-initialization-v27"
    ]
    ...
    for sequence in schedule:
        if not schedule_execution_valid(sequence):
            failures.append(f"schedule-process-shape:{sequence.get('sequence_id')}")
    ...
    initialization_artifact = pathlib.Path(raw_path).parent / (
        "PREARM-WRAPPER-INITIALIZATION-v27.json"
    )
    try:
    ...
    for row in operations:
        wrapper = row.get("wrapper", {})
        label = f"{wrapper.get('sequence_id')}:{wrapper.get('pair')}:{wrapper.get('role')}"
        if wrapper.get("product_process_shape") != "one-shot":
            failures.append(f"process-shape:{label}")
        precondition = wrapper.get("database_precondition")
        clone_entries = wrapper.get("clone_receipt", {}).get("entries", [])
        if wrapper.get("role") == "g4_verified":
            if precondition != {
                ...
                failures.append(f"database-precondition:{label}")
        else:
            database_entries = [
            ...
            if (
                type(precondition) is not dict
                or precondition.get("classification") != "MirrorsFrozenG4InternalPreTimerDatabaseHashRead"
                or precondition.get("timing_claim") != "CacheWarmPreconditionedNotColdReopen"
                or precondition.get("buffer_bytes") != 65_536
                or type(precondition.get("wall_ns")) is not int
                or precondition.get("wall_ns") < 0
                or precondition.get("inside_complete_wall") is not True
                or precondition.get("inside_product_timer") is not False
                or precondition.get("layerfs_sql_or_authentication_work") is not False
                or len(database_entries) != 1
                or precondition.get("bytes") != database_entries[0].get("bytes")
            ):
                failures.append(f"database-precondition:{label}")
    ...
    for terminal in terminals:
        ...
        if (
            ...
            or terminal.get("expected_rows") != 1
    ...
    validate_child_lifecycle(
        ...
        sentinel_rows,
        campaign,
        pathlib.Path(raw_path).parent / "PRODUCT-CHILD-LIFECYCLE-v27.json",
        failures,
    )
    validate_work_root_lifecycle(
        work_root_rows,
        operations,
        pathlib.Path(raw_path).parent / "WORK-ROOT-LIFECYCLE-v27.json",
        failures,
    )
    if campaign == "screen":
        counts = {}
    ...
    for (comparison, operation), roles in sorted(grouped.items()):
        ...
        for pair in sorted(control_by_pair):
            ...
            if comparison == "g4-verified-vs-g5-verified":
                expected = {
                    "canonical_bytes_authenticated": left.get("canonical_bytes_authenticated", 0) + 65,
                    "objects_authenticated": left.get("objects_authenticated", 0) + 1,
                    "canonical_authentication_hash_bytes": left.get("canonical_authentication_hash_bytes", 0) + 65,
                    "canonical_authentication_hashes": left.get("canonical_authentication_hashes", 0) + 1,
                    "statement_cache_acquisitions": left.get("statement_cache_acquisitions", 0) + 1,
                    "sql_calls": left.get("sql_calls", 0) + 1,
                    "sql_query_calls": left.get("sql_query_calls", 0) + 1,
                    "sql_rows_returned": left.get("sql_rows_returned", 0) + 1,
                    "row_blob_reads": left.get("row_blob_reads", 0) + 1,
                    "row_blob_copy_bytes": left.get("row_blob_copy_bytes", 0) + 65,
                }
                for name, wanted in expected.items():
                    if right.get(name) != wanted:
                        failures.append(f"{comparison}:{operation}:pair-{pair}:{name}")
                for name in (
                    "reused_object_id_authentications", "reused_object_id_authentication_bytes",
                ...
                for name in comparison_parity_fields(comparison):
                    if left.get(name) != right.get(name):
                        failures.append(f"{comparison}:{operation}:pair-{pair}:{name}")
        result = {
        ...
        if comparison == "g4-verified-vs-g5-verified":
            ...
            if result["paired_median_improvement_basis_points"] < 9_000:
                failures.append(f"same-g5:{operation}:improvement")
            if operation == "first-edit-after-reopen" and result["candidate_p95_ns"] > 25_000_000:
                failures.append("same-g5:first-edit-after-reopen:p95-safety")
    ...
    return {"schema": "phase4-g5-1-primary-analysis-v27", "status": "PASS" if not failures else "REVISE", "normalized": normalized}


def self_check():
    valid_execution = {"category": "performance", "execution": "one-shot-sequence-scoped-balanced"}
    assert schedule_execution_valid(valid_execution)
    assert not schedule_execution_valid({**valid_execution, "execution": "sequence-scoped-persistent-balanced"})
    ...
    print(compact({"status": "PASS", "checks": 16, "mutations_rejected": sorted(mutations) + sorted(prearm_mutations) + ["legacy-catalog", "paired-receipt-hash", "persistent-process-shape"]}))
    return 0
