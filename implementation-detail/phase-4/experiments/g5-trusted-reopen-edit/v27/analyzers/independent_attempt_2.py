...
def schedule_execution_valid(sequence):
    expected = (
        "pair-scoped-native-semantic" if sequence.get("category") == "fault"
        else "one-shot" if sequence.get("category") == "sentinel"
        else "one-shot-sequence-scoped-balanced"
    )
    return sequence.get("execution") == expected
...
S07_G4_SHA256 = "34009a0345e383bebd31ebf83ce2558c71c1fc433eca496305ec7a8f1bd3f411"
...
S07_RANGE_MEASUREMENT = dict(
    ...
    canonical_bytes_authenticated=1_051_290, objects_authenticated=55,
)
S07_RANGE = dict(
...
S07_FULL_RANGE_SHAPES = [
    dict(label="zero", start=0, end=0, returned_bytes=0, canonical_bytes_authenticated=89, objects_authenticated=1),
    dict(label="first-byte", start=0, end=1, returned_bytes=1, canonical_bytes_authenticated=34_806, objects_authenticated=3),
    dict(label="cross-chunk", start=32_767, end=32_769, returned_bytes=2, canonical_bytes_authenticated=52_174, objects_authenticated=4),
    dict(label="last-byte", start=1_048_575, end=1_048_576, returned_bytes=1, canonical_bytes_authenticated=17_580, objects_authenticated=3),
    dict(label="eof", start=1_048_576, end=1_048_576, returned_bytes=0, canonical_bytes_authenticated=89, objects_authenticated=1),
]
S07_WORK_FIELDS = (
...
LIFECYCLE_SCHEMA = "phase4-g5-1-product-child-lifecycle-v27"
...
CLONE_SCHEMA = "g5-v27-native-clone-receipt-v1"
CLONE_TOP = frozenset((
...
PHYSICAL_SCHEMA = "g5-v27-physical-allocation-observation-v1"
...
WORK_ROOT_SCHEMA = "phase4-g5-1-work-root-lifecycle-v27"
...
INPUT_MANIFEST = pathlib.Path(__file__).resolve().parents[1] / "method/INPUT-MANIFEST-v27.tsv"
DRY_RUN = pathlib.Path(__file__).resolve().parents[1] / "DRY-RUN-v27.json"
...
def audit_lifecycle(
    ...
    for (sequence, pair), arm_rows in grouped.items():
        ...
        required = [
            f"{int(arm.get('wrapper', {}).get('ordinal')):03d}-{sequence}-{role}"
            for arm, role in zip(arm_rows, roles)
            if str(role).startswith("g5_")
        ]
        peak = 0
        for arm, role in zip(arm_rows, roles):
        ...
        expected_scopes.append({
            ...
            "pair_status": "PASS", "active_product_children_after_pair": 0,
...
def audit_prearm(
    ...
    facts = (
        ...
        type(evidence) is dict
        and evidence.get("schema") == "phase4-g5-1-prearm-wrapper-initialization-evidence-v27",
        type(evidence) is dict
...
def recompute(raw_file, timing_file, schedule_file, expected_file):
    ...
    rows = [value for value in documents if value.get("schema") == "phase4-g5-1-operation-v27"]
    ...
    sentinel = [value for value in documents if value.get("schema") == "phase4-g5-1-protected-sentinel-v27"]
    ...
    initialization_rows = [
        ...
        if value.get("schema") == "phase4-g5-1-prearm-wrapper-initialization-v27"
    ]
    ...
    for sequence in scheduled:
        if not schedule_execution_valid(sequence):
            problems.add(f"schedule-process-shape:{sequence.get('sequence_id')}")
    ...
    artifact = pathlib.Path(raw_file).parent / "PREARM-WRAPPER-INITIALIZATION-v27.json"
    try:
        artifact_bytes = artifact.read_bytes()
    except OSError:
        artifact_bytes = b""
    evidence_path = pathlib.Path(raw_file).parent / (
        "PREARM-WRAPPER-INITIALIZATION-EVIDENCE-v27.json"
    )
    try:
    ...
    for value in rows:
        meta = value.get("wrapper", {})
        if meta.get("product_process_shape") != "one-shot":
            problems.add(f"process-shape:{meta.get('sequence_id')}:{meta.get('pair')}:{meta.get('role')}")
        precondition = meta.get("database_precondition")
        clone_entries = meta.get("clone_receipt", {}).get("entries", [])
        precondition_label = f"database-precondition:{meta.get('sequence_id')}:{meta.get('pair')}:{meta.get('role')}"
        if meta.get("role") == "g4_verified":
            expected_precondition = dict(
                classification="NotAppliedFrozenG4HashesDatabaseInternallyBeforeProductTimer",
                timing_claim="CacheWarmPreconditionedNotColdReopen",
                bytes=0, buffer_bytes=0, wall_ns=0,
                inside_complete_wall=True, inside_product_timer=False,
                layerfs_sql_or_authentication_work=False,
            )
            if precondition != expected_precondition:
                problems.add(precondition_label)
        else:
            database_entries = tuple(
                entry for entry in clone_entries
                if entry.get("path") == (precondition or {}).get("database")
                and str(entry.get("path", "")).endswith(".sqlite")
            )
            checks = (
                type(precondition) is dict,
                type(precondition) is dict and precondition.get("classification") == "MirrorsFrozenG4InternalPreTimerDatabaseHashRead",
                type(precondition) is dict and precondition.get("timing_claim") == "CacheWarmPreconditionedNotColdReopen",
                type(precondition) is dict and precondition.get("buffer_bytes") == 65_536,
                type(precondition) is dict and type(precondition.get("wall_ns")) is int and precondition.get("wall_ns") >= 0,
                type(precondition) is dict and precondition.get("inside_complete_wall") is True,
                type(precondition) is dict and precondition.get("inside_product_timer") is False,
                type(precondition) is dict and precondition.get("layerfs_sql_or_authentication_work") is False,
                len(database_entries) == 1,
                len(database_entries) == 1 and precondition.get("bytes") == database_entries[0].get("bytes"),
            )
            if not all(checks):
                problems.add(precondition_label)
        ...
        if checkpoint:
            ...
            if any(type(number) is not int or number <= 0 for number in roundtrip_numbers) or not lifecycle_equation_valid(meta.get("role"), product):
    ...
    for end in ends:
        ...
        if type(end.get("rows")) is not int or end.get("rows") <= 0 or end.get("expected_rows") != 1 or end.get("rows") != end.get("expected_rows"):
    ...
    audit_lifecycle(
        ...
        sentinel,
        campaign,
        pathlib.Path(raw_file).parent / "PRODUCT-CHILD-LIFECYCLE-v27.json",
        problems,
    )
    audit_work_roots(
        work_root_rows,
        rows,
        pathlib.Path(raw_file).parent / "WORK-ROOT-LIFECYCLE-v27.json",
        problems,
    )
    if campaign == "screen":
        counts = {}
        ...
        for value in sentinel:
            ...
            range_facts = (
                ...
                if route == "range" else [
                    ...
                    type(ranges) is list and len(ranges) == len(S07_FULL_RANGE_SHAPES),
                    type(ranges) is list and len(ranges) == len(S07_FULL_RANGE_SHAPES)
                    and all(
                        all(observed.get(field) == wanted for field, wanted in expected_shape.items())
                        and type(observed.get("wall_ns")) is int and observed["wall_ns"] >= 0
            ...
            exact_facts = [
                ...
                product.get("base_copy_method") == S07_PRODUCT_BASE_COPY_METHOD,
    ...
    for comparison in ("g4-verified-vs-g5-verified", "g5-verified-vs-g5-trusted"):
        ...
        for operation in operations:
            ...
            for pair in sorted(left):
                ...
                left_product = left[pair]["product"]
                right_product = right[pair]["product"]
                if comparison == "g4-verified-vs-g5-verified":
                    authority_expected = dict(
                        canonical_bytes_authenticated=left_product.get("canonical_bytes_authenticated", 0) + 65,
                        objects_authenticated=left_product.get("objects_authenticated", 0) + 1,
                        canonical_authentication_hash_bytes=left_product.get("canonical_authentication_hash_bytes", 0) + 65,
                        canonical_authentication_hashes=left_product.get("canonical_authentication_hashes", 0) + 1,
                        statement_cache_acquisitions=left_product.get("statement_cache_acquisitions", 0) + 1,
                        sql_calls=left_product.get("sql_calls", 0) + 1,
                        sql_query_calls=left_product.get("sql_query_calls", 0) + 1,
                        sql_rows_returned=left_product.get("sql_rows_returned", 0) + 1,
                        row_blob_reads=left_product.get("row_blob_reads", 0) + 1,
                        row_blob_copy_bytes=left_product.get("row_blob_copy_bytes", 0) + 65,
                    )
                    for field, wanted in authority_expected.items():
                        if right_product.get(field) != wanted:
                            problems.add(f"{comparison}:{operation}:pair-{pair}:{field}")
                    for field in (
                        "reused_object_id_authentications", "reused_object_id_authentication_bytes",
                        "sql_execute_calls", "sql_rows_changed", "row_blob_writes",
                        "borrowed_row_blob_reads", "borrowed_row_blob_bytes",
                        "leaf_batch_queries", "leaf_batch_references", "leaf_batch_references_max",
                        "blob_opens", "blob_reads", "blob_writes",
                    ):
                        if left_product.get(field) != right_product.get(field):
                            problems.add(f"{comparison}:{operation}:pair-{pair}:{field}")
                else:
                    for field in comparison_parity_fields(comparison):
                        if left_product.get(field) != right_product.get(field):
                            problems.add(f"{comparison}:{operation}:pair-{pair}:{field}")
            cell = {
                "pairs": len(control),
            ...
            if comparison == "g4-verified-vs-g5-verified":
                ...
                if middle(gains) < 9_000:
                    problems.add(f"same-g5:{operation}:improvement")
                if operation == "first-edit-after-reopen" and nearest_rank(candidate, 95) > 25_000_000:
                    problems.add("same-g5:first-edit-after-reopen:p95-safety")
    ...
    return {
        "schema": "phase4-g5-1-independent-recomputation-v27",
...
def self_check():
    valid_execution = dict(category="performance", execution="one-shot-sequence-scoped-balanced")
    assert schedule_execution_valid(valid_execution)
    assert not schedule_execution_valid(dict(valid_execution, execution="sequence-scoped-persistent-balanced"))
    ...
    print(packed({"status": "PASS", "checks": 16, "mutations_rejected": sorted(mutations) + sorted(prearm_mutations) + ["legacy-catalog", "paired-receipt-hash", "persistent-process-shape"]}))
    return 0
