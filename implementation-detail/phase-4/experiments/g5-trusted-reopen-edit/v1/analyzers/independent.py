...
def recompute(raw_file, timing_file, schedule_file, expected_file):
    ...
    rows = [value for value in documents if value.get("schema") == "phase4-g5-1-operation-v1"]
    ends = [value for value in documents if value.get("schema") == "phase4-g5-trusted-child-terminal-v1"]
    ...
    for value in rows:
        ...
        for field in (
            "q_current", "transactions", "commits", "root_id", "transition_id",
            "logical_database_digest", "edit_base_complete_scrub_calls",
    ...
    for end in ends:
        ...
        for owner in ("argument_owners", "schedule_owners", "timing_owners", "report_owners"):
            if end.get(owner) != 0:
    ...
    for comparison in ("g4-verified-vs-g5-verified", "g5-verified-vs-g5-trusted"):
        ...
        for operation in operations:
            ...
            for pair in sorted(left):
                ...
                for field in ("root_id", "transition_id", "logical_database_digest"):
    ...
    return {
        "schema": "phase4-g5-1-independent-recomputation-v1",
