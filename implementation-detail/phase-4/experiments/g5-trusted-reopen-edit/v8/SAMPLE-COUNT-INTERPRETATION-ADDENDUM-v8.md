# G5-1 v8 sample-count interpretation addendum

This append-only note corrects one sentence in `REVIEW-SYNTHESIS-v8.md`
without changing `PREREGISTRATION-v8.md` or its schedule.
...
separately inside each comparison. V8 nevertheless prospectively froze the
...
That is a deliberate v8 method choice, not an exact restatement of an
...
forecast is `<=120 s`. If it cannot, v8 is preserved as `PREMEASUREMENT_REVISE`;
only a prospectively created v9 may adopt a narrower interpretation.
