...
REPO = HERE.parents[4]
METHOD = HERE / "method"
SCHEDULE = METHOD / "SCHEDULE-v8.tsv"
EXPECTED = METHOD / "EXPECTED-OUTCOMES-v8.tsv"
INPUT_MANIFEST = METHOD / "INPUT-MANIFEST-v8.tsv"
METHOD_MANIFEST = METHOD / "METHOD-MANIFEST-v8.tsv"
SOURCE_FREEZE = METHOD / "SOURCE-FREEZE-v8.json"
LIMITATIONS = HERE / "LIMITATIONS-v8.json"
STATIC_CLOSURE = HERE / "STATIC-CLOSURE-v8.json"
DRY_RUN = HERE / "DRY-RUN-v8.json"
...
INPUT_ROOT = REPO / f"target/phase4-g5-trusted-reopen-edit-inputs-{DATE}-v8"
SCREEN_RESULT = REPO / f"target/phase4-g5-trusted-reopen-edit-{DATE}-v8-screen"
GATE_RESULT = REPO / f"target/phase4-g5-trusted-reopen-edit-{DATE}-v8"

# Frozen v8 Rust transport.
G5_CHILD_BINARY = HERE / "g5-benchmark/target/release/layerfs-g5-trusted-child-v8"
FIXTURE_FLAG = "--g5-fixture"
PREPARE_FLAG = "--g5-prepare"
CHILD_FLAG = "--g5-child"
SEMANTIC_FLAG = "--g5-semantic"
CHILD_READY_SCHEMA = "phase4-g5-trusted-child-ready-v8"
CHILD_ENVELOPE_SCHEMA = "phase4-g5-trusted-child-row-v8"
CHILD_TERMINAL_SCHEMA = "phase4-g5-trusted-child-terminal-v8"
FIXTURE_SCHEMA = "phase4-g5-trusted-fixture-v8"
PREPARE_SCHEMA = "phase4-g5-trusted-prepare-v8"
SEMANTIC_SCHEMA = "phase4-g5-trusted-semantic-v8"
SEMANTIC_TERMINAL_SCHEMA = "phase4-g5-trusted-semantic-terminal-v8"
SENTINEL_SCHEMA = "phase4-g5-1-protected-sentinel-v8"
REQUEST_FIELDS = (
...
def method_source_names():
    fixed = {
        ...
        for path in (
            HERE / "PREREGISTRATION-v8.md",
            HERE / "REVIEW-SYNTHESIS-v8.md",
            HERE / "SAMPLE-COUNT-INTERPRETATION-ADDENDUM-v8.md",
            HERE / "V7-SUPERSESSION-v8.json",
            LIMITATIONS,
            HERE / "runner.py",
            PRIMARY,
            INDEPENDENT,
...
def write_input_and_method_manifests():
    ...
    freeze = {
        "schema": "phase4-g5-1-source-freeze-v8",
        ...
        "forecast_model": "dry-run-calibrated-v8-python-and-external-shasum",
...
def run_preparation_command(
    ...
    chronology = evidence_root / "CHRONOLOGY-v8.jsonl"
    ...
    write_json(
        intent,
        {
            "schema": "phase4-g5-1-preparation-command-intent-v8",
            "ordinal": ordinal,
            "label": label,
            "argv": command,
    ...
    terminal = {
        "schema": "phase4-g5-1-preparation-command-terminal-v8",
        "ordinal": ordinal,
        "label": label,
...
def prepare_inputs():
    ...
    if any(path.exists() for path in (INPUT_MANIFEST, METHOD_MANIFEST, SOURCE_FREEZE, DRY_RUN, STATIC_CLOSURE)):
        raise RuntimeError("v8 method/freeze evidence already exists")
    ...
    fsync_dir(INPUT_ROOT.parent)
    evidence_root = INPUT_ROOT / "preparation-evidence-v8"
    ...
    try:
        ...
        append_text(
            evidence_root / "CHRONOLOGY-v8.jsonl",
            compact(
                {
        ...
        write_json(
            INPUT_ROOT / "PREPARATION-FAILED-v8.json",
            {
                "schema": "phase4-g5-1-input-preparation-failure-v8",
                "status": "REVISE",
        ...
        raise
    preparation_manifest = evidence_root / "PREPARATION-EVIDENCE-MANIFEST-v8.tsv"
    write_text(
    ...
    write_json(
        INPUT_ROOT / "PREPARATION-CUSTODY-v8.json",
        {
            "schema": "phase4-g5-1-input-preparation-v8",
            "status": "PASS",
...
def verify_freeze(require_static=False, require_dry=False):
    ...
    if require_static:
        ...
        screen_terminal = SCREEN_RESULT / "TERMINAL-VERIFICATION-v8.json"
        screen_final_manifest = SCREEN_RESULT / "FINAL-ARTIFACT-HASHES-v8.tsv"
        screen_final_verification = SCREEN_RESULT / "FINAL-READONLY-VERIFICATION-v8.json"
        screen_complete_wall = SCREEN_RESULT / "COMPLETE-WALL-v8.json"
        required = {
            "schema": "phase4-g5-1-static-closure-v8",
            "status": "PASS",
            "source_freeze_sha256": sha256(SOURCE_FREEZE),
...
def gate_hash_bytes():
    ...
    preserved_v7_bytes = V7_PREMEASUREMENT.stat().st_size + V7_PREPARATION_AUDIT.stat().st_size
    preserved_v7_bytes += sum(path.stat().st_size for path in V7_PARTIAL_INPUT_ROOT.rglob("*") if path.is_file())
    ...
    def isolated_clone_bytes(master):
        prefix = f"{pathlib.Path(master).relative_to(INPUT_ROOT)}/"
        selected = [item["bytes"] for name, item in VERIFIED_INPUT_CUSTODY.items() if name.startswith(prefix)]
        if not selected:
            raise RuntimeError(f"gate clone hash custody shape mismatch: {master}")
        return sum(selected)

    observations = expanded_observations("gate")
    pre_dispatch_clone_bytes = sum(isolated_clone_bytes(master_path(row)) for row in observations)
    post_row_bytes = sum(row_custody_bytes(master_path(row)) for row in observations)
    frozen_g4_internal_bytes = sum(
        row_custody_bytes(master_path(row)) for row in observations if row["role"] == "g4_verified"
    )
    frozen_g4_self_hash_bytes = sum(
        G4_EXECUTABLE.stat().st_size for row in observations if row["role"] == "g4_verified"
    )
    ...
    components = {
        ...
        "preserved_v7_failure_audit_and_partial_tree": preserved_v7_bytes,
        ...
        "two_hundred_isolated_clone_pre_dispatch_hashes": pre_dispatch_clone_bytes,
        "frozen_g4_internal_pre_dispatch_hashes": frozen_g4_internal_bytes,
        "frozen_g4_per_row_executable_self_hashes": frozen_g4_self_hash_bytes,
...
def hash_calibration():
    ...
    return {
        "schema": "phase4-g5-1-hash-calibration-v8",
...
def dry_run():
    ...
    if full_wrapper_forecast_ns > LIMIT_NS["gate"]:
        raise RuntimeError("v8 PREMEASUREMENT_REVISE: conservative full-wrapper forecast exceeds 120 seconds")
    ...
    value = {
        ...
        "gate_sequences": 14,
        "gate_arm_observations": GATE_ARM_OBSERVATIONS,
        "sample_count_interpretation": "deliberately-stricter-v8-choice-not-unambiguous-user-minimum",
...
def release_lock(lock, result, terminal_verification=None, state="release"):
    ...
    try:
        ...
        attestation = result / "BENCHMARK-LOCK-RELEASE-ATTESTATION-v8.json"
        ...
        value = {
            "schema": "phase4-g5-1-lock-release-v8",
        ...
        write_json(result / "LOCK-RELEASE-v8.json", value)
        return value
    finally:
        os.close(lock["fd"])
        lock["fd"] = None
...
def clone_master(master, destination):
    ...
    for source in sorted(path for path in master.rglob("*") if path.is_file()):
        ...
        if expected:
            ...
            if copied_sha256 != expected["sha256"]:
                raise RuntimeError(f"isolated clone manifest hash mismatch: {copied}")
            item["manifest_sha256"] = expected["sha256"]
            item["pre_dispatch_sha256"] = copied_sha256
        custody.append(item)
...
def prepared_clone_custody(custody):
    values = {}
    for item in custody:
        path = item["path"]
        if path.endswith(".sqlite"):
            values["database_sha256"] = item.get("pre_dispatch_sha256")
        elif path.endswith(".sqlite.authority"):
            values["authority_sha256"] = item.get("pre_dispatch_sha256")
        elif path.endswith(".sqlite.expectations"):
            values["expectations_sha256"] = item.get("pre_dispatch_sha256")
    if set(values) != {"database_sha256", "authority_sha256", "expectations_sha256"} or None in values.values():
        raise RuntimeError("isolated clone prepared custody unavailable")
    return values
...
def run_semantic(executable, case, root, result, label):
    stdout = result / f"children-v8/{label}.stdout"
    stderr = result / f"children-v8/{label}.stderr"
    sidecar = result / f"time-v8/{label}.txt"
...
def validate_child_row(envelope):
    ...
    return {
        "schema": "phase4-g5-1-operation-v8",
...
class PersistentChild:
    def __init__(
        ...
        self.stdout_path = result / f"children-v8/{label}.stdout"
        self.stderr_path = result / f"children-v8/{label}.stderr"
        self.time_path = result / f"time-v8/{label}.txt"
...
def analyze(result):
    ...
    for analyzer, name in (
        (PRIMARY, "PRIMARY-ANALYSIS-v8.json"),
        (INDEPENDENT, "INDEPENDENT-RECOMPUTATION-v8.json"),
    ):
        output = result / name
        completed = subprocess.run(
            [
                sys.executable, str(analyzer), str(result / "RAW-v8.jsonl"),
                str(result / "TIMINGS-v8.tsv"), str(SCHEDULE), str(EXPECTED), str(output),
            ],
            cwd=REPO,
            text=True,
            capture_output=True,
    ...
    write_json(
        result / "ANALYZER-AGREEMENT-v8.json",
        {
            "schema": "phase4-g5-1-analyzer-agreement-v8",
...
def ladder_prelock(campaign):
    ...
    if campaign == "gate":
        paths.extend(
            (
                SCREEN_RESULT / "TERMINAL-VERIFICATION-v8.json",
                SCREEN_RESULT / "FINAL-ARTIFACT-HASHES-v8.tsv",
                SCREEN_RESULT / "FINAL-READONLY-VERIFICATION-v8.json",
                SCREEN_RESULT / "COMPLETE-WALL-v8.json",
                STATIC_CLOSURE,
            )
        )
...
def run_campaign(campaign):
    ...
    try:
        ...
        work = result.parent / f"{result.name}-work-v8"
        work.mkdir(mode=0o700)
        for name in ("operands-v8", "children-v8", "time-v8"):
            (result / name).mkdir()
        fsync_dir(result)
        fsync_dir(result.parent)

        g4_copy = result / "operands-v8/frozen-g4-verified"
        g5_copy = result / "operands-v8/g5-verified-trusted"
        ...
        write_json(
            result / "OPERAND-CUSTODY-v8.json",
            {
                "schema": "phase4-g5-1-operand-custody-v8",
        ...
        write_json(
            result / "PREFLIGHT-v8.json",
            {
                "schema": "phase4-g5-1-preflight-v8",
                "status": "PASS",
                "campaign": campaign,
                "branch": BRANCH,
                "checkpoint": CHECKPOINT,
        ...
        write_json(
            result / "ENVIRONMENT-v8.json",
            {
                "schema": "phase4-g5-1-environment-v8",
        ...
        shutil.copyfile(INPUT_MANIFEST, result / "INPUT-CUSTODY-v8.tsv")
        with (result / "INPUT-CUSTODY-v8.tsv").open("rb") as handle:
            os.fsync(handle.fileno())
        ...
        try:
            for observation in observations:
                ...
                pre_dispatch_custody = prepared_clone_custody(custody)
                expected_custody = master_custody[master_path(observation)]
                if pre_dispatch_custody != expected_custody:
                    raise RuntimeError(f"isolated pre-dispatch custody mismatch: {observation['ordinal']}")
                    ...
                    semantics.extend(
                        {
                            ...
                            "expectation_id": "native-semantic-v8",
                            "status": value["status"],
                            "error": value["error"],
                        }
                        for value in values
        ...
        write_json(result / "COMMANDS-v8.json", {"schema": "phase4-g5-1-commands-v8", "commands": commands})
        analyze(result)

        if work.parent != result.parent or not work.name.endswith("-work-v8"):
            raise RuntimeError("refusing unsafe work cleanup")
        shutil.rmtree(work)
        fsync_dir(work.parent)
        residue = [str(path) for path in result.parent.glob(f"{result.name}-work-v8")]
        write_json(
            result / "CLEANUP-v8.json",
            {
                "schema": "phase4-g5-1-cleanup-v8",
        ...
        payload_excluded = {
            "PAYLOAD-MANIFEST-v8.tsv", "MEASURED-TERMINAL-v8.json",
            "TERMINAL-VERIFICATION-v8.json", "COMPLETE-WALL-v8.json",
            "BENCHMARK-LOCK-RELEASE-ATTESTATION-v8.json", "LOCK-RELEASE-v8.json",
            "FINAL-ARTIFACT-HASHES-v8.tsv", "FINAL-READONLY-VERIFICATION-v8.json",
        }
        payload = result / "PAYLOAD-MANIFEST-v8.tsv"
        ...
        terminal = result / "MEASURED-TERMINAL-v8.json"
        write_json(
            terminal,
            {
                "schema": "phase4-g5-1-measured-terminal-v8",
                "status": "PASS",
                "campaign": campaign,
                "rows": len(timings),
        ...
        verification = result / "TERMINAL-VERIFICATION-v8.json"
        write_json(
            verification,
            {
                "schema": "phase4-g5-1-terminal-verification-v8",
                "status": "PASS",
        ...
        final = result / "FINAL-ARTIFACT-HASHES-v8.tsv"
        write_text(
            final,
            manifest_text(
                result,
                excluded={final.name, "FINAL-READONLY-VERIFICATION-v8.json", "COMPLETE-WALL-v8.json"},
            ),
        )
        final_count = verify_manifest(result, final, "result_relative_path")
        write_json(
            result / "FINAL-READONLY-VERIFICATION-v8.json",
            {
                "schema": "phase4-g5-1-final-readonly-verification-v8",
                "status": "PASS",
        ...
        write_json(
            result / "COMPLETE-WALL-v8.json",
            {
                "schema": "phase4-g5-1-complete-wall-v8",
                "status": "PASS",
                "campaign": campaign,
                ...
                "terminal_self_exclusion": "COMPLETE-WALL-v8.json follows the verified final manifest",
            },
        )
        fsync_dir(result)
        ...
        if result.exists() and not (result / "FAILED-v8.json").exists():
            write_json(
                result / "FAILED-v8.json",
                {
                    "schema": "phase4-g5-1-failure-v8",
                    "status": "REVISE",
                    "error": str(error),
