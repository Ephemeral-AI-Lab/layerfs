...
OP_SCHEMA = "phase4-g5-1-operation-v8"
TERMINAL_SCHEMA = "phase4-g5-trusted-child-terminal-v8"
...
def analyze(raw_path, timing_path, schedule_path, expected_path):
    ...
    semantic_rows = [row for row in raw if row.get("schema") == "phase4-g5-trusted-semantic-v8"]
    semantic_terminals = [
        row for row in raw if row.get("schema") == "phase4-g5-trusted-semantic-terminal-v8"
    ]
    sentinel_rows = [row for row in raw if row.get("schema") == "phase4-g5-1-protected-sentinel-v8"]
    ...
    for row in operations:
        ...
        clone_custody = wrapper.get("base_custody")
        derived_pre = {}
        for item in clone_custody if isinstance(clone_custody, list) else ():
            if str(item.get("path", "")).endswith(".sqlite"):
                derived_pre["database_sha256"] = item.get("pre_dispatch_sha256")
            elif str(item.get("path", "")).endswith(".sqlite.authority"):
                derived_pre["authority_sha256"] = item.get("pre_dispatch_sha256")
            elif str(item.get("path", "")).endswith(".sqlite.expectations"):
                derived_pre["expectations_sha256"] = item.get("pre_dispatch_sha256")
        if (
            set(pre_dispatch) != {"database_sha256", "authority_sha256", "expectations_sha256"}
            or not isinstance(clone_custody, list)
            or any(item.get("pre_dispatch_sha256") != item.get("manifest_sha256") for item in clone_custody)
            or derived_pre != pre_dispatch
        ):
            failures.append(f"{label}:pre-dispatch-custody")
    ...
    return {"schema": "phase4-g5-1-primary-analysis-v8", "status": "PASS" if not failures else "REVISE", "normalized": normalized}
