# Phase 4 G5-1 v8 preregistration
...
Any failure preserves v8. No row is deleted, replaced, reordered, or selectively
rerun. A source or method repair creates v9.
...
The 20-pair primary is 10 AB/10 BA. The six five-pair secondary shapes
alternate which role receives the fifth first position by operation, producing
exactly 15 AB/15 BA secondary first positions inside each comparison.

The zero-row dry-run performs one labeled Python SHA-256 pass and one external
`shasum` pass over the frozen 100-MiB source, admits no Store, row, timer, or
copy, and sets the forecast hash floor to at most half the slower observed
throughput. It computes the exact preregistered gate hash-byte plan; forecast
failure preserves v8 without reducing rows.

## Preparation evidence law

V8 uses a new input root and never reads, repairs, removes, or reuses the v7
partial root. Before each fixture or prepared-master subprocess, it durably
writes an exclusive intent and chronology-start event. After return it writes
and fsyncs exact byte-mode stdout and stderr plus a terminal record containing
return code, times, hashes, and postinventory before any UTF-8 or JSON parse.
Fixture and prepare transports must emit empty stderr and exactly one
newline-terminated UTF-8 JSON object with the native v8 schema; no diagnostic
line is filtered or tolerated. First failure writes durable partial custody and
stops without manifest, freeze, sealing, cleanup, or retry. Complete success
verifies a preparation-evidence manifest and custody record before input/method
manifests and source freeze, then seals, fsyncs, and rehashes the input tree.
...
Screen requires the hash-bound dry-run. Static closure must postdate and bind
the screen terminal, complete wall, final manifest, and final read-only
verification. Gate rechecks that chain before and after acquiring the global
lock. Complete wall ends only after lock release, final manifest verification,
and final read-only verification are fsynced.
