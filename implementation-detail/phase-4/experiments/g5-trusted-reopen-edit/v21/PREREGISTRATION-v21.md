# Phase 4 G5-1 v21 preregistration

V21 preserves v18's 200-row RSS REVISE. It removes the ineffective allocator
...
V21 preserves the v17 attempt-2 200-row REVISE. It retains the K64 scrub and
...
V21 preserves v19's 115.699891208-second RSS REVISE and reduces only the
...
V21 preserves v20's 115.738195625-second RSS REVISE; its sole change is the
...
The candidate remains CAS + CDC + COW + canonical K64/F64 + SQLite. V21 changes
...
Any failure preserves v21. No observed row may be deleted, replaced, reordered,
or selectively rerun. A source or method repair creates v21.
...
`phase4-g5-1-wrapper-calibration-intent-v21`,
`phase4-g5-1-wrapper-initialization-sample-v21`,
`phase4-g5-1-wrapper-calibration-sample-v21`, and
`phase4-g5-1-wrapper-calibration-result-v21`.
...
`phase4-g5-1-wrapper-calibration-plan-v21` and records the complete candidate
...
must write and fsync the top-level `DRY-RUN-INTENT-v21.json` before invoking
...
write and fsync `DRY-RUN-FAILED-v21.json` first, then write and fsync a
`DRY-RUN-DISPOSITION-v21.json` with `status=REVISE` that binds the FAILED hash
and explicitly records absent optional artifacts. A success or calibrated
overrun writes and fsyncs complete `DRY-RUN-v21.json`, including both
...
`PREMEASUREMENT-REVISE-v21.json` and stops before screen. A missing dry-run or
...
V14 is byte-preserved by `V14-SUPERSESSION-v21.json`. Its zero-row passed, and
...
V21 changes the shared product timer equation and uses a fresh release. The
...
separate complete-lifecycle evidence. V21 reuses only the independently rehashed
...
are not copied, renamed, or represented as v21-generated. V21 reuses no v14
product row or timing observation. The new v21 input manifest rebinds the same sealed
files byte-for-byte before the v21 method manifest and source freeze are
generated.

V15 attempts 1–5 remain append-only and are not measurement authority for v21.
...
while the lifecycle sum added that authority again. V21 removes only that term
...
rows. Its G5 rows confirm the lifecycle equation repair. V21 changes only the
...
The final `PREARM-WRAPPER-INITIALIZATION-v21.json` write is outside that bound
...
schedule, expectation, limitation, and supersession byte has settled, v21
...
child. It writes `FREEZE-VERIFICATION-v21.json` with classification
...
V21 voluntarily freezes a deliberately stricter v1-lineage design: each
...
`phase4-g5-1-product-child-lifecycle-v21`. The two copies must be byte-value
...
child, v21 verifies the complete sealed input manifest exactly once: path,
...
`g5-v21-native-clone-receipt-v1`. Its ordered `entries` bind each source master
...
inspection for a future separate API. V21 therefore removes the external
...
`g5-v21-rooted-logical-state-v1` from one query-only, autocommit, constant-row
...
V21 does not claim physical SQLite file-byte parity. The sibling
`physical_allocation_observation` remains schema
`g5-v21-physical-allocation-observation-v1`, classification
...
Each wrapper receipt uses schema `phase4-g5-1-arm-cleanup-receipt-v21` and is
repeated in order by the raw and standalone
`phase4-g5-1-work-root-lifecycle-v21` record. Started and cleaned roots equal
