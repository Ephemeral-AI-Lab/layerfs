# G5-1 v21 review synthesis

V20 completed all 200 rows below 150 seconds and missed RSS only for one
Verified child by 81,920 bytes. V21 halves only the scrub statement batch width
...
the 20-MiB cap. V21 keeps the authenticated batch mechanism and uses width 32
...
G5 processes exceeded the 20-MiB per-child RSS cap. V21 changes only the common
...
20,971,520-byte cap. V21 releases free allocator pages only between rows after
...
## V16 screen and accepted v21 analyzer repair
...
V21 changes only those two analyzer predicates and the versioned child/report
...
product row or timing is promoted to v21 authority.

V21 keeps the 200-arm schedule, 56 checkpoints, half-slower calibration floor,
...
- use native APFS clone receipts, schema `g5-v21-native-clone-receipt-v1`, to
...
- emit constant-size rooted state, schema `g5-v21-rooted-logical-state-v1`,
...
`FREEZE-VERIFICATION-v21.json` is classified
...
`PRODUCT-CHILD-LIFECYCLE-v21.json` under schema
`phase4-g5-1-product-child-lifecycle-v21`. Per-arm cleanup receipts use schema
`phase4-g5-1-arm-cleanup-receipt-v21`; their ordered terminal record is likewise
duplicated as `WORK-ROOT-LIFECYCLE-v21.json` under schema
`phase4-g5-1-work-root-lifecycle-v21`.
...
`wrapper-calibration-intent-v21`, `wrapper-initialization-sample-v21`,
`wrapper-calibration-sample-v21`, and `wrapper-calibration-result-v21`. The
...
intent embeds `phase4-g5-1-wrapper-calibration-plan-v21`, retains the complete
...
`phase4-g5-1-prearm-wrapper-initialization-v21` record after lock/custody and
...
closes v21 as `PREMEASUREMENT_REVISE` before screen; no schedule or threshold
is reduced.

`DRY-RUN-INTENT-v21.json` is durably written before either calibration begins.
A single outer failure funnel then covers wrapper calibration, hash calibration,
forecasting, and sealing: it always writes `DRY-RUN-FAILED-v21.json` before a
REVISE `DRY-RUN-DISPOSITION-v21.json`, even when the wrapper result is absent.
...
V21 nevertheless voluntarily freezes the stricter v1-lineage design: each
