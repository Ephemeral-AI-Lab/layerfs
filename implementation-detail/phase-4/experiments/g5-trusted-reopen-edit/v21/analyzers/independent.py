...
LIFECYCLE_SCHEMA = "phase4-g5-1-product-child-lifecycle-v21"
...
CLONE_SCHEMA = "g5-v21-native-clone-receipt-v1"
CLONE_TOP = frozenset((
...
PHYSICAL_SCHEMA = "g5-v21-physical-allocation-observation-v1"
...
WORK_ROOT_SCHEMA = "phase4-g5-1-work-root-lifecycle-v21"
...
INPUT_MANIFEST = pathlib.Path(__file__).resolve().parents[1] / "method/INPUT-MANIFEST-v21.tsv"
DRY_RUN = pathlib.Path(__file__).resolve().parents[1] / "DRY-RUN-v21.json"
...
def audit_prearm(
    ...
    facts = (
        ...
        type(evidence) is dict
        and evidence.get("schema") == "phase4-g5-1-prearm-wrapper-initialization-evidence-v21",
        type(evidence) is dict
...
def recompute(raw_file, timing_file, schedule_file, expected_file):
    ...
    rows = [value for value in documents if value.get("schema") == "phase4-g5-1-operation-v21"]
    ...
    sentinel = [value for value in documents if value.get("schema") == "phase4-g5-1-protected-sentinel-v21"]
    ...
    initialization_rows = [
        ...
        if value.get("schema") == "phase4-g5-1-prearm-wrapper-initialization-v21"
    ]
    ...
    artifact = pathlib.Path(raw_file).parent / "PREARM-WRAPPER-INITIALIZATION-v21.json"
    try:
        artifact_bytes = artifact.read_bytes()
    except OSError:
        artifact_bytes = b""
    evidence_path = pathlib.Path(raw_file).parent / (
        "PREARM-WRAPPER-INITIALIZATION-EVIDENCE-v21.json"
    )
    try:
    ...
    audit_lifecycle(
        ...
        sentinel,
        campaign,
        pathlib.Path(raw_file).parent / "PRODUCT-CHILD-LIFECYCLE-v21.json",
        problems,
    )
    audit_work_roots(
        work_root_rows,
        rows,
        pathlib.Path(raw_file).parent / "WORK-ROOT-LIFECYCLE-v21.json",
        problems,
    )
    if campaign == "screen":
        counts = {}
            ...
            for pair in sorted(left):
                ...
                if comparison == "g4-verified-vs-g5-verified":
                    ...
                    if not (
                        ...
                        and type(query_count) is int and query_count == -(-reference_count // 16)
                        and type(batch_max) is int and 0 < batch_max <= 16
                    ):
    ...
    return {
        "schema": "phase4-g5-1-independent-recomputation-v21",
