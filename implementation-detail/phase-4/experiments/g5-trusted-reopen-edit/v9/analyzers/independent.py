...
def check_catalog(meta, product, problems, tag):
    ...
    page_size = catalog.get("sqlite_page_size", 0)
    facts = (
        ...
        numeric,
        512 <= page_size <= 65_536 and page_size & (page_size - 1) == 0,
        catalog.get("sqlite_page_count", 0) > 0,
        catalog.get("sqlite_freelist_count", 1) <= catalog.get("sqlite_page_count", 0),
        catalog.get("sqlite_logical_database_bytes") == page_size * catalog.get("sqlite_page_count", 0),
        catalog.get("sqlite_logical_database_bytes") == product.get("sqlite_post_logical_database_bytes"),
...
def recompute(raw_file, timing_file, schedule_file, expected_file):
    ...
    rows = [value for value in documents if value.get("schema") == "phase4-g5-1-operation-v9"]
    ends = [value for value in documents if value.get("schema") == "phase4-g5-trusted-child-terminal-v9"]
    semantic = [value for value in documents if value.get("schema") == "phase4-g5-trusted-semantic-v9"]
    semantic_ends = [value for value in documents if value.get("schema") == "phase4-g5-trusted-semantic-terminal-v9"]
    sentinel = [value for value in documents if value.get("schema") == "phase4-g5-1-protected-sentinel-v9"]
    ...
    return {
        "schema": "phase4-g5-1-independent-recomputation-v9",
