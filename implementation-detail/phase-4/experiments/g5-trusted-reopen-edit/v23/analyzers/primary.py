...
OP_SCHEMA = "phase4-g5-1-operation-v23"
...
def validate_prearm_initialization(
    ...
    if (
        ...
        != "phase4-g5-1-prearm-wrapper-initialization-evidence-v23"
...
def analyze(raw_path, timing_path, schedule_path, expected_path):
    ...
    sentinel_rows = [row for row in raw if row.get("schema") == "phase4-g5-1-protected-sentinel-v23"]
    ...
    initialization_rows = [
        row
        for row in raw
        if row.get("schema") == "phase4-g5-1-prearm-wrapper-initialization-v23"
    ]
    ...
    initialization_artifact = pathlib.Path(raw_path).parent / (
        "PREARM-WRAPPER-INITIALIZATION-v23.json"
    )
    try:
    ...
    validate_child_lifecycle(
        ...
        sentinel_rows,
        campaign,
        pathlib.Path(raw_path).parent / "PRODUCT-CHILD-LIFECYCLE-v23.json",
        failures,
    )
    validate_work_root_lifecycle(
        work_root_rows,
        operations,
        pathlib.Path(raw_path).parent / "WORK-ROOT-LIFECYCLE-v23.json",
        failures,
    )
    if campaign == "screen":
        counts = {}
    ...
    for (comparison, operation), roles in sorted(grouped.items()):
        ...
        for pair in sorted(control_by_pair):
            ...
            if comparison == "g4-verified-vs-g5-verified":
                references, batches, maximum = {
                    "plus1-early": (83, 2, 64),
                    "plus1-middle": (42, 1, 42),
                }.get(operation, (0, 0, 0))
                expected_batch = {
                    "leaf_batch_references": left.get("leaf_batch_references", 0) + references,
                    "leaf_batch_queries": left.get("leaf_batch_queries", 0) + batches,
                    "leaf_batch_references_max": max(
                        left.get("leaf_batch_references_max", 0), maximum
                    ),
                }
                for name, wanted in expected_batch.items():
                ...
                expected = {
                    ...
                    "statement_cache_acquisitions": left.get("statement_cache_acquisitions", 0) + 1 - references,
                    "sql_calls": left.get("sql_calls", 0) + 1 - references + batches,
                    "sql_query_calls": left.get("sql_query_calls", 0) + 1 - references + batches,
                    "sql_rows_returned": left.get("sql_rows_returned", 0) + 1,
                    "row_blob_reads": left.get("row_blob_reads", 0) + 1,
                    "borrowed_row_blob_reads": left.get("borrowed_row_blob_reads", 0) + references,
                }
                for name, wanted in expected.items():
                    if right.get(name) != wanted:
                        failures.append(f"{comparison}:{operation}:pair-{pair}:{name}")
                if (
                    right.get("row_blob_copy_bytes", 0) + right.get("borrowed_row_blob_bytes", 0)
                    != left.get("row_blob_copy_bytes", 0) + left.get("borrowed_row_blob_bytes", 0) + 65
                ):
                    failures.append(f"{comparison}:{operation}:pair-{pair}:authenticated-blob-bytes")
                for name in (
                    "reused_object_id_authentications", "reused_object_id_authentication_bytes",
    ...
    return {"schema": "phase4-g5-1-primary-analysis-v23", "status": "PASS" if not failures else "REVISE", "normalized": normalized}
