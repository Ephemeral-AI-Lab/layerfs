...
LIFECYCLE_SCHEMA = "phase4-g5-1-product-child-lifecycle-v23"
...
CLONE_SCHEMA = "g5-v23-native-clone-receipt-v1"
CLONE_TOP = frozenset((
...
PHYSICAL_SCHEMA = "g5-v23-physical-allocation-observation-v1"
...
WORK_ROOT_SCHEMA = "phase4-g5-1-work-root-lifecycle-v23"
...
INPUT_MANIFEST = pathlib.Path(__file__).resolve().parents[1] / "method/INPUT-MANIFEST-v23.tsv"
DRY_RUN = pathlib.Path(__file__).resolve().parents[1] / "DRY-RUN-v23.json"
...
def audit_prearm(
    ...
    facts = (
        ...
        type(evidence) is dict
        and evidence.get("schema") == "phase4-g5-1-prearm-wrapper-initialization-evidence-v23",
        type(evidence) is dict
...
def recompute(raw_file, timing_file, schedule_file, expected_file):
    ...
    rows = [value for value in documents if value.get("schema") == "phase4-g5-1-operation-v23"]
    ...
    sentinel = [value for value in documents if value.get("schema") == "phase4-g5-1-protected-sentinel-v23"]
    ...
    initialization_rows = [
        ...
        if value.get("schema") == "phase4-g5-1-prearm-wrapper-initialization-v23"
    ]
    ...
    artifact = pathlib.Path(raw_file).parent / "PREARM-WRAPPER-INITIALIZATION-v23.json"
    try:
        artifact_bytes = artifact.read_bytes()
    except OSError:
        artifact_bytes = b""
    evidence_path = pathlib.Path(raw_file).parent / (
        "PREARM-WRAPPER-INITIALIZATION-EVIDENCE-v23.json"
    )
    try:
    ...
    audit_lifecycle(
        ...
        sentinel,
        campaign,
        pathlib.Path(raw_file).parent / "PRODUCT-CHILD-LIFECYCLE-v23.json",
        problems,
    )
    audit_work_roots(
        work_root_rows,
        rows,
        pathlib.Path(raw_file).parent / "WORK-ROOT-LIFECYCLE-v23.json",
        problems,
    )
    if campaign == "screen":
        counts = {}
    ...
    for comparison in ("g4-verified-vs-g5-verified", "g5-verified-vs-g5-trusted"):
        ...
        for operation in operations:
            ...
            for pair in sorted(left):
                ...
                if comparison == "g4-verified-vs-g5-verified":
                    reference_count, query_count, batch_max = {
                        "plus1-early": (83, 2, 64),
                        "plus1-middle": (42, 1, 42),
                    }.get(operation, (0, 0, 0))
                    batch_expected = dict(
                        leaf_batch_references=left_product.get("leaf_batch_references", 0) + reference_count,
                        leaf_batch_queries=left_product.get("leaf_batch_queries", 0) + query_count,
                        leaf_batch_references_max=max(
                            left_product.get("leaf_batch_references_max", 0), batch_max
                        ),
                    )
                    for field, wanted in batch_expected.items():
                    ...
                    authority_expected = dict(
                        ...
                        statement_cache_acquisitions=left_product.get("statement_cache_acquisitions", 0) + 1 - reference_count,
                        sql_calls=left_product.get("sql_calls", 0) + 1 - reference_count + query_count,
                        sql_query_calls=left_product.get("sql_query_calls", 0) + 1 - reference_count + query_count,
                        sql_rows_returned=left_product.get("sql_rows_returned", 0) + 1,
                        row_blob_reads=left_product.get("row_blob_reads", 0) + 1,
                        borrowed_row_blob_reads=left_product.get("borrowed_row_blob_reads", 0) + reference_count,
                    )
                    for field, wanted in authority_expected.items():
                        if right_product.get(field) != wanted:
                            problems.add(f"{comparison}:{operation}:pair-{pair}:{field}")
                    if (
                        right_product.get("row_blob_copy_bytes", 0) + right_product.get("borrowed_row_blob_bytes", 0)
                        != left_product.get("row_blob_copy_bytes", 0) + left_product.get("borrowed_row_blob_bytes", 0) + 65
                    ):
                        problems.add(f"{comparison}:{operation}:pair-{pair}:authenticated-blob-bytes")
                    for field in (
                        "reused_object_id_authentications", "reused_object_id_authentication_bytes",
    ...
    return {
        "schema": "phase4-g5-1-independent-recomputation-v23",
