...
20,971,520-byte cap. V18 releases free allocator pages only between rows after
...
## V16 screen and accepted v18 analyzer repair
...
V18 changes only those two analyzer predicates and the versioned child/report
...
product row or timing is promoted to v18 authority.

V18 keeps the 200-arm schedule, 56 checkpoints, half-slower calibration floor,
...
- use native APFS clone receipts, schema `g5-v18-native-clone-receipt-v1`, to
...
- emit constant-size rooted state, schema `g5-v18-rooted-logical-state-v1`,
...
`FREEZE-VERIFICATION-v18.json` is classified
...
`PRODUCT-CHILD-LIFECYCLE-v18.json` under schema
`phase4-g5-1-product-child-lifecycle-v18`. Per-arm cleanup receipts use schema
`phase4-g5-1-arm-cleanup-receipt-v18`; their ordered terminal record is likewise
duplicated as `WORK-ROOT-LIFECYCLE-v18.json` under schema
`phase4-g5-1-work-root-lifecycle-v18`.
...
`wrapper-calibration-intent-v18`, `wrapper-initialization-sample-v18`,
`wrapper-calibration-sample-v18`, and `wrapper-calibration-result-v18`. The
...
intent embeds `phase4-g5-1-wrapper-calibration-plan-v18`, retains the complete
...
`phase4-g5-1-prearm-wrapper-initialization-v18` record after lock/custody and
...
closes v18 as `PREMEASUREMENT_REVISE` before screen; no schedule or threshold
is reduced.

`DRY-RUN-INTENT-v18.json` is durably written before either calibration begins.
A single outer failure funnel then covers wrapper calibration, hash calibration,
forecasting, and sealing: it always writes `DRY-RUN-FAILED-v18.json` before a
REVISE `DRY-RUN-DISPOSITION-v18.json`, even when the wrapper result is absent.
...
V18 nevertheless voluntarily freezes the stricter v1-lineage design: each
