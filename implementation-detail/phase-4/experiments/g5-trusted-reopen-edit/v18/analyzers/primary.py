...
OP_SCHEMA = "phase4-g5-1-operation-v18"
...
def validate_prearm_initialization(
    ...
    if (
        ...
        != "phase4-g5-1-prearm-wrapper-initialization-evidence-v18"
...
def analyze(raw_path, timing_path, schedule_path, expected_path):
    ...
    sentinel_rows = [row for row in raw if row.get("schema") == "phase4-g5-1-protected-sentinel-v18"]
    ...
    initialization_rows = [
        row
        for row in raw
        if row.get("schema") == "phase4-g5-1-prearm-wrapper-initialization-v18"
    ]
    ...
    initialization_artifact = pathlib.Path(raw_path).parent / (
        "PREARM-WRAPPER-INITIALIZATION-v18.json"
    )
    try:
    ...
    validate_child_lifecycle(
        ...
        sentinel_rows,
        campaign,
        pathlib.Path(raw_path).parent / "PRODUCT-CHILD-LIFECYCLE-v18.json",
        failures,
    )
    validate_work_root_lifecycle(
        work_root_rows,
        operations,
        pathlib.Path(raw_path).parent / "WORK-ROOT-LIFECYCLE-v18.json",
        failures,
    )
    if campaign == "screen":
        counts = {}
        ...
        for pair in sorted(control_by_pair):
            ...
            if comparison == "g4-verified-vs-g5-verified":
                ...
                if (
                    ...
                    or type(batches) is not int or batches != (references + 63) // 64
    ...
    return {"schema": "phase4-g5-1-primary-analysis-v18", "status": "PASS" if not failures else "REVISE", "normalized": normalized}
