# Phase 4 G5-1 v18 preregistration

V18 preserves the v17 attempt-2 200-row REVISE. It retains the K64 scrub and
...
The candidate remains CAS + CDC + COW + canonical K64/F64 + SQLite. V18 changes
...
Any failure preserves v18. No observed row may be deleted, replaced, reordered,
or selectively rerun. A source or method repair creates v18.
...
`phase4-g5-1-wrapper-calibration-intent-v18`,
`phase4-g5-1-wrapper-initialization-sample-v18`,
`phase4-g5-1-wrapper-calibration-sample-v18`, and
`phase4-g5-1-wrapper-calibration-result-v18`.
...
`phase4-g5-1-wrapper-calibration-plan-v18` and records the complete candidate
...
must write and fsync the top-level `DRY-RUN-INTENT-v18.json` before invoking
...
write and fsync `DRY-RUN-FAILED-v18.json` first, then write and fsync a
`DRY-RUN-DISPOSITION-v18.json` with `status=REVISE` that binds the FAILED hash
and explicitly records absent optional artifacts. A success or calibrated
overrun writes and fsyncs complete `DRY-RUN-v18.json`, including both
...
`PREMEASUREMENT-REVISE-v18.json` and stops before screen. A missing dry-run or
...
V14 is byte-preserved by `V14-SUPERSESSION-v18.json`. Its zero-row passed, and
...
V18 changes the shared product timer equation and uses a fresh release. The
...
separate complete-lifecycle evidence. V18 reuses only the independently rehashed
...
are not copied, renamed, or represented as v18-generated. V18 reuses no v14
product row or timing observation. The new v18 input manifest rebinds the same sealed
files byte-for-byte before the v18 method manifest and source freeze are
generated.

V15 attempts 1–5 remain append-only and are not measurement authority for v18.
...
while the lifecycle sum added that authority again. V18 removes only that term
...
rows. Its G5 rows confirm the lifecycle equation repair. V18 changes only the
...
The final `PREARM-WRAPPER-INITIALIZATION-v18.json` write is outside that bound
...
schedule, expectation, limitation, and supersession byte has settled, v18
...
child. It writes `FREEZE-VERIFICATION-v18.json` with classification
...
V18 voluntarily freezes a deliberately stricter v1-lineage design: each
...
`phase4-g5-1-product-child-lifecycle-v18`. The two copies must be byte-value
...
child, v18 verifies the complete sealed input manifest exactly once: path,
...
`g5-v18-native-clone-receipt-v1`. Its ordered `entries` bind each source master
...
inspection for a future separate API. V18 therefore removes the external
...
`g5-v18-rooted-logical-state-v1` from one query-only, autocommit, constant-row
...
V18 does not claim physical SQLite file-byte parity. The sibling
`physical_allocation_observation` remains schema
`g5-v18-physical-allocation-observation-v1`, classification
...
Each wrapper receipt uses schema `phase4-g5-1-arm-cleanup-receipt-v18` and is
repeated in order by the raw and standalone
`phase4-g5-1-work-root-lifecycle-v18` record. Started and cleaned roots equal
