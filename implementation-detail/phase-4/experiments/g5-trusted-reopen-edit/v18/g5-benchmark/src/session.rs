...
#[cfg(target_os = "macos")]
fn g5_release_free_allocator_pages() {
    unsafe extern "C" {
        fn malloc_zone_pressure_relief(zone: *mut libc::malloc_zone_t, goal: usize) -> usize;
    }
    // SAFETY: the default malloc zone is process-global and remains valid for
    // the process lifetime; goal 0 asks Darwin to release any free pages.
    unsafe {
...
fn g5_child() -> AnyResult<()> {
    ...
    while let Some(line) = g5_read_line(&mut input, &mut buffer)? {
        ...
        g5_terminal_q()?;
        g5_release_free_allocator_pages();
