...
OP_SCHEMA = "phase4-g5-1-operation-v19"
...
def validate_prearm_initialization(
    ...
    if (
        ...
        != "phase4-g5-1-prearm-wrapper-initialization-evidence-v19"
...
def analyze(raw_path, timing_path, schedule_path, expected_path):
    ...
    sentinel_rows = [row for row in raw if row.get("schema") == "phase4-g5-1-protected-sentinel-v19"]
    ...
    initialization_rows = [
        row
        for row in raw
        if row.get("schema") == "phase4-g5-1-prearm-wrapper-initialization-v19"
    ]
    ...
    initialization_artifact = pathlib.Path(raw_path).parent / (
        "PREARM-WRAPPER-INITIALIZATION-v19.json"
    )
    try:
    ...
    validate_child_lifecycle(
        ...
        sentinel_rows,
        campaign,
        pathlib.Path(raw_path).parent / "PRODUCT-CHILD-LIFECYCLE-v19.json",
        failures,
    )
    validate_work_root_lifecycle(
        work_root_rows,
        operations,
        pathlib.Path(raw_path).parent / "WORK-ROOT-LIFECYCLE-v19.json",
        failures,
    )
    if campaign == "screen":
        counts = {}
    ...
    return {"schema": "phase4-g5-1-primary-analysis-v19", "status": "PASS" if not failures else "REVISE", "normalized": normalized}
