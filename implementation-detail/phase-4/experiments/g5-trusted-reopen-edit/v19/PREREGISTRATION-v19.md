# Phase 4 G5-1 v19 preregistration

V19 preserves v18's 200-row RSS REVISE. It removes the ineffective allocator
...
V19 preserves the v17 attempt-2 200-row REVISE. It retains the K64 scrub and
...
`R-ceil(R/64)` query reduction; mutation and durability work remain equal.
...
The candidate remains CAS + CDC + COW + canonical K64/F64 + SQLite. V19 changes
...
Any failure preserves v19. No observed row may be deleted, replaced, reordered,
or selectively rerun. A source or method repair creates v19.
...
`phase4-g5-1-wrapper-calibration-intent-v19`,
`phase4-g5-1-wrapper-initialization-sample-v19`,
`phase4-g5-1-wrapper-calibration-sample-v19`, and
`phase4-g5-1-wrapper-calibration-result-v19`.
...
`phase4-g5-1-wrapper-calibration-plan-v19` and records the complete candidate
...
must write and fsync the top-level `DRY-RUN-INTENT-v19.json` before invoking
...
write and fsync `DRY-RUN-FAILED-v19.json` first, then write and fsync a
`DRY-RUN-DISPOSITION-v19.json` with `status=REVISE` that binds the FAILED hash
and explicitly records absent optional artifacts. A success or calibrated
overrun writes and fsyncs complete `DRY-RUN-v19.json`, including both
...
`PREMEASUREMENT-REVISE-v19.json` and stops before screen. A missing dry-run or
...
V14 is byte-preserved by `V14-SUPERSESSION-v19.json`. Its zero-row passed, and
...
V19 changes the shared product timer equation and uses a fresh release. The
...
separate complete-lifecycle evidence. V19 reuses only the independently rehashed
...
are not copied, renamed, or represented as v19-generated. V19 reuses no v14
product row or timing observation. The new v19 input manifest rebinds the same sealed
files byte-for-byte before the v19 method manifest and source freeze are
generated.

V15 attempts 1–5 remain append-only and are not measurement authority for v19.
...
while the lifecycle sum added that authority again. V19 removes only that term
...
rows. Its G5 rows confirm the lifecycle equation repair. V19 changes only the
...
The final `PREARM-WRAPPER-INITIALIZATION-v19.json` write is outside that bound
...
schedule, expectation, limitation, and supersession byte has settled, v19
...
child. It writes `FREEZE-VERIFICATION-v19.json` with classification
...
V19 voluntarily freezes a deliberately stricter v1-lineage design: each
...
`phase4-g5-1-product-child-lifecycle-v19`. The two copies must be byte-value
...
child, v19 verifies the complete sealed input manifest exactly once: path,
...
`g5-v19-native-clone-receipt-v1`. Its ordered `entries` bind each source master
...
inspection for a future separate API. V19 therefore removes the external
...
`g5-v19-rooted-logical-state-v1` from one query-only, autocommit, constant-row
...
V19 does not claim physical SQLite file-byte parity. The sibling
`physical_allocation_observation` remains schema
`g5-v19-physical-allocation-observation-v1`, classification
...
Each wrapper receipt uses schema `phase4-g5-1-arm-cleanup-receipt-v19` and is
repeated in order by the raw and standalone
`phase4-g5-1-work-root-lifecycle-v19` record. Started and cleaned roots equal
