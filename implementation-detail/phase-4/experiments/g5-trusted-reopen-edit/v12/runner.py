...
REPO = HERE.parents[4]
METHOD = HERE / "method"
SCHEDULE = METHOD / "SCHEDULE-v12.tsv"
EXPECTED = METHOD / "EXPECTED-OUTCOMES-v12.tsv"
INPUT_MANIFEST = METHOD / "INPUT-MANIFEST-v12.tsv"
METHOD_MANIFEST = METHOD / "METHOD-MANIFEST-v12.tsv"
SOURCE_FREEZE = METHOD / "SOURCE-FREEZE-v12.json"
FREEZE_VERIFICATION = HERE / "FREEZE-VERIFICATION-v12.json"
LIMITATIONS = HERE / "LIMITATIONS-v12.json"
STATIC_CLOSURE = HERE / "STATIC-CLOSURE-v12.json"
DRY_RUN = HERE / "DRY-RUN-v12.json"
DRY_RUN_INTENT = HERE / "DRY-RUN-INTENT-v12.json"
DRY_RUN_CALIBRATION_STDOUT = HERE / "DRY-RUN-CALIBRATION-v12.stdout"
DRY_RUN_CALIBRATION_STDERR = HERE / "DRY-RUN-CALIBRATION-v12.stderr"
DRY_RUN_CALIBRATION_TERMINAL = HERE / "DRY-RUN-CALIBRATION-TERMINAL-v12.json"
DRY_RUN_DISPOSITION = HERE / "DRY-RUN-DISPOSITION-v12.json"
DRY_RUN_FAILED = HERE / "DRY-RUN-FAILED-v12.json"
PREMEASUREMENT_REVISE = HERE / "PREMEASUREMENT-REVISE-v12.json"
WRAPPER_CALIBRATION_INTENT = HERE / "WRAPPER-CALIBRATION-INTENT-v12.json"
WRAPPER_CALIBRATION_RAW = HERE / "WRAPPER-CALIBRATION-RAW-v12.jsonl"
WRAPPER_CALIBRATION_RESULT = HERE / "WRAPPER-CALIBRATION-RESULT-v12.json"
...
SCREEN_RESULT = REPO / f"target/phase4-g5-trusted-reopen-edit-{DATE}-v12-screen"
GATE_RESULT = REPO / f"target/phase4-g5-trusted-reopen-edit-{DATE}-v12"
WRAPPER_CALIBRATION_ROOT = (
    REPO / f"target/phase4-g5-trusted-reopen-edit-wrapper-calibration-{DATE}-v12"
)

# V12 is a method-only revision and reuses the exact hash-verified v10 operands.
...
SENTINEL_SCHEMA = "phase4-g5-1-protected-sentinel-v12"
REQUEST_FIELDS = (
...
V11_SUPERSESSION = HERE / "V11-SUPERSESSION-v12.json"
...
FORECAST_MODEL_VERSION = "phase4-g5-1-v12-fast-law-forecast-v5"
...
CLONE_RECEIPT_SCHEMA = "g5-v12-native-clone-receipt-v1"
...
PHYSICAL_ALLOCATION_SCHEMA = "g5-v12-physical-allocation-observation-v1"
...
CHILD_LIFECYCLE_SCHEMA = "phase4-g5-1-product-child-lifecycle-v12"
...
WORK_ROOT_LIFECYCLE_SCHEMA = "phase4-g5-1-work-root-lifecycle-v12"
...
def method_source_names():
    fixed = {
        ...
        for path in (
            HERE / "PREREGISTRATION-v12.md",
            HERE / "REVIEW-SYNTHESIS-v12.md",
            HERE / "SAMPLE-COUNT-INTERPRETATION-ADDENDUM-v12.md",
            HERE / "FOCUSED-TEST-ATTEMPTS-v12.json",
            HERE / "PREMEASUREMENT-READINESS-AUDIT-v12.json",
            LIMITATIONS,
            HERE / "runner.py",
            PRIMARY,
            INDEPENDENT,
...
def freeze_interface_contract():
    return {
        ...
        "wrapper_calibration": {
            ...
            "intent_schema": "phase4-g5-1-wrapper-calibration-intent-v12",
            "initialization_schema": "phase4-g5-1-wrapper-initialization-sample-v12",
            "sample_schema": "phase4-g5-1-wrapper-calibration-sample-v12",
            "result_schema": "phase4-g5-1-wrapper-calibration-result-v12",
...
def write_input_and_method_manifests():
    ...
    freeze = {
        "schema": "phase4-g5-1-source-freeze-v12",
        ...
        "post_freeze_verification": {
            ...
            "schema": "phase4-g5-1-freeze-verification-v12",
...
def run_preparation_command(
    ...
    chronology = evidence_root / "CHRONOLOGY-v12.jsonl"
    ...
    write_json(
        intent,
        {
            "schema": "phase4-g5-1-preparation-command-intent-v12",
            "ordinal": ordinal,
            "label": label,
            "argv": command,
    ...
    terminal = {
        "schema": "phase4-g5-1-preparation-command-terminal-v12",
        "ordinal": ordinal,
        "label": label,
...
def prepare_inputs():
    ...
    if any(
        ...
        raise RuntimeError("v12 method/freeze evidence already exists")
    ...
    if future_input not in sources or missing:
        raise RuntimeError(f"v12 method source preflight mismatch: {missing}")
    write_input_and_method_manifests()
    if verify_sealed_input_manifest() != 93:
        raise RuntimeError("v12 reused input manifest cardinality mismatch")
...
def verify_dry_run(freeze):
    ...
    required = {
        "schema": "phase4-g5-1-dry-run-v12",
        "status": "PASS",
        "measured_rows": 0,
    ...
    intent_required = {
        "schema": "phase4-g5-1-dry-run-intent-v12",
        "status": "STARTED",
        "branch": BRANCH,
        "head": CHECKPOINT,
    ...
    if (
        ...
        or disposition.get("schema") != "phase4-g5-1-dry-run-disposition-v12"
...
def verify_freeze(require_static=False, require_dry=False):
    ...
    if require_static:
        ...
        screen_terminal = SCREEN_RESULT / "TERMINAL-VERIFICATION-v12.json"
        screen_final_manifest = SCREEN_RESULT / "FINAL-ARTIFACT-HASHES-v12.tsv"
        screen_final_verification = SCREEN_RESULT / "FINAL-READONLY-VERIFICATION-v12.json"
        screen_complete_wall = SCREEN_RESULT / "COMPLETE-WALL-v12.json"
        required = {
            "schema": "phase4-g5-1-static-closure-v12",
            "status": "PASS",
            "source_freeze_sha256": sha256(SOURCE_FREEZE),
            "tracked_diff_sha256": freeze["tracked_diff_sha256"],
...
def gate_hash_bytes():
    ...
    prior_method_bytes = sum(
        int(row["bytes"])
        for manifest in (V10_METHOD_MANIFEST, V11_METHOD_MANIFEST)
    ...
    components = {
        ...
        "preserved_v7_v8_v9_v10_v11_failure_evidence": preserved_history_bytes,
...
def gate_workload_enumeration():
    ...
    return {
        "schema": "phase4-g5-1-prospective-workload-v12",
...
def hash_calibration():
    ...
    write_json(
        DRY_RUN_CALIBRATION_TERMINAL,
        {
            "schema": "phase4-g5-1-dry-run-calibration-terminal-v12",
            "status": "RETURNED",
            "return_code": completed.returncode,
    ...
    return {
        "schema": "phase4-g5-1-hash-calibration-v12",
...
def wrapper_calibration_plan():
    ...
    plan = {
        "schema": "phase4-g5-1-wrapper-calibration-plan-v12",
...
def run_wrapper_calibration(freeze, plan):
    ...
    intent = {
        "schema": "phase4-g5-1-wrapper-calibration-intent-v12",
        "status": "STARTED",
    ...
    try:
        ...
        initialization_evidence = {
            "schema": "phase4-g5-1-wrapper-initialization-evidence-v12",
        ...
        initialization = {
            "schema": "phase4-g5-1-wrapper-initialization-sample-v12",
            "status": "PASS",
        ...
        for sample_index in range(1, WRAPPER_CALIBRATION_SAMPLES + 1):
            ...
            representative = {
                "schema": "phase4-g5-1-wrapper-calibration-representative-arm-v12",
                "sample": sample_index,
                "master": master["relative_path"],
            ...
            sample = {
                "schema": "phase4-g5-1-wrapper-calibration-sample-v12",
                "status": "PASS",
                "sample": sample_index,
                "master": master["relative_path"],
                "phases_ns": phases,
        ...
        try:
            ...
            if WRAPPER_CALIBRATION_ROOT.exists():
                if (
                    ...
                    or not WRAPPER_CALIBRATION_ROOT.name.endswith(f"-{DATE}-v12")
                ):
    ...
    result = {
        "schema": "phase4-g5-1-wrapper-calibration-result-v12",
        "status": "PASS",
        "classification": "ZeroProductWrapperCalibration",
...
def verify_wrapper_calibration(freeze):
    ...
    if (
        ...
        != "phase4-g5-1-wrapper-initialization-sample-v12"
    ...
    for sample_index, sample in enumerate(samples, start=1):
        ...
        if (
            sample.get("schema") != "phase4-g5-1-wrapper-calibration-sample-v12"
...
def dry_failure_documents(error_type, error_message, intent_sha256, collected, artifacts):
    failed = {
        "schema": "phase4-g5-1-dry-run-failed-v12",
        "status": "REVISE",
    ...
    disposition = {
        "schema": "phase4-g5-1-dry-run-disposition-v12",
        "status": "REVISE",
        "classification": "UNEXPECTED_DRY_RUN_FAILURE",
        "intent_sha256": intent_sha256,
...
def dry_run():
    ...
    if any(path.exists() for path in evidence_paths):
        raise RuntimeError("v12 dry-run evidence already exists")
    if not FREEZE_VERIFICATION.is_file():
        raise RuntimeError("v12 dry-run requires durable freeze verification")
    ...
    try:
        ...
        value = {
            ...
            "sample_count_interpretation": "deliberately-stricter-v12-choice-not-unambiguous-user-minimum",
        ...
        if status == "REVISE":
            write_json(
                PREMEASUREMENT_REVISE,
                {
                    "schema": "phase4-g5-1-premeasurement-revise-v12",
                    "status": "REVISE",
...
def release_lock(lock, result, terminal_verification=None, state="release"):
    ...
    try:
        ...
        attestation = result / "BENCHMARK-LOCK-RELEASE-ATTESTATION-v12.json"
        ...
        value = {
            "schema": "phase4-g5-1-lock-release-v12",
        ...
        write_json(result / "LOCK-RELEASE-v12.json", value)
        return value
    finally:
        os.close(lock["fd"])
        lock["fd"] = None
...
def run_semantic(executable, case, root, result, label, lifecycle=None):
    stdout = result / f"children-v12/{label}.stdout"
    stderr = result / f"children-v12/{label}.stderr"
    sidecar = result / f"time-v12/{label}.txt"
...
def validate_child_row(envelope):
    ...
    return {
        "schema": "phase4-g5-1-operation-v12",
...
class PersistentChild:
    def __init__(
        ...
        self.stdout_path = result / f"children-v12/{self.label}.stdout"
        self.stderr_path = result / f"children-v12/{self.label}.stderr"
        self.time_path = result / f"time-v12/{self.label}.txt"
...
def analyze(result):
    ...
    for analyzer, name in (
        (PRIMARY, "PRIMARY-ANALYSIS-v12.json"),
        (INDEPENDENT, "INDEPENDENT-RECOMPUTATION-v12.json"),
    ):
        output = result / name
        completed = subprocess.run(
            [
                sys.executable, str(analyzer), str(result / "RAW-v12.jsonl"),
                str(result / "TIMINGS-v12.tsv"), str(SCHEDULE), str(EXPECTED), str(output),
            ],
            cwd=REPO,
            text=True,
            capture_output=True,
    ...
    write_json(
        result / "ANALYZER-AGREEMENT-v12.json",
        {
            "schema": "phase4-g5-1-analyzer-agreement-v12",
...
def run_prearm_wrapper_initialization(result, freeze, dry, lock):
    ...
    evidence_path = result / "PREARM-WRAPPER-INITIALIZATION-EVIDENCE-v12.json"
    ...
    write_json(result / "PREARM-WRAPPER-INITIALIZATION-v12.json", value)
    if value["status"] != "PASS":
...
def ladder_prelock(campaign):
    ...
    if campaign == "gate":
        paths.extend(
            (
                SCREEN_RESULT / "TERMINAL-VERIFICATION-v12.json",
                SCREEN_RESULT / "FINAL-ARTIFACT-HASHES-v12.tsv",
                SCREEN_RESULT / "FINAL-READONLY-VERIFICATION-v12.json",
                SCREEN_RESULT / "COMPLETE-WALL-v12.json",
                STATIC_CLOSURE,
            )
        )
...
def run_campaign(campaign):
    ...
    try:
        ...
        work = result.parent / f"{result.name}-work-v12"
        work.mkdir(mode=0o700)
        work_roots = ArmWorkRootLifecycle(work)
        for name in ("operands-v12", "children-v12", "time-v12"):
            (result / name).mkdir()
        fsync_dir(result)
        fsync_dir(result.parent)

        g4_copy = result / "operands-v12/frozen-g4-verified"
        g5_copy = result / "operands-v12/g5-verified-trusted"
        ...
        write_json(
            result / "OPERAND-CUSTODY-v12.json",
            {
                "schema": "phase4-g5-1-operand-custody-v12",
        ...
        write_json(
            result / "PREFLIGHT-v12.json",
            {
                "schema": "phase4-g5-1-preflight-v12",
                "status": "PASS",
                "campaign": campaign,
                "branch": BRANCH,
                "checkpoint": CHECKPOINT,
        ...
        write_json(
            result / "ENVIRONMENT-v12.json",
            {
                "schema": "phase4-g5-1-environment-v12",
        ...
        shutil.copyfile(INPUT_MANIFEST, result / "INPUT-CUSTODY-v12.tsv")
        with (result / "INPUT-CUSTODY-v12.tsv").open("rb") as handle:
            os.fsync(handle.fileno())
                    ...
                    semantics.extend(
                        {
                            ...
                            "expectation_id": "native-semantic-v12",
                            "status": value["status"],
                            "error": value["error"],
                        }
                        for value in values
        ...
        write_json(result / "PRODUCT-CHILD-LIFECYCLE-v12.json", child_lifecycle)
        write_json(result / "WORK-ROOT-LIFECYCLE-v12.json", work_root_lifecycle)

        write_text(result / "RAW-v12.jsonl", "".join(compact(row) + "\n" for row in raw))
        timing_fields = (
        ...
        write_text(
            result / "TIMINGS-v12.tsv",
            "\t".join(timing_fields) + "\n"
            + "".join("\t".join(str(row[name]) for name in timing_fields) + "\n" for row in timings),
        )
        write_text(
            result / "SEMANTIC-FAULT-RESULTS-v12.tsv",
        ...
        write_json(result / "COMMANDS-v12.json", {"schema": "phase4-g5-1-commands-v12", "commands": commands})
        analyze(result)

        if work.parent != result.parent or not work.name.endswith("-work-v12"):
            raise RuntimeError("refusing unsafe work cleanup")
        shutil.rmtree(work)
        fsync_dir(work.parent)
        residue = [str(path) for path in result.parent.glob(f"{result.name}-work-v12")]
        write_json(
            result / "CLEANUP-v12.json",
            {
                "schema": "phase4-g5-1-cleanup-v12",
        ...
        payload_excluded = {
            "PAYLOAD-MANIFEST-v12.tsv", "MEASURED-TERMINAL-v12.json",
            "TERMINAL-VERIFICATION-v12.json", "COMPLETE-WALL-v12.json",
            "BENCHMARK-LOCK-RELEASE-ATTESTATION-v12.json", "LOCK-RELEASE-v12.json",
            "FINAL-ARTIFACT-HASHES-v12.tsv", "FINAL-READONLY-VERIFICATION-v12.json",
        }
        payload = result / "PAYLOAD-MANIFEST-v12.tsv"
        ...
        terminal = result / "MEASURED-TERMINAL-v12.json"
        write_json(
            terminal,
            {
                "schema": "phase4-g5-1-measured-terminal-v12",
                "status": "PASS",
                "campaign": campaign,
                "rows": len(timings),
                ...
                "product_child_lifecycle_sha256": sha256(
                    result / "PRODUCT-CHILD-LIFECYCLE-v12.json"
                ),
                "work_root_lifecycle_sha256": sha256(
                    result / "WORK-ROOT-LIFECYCLE-v12.json"
                ),
                "prearm_wrapper_initialization_sha256": sha256(
                    result / "PREARM-WRAPPER-INITIALIZATION-v12.json"
                ),
        ...
        verification = result / "TERMINAL-VERIFICATION-v12.json"
        write_json(
            verification,
            {
                "schema": "phase4-g5-1-terminal-verification-v12",
                "status": "PASS",
        ...
        final = result / "FINAL-ARTIFACT-HASHES-v12.tsv"
        write_text(
            final,
            manifest_text(
                result,
                excluded={final.name, "FINAL-READONLY-VERIFICATION-v12.json", "COMPLETE-WALL-v12.json"},
            ),
        )
        final_count = verify_manifest(result, final, "result_relative_path")
        write_json(
            result / "FINAL-READONLY-VERIFICATION-v12.json",
            {
                "schema": "phase4-g5-1-final-readonly-verification-v12",
                "status": "PASS",
        ...
        write_json(
            result / "COMPLETE-WALL-v12.json",
            {
                "schema": "phase4-g5-1-complete-wall-v12",
                "status": "PASS",
                "campaign": campaign,
                ...
                "terminal_self_exclusion": "COMPLETE-WALL-v12.json follows the verified final manifest",
            },
        )
        fsync_dir(result)
        ...
        if result.exists() and not (result / "FAILED-v12.json").exists():
            write_json(
                result / "FAILED-v12.json",
                {
                    "schema": "phase4-g5-1-failure-v12",
                    "status": "REVISE",
                    "error": str(error),
...
def run_self_checks():
    ...
    with tempfile.TemporaryDirectory(prefix="layerfs-g5-v12-self-check-") as temporary:
        work = pathlib.Path(temporary) / "work-v12"
        work.mkdir()
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="layerfs-g5-v12-forecast-check-") as temporary:
    ...
    assert FORECAST_MODEL_VERSION == "phase4-g5-1-v12-fast-law-forecast-v5"
