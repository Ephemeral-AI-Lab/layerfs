...
## V11 terminal and accepted v12 method repair

V11 honestly closed as `PREMEASUREMENT_REVISE`: `129.202697050 s` against the
fixed `120 s` limit, zero product rows, screen, gate, or lock. Its first
complete-wrapper sample spent `27.711375 ms` in the process-first
published-state call, while identical later calls spent `0.476375/1.122917
ms`; v11 multiplied that one-time initialization by `4×200`. The frozen v11
calibration also had a semantic contradiction: nested calibration-only
semantics but product semantics in the surrounding wrapper evidence. The full
chain and both defects are bound by `V11-SUPERSESSION-v12.json`.

V12 changes only the runner, analyzers, and method. It derives top-level wrapper
semantics from the nested rooted state. It measures one process-first wrapper
...
external operands; no v11 row or timing is promoted to v12 authority.

V12 keeps the 200-arm schedule, 56 checkpoints, half-slower calibration floor,
...
- use native APFS clone receipts, schema `g5-v12-native-clone-receipt-v1`, to
...
- emit constant-size rooted state, schema `g5-v12-rooted-logical-state-v1`,
...
`FREEZE-VERIFICATION-v12.json` is classified
...
`PRODUCT-CHILD-LIFECYCLE-v12.json` under schema
`phase4-g5-1-product-child-lifecycle-v12`. Per-arm cleanup receipts use schema
`phase4-g5-1-arm-cleanup-receipt-v12`; their ordered terminal record is likewise
duplicated as `WORK-ROOT-LIFECYCLE-v12.json` under schema
`phase4-g5-1-work-root-lifecycle-v12`.
...
`wrapper-calibration-intent-v12`, `wrapper-initialization-sample-v12`,
`wrapper-calibration-sample-v12`, and `wrapper-calibration-result-v12`. The
...
intent embeds `phase4-g5-1-wrapper-calibration-plan-v12`, retains the complete
...
`phase4-g5-1-prearm-wrapper-initialization-v12` record after lock/custody and
...
closes v12 as `PREMEASUREMENT_REVISE` before screen; no schedule or threshold
is reduced.

`DRY-RUN-INTENT-v12.json` is durably written before either calibration begins.
A single outer failure funnel then covers wrapper calibration, hash calibration,
forecasting, and sealing: it always writes `DRY-RUN-FAILED-v12.json` before a
REVISE `DRY-RUN-DISPOSITION-v12.json`, even when the wrapper result is absent.
...
V12 nevertheless voluntarily freezes the stricter v1-lineage design: each
