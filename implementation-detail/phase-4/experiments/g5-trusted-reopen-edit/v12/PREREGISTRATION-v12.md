# Phase 4 G5-1 v12 preregistration
...
The candidate remains CAS + CDC + COW + canonical K64/F64 + SQLite. V12 changes
...
Any failure preserves v12. No observed row may be deleted, replaced, reordered,
or selectively rerun. A source or method repair creates v13.
...
`phase4-g5-1-wrapper-calibration-intent-v12`,
`phase4-g5-1-wrapper-initialization-sample-v12`,
`phase4-g5-1-wrapper-calibration-sample-v12`, and
`phase4-g5-1-wrapper-calibration-result-v12`.
...
`phase4-g5-1-wrapper-calibration-plan-v12` and records the complete candidate
...
must write and fsync the top-level `DRY-RUN-INTENT-v12.json` before invoking
...
write and fsync `DRY-RUN-FAILED-v12.json` first, then write and fsync a
`DRY-RUN-DISPOSITION-v12.json` with `status=REVISE` that binds the FAILED hash
and explicitly records absent optional artifacts. A success or calibrated
overrun writes and fsyncs complete `DRY-RUN-v12.json`, including both
...
`PREMEASUREMENT-REVISE-v12.json` and stops before screen. A missing dry-run or
failure disposition is never treated as an authoritative PASS or reconstructed
from an earlier attempt.

V11 is byte-preserved and superseded by `V11-SUPERSESSION-v12.json`. It binds the
...
V12 is a runner/analyzer/method-only revision. It reuses the unchanged v10
...
are not copied, renamed, or represented as v12-generated. V12 reuses no v10
row or timing observation. The new v12 input manifest rebinds the same sealed
files byte-for-byte before the v12 method manifest and source freeze are
generated.
...
The final `PREARM-WRAPPER-INITIALIZATION-v12.json` write is outside that bound
...
schedule, expectation, limitation, and supersession byte has settled, v12
...
child. It writes `FREEZE-VERIFICATION-v12.json` with classification
...
V12 voluntarily freezes a deliberately stricter v1-lineage design: each
...
`phase4-g5-1-product-child-lifecycle-v12`. The two copies must be byte-value
...
child, v12 verifies the complete sealed input manifest exactly once: path,
...
`g5-v12-native-clone-receipt-v1`. Its ordered `entries` bind each source master
...
inspection for a future separate API. V12 therefore removes the external
...
`g5-v12-rooted-logical-state-v1` from one query-only, autocommit, constant-row
...
V12 does not claim physical SQLite file-byte parity. The sibling
`physical_allocation_observation` remains schema
`g5-v12-physical-allocation-observation-v1`, classification
...
Each wrapper receipt uses schema `phase4-g5-1-arm-cleanup-receipt-v12` and is
repeated in order by the raw and standalone
`phase4-g5-1-work-root-lifecycle-v12` record. Started and cleaned roots equal
