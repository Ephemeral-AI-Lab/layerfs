# Phase 4 G5-1 v15 preregistration
...
The candidate remains CAS + CDC + COW + canonical K64/F64 + SQLite. V15 changes
...
Any failure preserves v15. No observed row may be deleted, replaced, reordered,
...
`phase4-g5-1-wrapper-calibration-intent-v15`,
`phase4-g5-1-wrapper-initialization-sample-v15`,
`phase4-g5-1-wrapper-calibration-sample-v15`, and
`phase4-g5-1-wrapper-calibration-result-v15`.
...
`phase4-g5-1-wrapper-calibration-plan-v15` and records the complete candidate
...
must write and fsync the top-level `DRY-RUN-INTENT-v15.json` before invoking
...
write and fsync `DRY-RUN-FAILED-v15.json` first, then write and fsync a
`DRY-RUN-DISPOSITION-v15.json` with `status=REVISE` that binds the FAILED hash
and explicitly records absent optional artifacts. A success or calibrated
overrun writes and fsyncs complete `DRY-RUN-v15.json`, including both
...
`PREMEASUREMENT-REVISE-v15.json` and stops before screen. A missing dry-run or
...
V14 is byte-preserved by `V14-SUPERSESSION-v15.json`. Its zero-row passed, and
...
V15 changes the shared product timer equation and uses a fresh release. The
...
separate complete-lifecycle evidence. V15 reuses only the independently rehashed
...
are not copied, renamed, or represented as v15-generated. V15 reuses no v14
product row or timing observation. The new v15 input manifest rebinds the same sealed
files byte-for-byte before the v15 method manifest and source freeze are
generated.
...
The final `PREARM-WRAPPER-INITIALIZATION-v15.json` write is outside that bound
...
schedule, expectation, limitation, and supersession byte has settled, v15
...
child. It writes `FREEZE-VERIFICATION-v15.json` with classification
...
V15 voluntarily freezes a deliberately stricter v1-lineage design: each
...
`phase4-g5-1-product-child-lifecycle-v15`. The two copies must be byte-value
...
child, v15 verifies the complete sealed input manifest exactly once: path,
...
`g5-v15-native-clone-receipt-v1`. Its ordered `entries` bind each source master
...
inspection for a future separate API. V15 therefore removes the external
...
`g5-v15-rooted-logical-state-v1` from one query-only, autocommit, constant-row
...
V15 does not claim physical SQLite file-byte parity. The sibling
`physical_allocation_observation` remains schema
`g5-v15-physical-allocation-observation-v1`, classification
...
Each wrapper receipt uses schema `phase4-g5-1-arm-cleanup-receipt-v15` and is
repeated in order by the raw and standalone
`phase4-g5-1-work-root-lifecycle-v15` record. Started and cleaned roots equal
