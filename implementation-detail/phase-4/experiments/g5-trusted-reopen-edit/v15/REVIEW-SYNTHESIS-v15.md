...
## V14 terminal and accepted v15 product repair

V14 passed its zero-row and reached one G5 Verified CompleteRoundTrip screen
row. That product row reported `first_edit_component_sum=11,723,249 ns`,
`first_edit_equation_total=9,285,958 ns`, and
`first_edit_timer_equation_matches=false`. The post-COMMIT reopen had overwritten
the initial reopen aggregate used by the first-edit equation. The partial screen
row, failure, cleanup, Q0, and released lock are bound by
`V14-SUPERSESSION-v15.json` and are never reused.

V15 changes the shared product first-edit timer equation and builds a fresh
release. It sums the initial Store preflight, SQLite/profile, visible head,
same-open authority, and durable capture aggregates; post-COMMIT reopen remains
separate lifecycle evidence. It retains v14's lock-context repair, split intent
...
product row or timing is promoted to v15 authority.

V15 keeps the 200-arm schedule, 56 checkpoints, half-slower calibration floor,
...
- use native APFS clone receipts, schema `g5-v15-native-clone-receipt-v1`, to
...
- emit constant-size rooted state, schema `g5-v15-rooted-logical-state-v1`,
...
`FREEZE-VERIFICATION-v15.json` is classified
...
`PRODUCT-CHILD-LIFECYCLE-v15.json` under schema
`phase4-g5-1-product-child-lifecycle-v15`. Per-arm cleanup receipts use schema
`phase4-g5-1-arm-cleanup-receipt-v15`; their ordered terminal record is likewise
duplicated as `WORK-ROOT-LIFECYCLE-v15.json` under schema
`phase4-g5-1-work-root-lifecycle-v15`.
...
`wrapper-calibration-intent-v15`, `wrapper-initialization-sample-v15`,
`wrapper-calibration-sample-v15`, and `wrapper-calibration-result-v15`. The
...
intent embeds `phase4-g5-1-wrapper-calibration-plan-v15`, retains the complete
...
`phase4-g5-1-prearm-wrapper-initialization-v15` record after lock/custody and
...
closes v15 as `PREMEASUREMENT_REVISE` before screen; no schedule or threshold
is reduced.

`DRY-RUN-INTENT-v15.json` is durably written before either calibration begins.
A single outer failure funnel then covers wrapper calibration, hash calibration,
forecasting, and sealing: it always writes `DRY-RUN-FAILED-v15.json` before a
REVISE `DRY-RUN-DISPOSITION-v15.json`, even when the wrapper result is absent.
...
V15 nevertheless voluntarily freezes the stricter v1-lineage design: each
