...
REPO = HERE.parents[4]
METHOD = HERE / "method"
SCHEDULE = METHOD / "SCHEDULE-v5.tsv"
EXPECTED = METHOD / "EXPECTED-OUTCOMES-v5.tsv"
INPUT_MANIFEST = METHOD / "INPUT-MANIFEST-v5.tsv"
METHOD_MANIFEST = METHOD / "METHOD-MANIFEST-v5.tsv"
SOURCE_FREEZE = METHOD / "SOURCE-FREEZE-v5.json"
LIMITATIONS = HERE / "LIMITATIONS-v5.json"
STATIC_CLOSURE = HERE / "STATIC-CLOSURE-v5.json"
...
INPUT_ROOT = REPO / f"target/phase4-g5-trusted-reopen-edit-inputs-{DATE}-v5"
SCREEN_RESULT = REPO / f"target/phase4-g5-trusted-reopen-edit-{DATE}-v5-screen"
GATE_RESULT = REPO / f"target/phase4-g5-trusted-reopen-edit-{DATE}-v5"

# Frozen v5 Rust transport.
G5_CHILD_BINARY = HERE / "g5-benchmark/target/release/layerfs-g5-trusted-child-v5"
FIXTURE_FLAG = "--g5-fixture"
PREPARE_FLAG = "--g5-prepare"
CHILD_FLAG = "--g5-child"
SEMANTIC_FLAG = "--g5-semantic"
CHILD_READY_SCHEMA = "phase4-g5-trusted-child-ready-v5"
CHILD_ENVELOPE_SCHEMA = "phase4-g5-trusted-child-row-v5"
CHILD_TERMINAL_SCHEMA = "phase4-g5-trusted-child-terminal-v5"
FIXTURE_SCHEMA = "phase4-g5-trusted-fixture-v5"
PREPARE_SCHEMA = "phase4-g5-trusted-prepare-v5"
SEMANTIC_SCHEMA = "phase4-g5-trusted-semantic-v5"
SEMANTIC_TERMINAL_SCHEMA = "phase4-g5-trusted-semantic-terminal-v5"
SENTINEL_SCHEMA = "phase4-g5-1-protected-sentinel-v5"
REQUEST_FIELDS = (
...
def method_source_names():
    fixed = {
        ...
        for path in (
            HERE / "PREREGISTRATION-v5.md",
            HERE / "REVIEW-SYNTHESIS-v5.md",
            HERE / "SAMPLE-COUNT-INTERPRETATION-ADDENDUM-v5.md",
            HERE / "V4-SUPERSESSION-v5.json",
            LIMITATIONS,
            HERE / "runner.py",
            PRIMARY,
            INDEPENDENT,
...
def write_input_and_method_manifests():
    ...
    freeze = {
        "schema": "phase4-g5-1-source-freeze-v5",
...
def prepare_inputs():
    ...
    if any(path.exists() for path in (INPUT_MANIFEST, METHOD_MANIFEST, SOURCE_FREEZE, DRY_RUN, STATIC_CLOSURE)):
        raise RuntimeError("v5 method/freeze evidence already exists")
    ...
    write_json(
        INPUT_ROOT / "PREPARATION-CUSTODY-v5.json",
        {
            "schema": "phase4-g5-1-input-preparation-v5",
            "status": "PASS",
...
def dry_run():
    ...
    if FULL_WRAPPER_FORECAST_NS > LIMIT_NS["gate"]:
        raise RuntimeError("v5 PREMEASUREMENT_REVISE: conservative full-wrapper forecast exceeds 120 seconds")
    generated_residue = sorted(
        str(path.relative_to(REPO)) for path in HERE.rglob("__pycache__") if path.is_dir()
    )
    value = {
        "schema": "phase4-g5-1-dry-run-v5",
        "status": "PASS",
        "measured_rows": 0,
        ...
        "sample_count_interpretation": "deliberately-stricter-v5-choice-not-unambiguous-user-minimum",
...
def release_lock(lock, result, terminal_verification=None, state="release"):
    ...
    try:
        ...
        value = {
            "schema": "phase4-g5-1-lock-release-v5",
        ...
        write_json(result / "LOCK-RELEASE-v5.json", value)
        return value
    finally:
        os.close(lock["fd"])
        lock["fd"] = None
...
def run_semantic(executable, case, root, result, label):
    stdout = result / f"children-v5/{label}.stdout"
    stderr = result / f"children-v5/{label}.stderr"
    sidecar = result / f"time-v5/{label}.txt"
...
def validate_child_row(envelope):
    ...
    return {
        "schema": "phase4-g5-1-operation-v5",
...
class PersistentChild:
    def __init__(
        ...
        self.stdout_path = result / f"children-v5/{label}.stdout"
        self.stderr_path = result / f"children-v5/{label}.stderr"
        self.time_path = result / f"time-v5/{label}.txt"
...
def analyze(result):
    ...
    for analyzer, name in (
        (PRIMARY, "PRIMARY-ANALYSIS-v5.json"),
        (INDEPENDENT, "INDEPENDENT-RECOMPUTATION-v5.json"),
    ):
        output = result / name
        completed = subprocess.run(
            [
                sys.executable, str(analyzer), str(result / "RAW-v5.jsonl"),
                str(result / "TIMINGS-v5.tsv"), str(SCHEDULE), str(EXPECTED), str(output),
            ],
            cwd=REPO,
            text=True,
            capture_output=True,
    ...
    write_json(
        result / "ANALYZER-AGREEMENT-v5.json",
        {
            "schema": "phase4-g5-1-analyzer-agreement-v5",
...
def run_campaign(campaign):
    ...
    try:
        ...
        work = result.parent / f"{result.name}-work-v5"
        work.mkdir(mode=0o700)
        for name in ("operands-v5", "children-v5", "time-v5"):
            (result / name).mkdir()
        fsync_dir(result)
        fsync_dir(result.parent)

        g4_copy = result / "operands-v5/frozen-g4-verified"
        g5_copy = result / "operands-v5/g5-verified-trusted"
        ...
        write_json(
            result / "OPERAND-CUSTODY-v5.json",
            {
                "schema": "phase4-g5-1-operand-custody-v5",
        ...
        write_json(
            result / "PREFLIGHT-v5.json",
            {
                "schema": "phase4-g5-1-preflight-v5",
                "status": "PASS",
                "campaign": campaign,
                "branch": BRANCH,
                "checkpoint": CHECKPOINT,
        ...
        write_json(
            result / "ENVIRONMENT-v5.json",
            {
                "schema": "phase4-g5-1-environment-v5",
        ...
        shutil.copyfile(INPUT_MANIFEST, result / "INPUT-CUSTODY-v5.tsv")
        with (result / "INPUT-CUSTODY-v5.tsv").open("rb") as handle:
            os.fsync(handle.fileno())
                    ...
                    semantics.extend(
                        {
                            ...
                            "expectation_id": "native-semantic-v5",
                            "status": value["status"],
                            "error": value["error"],
                        }
                        for value in values
        ...
        write_text(
            result / "TIMINGS-v5.tsv",
            "\t".join(timing_fields) + "\n"
            + "".join("\t".join(str(row[name]) for name in timing_fields) + "\n" for row in timings),
        )
        write_text(
            result / "SEMANTIC-FAULT-RESULTS-v5.tsv",
        ...
        write_json(result / "COMMANDS-v5.json", {"schema": "phase4-g5-1-commands-v5", "commands": commands})
        analyze(result)

        if work.parent != result.parent or not work.name.endswith("-work-v5"):
            raise RuntimeError("refusing unsafe work cleanup")
        shutil.rmtree(work)
        fsync_dir(work.parent)
        residue = [str(path) for path in result.parent.glob(f"{result.name}-work-v5")]
        write_json(
            result / "CLEANUP-v5.json",
            {
                "schema": "phase4-g5-1-cleanup-v5",
        ...
        payload_excluded = {
            "PAYLOAD-MANIFEST-v5.tsv", "MEASURED-TERMINAL-v5.json",
            "TERMINAL-VERIFICATION-v5.json", "COMPLETE-WALL-v5.json",
            "BENCHMARK-LOCK-RELEASE-ATTESTATION-v5.json", "LOCK-RELEASE-v5.json",
            "FINAL-ARTIFACT-HASHES-v5.tsv", "FINAL-READONLY-VERIFICATION-v5.json",
        }
        payload = result / "PAYLOAD-MANIFEST-v5.tsv"
        ...
        write_json(
            terminal,
            {
                "schema": "phase4-g5-1-measured-terminal-v5",
                "status": "PASS",
                "campaign": campaign,
                "rows": len(timings),
        ...
        verification = result / "TERMINAL-VERIFICATION-v5.json"
        write_json(
            verification,
            {
                "schema": "phase4-g5-1-terminal-verification-v5",
                "status": "PASS",
        ...
        write_json(
            result / "COMPLETE-WALL-v5.json",
            {
                "schema": "phase4-g5-1-complete-wall-v5",
                "status": "PASS",
                "campaign": campaign,
        ...
        final = result / "FINAL-ARTIFACT-HASHES-v5.tsv"
        write_text(final, manifest_text(result, excluded={final.name, "FINAL-READONLY-VERIFICATION-v5.json"}))
        final_count = verify_manifest(result, final, "result_relative_path")
        write_json(
            result / "FINAL-READONLY-VERIFICATION-v5.json",
            {
                "schema": "phase4-g5-1-final-readonly-verification-v5",
                "status": "PASS",
        ...
        if result.exists() and not (result / "FAILED-v5.json").exists():
            write_json(
                result / "FAILED-v5.json",
                {
                    "schema": "phase4-g5-1-failure-v5",
                    "status": "REVISE",
                    "error": str(error),
