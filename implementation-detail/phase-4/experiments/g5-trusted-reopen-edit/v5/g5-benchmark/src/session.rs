const G5_REQUEST_BYTES_V5: usize = 4_096;
...
fn g5_read_line<'a>(
    ...
    buffer: &'a mut [u8; G5_REQUEST_BYTES_V5],
...
fn g5_child() -> AnyResult<()> {
    ...
    let mut buffer = [0_u8; G5_REQUEST_BYTES_V5];
    let mut rows = 0_u64;

    writeln!(output, "{{\"status\":\"READY\",\"schema\":\"phase4-g5-trusted-child-ready-v5\",\"integrity_mode\":\"{mode_name}\",\"mode_provenance\":\"fixed-at-child-start\",\"size_bytes\":{size},\"operation\":\"{operation}\",\"expected_rows\":{expected_rows},\"full_wrapper_forecast_ns\":{forecast_ns},\"full_wrapper_limit_ns\":{limit_ns},\"custody\":\"runner-preverified-borrowed\",\"request_schema\":\"id\\troot\\titeration\\twarmup\\tvalidation\"}}")?;
    output.flush()?;
    while let Some(line) = g5_read_line(&mut input, &mut buffer)? {
        ...
        writeln!(output, "{{\"status\":\"PASS\",\"schema\":\"phase4-g5-trusted-child-row-v5\",\"request_id\":\"{}\",\"integrity_mode\":\"{mode_name}\",\"mode_provenance\":\"fixed-at-child-start\",\"row\":{}}}", request.id, &*row)?;
        output.flush()?;
        drop(row);
        drop(session);
        drop(expectations);
    ...
    g5_terminal_q()?;
    writeln!(output, "{{\"status\":\"PASS\",\"schema\":\"phase4-g5-trusted-child-terminal-v5\",\"integrity_mode\":\"{mode_name}\",\"rows\":{rows},\"expected_rows\":{expected_rows},\"argument_owners\":0,\"request_owners\":0,\"schedule_owners\":0,\"timing_owners\":0,\"report_owners\":0,\"q_current\":0,\"rss\":\"external-high-water-only\"}}")?;
    output.flush()?;
    Ok(())
}
...
fn g5_semantic(case: &str, root: &Path) -> AnyResult<()> {
    ...
    g5_terminal_q()?;
    writeln!(output, "{{\"status\":\"PASS\",\"schema\":\"phase4-g5-trusted-semantic-terminal-v5\",\"case\":\"{case}\",\"q_current\":0}}")?;
    output.flush()?;
    Ok(())
}
...
pub(super) fn g5_transport_main() -> AnyResult<()> {
    match g5_arg(1)? {
        "--g5-fixture" => {
            ...
            g5_terminal_q()?;
            println!("{{\"status\":\"PASS\",\"schema\":\"phase4-g5-trusted-fixture-v5\",\"size_bytes\":{size},\"q_current\":0}}" );
            Ok(())
        }
        "--g5-prepare" => {
            g5_require_args(6)?;
            ...
            g5_terminal_q()?;
            println!("{{\"status\":\"PASS\",\"schema\":\"phase4-g5-trusted-prepare-v5\",\"size_bytes\":{size},\"operation\":\"{operation}\",\"iteration\":{iteration},\"q_current\":0}}" );
            Ok(())
        }
        "--g5-child" => g5_child(),
        "--g5-semantic" => {
