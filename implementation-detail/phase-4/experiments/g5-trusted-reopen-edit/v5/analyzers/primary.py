...
OP_SCHEMA = "phase4-g5-1-operation-v5"
TERMINAL_SCHEMA = "phase4-g5-trusted-child-terminal-v5"
...
def analyze(raw_path, timing_path, schedule_path, expected_path):
    ...
    semantic_rows = [row for row in raw if row.get("schema") == "phase4-g5-trusted-semantic-v5"]
    semantic_terminals = [
        row for row in raw if row.get("schema") == "phase4-g5-trusted-semantic-terminal-v5"
    ]
    sentinel_rows = [row for row in raw if row.get("schema") == "phase4-g5-1-protected-sentinel-v5"]
    ...
    return {"schema": "phase4-g5-1-primary-analysis-v5", "status": "PASS" if not failures else "REVISE", "normalized": normalized}
