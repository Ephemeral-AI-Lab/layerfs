# G5-1 v5 sample-count interpretation addendum

This append-only note corrects one sentence in `REVIEW-SYNTHESIS-v5.md`
without changing `PREREGISTRATION-v5.md` or its schedule.
...
separately inside each comparison. V5 nevertheless prospectively froze the
...
That is a deliberate v5 method choice, not an exact restatement of an
...
forecast is `<=120 s`. If it cannot, v5 is preserved as `PREMEASUREMENT_REVISE`;
only a prospectively created v5 may adopt a narrower interpretation.
