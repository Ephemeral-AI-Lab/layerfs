...
REPO = HERE.parents[4]
METHOD = HERE / "method"
SCHEDULE = METHOD / "SCHEDULE-v7.tsv"
EXPECTED = METHOD / "EXPECTED-OUTCOMES-v7.tsv"
INPUT_MANIFEST = METHOD / "INPUT-MANIFEST-v7.tsv"
METHOD_MANIFEST = METHOD / "METHOD-MANIFEST-v7.tsv"
SOURCE_FREEZE = METHOD / "SOURCE-FREEZE-v7.json"
LIMITATIONS = HERE / "LIMITATIONS-v7.json"
STATIC_CLOSURE = HERE / "STATIC-CLOSURE-v7.json"
...
INPUT_ROOT = REPO / f"target/phase4-g5-trusted-reopen-edit-inputs-{DATE}-v7"
SCREEN_RESULT = REPO / f"target/phase4-g5-trusted-reopen-edit-{DATE}-v7-screen"
GATE_RESULT = REPO / f"target/phase4-g5-trusted-reopen-edit-{DATE}-v7"

# Frozen v7 Rust transport.
G5_CHILD_BINARY = HERE / "g5-benchmark/target/release/layerfs-g5-trusted-child-v7"
FIXTURE_FLAG = "--g5-fixture"
PREPARE_FLAG = "--g5-prepare"
CHILD_FLAG = "--g5-child"
SEMANTIC_FLAG = "--g5-semantic"
CHILD_READY_SCHEMA = "phase4-g5-trusted-child-ready-v7"
CHILD_ENVELOPE_SCHEMA = "phase4-g5-trusted-child-row-v7"
CHILD_TERMINAL_SCHEMA = "phase4-g5-trusted-child-terminal-v7"
FIXTURE_SCHEMA = "phase4-g5-trusted-fixture-v7"
PREPARE_SCHEMA = "phase4-g5-trusted-prepare-v7"
SEMANTIC_SCHEMA = "phase4-g5-trusted-semantic-v7"
SEMANTIC_TERMINAL_SCHEMA = "phase4-g5-trusted-semantic-terminal-v7"
SENTINEL_SCHEMA = "phase4-g5-1-protected-sentinel-v7"
REQUEST_FIELDS = (
...
def method_source_names():
    fixed = {
        ...
        for path in (
            HERE / "PREREGISTRATION-v7.md",
            HERE / "REVIEW-SYNTHESIS-v7.md",
            HERE / "SAMPLE-COUNT-INTERPRETATION-ADDENDUM-v7.md",
            HERE / "V6-SUPERSESSION-v7.json",
            LIMITATIONS,
            HERE / "runner.py",
            PRIMARY,
            INDEPENDENT,
...
def write_input_and_method_manifests():
    ...
    freeze = {
        "schema": "phase4-g5-1-source-freeze-v7",
        ...
        "forecast_model": "dry-run-calibrated-v7-python-and-external-shasum",
...
def prepare_inputs():
    ...
    if any(path.exists() for path in (INPUT_MANIFEST, METHOD_MANIFEST, SOURCE_FREEZE, DRY_RUN, STATIC_CLOSURE)):
        raise RuntimeError("v7 method/freeze evidence already exists")
    ...
    write_json(
        INPUT_ROOT / "PREPARATION-CUSTODY-v7.json",
        {
            "schema": "phase4-g5-1-input-preparation-v7",
            "status": "PASS",
...
def verify_freeze(require_static=False, require_dry=False):
    ...
    if require_static:
        ...
        screen_terminal = SCREEN_RESULT / "TERMINAL-VERIFICATION-v7.json"
        screen_final_manifest = SCREEN_RESULT / "FINAL-ARTIFACT-HASHES-v7.tsv"
        screen_final_verification = SCREEN_RESULT / "FINAL-READONLY-VERIFICATION-v7.json"
        screen_complete_wall = SCREEN_RESULT / "COMPLETE-WALL-v7.json"
        required = {
            "schema": "phase4-g5-1-static-closure-v7",
            "status": "PASS",
            "source_freeze_sha256": sha256(SOURCE_FREEZE),
...
def hash_calibration():
    ...
    return {
        "schema": "phase4-g5-1-hash-calibration-v7",
...
def dry_run():
    ...
    if full_wrapper_forecast_ns > LIMIT_NS["gate"]:
        raise RuntimeError("v7 PREMEASUREMENT_REVISE: conservative full-wrapper forecast exceeds 120 seconds")
    ...
    value = {
        ...
        "gate_sequences": 14,
        "gate_arm_observations": GATE_ARM_OBSERVATIONS,
        "sample_count_interpretation": "deliberately-stricter-v7-choice-not-unambiguous-user-minimum",
...
def release_lock(lock, result, terminal_verification=None, state="release"):
    ...
    try:
        ...
        attestation = result / "BENCHMARK-LOCK-RELEASE-ATTESTATION-v7.json"
        ...
        value = {
            "schema": "phase4-g5-1-lock-release-v7",
        ...
        write_json(result / "LOCK-RELEASE-v7.json", value)
        return value
    finally:
        os.close(lock["fd"])
        lock["fd"] = None
...
def run_semantic(executable, case, root, result, label):
    stdout = result / f"children-v7/{label}.stdout"
    stderr = result / f"children-v7/{label}.stderr"
    sidecar = result / f"time-v7/{label}.txt"
...
def validate_child_row(envelope):
    ...
    return {
        "schema": "phase4-g5-1-operation-v7",
...
class PersistentChild:
    def __init__(
        ...
        self.stdout_path = result / f"children-v7/{label}.stdout"
        self.stderr_path = result / f"children-v7/{label}.stderr"
        self.time_path = result / f"time-v7/{label}.txt"
...
def analyze(result):
    ...
    for analyzer, name in (
        (PRIMARY, "PRIMARY-ANALYSIS-v7.json"),
        (INDEPENDENT, "INDEPENDENT-RECOMPUTATION-v7.json"),
    ):
        output = result / name
        completed = subprocess.run(
            [
                sys.executable, str(analyzer), str(result / "RAW-v7.jsonl"),
                str(result / "TIMINGS-v7.tsv"), str(SCHEDULE), str(EXPECTED), str(output),
            ],
            cwd=REPO,
            text=True,
            capture_output=True,
    ...
    write_json(
        result / "ANALYZER-AGREEMENT-v7.json",
        {
            "schema": "phase4-g5-1-analyzer-agreement-v7",
...
def ladder_prelock(campaign):
    ...
    if campaign == "gate":
        paths.extend(
            (
                SCREEN_RESULT / "TERMINAL-VERIFICATION-v7.json",
                SCREEN_RESULT / "FINAL-ARTIFACT-HASHES-v7.tsv",
                SCREEN_RESULT / "FINAL-READONLY-VERIFICATION-v7.json",
                SCREEN_RESULT / "COMPLETE-WALL-v7.json",
                STATIC_CLOSURE,
            )
        )
...
def run_campaign(campaign):
    ...
    try:
        ...
        work = result.parent / f"{result.name}-work-v7"
        work.mkdir(mode=0o700)
        for name in ("operands-v7", "children-v7", "time-v7"):
            (result / name).mkdir()
        fsync_dir(result)
        fsync_dir(result.parent)

        g4_copy = result / "operands-v7/frozen-g4-verified"
        g5_copy = result / "operands-v7/g5-verified-trusted"
        ...
        write_json(
            result / "OPERAND-CUSTODY-v7.json",
            {
                "schema": "phase4-g5-1-operand-custody-v7",
        ...
        write_json(
            result / "PREFLIGHT-v7.json",
            {
                "schema": "phase4-g5-1-preflight-v7",
                "status": "PASS",
                "campaign": campaign,
                "branch": BRANCH,
                "checkpoint": CHECKPOINT,
        ...
        write_json(
            result / "ENVIRONMENT-v7.json",
            {
                "schema": "phase4-g5-1-environment-v7",
        ...
        shutil.copyfile(INPUT_MANIFEST, result / "INPUT-CUSTODY-v7.tsv")
        with (result / "INPUT-CUSTODY-v7.tsv").open("rb") as handle:
            os.fsync(handle.fileno())
                    ...
                    semantics.extend(
                        {
                            ...
                            "expectation_id": "native-semantic-v7",
                            "status": value["status"],
                            "error": value["error"],
                        }
                        for value in values
        ...
        write_json(result / "COMMANDS-v7.json", {"schema": "phase4-g5-1-commands-v7", "commands": commands})
        analyze(result)

        if work.parent != result.parent or not work.name.endswith("-work-v7"):
            raise RuntimeError("refusing unsafe work cleanup")
        shutil.rmtree(work)
        fsync_dir(work.parent)
        residue = [str(path) for path in result.parent.glob(f"{result.name}-work-v7")]
        write_json(
            result / "CLEANUP-v7.json",
            {
                "schema": "phase4-g5-1-cleanup-v7",
        ...
        payload_excluded = {
            "PAYLOAD-MANIFEST-v7.tsv", "MEASURED-TERMINAL-v7.json",
            "TERMINAL-VERIFICATION-v7.json", "COMPLETE-WALL-v7.json",
            "BENCHMARK-LOCK-RELEASE-ATTESTATION-v7.json", "LOCK-RELEASE-v7.json",
            "FINAL-ARTIFACT-HASHES-v7.tsv", "FINAL-READONLY-VERIFICATION-v7.json",
        }
        payload = result / "PAYLOAD-MANIFEST-v7.tsv"
        ...
        terminal = result / "MEASURED-TERMINAL-v7.json"
        write_json(
            terminal,
            {
                "schema": "phase4-g5-1-measured-terminal-v7",
                "status": "PASS",
                "campaign": campaign,
                "rows": len(timings),
        ...
        verification = result / "TERMINAL-VERIFICATION-v7.json"
        write_json(
            verification,
            {
                "schema": "phase4-g5-1-terminal-verification-v7",
                "status": "PASS",
        ...
        final = result / "FINAL-ARTIFACT-HASHES-v7.tsv"
        write_text(
            final,
            manifest_text(
                result,
                excluded={final.name, "FINAL-READONLY-VERIFICATION-v7.json", "COMPLETE-WALL-v7.json"},
            ),
        )
        final_count = verify_manifest(result, final, "result_relative_path")
        write_json(
            result / "FINAL-READONLY-VERIFICATION-v7.json",
            {
                "schema": "phase4-g5-1-final-readonly-verification-v7",
                "status": "PASS",
        ...
        write_json(
            result / "COMPLETE-WALL-v7.json",
            {
                "schema": "phase4-g5-1-complete-wall-v7",
                "status": "PASS",
                "campaign": campaign,
                ...
                "terminal_self_exclusion": "COMPLETE-WALL-v7.json follows the verified final manifest",
            },
        )
        fsync_dir(result)
        ...
        if result.exists() and not (result / "FAILED-v7.json").exists():
            write_json(
                result / "FAILED-v7.json",
                {
                    "schema": "phase4-g5-1-failure-v7",
                    "status": "REVISE",
                    "error": str(error),
