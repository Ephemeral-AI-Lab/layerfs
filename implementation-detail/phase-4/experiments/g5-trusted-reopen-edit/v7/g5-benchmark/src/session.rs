const G5_REQUEST_BYTES_V7: usize = 4_096;
...
impl G5OwnedSemanticPaths {
    fn new(root: &Path, label: &str, metrics: &mut Metrics) -> AnyResult<Self> {
        ...
        let database_capacity =
            g5_checked_sum(&[prefix, "g5-semantic-v7-".len(), label.len(), ".sqlite".len()])?;
        let authority_capacity = database_capacity
            .checked_add(".authority".len())
            .ok_or(CoreError::LengthOverflow)?;
        let expectations_capacity = database_capacity
        ...
        let charge = charge_capacity(metrics, combined)?;
        let database_parts = ["g5-semantic-v7-", label, ".sqlite"];
        let database = g5_rooted_path(root, separator, database_capacity, &database_parts)?;
        let authority = g5_rooted_path(
            root,
            separator,
            authority_capacity,
            &["g5-semantic-v7-", label, ".sqlite.authority"],
        )?;
        let expectations = g5_rooted_path(
            root,
            separator,
            expectations_capacity,
            &["g5-semantic-v7-", label, ".sqlite.expectations"],
        )?;
        Ok(Self {
            database,
            authority,
...
fn g5_read_line<'a>(
    ...
    buffer: &'a mut [u8; G5_REQUEST_BYTES_V7],
...
fn g5_child() -> AnyResult<()> {
    ...
    let mut buffer = [0_u8; G5_REQUEST_BYTES_V7];
    let mut rows = 0_u64;
    let mut owners = G5ChildOwners::default();

    writeln!(output, "{{\"status\":\"READY\",\"schema\":\"phase4-g5-trusted-child-ready-v7\",\"integrity_mode\":\"{mode_name}\",\"mode_provenance\":\"fixed-at-child-start\",\"size_bytes\":{size},\"operation\":\"{operation}\",\"expected_rows\":{expected_rows},\"full_wrapper_forecast_ns\":{forecast_ns},\"full_wrapper_limit_ns\":{limit_ns},\"custody\":\"runner-preverified-borrowed\",\"request_schema\":\"id\\troot\\titeration\\twarmup\\tvalidation\"}}")?;
    output.flush()?;
    while let Some(line) = g5_read_line(&mut input, &mut buffer)? {
        ...
        writeln!(output, "{{\"status\":\"PASS\",\"schema\":\"phase4-g5-trusted-child-row-v7\",\"request_id\":\"{}\",\"integrity_mode\":\"{mode_name}\",\"mode_provenance\":\"fixed-at-child-start\",\"row\":{}}}", request.id, &**report)?;
        output.flush()?;
    ...
    writeln!(output, "{{\"status\":\"PASS\",\"schema\":\"phase4-g5-trusted-child-terminal-v7\",\"integrity_mode\":\"{mode_name}\",\"rows\":{rows},\"expected_rows\":{expected_rows},\"argument_owners\":{},\"request_owners\":{},\"schedule_owners\":{},\"timing_owners\":{},\"report_owners\":{},\"q_current\":{},\"rss\":\"external-high-water-only\"}}", terminal.argument, terminal.request, terminal.schedule, terminal.timing, terminal.report, q_current())?;
    output.flush()?;
    Ok(())
}
...
fn g5_semantic(case: &str, root: &Path) -> AnyResult<()> {
    ...
    g5_terminal_q()?;
    writeln!(output, "{{\"status\":\"PASS\",\"schema\":\"phase4-g5-trusted-semantic-terminal-v7\",\"case\":\"{case}\",\"q_current\":0}}")?;
    output.flush()?;
    Ok(())
}
...
pub(super) fn g5_transport_main() -> AnyResult<()> {
    match g5_arg(1)? {
        "--g5-fixture" => {
            ...
            g5_terminal_q()?;
            println!("{{\"status\":\"PASS\",\"schema\":\"phase4-g5-trusted-fixture-v7\",\"size_bytes\":{size},\"q_current\":0}}" );
            Ok(())
        }
        "--g5-prepare" => {
            g5_require_args(6)?;
            ...
            g5_terminal_q()?;
            println!("{{\"status\":\"PASS\",\"schema\":\"phase4-g5-trusted-prepare-v7\",\"size_bytes\":{size},\"operation\":\"{operation}\",\"iteration\":{iteration},\"q_current\":0}}" );
            Ok(())
        }
        "--g5-child" => g5_child(),
        "--g5-semantic" => {
...
mod g5_transport_tests {
    ...
    fn row_and_semantic_paths_are_exactly_charged_and_dropped() {
        ...
        let root = Path::new("/tmp/layerfs-g5-v7-owned-paths");
        let mut metrics = Metrics::default();
    ...
    fn row_path_error_unwinds_before_decharge() {
        ...
        let result = (|| -> AnyResult<()> {
            let mut metrics = Metrics::default();
            let paths = G5OwnedRowPaths::new(
                Path::new("/tmp/layerfs-g5-v7-error-paths"),
                SOURCE_1,
                "plus1-middle",
                7,
                &mut metrics,
    ...
    fn nested_path_and_report_q_and_terminal_owners_are_derived() {
        ...
        owners.request = Some(
            G5OwnedRowPaths::new(
                Path::new("/tmp/layerfs-g5-v7-nested-q"),
                SOURCE_1,
                "same-middle",
                0,
                &mut outer_metrics,
