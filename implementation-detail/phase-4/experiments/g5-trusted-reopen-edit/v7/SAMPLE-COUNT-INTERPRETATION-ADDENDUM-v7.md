# G5-1 v7 sample-count interpretation addendum

This append-only note corrects one sentence in `REVIEW-SYNTHESIS-v7.md`
without changing `PREREGISTRATION-v7.md` or its schedule.
...
separately inside each comparison. V7 nevertheless prospectively froze the
...
That is a deliberate v7 method choice, not an exact restatement of an
...
forecast is `<=120 s`. If it cannot, v7 is preserved as `PREMEASUREMENT_REVISE`;
only a prospectively created v8 may adopt a narrower interpretation.
