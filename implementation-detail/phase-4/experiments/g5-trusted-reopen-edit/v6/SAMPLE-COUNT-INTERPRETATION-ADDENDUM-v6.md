# G5-1 v6 sample-count interpretation addendum

This append-only note corrects one sentence in `REVIEW-SYNTHESIS-v6.md`
without changing `PREREGISTRATION-v6.md` or its schedule.
...
separately inside each comparison. V6 nevertheless prospectively froze the
...
That is a deliberate v6 method choice, not an exact restatement of an
...
forecast is `<=120 s`. If it cannot, v6 is preserved as `PREMEASUREMENT_REVISE`;
only a prospectively created v7 may adopt a narrower interpretation.
