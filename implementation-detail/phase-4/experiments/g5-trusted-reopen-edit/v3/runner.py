...
REPO = HERE.parents[4]
METHOD = HERE / "method"
SCHEDULE = METHOD / "SCHEDULE-v3.tsv"
EXPECTED = METHOD / "EXPECTED-OUTCOMES-v3.tsv"
INPUT_MANIFEST = METHOD / "INPUT-MANIFEST-v3.tsv"
METHOD_MANIFEST = METHOD / "METHOD-MANIFEST-v3.tsv"
SOURCE_FREEZE = METHOD / "SOURCE-FREEZE-v3.json"
LIMITATIONS = HERE / "LIMITATIONS-v3.json"
STATIC_CLOSURE = HERE / "STATIC-CLOSURE-v3.json"
...
INPUT_ROOT = REPO / f"target/phase4-g5-trusted-reopen-edit-inputs-{DATE}-v3"
SCREEN_RESULT = REPO / f"target/phase4-g5-trusted-reopen-edit-{DATE}-v3-screen"
GATE_RESULT = REPO / f"target/phase4-g5-trusted-reopen-edit-{DATE}-v3"

# Frozen v3 Rust transport.
G5_CHILD_BINARY = HERE / "g5-benchmark/target/release/layerfs-g5-trusted-child-v3"
FIXTURE_FLAG = "--g5-fixture"
PREPARE_FLAG = "--g5-prepare"
CHILD_FLAG = "--g5-child"
SEMANTIC_FLAG = "--g5-semantic"
CHILD_READY_SCHEMA = "phase4-g5-trusted-child-ready-v3"
CHILD_ENVELOPE_SCHEMA = "phase4-g5-trusted-child-row-v3"
CHILD_TERMINAL_SCHEMA = "phase4-g5-trusted-child-terminal-v3"
FIXTURE_SCHEMA = "phase4-g5-trusted-fixture-v3"
PREPARE_SCHEMA = "phase4-g5-trusted-prepare-v3"
SEMANTIC_SCHEMA = "phase4-g5-trusted-semantic-v3"
SEMANTIC_TERMINAL_SCHEMA = "phase4-g5-trusted-semantic-terminal-v3"
REQUEST_FIELDS = (
...
def method_source_names():
    fixed = {
        ...
        for path in (
            HERE / "PREREGISTRATION-v3.md",
            HERE / "REVIEW-SYNTHESIS-v3.md",
            HERE / "SAMPLE-COUNT-INTERPRETATION-ADDENDUM-v3.md",
            HERE / "V2-SUPERSESSION-v3.json",
            LIMITATIONS,
            HERE / "runner.py",
            PRIMARY,
            INDEPENDENT,
...
def write_input_and_method_manifests():
    ...
    freeze = {
        "schema": "phase4-g5-1-source-freeze-v3",
...
def prepare_inputs():
    ...
    if any(path.exists() for path in (INPUT_MANIFEST, METHOD_MANIFEST, SOURCE_FREEZE, DRY_RUN, STATIC_CLOSURE)):
        raise RuntimeError("v3 method/freeze evidence already exists")
    ...
    write_json(
        INPUT_ROOT / "PREPARATION-CUSTODY-v3.json",
        {
            "schema": "phase4-g5-1-input-preparation-v3",
            "status": "PASS",
...
def dry_run():
    ...
    if FULL_WRAPPER_FORECAST_NS > LIMIT_NS["gate"]:
        raise RuntimeError("v3 PREMEASUREMENT_REVISE: conservative full-wrapper forecast exceeds 120 seconds")
    generated_residue = sorted(
        str(path.relative_to(REPO)) for path in HERE.rglob("__pycache__") if path.is_dir()
    )
    value = {
        "schema": "phase4-g5-1-dry-run-v3",
        "status": "PASS",
        "measured_rows": 0,
        ...
        "sample_count_interpretation": "deliberately-stricter-v3-choice-not-unambiguous-user-minimum",
...
def release_lock(lock, result, terminal_verification=None, state="release"):
    ...
    try:
        ...
        attestation = result / "BENCHMARK-LOCK-RELEASE-ATTESTATION-v3.json"
        ...
        value = {
            "schema": "phase4-g5-1-lock-release-v3",
        ...
        write_json(result / "LOCK-RELEASE-v3.json", value)
        return value
    finally:
        os.close(lock["fd"])
        lock["fd"] = None
...
def run_semantic(executable, case, root, result, label):
    stdout = result / f"children-v3/{label}.stdout"
    stderr = result / f"children-v3/{label}.stderr"
    sidecar = result / f"time-v3/{label}.txt"
...
def validate_child_row(envelope):
    ...
    return {
        "schema": "phase4-g5-1-operation-v3",
...
class PersistentChild:
    def __init__(
        ...
        self.stdout_path = result / f"children-v3/{label}.stdout"
        self.stderr_path = result / f"children-v3/{label}.stderr"
        self.time_path = result / f"time-v3/{label}.txt"
...
def analyze(result):
    ...
    for analyzer, name in (
        (PRIMARY, "PRIMARY-ANALYSIS-v3.json"),
        (INDEPENDENT, "INDEPENDENT-RECOMPUTATION-v3.json"),
    ):
        output = result / name
        completed = subprocess.run(
            [
                sys.executable, str(analyzer), str(result / "RAW-v3.jsonl"),
                str(result / "TIMINGS-v3.tsv"), str(SCHEDULE), str(EXPECTED), str(output),
            ],
            cwd=REPO,
            text=True,
            capture_output=True,
    ...
    write_json(
        result / "ANALYZER-AGREEMENT-v3.json",
        {
            "schema": "phase4-g5-1-analyzer-agreement-v3",
...
def run_campaign(campaign):
    ...
    if campaign == "screen":
        raise RuntimeError(
            "v3 screen interface incomplete: S07 protected full-create/range sentinel has no dispatched command; refusing to acquire global lock"
        )
    started, lock = acquire_lock()
    try:
        freeze = verify_freeze(require_static=campaign == "gate")
        result.mkdir(mode=0o700)
        work = result.parent / f"{result.name}-work-v3"
        work.mkdir(mode=0o700)
        for name in ("operands-v3", "children-v3", "time-v3"):
            (result / name).mkdir()
        fsync_dir(result)
        fsync_dir(result.parent)

        g4_copy = result / "operands-v3/frozen-g4-verified"
        g5_copy = result / "operands-v3/g5-verified-trusted"
        ...
        write_json(
            result / "OPERAND-CUSTODY-v3.json",
            {
                "schema": "phase4-g5-1-operand-custody-v3",
        ...
        write_json(
            result / "PREFLIGHT-v3.json",
            {
                "schema": "phase4-g5-1-preflight-v3",
                "status": "PASS",
                "campaign": campaign,
                "branch": BRANCH,
                "checkpoint": CHECKPOINT,
        ...
        write_json(
            result / "ENVIRONMENT-v3.json",
            {
                "schema": "phase4-g5-1-environment-v3",
        ...
        shutil.copyfile(INPUT_MANIFEST, result / "INPUT-CUSTODY-v3.tsv")
        with (result / "INPUT-CUSTODY-v3.tsv").open("rb") as handle:
            os.fsync(handle.fileno())
                    ...
                    semantics.extend(
                        {
                            ...
                            "expectation_id": "native-semantic-v3",
                            "status": value["status"],
                            "error": value["error"],
                        }
                        for value in values
        ...
        write_json(result / "COMMANDS-v3.json", {"schema": "phase4-g5-1-commands-v3", "commands": commands})
        analyze(result)

        if work.parent != result.parent or not work.name.endswith("-work-v3"):
            raise RuntimeError("refusing unsafe work cleanup")
        shutil.rmtree(work)
        fsync_dir(work.parent)
        residue = [str(path) for path in result.parent.glob(f"{result.name}-work-v3")]
        write_json(
            result / "CLEANUP-v3.json",
            {
                "schema": "phase4-g5-1-cleanup-v3",
        ...
        payload_excluded = {
            "PAYLOAD-MANIFEST-v3.tsv", "MEASURED-TERMINAL-v3.json",
            "TERMINAL-VERIFICATION-v3.json", "COMPLETE-WALL-v3.json",
            "BENCHMARK-LOCK-RELEASE-ATTESTATION-v3.json", "LOCK-RELEASE-v3.json",
            "FINAL-ARTIFACT-HASHES-v3.tsv", "FINAL-READONLY-VERIFICATION-v3.json",
        }
        payload = result / "PAYLOAD-MANIFEST-v3.tsv"
        ...
        terminal = result / "MEASURED-TERMINAL-v3.json"
        write_json(
            terminal,
            {
                "schema": "phase4-g5-1-measured-terminal-v3",
                "status": "PASS",
                "campaign": campaign,
                "rows": len(timings),
        ...
        verification = result / "TERMINAL-VERIFICATION-v3.json"
        write_json(
            verification,
            {
                "schema": "phase4-g5-1-terminal-verification-v3",
                "status": "PASS",
        ...
        write_json(
            result / "COMPLETE-WALL-v3.json",
            {
                "schema": "phase4-g5-1-complete-wall-v3",
                "status": "PASS",
                "campaign": campaign,
        ...
        final = result / "FINAL-ARTIFACT-HASHES-v3.tsv"
        write_text(final, manifest_text(result, excluded={final.name, "FINAL-READONLY-VERIFICATION-v3.json"}))
        final_count = verify_manifest(result, final, "result_relative_path")
        write_json(
            result / "FINAL-READONLY-VERIFICATION-v3.json",
            {
                "schema": "phase4-g5-1-final-readonly-verification-v3",
                "status": "PASS",
        ...
        if result.exists() and not (result / "FAILED-v3.json").exists():
            write_json(
                result / "FAILED-v3.json",
                {
                    "schema": "phase4-g5-1-failure-v3",
                    "status": "REVISE",
                    "error": str(error),
