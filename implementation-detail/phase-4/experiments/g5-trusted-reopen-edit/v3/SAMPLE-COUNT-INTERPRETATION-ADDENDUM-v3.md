# G5-1 v3 sample-count interpretation addendum

This append-only note corrects one sentence in `REVIEW-SYNTHESIS-v3.md`
without changing `PREREGISTRATION-v3.md` or its schedule.
...
separately inside each comparison. V3 nevertheless prospectively froze the
...
That is a deliberate v3 method choice, not an exact restatement of an
...
forecast is `<=120 s`. If it cannot, v3 is preserved as `PREMEASUREMENT_REVISE`;
only a prospectively created v3 may adopt a narrower interpretation.
