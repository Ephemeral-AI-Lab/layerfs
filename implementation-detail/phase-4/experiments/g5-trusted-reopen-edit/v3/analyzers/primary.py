...
OP_SCHEMA = "phase4-g5-1-operation-v3"
TERMINAL_SCHEMA = "phase4-g5-trusted-child-terminal-v3"
...
def analyze(raw_path, timing_path, schedule_path, expected_path):
    ...
    semantic_rows = [row for row in raw if row.get("schema") == "phase4-g5-trusted-semantic-v3"]
    semantic_terminals = [
        row for row in raw if row.get("schema") == "phase4-g5-trusted-semantic-terminal-v3"
    ]
    ...
    return {"schema": "phase4-g5-1-primary-analysis-v3", "status": "PASS" if not failures else "REVISE", "normalized": normalized}
