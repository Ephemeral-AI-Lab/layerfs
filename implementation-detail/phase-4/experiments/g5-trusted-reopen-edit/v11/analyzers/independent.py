...
def recompute(raw_file, timing_file, schedule_file, expected_file):
    ...
    rows = [value for value in documents if value.get("schema") == "phase4-g5-1-operation-v11"]
    ...
    sentinel = [value for value in documents if value.get("schema") == "phase4-g5-1-protected-sentinel-v11"]
    ...
    audit_lifecycle(
        ...
        sentinel,
        campaign,
        pathlib.Path(raw_file).parent / "PRODUCT-CHILD-LIFECYCLE-v11.json",
        problems,
    )
    audit_work_roots(
        work_root_rows,
        rows,
        pathlib.Path(raw_file).parent / "WORK-ROOT-LIFECYCLE-v11.json",
        problems,
    )
    if campaign == "screen":
        counts = {}
    ...
    return {
        "schema": "phase4-g5-1-independent-recomputation-v11",
...
def self_check():
    ...
    print(packed({"status": "PASS", "checks": 9, "mutations_rejected": sorted(mutations) + ["legacy-catalog", "paired-receipt-hash"]}))
    return 0
