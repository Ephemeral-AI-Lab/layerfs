...
OP_SCHEMA = "phase4-g5-1-operation-v11"
...
LIFECYCLE_SCHEMA = "phase4-g5-1-product-child-lifecycle-v11"
...
CLONE_SCHEMA = "g5-v11-native-clone-receipt-v1"
CLONE_FIELDS = {
...
PHYSICAL_SCHEMA = "g5-v11-physical-allocation-observation-v1"
PHYSICAL_FIELDS = {
...
WORK_ROOT_SCHEMA = "phase4-g5-1-work-root-lifecycle-v11"
WORK_ROOT_FIELDS = {
...
INPUT_MANIFEST = pathlib.Path(__file__).resolve().parents[1] / "method/INPUT-MANIFEST-v11.tsv"
...
def analyze(raw_path, timing_path, schedule_path, expected_path):
    ...
    sentinel_rows = [row for row in raw if row.get("schema") == "phase4-g5-1-protected-sentinel-v11"]
    ...
    validate_child_lifecycle(
        ...
        sentinel_rows,
        campaign,
        pathlib.Path(raw_path).parent / "PRODUCT-CHILD-LIFECYCLE-v11.json",
        failures,
    )
    validate_work_root_lifecycle(
        work_root_rows,
        operations,
        pathlib.Path(raw_path).parent / "WORK-ROOT-LIFECYCLE-v11.json",
        failures,
    )
    if campaign == "screen":
        counts = {}
    ...
    return {"schema": "phase4-g5-1-primary-analysis-v11", "status": "PASS" if not failures else "REVISE", "normalized": normalized}
...
def self_check():
    ...
    print(compact({"status": "PASS", "checks": 9, "mutations_rejected": sorted(mutations) + ["legacy-catalog", "paired-receipt-hash"]}))
    return 0
