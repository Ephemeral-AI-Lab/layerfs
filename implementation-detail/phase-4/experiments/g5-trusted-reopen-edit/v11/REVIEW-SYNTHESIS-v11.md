...
## V10 terminal and accepted v11 method repair

V10 honestly closed as `PREMEASUREMENT_REVISE` after its zero-row forecast
reached `442.019522623 s` against the fixed `120 s` limit. It acquired no lock,
started no screen or gate, and emitted zero measured rows. The complete release,
93-row input manifest, 66-row method manifest, freeze/rehash, calibration, dry,
REVISE, and disposition chain is bound by `V10-SUPERSESSION-v11.json`.

All three targeted audits agreed that the product/trust path is sound and the
repeated whole-object-table Python scan is the singular forecast blocker: it
consumed about 98.7% of each wrapper sample. The controlling plan excludes
unreachable rows from snapshot verification and reserves all-row CAS audit for
a future separate API. V11 therefore changes only the runner, analyzers, and
method. It reuses the exact unchanged v10 release and 93-file sealed v10 input
root as named, rehashed external operands (`release_reuse=true`,
`input_reuse=true`) and reuses no row or timing result.

V11 keeps the 200-arm schedule, 56 checkpoints, half-slower calibration floor,
...
- use native APFS clone receipts, schema `g5-v11-native-clone-receipt-v1`, to
...
- emit constant-size rooted state, schema `g5-v11-rooted-logical-state-v1`,
...
`FREEZE-VERIFICATION-v11.json` is classified
...
`PRODUCT-CHILD-LIFECYCLE-v11.json` under schema
`phase4-g5-1-product-child-lifecycle-v11`. Per-arm cleanup receipts use schema
`phase4-g5-1-arm-cleanup-receipt-v11`; their ordered terminal record is likewise
duplicated as `WORK-ROOT-LIFECYCLE-v11.json` under schema
`phase4-g5-1-work-root-lifecycle-v11`.
...
samples each cover native clone, allowed-inventory evidence, constant-size
published-visible-state extraction, sidecar hashes, shared mutation-work hash
and wrapper assembly, evidence file/directory
fsync, and immediate cleanup. Four times the slowest sample is the conservative per-arm
...
`wrapper-calibration-intent-v11`, `wrapper-calibration-sample-v11`, and
`wrapper-calibration-result-v11`, all under the `phase4-g5-1-` prefix. The
...
intent embeds `phase4-g5-1-wrapper-calibration-plan-v11`, retains the complete
...
closes v11 as `PREMEASUREMENT_REVISE` before screen; no schedule or threshold
is reduced.

`DRY-RUN-INTENT-v11.json` is durably written before either calibration begins.
A single outer failure funnel then covers wrapper calibration, hash calibration,
forecasting, and sealing: it always writes `DRY-RUN-FAILED-v11.json` before a
REVISE `DRY-RUN-DISPOSITION-v11.json`, even when the wrapper result is absent.
...
V11 nevertheless voluntarily freezes the stricter v1-lineage design: each
