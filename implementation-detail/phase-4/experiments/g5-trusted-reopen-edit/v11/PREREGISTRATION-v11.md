# Phase 4 G5-1 v11 preregistration
...
The candidate remains CAS + CDC + COW + canonical K64/F64 + SQLite. V11 changes
...
Any failure preserves v11. No observed row may be deleted, replaced, reordered,
or selectively rerun. A source or method repair creates v12.
...
inventory evidence, constant-size published-visible-state extraction (two
query-only control PRAGMAs, one head row, three physical PRAGMAs, and the fixed
schema-rootpage rows), sidecar hashing, the
shared mutation-work hash and wrapper-evidence assembly, evidence-file and
evidence-directory fsync, and immediate isolated-root
...
`phase4-g5-1-wrapper-calibration-intent-v11`,
`phase4-g5-1-wrapper-calibration-sample-v11`, and
`phase4-g5-1-wrapper-calibration-result-v11`.
...
`phase4-g5-1-wrapper-calibration-plan-v11` and records the complete candidate
...
must write and fsync the top-level `DRY-RUN-INTENT-v11.json` before invoking
...
write and fsync `DRY-RUN-FAILED-v11.json` first, then write and fsync a
`DRY-RUN-DISPOSITION-v11.json` with `status=REVISE` that binds the FAILED hash
and explicitly records absent optional artifacts. A success or calibrated
overrun writes and fsyncs complete `DRY-RUN-v11.json`, including both
...
`PREMEASUREMENT-REVISE-v11.json` and stops before screen. A missing dry-run or
failure disposition is never treated as an authoritative PASS or reconstructed
from an earlier attempt.

V10 is immutably superseded by `V10-SUPERSESSION-v11.json`, which binds the
complete v10 release, input/method manifests, source freeze, independent freeze
verification, both calibration intents/results/raw files, dry run,
`PREMEASUREMENT-REVISE-v10.json`, and terminal disposition. V10 started no
screen or gate, acquired no benchmark lock, and emitted zero measured rows.

V11 is a runner/analyzer/method-only revision. It reuses the unchanged v10
...
are not copied, renamed, or represented as v11-generated. V11 reuses no v10
row or timing observation. The new v11 input manifest rebinds the same sealed
files byte-for-byte before the v11 method manifest and source freeze are
generated.

After every controlling product, harness, runner, analyzer, document,
schedule, expectation, limitation, and supersession byte has settled, v11
...
child. It writes `FREEZE-VERIFICATION-v11.json` with classification
...
V11 voluntarily freezes a deliberately stricter v1-lineage design: each
...
`phase4-g5-1-product-child-lifecycle-v11`. The two copies must be byte-value
...
child, v11 verifies the complete sealed input manifest exactly once: path,
...
`g5-v11-native-clone-receipt-v1`. Its ordered `entries` bind each source master
...
inspection for a future separate API. V11 therefore removes the external
...
`g5-v11-rooted-logical-state-v1` from one query-only, autocommit, constant-row
...
V11 does not claim physical SQLite file-byte parity. The sibling
`physical_allocation_observation` remains schema
`g5-v11-physical-allocation-observation-v1`, classification
...
Each wrapper receipt uses schema `phase4-g5-1-arm-cleanup-receipt-v11` and is
repeated in order by the raw and standalone
`phase4-g5-1-work-root-lifecycle-v11` record. Started and cleaned roots equal
