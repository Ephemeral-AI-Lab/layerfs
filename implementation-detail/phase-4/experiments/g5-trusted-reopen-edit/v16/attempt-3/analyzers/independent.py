...
LIFECYCLE_SCHEMA = "phase4-g5-1-product-child-lifecycle-v16"
...
CLONE_SCHEMA = "g5-v16-native-clone-receipt-v1"
CLONE_TOP = frozenset((
...
PHYSICAL_SCHEMA = "g5-v16-physical-allocation-observation-v1"
...
WORK_ROOT_SCHEMA = "phase4-g5-1-work-root-lifecycle-v16"
...
INPUT_MANIFEST = pathlib.Path(__file__).resolve().parents[2] / "method/INPUT-MANIFEST-v16.tsv"
DRY_RUN = pathlib.Path(__file__).resolve().parents[2] / "DRY-RUN-v16.json"
...
def audit_prearm(
    ...
    facts = (
        ...
        type(evidence) is dict
        and evidence.get("schema") == "phase4-g5-1-prearm-wrapper-initialization-evidence-v16",
        type(evidence) is dict
...
def recompute(raw_file, timing_file, schedule_file, expected_file):
    ...
    rows = [value for value in documents if value.get("schema") == "phase4-g5-1-operation-v16"]
    ...
    sentinel = [value for value in documents if value.get("schema") == "phase4-g5-1-protected-sentinel-v16"]
    ...
    initialization_rows = [
        ...
        if value.get("schema") == "phase4-g5-1-prearm-wrapper-initialization-v16"
    ]
    ...
    artifact = pathlib.Path(raw_file).parent / "PREARM-WRAPPER-INITIALIZATION-v16.json"
    try:
        artifact_bytes = artifact.read_bytes()
    except OSError:
        artifact_bytes = b""
    evidence_path = pathlib.Path(raw_file).parent / (
        "PREARM-WRAPPER-INITIALIZATION-EVIDENCE-v16.json"
    )
    try:
    ...
    audit_lifecycle(
        ...
        sentinel,
        campaign,
        pathlib.Path(raw_file).parent / "PRODUCT-CHILD-LIFECYCLE-v16.json",
        problems,
    )
    audit_work_roots(
        work_root_rows,
        rows,
        pathlib.Path(raw_file).parent / "WORK-ROOT-LIFECYCLE-v16.json",
        problems,
    )
    if campaign == "screen":
        counts = {}
    ...
    return {
        "schema": "phase4-g5-1-independent-recomputation-v16",
