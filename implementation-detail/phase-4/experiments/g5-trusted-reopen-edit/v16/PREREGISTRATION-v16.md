# Phase 4 G5-1 v16 preregistration
...
The candidate remains CAS + CDC + COW + canonical K64/F64 + SQLite. V16 changes
...
Any failure preserves v16. No observed row may be deleted, replaced, reordered,
or selectively rerun. A source or method repair creates v16.
...
`phase4-g5-1-wrapper-calibration-intent-v16`,
`phase4-g5-1-wrapper-initialization-sample-v16`,
`phase4-g5-1-wrapper-calibration-sample-v16`, and
`phase4-g5-1-wrapper-calibration-result-v16`.
...
`phase4-g5-1-wrapper-calibration-plan-v16` and records the complete candidate
...
must write and fsync the top-level `DRY-RUN-INTENT-v16.json` before invoking
...
write and fsync `DRY-RUN-FAILED-v16.json` first, then write and fsync a
`DRY-RUN-DISPOSITION-v16.json` with `status=REVISE` that binds the FAILED hash
and explicitly records absent optional artifacts. A success or calibrated
overrun writes and fsyncs complete `DRY-RUN-v16.json`, including both
...
`PREMEASUREMENT-REVISE-v16.json` and stops before screen. A missing dry-run or
...
V14 is byte-preserved by `V14-SUPERSESSION-v16.json`. Its zero-row passed, and
...
V16 changes the shared product timer equation and uses a fresh release. The
...
separate complete-lifecycle evidence. V16 reuses only the independently rehashed
...
are not copied, renamed, or represented as v16-generated. V16 reuses no v14
product row or timing observation. The new v16 input manifest rebinds the same sealed
files byte-for-byte before the v16 method manifest and source freeze are
generated.

V15 attempts 1–5 remain append-only and are not measurement authority for v16.
...
while the lifecycle sum added that authority again. V16 removes only that term
...
The final `PREARM-WRAPPER-INITIALIZATION-v16.json` write is outside that bound
...
schedule, expectation, limitation, and supersession byte has settled, v16
...
child. It writes `FREEZE-VERIFICATION-v16.json` with classification
...
V16 voluntarily freezes a deliberately stricter v1-lineage design: each
...
`phase4-g5-1-product-child-lifecycle-v16`. The two copies must be byte-value
...
child, v16 verifies the complete sealed input manifest exactly once: path,
...
`g5-v16-native-clone-receipt-v1`. Its ordered `entries` bind each source master
...
inspection for a future separate API. V16 therefore removes the external
...
`g5-v16-rooted-logical-state-v1` from one query-only, autocommit, constant-row
...
V16 does not claim physical SQLite file-byte parity. The sibling
`physical_allocation_observation` remains schema
`g5-v16-physical-allocation-observation-v1`, classification
...
Each wrapper receipt uses schema `phase4-g5-1-arm-cleanup-receipt-v16` and is
repeated in order by the raw and standalone
`phase4-g5-1-work-root-lifecycle-v16` record. Started and cleaned roots equal
