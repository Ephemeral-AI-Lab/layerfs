...
## V15 terminal and accepted v16 product repair
...
V16 removes same-open authority only from the CompleteRoundTrip lifecycle sum
and builds a fresh release. The first-edit decision equation, workload, trust,
...
product row or timing is promoted to v16 authority.

V16 keeps the 200-arm schedule, 56 checkpoints, half-slower calibration floor,
...
- use native APFS clone receipts, schema `g5-v16-native-clone-receipt-v1`, to
...
- emit constant-size rooted state, schema `g5-v16-rooted-logical-state-v1`,
...
`FREEZE-VERIFICATION-v16.json` is classified
...
`PRODUCT-CHILD-LIFECYCLE-v16.json` under schema
`phase4-g5-1-product-child-lifecycle-v16`. Per-arm cleanup receipts use schema
`phase4-g5-1-arm-cleanup-receipt-v16`; their ordered terminal record is likewise
duplicated as `WORK-ROOT-LIFECYCLE-v16.json` under schema
`phase4-g5-1-work-root-lifecycle-v16`.
...
`wrapper-calibration-intent-v16`, `wrapper-initialization-sample-v16`,
`wrapper-calibration-sample-v16`, and `wrapper-calibration-result-v16`. The
...
intent embeds `phase4-g5-1-wrapper-calibration-plan-v16`, retains the complete
...
`phase4-g5-1-prearm-wrapper-initialization-v16` record after lock/custody and
...
closes v16 as `PREMEASUREMENT_REVISE` before screen; no schedule or threshold
is reduced.

`DRY-RUN-INTENT-v16.json` is durably written before either calibration begins.
A single outer failure funnel then covers wrapper calibration, hash calibration,
forecasting, and sealing: it always writes `DRY-RUN-FAILED-v16.json` before a
REVISE `DRY-RUN-DISPOSITION-v16.json`, even when the wrapper result is absent.
...
V16 nevertheless voluntarily freezes the stricter v1-lineage design: each
