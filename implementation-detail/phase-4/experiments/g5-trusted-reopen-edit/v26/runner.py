...
V24 = HISTORY / "v24"
METHOD = HERE / "method"
SCHEDULE = METHOD / "SCHEDULE-v26.tsv"
EXPECTED = METHOD / "EXPECTED-OUTCOMES-v26.tsv"
INPUT_MANIFEST = METHOD / "INPUT-MANIFEST-v26.tsv"
METHOD_MANIFEST = METHOD / "METHOD-MANIFEST-v26.tsv"
SOURCE_FREEZE = METHOD / "SOURCE-FREEZE-v26.json"
FREEZE_VERIFICATION = HERE / "FREEZE-VERIFICATION-v26.json"
LIMITATIONS = HERE / "LIMITATIONS-v26.json"
STATIC_CLOSURE = HERE / "STATIC-CLOSURE-ATTEMPT-2-v26.json"
DRY_RUN = HERE / "DRY-RUN-v26.json"
DRY_RUN_INTENT = HERE / "DRY-RUN-INTENT-v26.json"
DRY_RUN_CALIBRATION_STDOUT = HERE / "DRY-RUN-CALIBRATION-v26.stdout"
DRY_RUN_CALIBRATION_STDERR = HERE / "DRY-RUN-CALIBRATION-v26.stderr"
DRY_RUN_CALIBRATION_TERMINAL = HERE / "DRY-RUN-CALIBRATION-TERMINAL-v26.json"
DRY_RUN_DISPOSITION = HERE / "DRY-RUN-DISPOSITION-v26.json"
DRY_RUN_FAILED = HERE / "DRY-RUN-FAILED-v26.json"
PREMEASUREMENT_REVISE = HERE / "PREMEASUREMENT-REVISE-v26.json"
WRAPPER_CALIBRATION_INTENT = HERE / "WRAPPER-CALIBRATION-INTENT-v26.json"
WRAPPER_CALIBRATION_RAW = HERE / "WRAPPER-CALIBRATION-RAW-v26.jsonl"
WRAPPER_CALIBRATION_RESULT = HERE / "WRAPPER-CALIBRATION-RESULT-v26.json"
...
SCREEN_RESULT = REPO / f"target/phase4-g5-trusted-reopen-edit-{DATE}-v26-screen"
GATE_RESULT = REPO / f"target/phase4-g5-trusted-reopen-edit-{DATE}-v26"
WRAPPER_CALIBRATION_ROOT = (
    REPO / f"target/phase4-g5-trusted-reopen-edit-wrapper-calibration-{DATE}-v26"
)

# V26 is a clean prospective method rerun of the unchanged hash-bound v24 product.
G5_CHILD_BINARY = V24 / "g5-benchmark/target/release/layerfs-g5-trusted-child-v24"
...
SENTINEL_SCHEMA = "phase4-g5-1-protected-sentinel-v26"
REQUEST_FIELDS = (
...
V24_SUPERSESSION = HERE / "V24-SUPERSESSION-v26.json"
...
FORECAST_MODEL_VERSION = "phase4-g5-1-v26-fast-law-forecast-v5"
...
PHYSICAL_ALLOCATION_SCHEMA = "g5-v26-physical-allocation-observation-v1"
...
CHILD_LIFECYCLE_SCHEMA = "phase4-g5-1-product-child-lifecycle-v26"
RSS_CLASSIFICATION = "PerProductChildRetainedPeak"
SYNCHRONOUS_RSS_KIND = "synchronous-one-shot"
ARM_CLEANUP_SCHEMA = "phase4-g5-1-arm-cleanup-receipt-v26"
WORK_ROOT_LIFECYCLE_SCHEMA = "phase4-g5-1-work-root-lifecycle-v26"
...
def verify_repository_identity():
    ...
    for path, expected in (
        ...
        (G5_CHILD_BINARY, V24_RELEASE_SHA256),
        (V24_ANTI_CHEAT, V24_ANTI_CHEAT_SHA256),
        (V24_SUPERSESSION, V24_SUPERSESSION_SHA256),
        (PRECONDITION_DIAGNOSTIC, PRECONDITION_DIAGNOSTIC_SHA256),
    ):
        verify_file(path, expected)
    supersession = json.loads(V24_SUPERSESSION.read_text(encoding="utf-8"))
    if (
        supersession.get("status") != "PASS"
        or supersession.get("product_release_reuse") is not True
        or supersession.get("sealed_input_reuse") is not True
        or supersession.get("measured_rows_reused") is not False
        or supersession.get("v26_authority_requires_fresh_rows") is not True
    ):
        raise RuntimeError("v24-to-v26 supersession control mismatch")
...
def method_source_names():
    fixed = {
        ...
        for path in (
            HERE / "PREREGISTRATION-v26.md",
            HERE / "REVIEW-SYNTHESIS-v26.md",
            HERE / "SAMPLE-COUNT-INTERPRETATION-ADDENDUM-v26.md",
            HERE / "FOCUSED-TEST-ATTEMPTS-v26.json",
            HERE / "PREMEASUREMENT-READINESS-AUDIT-v26.json",
            LIMITATIONS,
            HERE / "runner.py",
            PRIMARY,
            INDEPENDENT,
            ...
            V24_SUPERSESSION,
            PRECONDITION_DIAGNOSTIC,
            V24 / "g5-benchmark/Cargo.toml",
            V24 / "g5-benchmark/Cargo.lock",
            V24 / "g5-benchmark/build.rs",
            V24 / "g5-benchmark/src/main.rs",
            V24 / "g5-benchmark/src/session.rs",
        )
    }
...
def freeze_interface_contract():
    return {
        "product_operand_version": "v24",
        "product_release_reuse": True,
        "product_release_sha256": V24_RELEASE_SHA256,
        ...
        "wrapper_calibration": {
            ...
            "intent_schema": "phase4-g5-1-wrapper-calibration-intent-v26",
            "initialization_schema": "phase4-g5-1-wrapper-initialization-sample-v26",
            "sample_schema": "phase4-g5-1-wrapper-calibration-sample-v26",
            "result_schema": "phase4-g5-1-wrapper-calibration-result-v26",
...
def write_input_and_method_manifests():
    ...
    freeze = {
        "schema": "phase4-g5-1-source-freeze-v26",
        ...
        "post_freeze_verification": {
            ...
            "schema": "phase4-g5-1-freeze-verification-v26",
...
def current_freeze_bindings(freeze):
    ...
    required_authority_relatives = {
        str(V24_SUPERSESSION.relative_to(REPO)),
    ...
    return {
        ...
        "v24_supersession_sha256": sha256(V24_SUPERSESSION),
...
def run_preparation_command(
    ...
    chronology = evidence_root / "CHRONOLOGY-v26.jsonl"
    ...
    write_json(
        intent,
        {
            "schema": "phase4-g5-1-preparation-command-intent-v26",
            "ordinal": ordinal,
            "label": label,
            "argv": command,
    ...
    terminal = {
        "schema": "phase4-g5-1-preparation-command-terminal-v26",
        "ordinal": ordinal,
        "label": label,
...
def prepare_inputs():
    ...
    if any(
        ...
        raise RuntimeError("v26 method/freeze evidence already exists")
    if not os.access(G5_CHILD_BINARY, os.X_OK):
        raise RuntimeError(f"hash-bound v26 child is not executable: {G5_CHILD_BINARY}")
    ...
    if future_input not in sources or missing:
        raise RuntimeError(f"v26 method source preflight mismatch: {missing}")
    write_input_and_method_manifests()
    if verify_sealed_input_manifest() != 93:
        raise RuntimeError("v26 reused input manifest cardinality mismatch")
...
def verify_dry_run(freeze):
    ...
    required = {
        "schema": "phase4-g5-1-dry-run-v26",
        "status": "PASS",
        "measured_rows": 0,
        "benchmark_child_processes_started": 0,
        "stores_opened": 0,
    ...
    intent_required = {
        "schema": "phase4-g5-1-dry-run-intent-v26",
        "status": "STARTED",
        "branch": BRANCH,
        "head": CHECKPOINT,
    ...
    if (
        ...
        or disposition.get("schema") != "phase4-g5-1-dry-run-disposition-v26"
...
def verify_freeze(require_static=False, require_dry=False):
    ...
    if require_static:
        ...
        screen_terminal = SCREEN_RESULT / "TERMINAL-VERIFICATION-v26.json"
        screen_final_manifest = SCREEN_RESULT / "FINAL-ARTIFACT-HASHES-v26.tsv"
        screen_final_verification = SCREEN_RESULT / "FINAL-READONLY-VERIFICATION-v26.json"
        screen_complete_wall = SCREEN_RESULT / "COMPLETE-WALL-v26.json"
        required = {
            "schema": "phase4-g5-1-static-closure-v26",
            "status": "PASS",
            "source_freeze_sha256": sha256(SOURCE_FREEZE),
            "tracked_diff_sha256": freeze["tracked_diff_sha256"],
...
def gate_workload_enumeration():
    ...
    return {
        "schema": "phase4-g5-1-prospective-workload-v26",
...
def hash_calibration():
    ...
    write_json(
        DRY_RUN_CALIBRATION_TERMINAL,
        {
            "schema": "phase4-g5-1-dry-run-calibration-terminal-v26",
            "status": "RETURNED",
            "return_code": completed.returncode,
    ...
    return {
        "schema": "phase4-g5-1-hash-calibration-v26",
...
def wrapper_calibration_plan():
    ...
    plan = {
        "schema": "phase4-g5-1-wrapper-calibration-plan-v26",
...
def run_wrapper_calibration(freeze, plan):
    ...
    intent = {
        "schema": "phase4-g5-1-wrapper-calibration-intent-v26",
        "status": "STARTED",
    ...
    try:
        ...
        initialization_evidence = {
            "schema": "phase4-g5-1-wrapper-initialization-evidence-v26",
        ...
        initialization = {
            "schema": "phase4-g5-1-wrapper-initialization-sample-v26",
            "status": "PASS",
        ...
        for sample_index in range(1, WRAPPER_CALIBRATION_SAMPLES + 1):
            ...
            representative = {
                "schema": "phase4-g5-1-wrapper-calibration-representative-arm-v26",
                "sample": sample_index,
                "master": master["relative_path"],
            ...
            sample = {
                "schema": "phase4-g5-1-wrapper-calibration-sample-v26",
                "status": "PASS",
                "sample": sample_index,
                "master": master["relative_path"],
                "phases_ns": phases,
        ...
        try:
            ...
            if WRAPPER_CALIBRATION_ROOT.exists():
                if (
                    ...
                    != f"phase4-g5-trusted-reopen-edit-wrapper-calibration-{DATE}-v26"
                ):
    ...
    result = {
        "schema": "phase4-g5-1-wrapper-calibration-result-v26",
        "status": "PASS",
        "classification": "ZeroProductWrapperCalibration",
...
def verify_wrapper_calibration(freeze):
    ...
    if (
        ...
        != "phase4-g5-1-wrapper-initialization-sample-v26"
    ...
    for sample_index, sample in enumerate(samples, start=1):
        ...
        if (
            sample.get("schema") != "phase4-g5-1-wrapper-calibration-sample-v26"
...
def dry_failure_documents(error_type, error_message, intent_sha256, collected, artifacts):
    failed = {
        "schema": "phase4-g5-1-dry-run-failed-v26",
        "status": "REVISE",
    ...
    disposition = {
        "schema": "phase4-g5-1-dry-run-disposition-v26",
        "status": "REVISE",
        "classification": "UNEXPECTED_DRY_RUN_FAILURE",
        "intent_sha256": intent_sha256,
...
def dry_run():
    ...
    if any(path.exists() for path in evidence_paths):
        raise RuntimeError("v26 dry-run evidence already exists")
    if not FREEZE_VERIFICATION.is_file():
        raise RuntimeError("v26 dry-run requires durable freeze verification")
    ...
    try:
        ...
        value = {
            ...
            "sample_count_interpretation": "deliberately-stricter-v26-choice-not-unambiguous-user-minimum",
        ...
        if status == "REVISE":
            write_json(
                PREMEASUREMENT_REVISE,
                {
                    "schema": "phase4-g5-1-premeasurement-revise-v26",
                    "status": "REVISE",
...
def release_lock(lock, result, terminal_verification=None, state="release"):
    ...
    try:
        ...
        attestation = result / "BENCHMARK-LOCK-RELEASE-ATTESTATION-v26.json"
        ...
        value = {
            "schema": "phase4-g5-1-lock-release-v26",
        ...
        write_json(result / "LOCK-RELEASE-v26.json", value)
        return value
    finally:
        os.close(lock["fd"])
        lock["fd"] = None
...
def run_semantic(executable, case, root, result, label, lifecycle=None):
    stdout = result / f"children-v26/{label}.stdout"
    stderr = result / f"children-v26/{label}.stderr"
    sidecar = result / f"time-v26/{label}.txt"
...
def validate_child_row(envelope):
    ...
    return {
        "schema": "phase4-g5-1-operation-v26",
...
class PersistentChild:
    def __init__(
        ...
        self.stdout_path = result / f"children-v26/{self.label}.stdout"
        self.stderr_path = result / f"children-v26/{self.label}.stderr"
        self.time_path = result / f"time-v26/{self.label}.txt"
...
def analyze(result):
    ...
    for analyzer, name in (
        (PRIMARY, "PRIMARY-ANALYSIS-v26.json"),
        (INDEPENDENT, "INDEPENDENT-RECOMPUTATION-v26.json"),
    ):
        output = result / name
        completed = subprocess.run(
            [
                sys.executable, str(analyzer), str(result / "RAW-v26.jsonl"),
                str(result / "TIMINGS-v26.tsv"), str(SCHEDULE), str(EXPECTED), str(output),
            ],
            cwd=REPO,
            text=True,
            capture_output=True,
    ...
    write_json(
        result / "ANALYZER-AGREEMENT-v26.json",
        {
            "schema": "phase4-g5-1-analyzer-agreement-v26",
...
def run_prearm_wrapper_initialization(result, freeze, dry, lock):
    ...
    evidence = {
        "schema": "phase4-g5-1-prearm-wrapper-initialization-evidence-v26",
    ...
    evidence_path = result / "PREARM-WRAPPER-INITIALIZATION-EVIDENCE-v26.json"
    write_json(evidence_path, evidence)
    total_ns = max(1, time.monotonic_ns() - started)
    value = {
        **evidence,
        "schema": "phase4-g5-1-prearm-wrapper-initialization-v26",
    ...
    write_json(result / "PREARM-WRAPPER-INITIALIZATION-v26.json", value)
    if value["status"] != "PASS":
...
def ladder_prelock(campaign):
    ...
    if campaign == "gate":
        paths.extend(
            (
                SCREEN_RESULT / "TERMINAL-VERIFICATION-v26.json",
                SCREEN_RESULT / "FINAL-ARTIFACT-HASHES-v26.tsv",
                SCREEN_RESULT / "FINAL-READONLY-VERIFICATION-v26.json",
                SCREEN_RESULT / "COMPLETE-WALL-v26.json",
                STATIC_CLOSURE,
            )
        )
...
def run_campaign(campaign):
    ...
    try:
        ...
        work = result.parent / f"{result.name}-work-v26"
        work.mkdir(mode=0o700)
        work_roots = ArmWorkRootLifecycle(work)
        for name in ("operands-v26", "children-v26", "time-v26"):
            (result / name).mkdir()
        fsync_dir(result)
        fsync_dir(result.parent)

        g4_copy = result / "operands-v26/frozen-g4-verified"
        g5_copy = result / "operands-v26/g5-verified-trusted"
        ...
        write_json(
            result / "OPERAND-CUSTODY-v26.json",
            {
                "schema": "phase4-g5-1-operand-custody-v26",
        ...
        write_json(
            result / "PREFLIGHT-v26.json",
            {
                "schema": "phase4-g5-1-preflight-v26",
                "status": "PASS",
                "campaign": campaign,
                "branch": BRANCH,
                "checkpoint": CHECKPOINT,
        ...
        write_json(
            result / "DATABASE-PRECONDITION-FORECAST-v26.json",
            {
                "schema": "phase4-g5-1-database-precondition-forecast-v26",
                "status": "PASS",
                "campaign": campaign,
                "timing_claim": "CacheWarmPreconditionedNotColdReopen",
        ...
        write_json(
            result / "ENVIRONMENT-v26.json",
            {
                "schema": "phase4-g5-1-environment-v26",
        ...
        shutil.copyfile(INPUT_MANIFEST, result / "INPUT-CUSTODY-v26.tsv")
        with (result / "INPUT-CUSTODY-v26.tsv").open("rb") as handle:
            os.fsync(handle.fileno())
                    ...
                    semantics.extend(
                        {
                            ...
                            "expectation_id": "native-semantic-v26",
                            "status": value["status"],
                            "error": value["error"],
                        }
                        for value in values
        ...
        write_json(result / "PRODUCT-CHILD-LIFECYCLE-v26.json", child_lifecycle)
        write_json(result / "WORK-ROOT-LIFECYCLE-v26.json", work_root_lifecycle)

        write_text(result / "RAW-v26.jsonl", "".join(compact(row) + "\n" for row in raw))
        timing_fields = (
        ...
        write_text(
            result / "TIMINGS-v26.tsv",
            "\t".join(timing_fields) + "\n"
            + "".join("\t".join(str(row[name]) for name in timing_fields) + "\n" for row in timings),
        )
        write_text(
            result / "SEMANTIC-FAULT-RESULTS-v26.tsv",
        ...
        write_json(result / "COMMANDS-v26.json", {"schema": "phase4-g5-1-commands-v26", "commands": commands})
        analyze(result)

        if work.parent != result.parent or not work.name.endswith("-work-v26"):
            raise RuntimeError("refusing unsafe work cleanup")
        shutil.rmtree(work)
        fsync_dir(work.parent)
        residue = [str(path) for path in result.parent.glob(f"{result.name}-work-v26")]
        write_json(
            result / "CLEANUP-v26.json",
            {
                "schema": "phase4-g5-1-cleanup-v26",
        ...
        payload_excluded = {
            "PAYLOAD-MANIFEST-v26.tsv", "MEASURED-TERMINAL-v26.json",
            "TERMINAL-VERIFICATION-v26.json", "COMPLETE-WALL-v26.json",
            "BENCHMARK-LOCK-RELEASE-ATTESTATION-v26.json", "LOCK-RELEASE-v26.json",
            "FINAL-ARTIFACT-HASHES-v26.tsv", "FINAL-READONLY-VERIFICATION-v26.json",
        }
        payload = result / "PAYLOAD-MANIFEST-v26.tsv"
        ...
        terminal = result / "MEASURED-TERMINAL-v26.json"
        write_json(
            terminal,
            {
                "schema": "phase4-g5-1-measured-terminal-v26",
                "status": "PASS",
                "campaign": campaign,
                "rows": len(timings),
                ...
                "product_child_lifecycle_sha256": sha256(
                    result / "PRODUCT-CHILD-LIFECYCLE-v26.json"
                ),
                "work_root_lifecycle_sha256": sha256(
                    result / "WORK-ROOT-LIFECYCLE-v26.json"
                ),
                "prearm_wrapper_initialization_sha256": sha256(
                    result / "PREARM-WRAPPER-INITIALIZATION-v26.json"
                ),
        ...
        verification = result / "TERMINAL-VERIFICATION-v26.json"
        write_json(
            verification,
            {
                "schema": "phase4-g5-1-terminal-verification-v26",
                "status": "PASS",
        ...
        final = result / "FINAL-ARTIFACT-HASHES-v26.tsv"
        write_text(
            final,
            manifest_text(
                result,
                excluded={final.name, "FINAL-READONLY-VERIFICATION-v26.json", "COMPLETE-WALL-v26.json"},
            ),
        )
        final_count = verify_manifest(result, final, "result_relative_path")
        write_json(
            result / "FINAL-READONLY-VERIFICATION-v26.json",
            {
                "schema": "phase4-g5-1-final-readonly-verification-v26",
                "status": "PASS",
        ...
        write_json(
            result / "COMPLETE-WALL-v26.json",
            {
                "schema": "phase4-g5-1-complete-wall-v26",
                "status": "PASS",
                "campaign": campaign,
                ...
                "terminal_self_exclusion": "COMPLETE-WALL-v26.json follows the verified final manifest",
            },
        )
        fsync_dir(result)
        ...
        if result.exists() and not (result / "FAILED-v26.json").exists():
            write_json(
                result / "FAILED-v26.json",
                {
                    "schema": "phase4-g5-1-failure-v26",
                    "status": "REVISE",
                    "error": str(error),
...
def run_self_checks():
    ...
    with tempfile.TemporaryDirectory(prefix="layerfs-g5-v26-self-check-") as temporary:
        work = pathlib.Path(temporary) / "work-v26"
        work.mkdir()
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="layerfs-g5-v26-forecast-check-") as temporary:
    ...
    assert FORECAST_MODEL_VERSION == "phase4-g5-1-v26-fast-law-forecast-v5"
    ...
    assert str(V24_SUPERSESSION.relative_to(REPO)) in sources
...
def require_action_authorized(action):
    path = HERE / "PREMEASUREMENT-READINESS-AUDIT-v26.json"
    try:
        readiness = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as error:
        raise RuntimeError("v26 readiness authority is absent or invalid") from error
    if not action_authorized(readiness, action):
        raise RuntimeError(f"v26 action is not authorized: {action}")
