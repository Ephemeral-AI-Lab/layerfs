...
OP_SCHEMA = "phase4-g5-1-operation-v10"
...
def validate_catalog(wrapper, product, failures, label):
    ...
    if (
        ...
        or catalog.get("sqlite_schema_hash_semantics")
        != "type-name-table-sql-excludes-rootpage"
        or type(catalog.get("sqlite_schema_row_count")) is not int
        or catalog["sqlite_schema_row_count"] <= 0
...
def analyze(raw_path, timing_path, schedule_path, expected_path):
    ...
    sentinel_rows = [row for row in raw if row.get("schema") == "phase4-g5-1-protected-sentinel-v10"]
    ...
    validate_child_lifecycle(
        ...
        sentinel_rows,
        campaign,
        pathlib.Path(raw_path).parent / "PRODUCT-CHILD-LIFECYCLE-v10.json",
        failures,
    )
    validate_work_root_lifecycle(
        work_root_rows,
        operations,
        pathlib.Path(raw_path).parent / "WORK-ROOT-LIFECYCLE-v10.json",
        failures,
    )
    if campaign == "screen":
        counts = {}
    ...
    return {"schema": "phase4-g5-1-primary-analysis-v10", "status": "PASS" if not failures else "REVISE", "normalized": normalized}
