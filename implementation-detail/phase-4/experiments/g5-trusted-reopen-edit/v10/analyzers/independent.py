...
LIFECYCLE_SCHEMA = "phase4-g5-1-product-child-lifecycle-v10"
...
CLONE_SCHEMA = "g5-v10-native-clone-receipt-v1"
CLONE_TOP = frozenset((
...
CATALOG_SCHEMA = "g5-v10-ordered-logical-catalog-v1"
CATALOG_KEYS = frozenset((
    "schema", "hash_semantics", "query_only", "autocommit",
    "sqlite_schema_hash_semantics", "sqlite_schema_row_count",
...
PHYSICAL_SCHEMA = "g5-v10-physical-allocation-observation-v1"
...
WORK_ROOT_SCHEMA = "phase4-g5-1-work-root-lifecycle-v10"
...
CATALOG_DIGEST_KEYS = frozenset((
    "sqlite_schema_sha256", "meta_sha256", "object_catalog_sha256",
    "head_receipt_sha256", "logical_catalog_sha256",
))
INPUT_MANIFEST = pathlib.Path(__file__).resolve().parents[1] / "method/INPUT-MANIFEST-v10.tsv"
...
def check_catalog(meta, product, problems, tag):
    catalog = meta.get("logical_catalog")
    if type(catalog) is not dict or frozenset(catalog) != CATALOG_KEYS:
        problems.add(f"{tag}:logical-catalog-shape")
        return
    numbers = (
        "sqlite_schema_row_count", "meta_row_count", "object_count",
        "canonical_length_sum", "blob_length_sum", "head_row_count",
        "head_generation", "head_receipt_bytes",
    )
    numeric = all(type(catalog.get(field)) is int and catalog.get(field) >= 0 for field in numbers)
    facts = (
        catalog.get("schema") == CATALOG_SCHEMA,
        catalog.get("hash_semantics") == "ordered-logical-catalog-content-address-digest-not-physical-sqlite-bytes",
        catalog.get("query_only") is True,
        catalog.get("autocommit") is True,
        catalog.get("sqlite_schema_hash_semantics") == "type-name-table-sql-excludes-rootpage",
        type(catalog.get("sqlite_schema_row_count")) is int
        and catalog["sqlite_schema_row_count"] > 0,
        numeric,
        catalog.get("meta_row_count") == 1,
        catalog.get("object_count", 0) > 0,
        catalog.get("canonical_length_sum") == catalog.get("blob_length_sum"),
        catalog.get("head_row_count") == 1,
        catalog.get("head_receipt_bytes") == 216,
        catalog.get("head_root_id") == product.get("root_id"),
        catalog.get("head_transition_id") == product.get("transition_id"),
        digest(catalog.get("head_root_id")),
        digest(catalog.get("head_transition_id")),
        all(digest(catalog.get(field)) for field in CATALOG_DIGEST_KEYS),
    )
    if not all(facts):
        problems.add(f"{tag}:logical-catalog-hard")
    if "post_database_sha256" in meta:
        problems.add(f"{tag}:physical-database-claim")
    if (
        meta.get("post_database_hash_semantics")
...
def recompute(raw_file, timing_file, schedule_file, expected_file):
    ...
    rows = [value for value in documents if value.get("schema") == "phase4-g5-1-operation-v10"]
    ...
    sentinel = [value for value in documents if value.get("schema") == "phase4-g5-1-protected-sentinel-v10"]
    ...
    for value in rows:
        ...
        check_catalog(meta, product, problems, tag)
    ...
    audit_lifecycle(
        ...
        sentinel,
        campaign,
        pathlib.Path(raw_file).parent / "PRODUCT-CHILD-LIFECYCLE-v10.json",
        problems,
    )
    audit_work_roots(
        work_root_rows,
        rows,
        pathlib.Path(raw_file).parent / "WORK-ROOT-LIFECYCLE-v10.json",
        problems,
    )
    if campaign == "screen":
        counts = {}
        ...
        for value in sentinel:
            ...
            check_catalog(value, product, problems, f"S07:{route}")
            exact_facts = [
                ...
                value.get("post_database_hash_semantics")
                == "ordered-logical-catalog-content-address-digest-not-physical-sqlite-bytes",
            ...
            for pair in sorted(left):
                ...
                for field in (
                    ...
                    if left[pair]["wrapper"].get(field) != right[pair]["wrapper"].get(field):
                        problems.add(f"{comparison}:{operation}:pair-{pair}:{field}")
                if left[pair]["wrapper"].get("logical_catalog") != right[pair]["wrapper"].get("logical_catalog"):
                    problems.add(f"{comparison}:{operation}:pair-{pair}:logical-catalog")
                parity_fields = (
                    COMMON_PARITY_FIELDS
                    if comparison == "g4-verified-vs-g5-verified"
    ...
    normalized = {
        ...
        "gate_custody_results": sorted(
            ({
                ...
                "logical_catalog": value.get("wrapper", {}).get("logical_catalog"),
        ...
        "protected_sentinel_results": sorted(
            ({
                ...
                "logical_catalog": value.get("logical_catalog"),
    ...
    return {
        "schema": "phase4-g5-1-independent-recomputation-v10",
