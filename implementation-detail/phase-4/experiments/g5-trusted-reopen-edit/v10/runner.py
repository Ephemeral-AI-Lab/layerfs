...
REPO = HERE.parents[4]
METHOD = HERE / "method"
SCHEDULE = METHOD / "SCHEDULE-v10.tsv"
EXPECTED = METHOD / "EXPECTED-OUTCOMES-v10.tsv"
INPUT_MANIFEST = METHOD / "INPUT-MANIFEST-v10.tsv"
METHOD_MANIFEST = METHOD / "METHOD-MANIFEST-v10.tsv"
SOURCE_FREEZE = METHOD / "SOURCE-FREEZE-v10.json"
FREEZE_VERIFICATION = HERE / "FREEZE-VERIFICATION-v10.json"
LIMITATIONS = HERE / "LIMITATIONS-v10.json"
STATIC_CLOSURE = HERE / "STATIC-CLOSURE-v10.json"
DRY_RUN = HERE / "DRY-RUN-v10.json"
DRY_RUN_INTENT = HERE / "DRY-RUN-INTENT-v10.json"
DRY_RUN_CALIBRATION_STDOUT = HERE / "DRY-RUN-CALIBRATION-v10.stdout"
DRY_RUN_CALIBRATION_STDERR = HERE / "DRY-RUN-CALIBRATION-v10.stderr"
DRY_RUN_CALIBRATION_TERMINAL = HERE / "DRY-RUN-CALIBRATION-TERMINAL-v10.json"
DRY_RUN_DISPOSITION = HERE / "DRY-RUN-DISPOSITION-v10.json"
DRY_RUN_FAILED = HERE / "DRY-RUN-FAILED-v10.json"
PREMEASUREMENT_REVISE = HERE / "PREMEASUREMENT-REVISE-v10.json"
WRAPPER_CALIBRATION_INTENT = HERE / "WRAPPER-CALIBRATION-INTENT-v10.json"
WRAPPER_CALIBRATION_RAW = HERE / "WRAPPER-CALIBRATION-RAW-v10.jsonl"
WRAPPER_CALIBRATION_RESULT = HERE / "WRAPPER-CALIBRATION-RESULT-v10.json"
...
SCREEN_RESULT = REPO / f"target/phase4-g5-trusted-reopen-edit-{DATE}-v10-screen"
GATE_RESULT = REPO / f"target/phase4-g5-trusted-reopen-edit-{DATE}-v10"
WRAPPER_CALIBRATION_ROOT = (
    REPO / f"target/phase4-g5-trusted-reopen-edit-wrapper-calibration-{DATE}-v10"
)

# Frozen v10 Rust transport.
G5_CHILD_BINARY = HERE / "g5-benchmark/target/release/layerfs-g5-trusted-child-v10"
...
SENTINEL_SCHEMA = "phase4-g5-1-protected-sentinel-v10"
REQUEST_FIELDS = (
...
V9_SUPERSESSION = HERE / "V9-SUPERSESSION-v10.json"
...
FORECAST_MODEL_VERSION = "phase4-g5-1-v10-fast-law-forecast-v3"
...
CHILD_LIFECYCLE_SCHEMA = "phase4-g5-1-product-child-lifecycle-v10"
RSS_CLASSIFICATION = "PerProductChildRetainedPeak"
SYNCHRONOUS_RSS_KIND = "synchronous-one-shot"
ARM_CLEANUP_SCHEMA = "phase4-g5-1-arm-cleanup-receipt-v10"
WORK_ROOT_LIFECYCLE_SCHEMA = "phase4-g5-1-work-root-lifecycle-v10"
...
def method_source_names():
    fixed = {
        ...
        for path in (
            HERE / "PREREGISTRATION-v10.md",
            HERE / "REVIEW-SYNTHESIS-v10.md",
            HERE / "SAMPLE-COUNT-INTERPRETATION-ADDENDUM-v10.md",
            HERE / "FOCUSED-TEST-ATTEMPTS-v10.json",
            HERE / "PREMEASUREMENT-READINESS-AUDIT-v10.json",
            LIMITATIONS,
            HERE / "runner.py",
            PRIMARY,
            INDEPENDENT,
...
def freeze_interface_contract():
    return {
        ...
        "wrapper_calibration": {
            ...
            "intent_schema": "phase4-g5-1-wrapper-calibration-intent-v10",
            "sample_schema": "phase4-g5-1-wrapper-calibration-sample-v10",
            "result_schema": "phase4-g5-1-wrapper-calibration-result-v10",
            "samples": WRAPPER_CALIBRATION_SAMPLES,
...
def write_input_and_method_manifests():
    ...
    freeze = {
        "schema": "phase4-g5-1-source-freeze-v10",
        ...
        "post_freeze_verification": {
            ...
            "schema": "phase4-g5-1-freeze-verification-v10",
...
def current_freeze_bindings(freeze):
    ...
    supersession_relative = str(V9_SUPERSESSION.relative_to(REPO))
    if verification_relative in explicit_sources or supersession_relative not in explicit_sources:
        raise RuntimeError("freeze verification cycle or v9 supersession omission")
...
def run_preparation_command(
    ...
    chronology = evidence_root / "CHRONOLOGY-v10.jsonl"
    ...
    write_json(
        intent,
        {
            "schema": "phase4-g5-1-preparation-command-intent-v10",
            "ordinal": ordinal,
            "label": label,
            "argv": command,
    ...
    terminal = {
        "schema": "phase4-g5-1-preparation-command-terminal-v10",
        "ordinal": ordinal,
        "label": label,
...
def prepare_inputs():
    ...
    if any(
        ...
        raise RuntimeError("v10 method/freeze evidence already exists")
    ...
    fsync_dir(INPUT_ROOT.parent)
    evidence_root = INPUT_ROOT / "preparation-evidence-v10"
    ...
    try:
        ...
        append_text(
            evidence_root / "CHRONOLOGY-v10.jsonl",
            compact(
                {
        ...
        write_json(
            INPUT_ROOT / "PREPARATION-FAILED-v10.json",
            {
                "schema": "phase4-g5-1-input-preparation-failure-v10",
                "status": "REVISE",
        ...
        raise
    preparation_manifest = evidence_root / "PREPARATION-EVIDENCE-MANIFEST-v10.tsv"
    write_text(
    ...
    write_json(
        INPUT_ROOT / "PREPARATION-CUSTODY-v10.json",
        {
            "schema": "phase4-g5-1-input-preparation-v10",
            "status": "PASS",
...
def verify_dry_run(freeze):
    ...
    required = {
        "schema": "phase4-g5-1-dry-run-v10",
        "status": "PASS",
        "measured_rows": 0,
        ...
        "wrapper_calibration_samples_completed": WRAPPER_CALIBRATION_SAMPLES,
    ...
    intent_required = {
        "schema": "phase4-g5-1-dry-run-intent-v10",
        "status": "STARTED",
        "branch": BRANCH,
        "head": CHECKPOINT,
    ...
    if (
        ...
        or disposition.get("schema") != "phase4-g5-1-dry-run-disposition-v10"
    ...
    expected_forecast_components = {
        **BASE_FORECAST_COMPONENTS_NS,
        "calibrated_per_arm_wrapper_work": wrapper_calibration[
            "forecast_component_ns"
        ],
...
def verify_freeze(require_static=False, require_dry=False):
    ...
    if require_static:
        ...
        screen_terminal = SCREEN_RESULT / "TERMINAL-VERIFICATION-v10.json"
        screen_final_manifest = SCREEN_RESULT / "FINAL-ARTIFACT-HASHES-v10.tsv"
        screen_final_verification = SCREEN_RESULT / "FINAL-READONLY-VERIFICATION-v10.json"
        screen_complete_wall = SCREEN_RESULT / "COMPLETE-WALL-v10.json"
        required = {
            "schema": "phase4-g5-1-static-closure-v10",
            "status": "PASS",
            "source_freeze_sha256": sha256(SOURCE_FREEZE),
            "tracked_diff_sha256": freeze["tracked_diff_sha256"],
...
def gate_hash_bytes():
    ...
    components = {
        ...
        "preserved_v7_v8_v9_failure_evidence": preserved_history_bytes,
...
def gate_workload_enumeration():
    ...
    return {
        "schema": "phase4-g5-1-prospective-workload-v10",
        ...
        "logical_catalog": {
            ...
            "ordered_object_all_row_scans": len(observations),
            "ordered_sqlite_schema_all_row_scans": len(observations),
            "exact_meta_rows_required": len(observations),
            "exact_head_rows_required": len(observations),
            "per_arm_object_and_schema_row_counts": "EmittedExactlyByArmNotAssignedUnsupportedTime",
        },
...
def hash_calibration():
    ...
    write_json(
        DRY_RUN_CALIBRATION_TERMINAL,
        {
            "schema": "phase4-g5-1-dry-run-calibration-terminal-v10",
            "status": "RETURNED",
            "return_code": completed.returncode,
    ...
    return {
        "schema": "phase4-g5-1-hash-calibration-v10",
...
def wrapper_calibration_plan():
    ...
    plan = {
        "schema": "phase4-g5-1-wrapper-calibration-plan-v10",
...
def wrapper_calibration_forecast(sample_totals_ns):
    if (
        len(sample_totals_ns) != WRAPPER_CALIBRATION_SAMPLES
    ...
    return {
        "max_sample_total_ns": maximum,
        "conservative_factor": WRAPPER_CALIBRATION_CONSERVATIVE_FACTOR,
        "per_arm_bound_ns": per_arm_bound_ns,
        "gate_arm_observations": GATE_ARM_OBSERVATIONS,
        "forecast_component_ns": GATE_ARM_OBSERVATIONS * per_arm_bound_ns,
    }
...
def run_wrapper_calibration(freeze, plan):
    ...
    intent = {
        "schema": "phase4-g5-1-wrapper-calibration-intent-v10",
        "status": "STARTED",
        ...
        "plan": plan,
        "sample_count": WRAPPER_CALIBRATION_SAMPLES,
        "conservative_factor": WRAPPER_CALIBRATION_CONSERVATIVE_FACTOR,
        "gate_arm_observations": GATE_ARM_OBSERVATIONS,
    ...
    try:
        for sample_index in range(1, WRAPPER_CALIBRATION_SAMPLES + 1):
            ...
            (catalog, physical), logical_catalog_ns = elapsed_action(
                lambda: logical_catalog(database)
            )
            ...
            action_counts = {
                **planned_actions,
                "logical_catalog_object_rows": catalog["object_count"],
                "logical_catalog_schema_rows": catalog["sqlite_schema_row_count"],
                "logical_catalog_meta_rows": catalog["meta_row_count"],
                "logical_catalog_head_rows": catalog["head_row_count"],
            }
            representative = {
                "schema": "phase4-g5-1-wrapper-calibration-representative-arm-v10",
                "sample": sample_index,
                "master": master["relative_path"],
                "action_counts": action_counts,
                "logical_catalog_sha256": catalog["logical_catalog_sha256"],
            ...
            phases = {
                ...
                "logical_catalog_ns": logical_catalog_ns,
            ...
            sample = {
                "schema": "phase4-g5-1-wrapper-calibration-sample-v10",
                "status": "PASS",
                "sample": sample_index,
                "master": master["relative_path"],
                "phases_ns": phases,
        ...
        try:
            ...
            if WRAPPER_CALIBRATION_ROOT.exists():
                if (
                    ...
                    or not WRAPPER_CALIBRATION_ROOT.name.endswith(f"-{DATE}-v10")
                ):
    ...
    result = {
        "schema": "phase4-g5-1-wrapper-calibration-result-v10",
        "status": "PASS",
        "classification": "ZeroProductWrapperCalibration",
        "intent_sha256": sha256(WRAPPER_CALIBRATION_INTENT),
        "raw_sha256": sha256(WRAPPER_CALIBRATION_RAW),
        "sample_count": len(samples),
        "samples_sha256": sha256_bytes(compact(samples).encode()),
...
def verify_wrapper_calibration(freeze):
    ...
    raw_reconstructed = "".join(compact(sample) + "\n" for sample in samples).encode()
    if (
        len(samples) != WRAPPER_CALIBRATION_SAMPLES
        or WRAPPER_CALIBRATION_RAW.read_bytes() != raw_reconstructed
    ):
    ...
    for sample_index, sample in enumerate(samples, start=1):
        ...
        if (
            sample.get("schema") != "phase4-g5-1-wrapper-calibration-sample-v10"
            ...
            or set(actions) != set(planned_actions) | {
                "logical_catalog_object_rows",
                "logical_catalog_schema_rows",
                "logical_catalog_meta_rows",
                "logical_catalog_head_rows",
            }
            or any(actions.get(key) != value for key, value in planned_actions.items())
            or actions != dynamic_actions
            or type(actions.get("logical_catalog_object_rows")) is not int
            or actions.get("logical_catalog_object_rows") <= 0
            or type(actions.get("logical_catalog_schema_rows")) is not int
            or actions.get("logical_catalog_schema_rows") <= 0
            or actions.get("logical_catalog_meta_rows") != 1
            or actions.get("logical_catalog_head_rows") != 1
    ...
    if (
        ...
        or WRAPPER_CALIBRATION_ROOT.exists()
        or LOCK.exists()
...
def dry_failure_documents(error_type, error_message, intent_sha256, collected, artifacts):
    failed = {
        "schema": "phase4-g5-1-dry-run-failed-v10",
        "status": "REVISE",
    ...
    disposition = {
        "schema": "phase4-g5-1-dry-run-disposition-v10",
        "status": "REVISE",
        "classification": "UNEXPECTED_DRY_RUN_FAILURE",
        "intent_sha256": intent_sha256,
...
def dry_run():
    ...
    if any(path.exists() for path in evidence_paths):
        raise RuntimeError("v10 dry-run evidence already exists")
    if not FREEZE_VERIFICATION.is_file():
        raise RuntimeError("v10 dry-run requires durable freeze verification")
    ...
    try:
        ...
        value = {
            ...
            "sample_count_interpretation": "deliberately-stricter-v10-choice-not-unambiguous-user-minimum",
        ...
        if status == "REVISE":
            write_json(
                PREMEASUREMENT_REVISE,
                {
                    "schema": "phase4-g5-1-premeasurement-revise-v10",
                    "status": "REVISE",
...
def release_lock(lock, result, terminal_verification=None, state="release"):
    ...
    try:
        ...
        attestation = result / "BENCHMARK-LOCK-RELEASE-ATTESTATION-v10.json"
        ...
        value = {
            "schema": "phase4-g5-1-lock-release-v10",
        ...
        write_json(result / "LOCK-RELEASE-v10.json", value)
        return value
    finally:
        os.close(lock["fd"])
        lock["fd"] = None
...
def run_semantic(executable, case, root, result, label, lifecycle=None):
    stdout = result / f"children-v10/{label}.stdout"
    stderr = result / f"children-v10/{label}.stderr"
    sidecar = result / f"time-v10/{label}.txt"
...
def validate_child_row(envelope):
    ...
    return {
        "schema": "phase4-g5-1-operation-v10",
...
def logical_catalog_identity(
    ...
    return catalog_digest(
        "layerfs-g5-v10-logical-catalog-v1",
        [
            (
                sqlite_schema_sha256,
                meta_sha256,
...
def logical_catalog(database):
    ...
    try:
        ...
        sqlite_allocated_bytes = sqlite_page_size * sqlite_page_count
        sqlite_freelist_bytes = sqlite_page_size * sqlite_freelist_count

        schema_rows = connection.execute(
            "SELECT type, name, tbl_name, sql FROM sqlite_schema "
            "WHERE name NOT LIKE 'sqlite_%' ORDER BY type, name"
        ).fetchall()
        schema_rootpages = connection.execute(
            "SELECT type, name, tbl_name, rootpage FROM sqlite_schema "
            "WHERE name NOT LIKE 'sqlite_%' ORDER BY type, name"
        ).fetchall()
        sqlite_schema_sha256 = catalog_digest(
            "layerfs-g5-v10-sqlite-schema-v1", schema_rows
        )
        ...
        meta_sha256 = catalog_digest("layerfs-g5-v10-meta-v1", meta_rows)

        object_digest = hashlib.sha256()
        catalog_feed(object_digest, "domain", "layerfs-g5-v10-object-catalog-v1")
        object_count = 0
        ...
        head_sha256 = catalog_digest("layerfs-g5-v10-visible-head-v1", head_rows)

        logical_catalog_sha256 = logical_catalog_identity(
            sqlite_schema_sha256,
            meta_sha256,
            object_catalog_sha256,
            head_sha256,
        ...
        return (
            {
                ...
                "sqlite_schema_row_count": len(schema_rows),
            ...
            },
            {
                ...
                "sqlite_allocated_bytes": sqlite_allocated_bytes,
                "sqlite_freelist_bytes": sqlite_freelist_bytes,
                "sqlite_schema_rootpages": [
                    ...
                    for row_type, name, table_name, rootpage in schema_rootpages
                ],
            },
        )
    finally:
...
def post_row_state(root, product, allowed_inventory):
    ...
    catalog, physical_allocation = logical_catalog(database)
    if (
...
class PersistentChild:
    def __init__(
        ...
        self.stdout_path = result / f"children-v10/{self.label}.stdout"
        self.stderr_path = result / f"children-v10/{self.label}.stderr"
        self.time_path = result / f"time-v10/{self.label}.txt"
...
def analyze(result):
    ...
    for analyzer, name in (
        (PRIMARY, "PRIMARY-ANALYSIS-v10.json"),
        (INDEPENDENT, "INDEPENDENT-RECOMPUTATION-v10.json"),
    ):
        output = result / name
        completed = subprocess.run(
            [
                sys.executable, str(analyzer), str(result / "RAW-v10.jsonl"),
                str(result / "TIMINGS-v10.tsv"), str(SCHEDULE), str(EXPECTED), str(output),
            ],
            cwd=REPO,
            text=True,
            capture_output=True,
    ...
    write_json(
        result / "ANALYZER-AGREEMENT-v10.json",
        {
            "schema": "phase4-g5-1-analyzer-agreement-v10",
...
def ladder_prelock(campaign):
    ...
    if campaign == "gate":
        paths.extend(
            (
                SCREEN_RESULT / "TERMINAL-VERIFICATION-v10.json",
                SCREEN_RESULT / "FINAL-ARTIFACT-HASHES-v10.tsv",
                SCREEN_RESULT / "FINAL-READONLY-VERIFICATION-v10.json",
                SCREEN_RESULT / "COMPLETE-WALL-v10.json",
                STATIC_CLOSURE,
            )
        )
...
def run_campaign(campaign):
    ...
    try:
        ...
        work = result.parent / f"{result.name}-work-v10"
        work.mkdir(mode=0o700)
        work_roots = ArmWorkRootLifecycle(work)
        for name in ("operands-v10", "children-v10", "time-v10"):
            (result / name).mkdir()
        fsync_dir(result)
        fsync_dir(result.parent)

        g4_copy = result / "operands-v10/frozen-g4-verified"
        g5_copy = result / "operands-v10/g5-verified-trusted"
        ...
        write_json(
            result / "OPERAND-CUSTODY-v10.json",
            {
                "schema": "phase4-g5-1-operand-custody-v10",
        ...
        write_json(
            result / "PREFLIGHT-v10.json",
            {
                "schema": "phase4-g5-1-preflight-v10",
                "status": "PASS",
                "campaign": campaign,
                "branch": BRANCH,
                "checkpoint": CHECKPOINT,
        ...
        write_json(
            result / "ENVIRONMENT-v10.json",
            {
                "schema": "phase4-g5-1-environment-v10",
        ...
        shutil.copyfile(INPUT_MANIFEST, result / "INPUT-CUSTODY-v10.tsv")
        with (result / "INPUT-CUSTODY-v10.tsv").open("rb") as handle:
            os.fsync(handle.fileno())
                        ...
                        try:
                            for observation in pair_rows:
                                ...
                                state = post_row_state(
                                    row_root, value["product"], allowed_inventory
                                )
                    ...
                    semantics.extend(
                        {
                            ...
                            "expectation_id": "native-semantic-v10",
                            "status": value["status"],
                            "error": value["error"],
                        }
                        for value in values
        ...
        write_json(result / "PRODUCT-CHILD-LIFECYCLE-v10.json", child_lifecycle)
        write_json(result / "WORK-ROOT-LIFECYCLE-v10.json", work_root_lifecycle)

        write_text(result / "RAW-v10.jsonl", "".join(compact(row) + "\n" for row in raw))
        timing_fields = (
        ...
        write_text(
            result / "TIMINGS-v10.tsv",
            "\t".join(timing_fields) + "\n"
            + "".join("\t".join(str(row[name]) for name in timing_fields) + "\n" for row in timings),
        )
        write_text(
            result / "SEMANTIC-FAULT-RESULTS-v10.tsv",
        ...
        write_json(result / "COMMANDS-v10.json", {"schema": "phase4-g5-1-commands-v10", "commands": commands})
        analyze(result)

        if work.parent != result.parent or not work.name.endswith("-work-v10"):
            raise RuntimeError("refusing unsafe work cleanup")
        shutil.rmtree(work)
        fsync_dir(work.parent)
        residue = [str(path) for path in result.parent.glob(f"{result.name}-work-v10")]
        write_json(
            result / "CLEANUP-v10.json",
            {
                "schema": "phase4-g5-1-cleanup-v10",
        ...
        payload_excluded = {
            "PAYLOAD-MANIFEST-v10.tsv", "MEASURED-TERMINAL-v10.json",
            "TERMINAL-VERIFICATION-v10.json", "COMPLETE-WALL-v10.json",
            "BENCHMARK-LOCK-RELEASE-ATTESTATION-v10.json", "LOCK-RELEASE-v10.json",
            "FINAL-ARTIFACT-HASHES-v10.tsv", "FINAL-READONLY-VERIFICATION-v10.json",
        }
        payload = result / "PAYLOAD-MANIFEST-v10.tsv"
        ...
        terminal = result / "MEASURED-TERMINAL-v10.json"
        write_json(
            terminal,
            {
                "schema": "phase4-g5-1-measured-terminal-v10",
                "status": "PASS",
                "campaign": campaign,
                "rows": len(timings),
                ...
                "product_child_lifecycle_sha256": sha256(
                    result / "PRODUCT-CHILD-LIFECYCLE-v10.json"
                ),
                "work_root_lifecycle_sha256": sha256(
                    result / "WORK-ROOT-LIFECYCLE-v10.json"
                ),
        ...
        verification = result / "TERMINAL-VERIFICATION-v10.json"
        write_json(
            verification,
            {
                "schema": "phase4-g5-1-terminal-verification-v10",
                "status": "PASS",
        ...
        final = result / "FINAL-ARTIFACT-HASHES-v10.tsv"
        write_text(
            final,
            manifest_text(
                result,
                excluded={final.name, "FINAL-READONLY-VERIFICATION-v10.json", "COMPLETE-WALL-v10.json"},
            ),
        )
        final_count = verify_manifest(result, final, "result_relative_path")
        write_json(
            result / "FINAL-READONLY-VERIFICATION-v10.json",
            {
                "schema": "phase4-g5-1-final-readonly-verification-v10",
                "status": "PASS",
        ...
        write_json(
            result / "COMPLETE-WALL-v10.json",
            {
                "schema": "phase4-g5-1-complete-wall-v10",
                "status": "PASS",
                "campaign": campaign,
                ...
                "terminal_self_exclusion": "COMPLETE-WALL-v10.json follows the verified final manifest",
            },
        )
        fsync_dir(result)
        ...
        if result.exists() and not (result / "FAILED-v10.json").exists():
            write_json(
                result / "FAILED-v10.json",
                {
                    "schema": "phase4-g5-1-failure-v10",
                    "status": "REVISE",
                    "error": str(error),
...
def run_self_checks():
    ...
    with tempfile.TemporaryDirectory(prefix="layerfs-g5-v10-self-check-") as temporary:
        work = pathlib.Path(temporary) / "work-v10"
        work.mkdir()
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="layerfs-g5-v10-forecast-check-") as temporary:
    ...
    logical = logical_catalog_identity("a", "b", "c", "d", 1, 2, 3)
    changed_physical = logical_catalog_identity("a", "b", "c", "d", 1, 2, 3)
    assert logical == changed_physical
    logical_keys = {
        "sqlite_schema_hash_semantics",
        "sqlite_schema_sha256",
        "meta_sha256",
        "object_catalog_sha256",
        "head_receipt_sha256",
    ...
    assert logical_keys.isdisjoint(physical_keys)
    ...
    synthetic_wrapper = wrapper_calibration_forecast([10, 20, 30])
    assert synthetic_wrapper == {
        "max_sample_total_ns": 30,
        "conservative_factor": 4,
        "per_arm_bound_ns": 120,
        "gate_arm_observations": 200,
        "forecast_component_ns": 24_000,
    }
    synthetic_components = {
        **BASE_FORECAST_COMPONENTS_NS,
        "calibrated_per_arm_wrapper_work": synthetic_wrapper["forecast_component_ns"],
        "external_bulk_hash_bytes_at_calibrated_floor": 1,
    }
    assert len(synthetic_components) == 4
    ...
    assert FORECAST_MODEL_VERSION == "phase4-g5-1-v10-fast-law-forecast-v3"
    ...
    print(compact({"status": "PASS", "checks": 13, "gate_arms": 200, "checkpoints": 56}))
    return 0
