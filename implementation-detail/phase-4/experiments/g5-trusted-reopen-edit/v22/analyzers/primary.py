...
OP_SCHEMA = "phase4-g5-1-operation-v22"
...
def validate_prearm_initialization(
    ...
    if (
        ...
        != "phase4-g5-1-prearm-wrapper-initialization-evidence-v22"
...
def analyze(raw_path, timing_path, schedule_path, expected_path):
    ...
    sentinel_rows = [row for row in raw if row.get("schema") == "phase4-g5-1-protected-sentinel-v22"]
    ...
    initialization_rows = [
        row
        for row in raw
        if row.get("schema") == "phase4-g5-1-prearm-wrapper-initialization-v22"
    ]
    ...
    initialization_artifact = pathlib.Path(raw_path).parent / (
        "PREARM-WRAPPER-INITIALIZATION-v22.json"
    )
    try:
    ...
    validate_child_lifecycle(
        ...
        sentinel_rows,
        campaign,
        pathlib.Path(raw_path).parent / "PRODUCT-CHILD-LIFECYCLE-v22.json",
        failures,
    )
    validate_work_root_lifecycle(
        work_root_rows,
        operations,
        pathlib.Path(raw_path).parent / "WORK-ROOT-LIFECYCLE-v22.json",
        failures,
    )
    if campaign == "screen":
        counts = {}
    ...
    for (comparison, operation), roles in sorted(grouped.items()):
        ...
        for pair in sorted(control_by_pair):
            ...
            if comparison == "g4-verified-vs-g5-verified":
                references = right.get("leaf_batch_references")
                batches = right.get("leaf_batch_queries")
                maximum = right.get("leaf_batch_references_max")
                if (
                    type(references) is not int or references <= 0
                    or type(batches) is not int or batches != (references + 15) // 16
                    or type(maximum) is not int or not 0 < maximum <= 16
                ):
                    failures.append(f"{comparison}:{operation}:pair-{pair}:leaf-batch-equation")
                else:
                    reduction = references - batches
                    expected = {
                        ...
                        "sql_calls": left.get("sql_calls", 0) + 1 - reduction,
                        "sql_query_calls": left.get("sql_query_calls", 0) + 1 - reduction,
    ...
    return {"schema": "phase4-g5-1-primary-analysis-v22", "status": "PASS" if not failures else "REVISE", "normalized": normalized}
