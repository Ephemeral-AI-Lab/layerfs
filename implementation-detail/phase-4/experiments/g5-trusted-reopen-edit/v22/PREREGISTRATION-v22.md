# Phase 4 G5-1 v22 preregistration

V22 preserves v21's 116.457371791-second RSS REVISE. Its only change calls
SQLite's process-global `sqlite3_release_memory` between persistent requests,
after the Store/row owners are dropped and Q0 is proven. Batch width remains
16; process environment, schedule, thresholds, and 150-second cap are fixed.

V22 preserves v18's 200-row RSS REVISE. It removes the ineffective allocator
...
V22 preserves the v17 attempt-2 200-row REVISE. It retains the K64 scrub and
...
`R-ceil(R/16)` query reduction; mutation and durability work remain equal.

V22 preserves v19's 115.699891208-second RSS REVISE and reduces only the
scrub batch width to 16. This lowers the simultaneously
live SQLite statement/parameter footprint; K64 remains the hard upper bound.

V22 preserves v20's 115.738195625-second RSS REVISE; its sole change is the
32-to-16 batch-width reduction after the prior gate missed RSS by 81,920 bytes.
...
The candidate remains CAS + CDC + COW + canonical K64/F64 + SQLite. V22 changes
...
Any failure preserves v22. No observed row may be deleted, replaced, reordered,
or selectively rerun. A source or method repair creates v22.
...
`phase4-g5-1-wrapper-calibration-intent-v22`,
`phase4-g5-1-wrapper-initialization-sample-v22`,
`phase4-g5-1-wrapper-calibration-sample-v22`, and
`phase4-g5-1-wrapper-calibration-result-v22`.
...
`phase4-g5-1-wrapper-calibration-plan-v22` and records the complete candidate
...
must write and fsync the top-level `DRY-RUN-INTENT-v22.json` before invoking
...
write and fsync `DRY-RUN-FAILED-v22.json` first, then write and fsync a
`DRY-RUN-DISPOSITION-v22.json` with `status=REVISE` that binds the FAILED hash
and explicitly records absent optional artifacts. A success or calibrated
overrun writes and fsyncs complete `DRY-RUN-v22.json`, including both
...
`PREMEASUREMENT-REVISE-v22.json` and stops before screen. A missing dry-run or
...
V14 is byte-preserved by `V14-SUPERSESSION-v22.json`. Its zero-row passed, and
...
V22 changes the shared product timer equation and uses a fresh release. The
...
separate complete-lifecycle evidence. V22 reuses only the independently rehashed
...
are not copied, renamed, or represented as v22-generated. V22 reuses no v14
product row or timing observation. The new v22 input manifest rebinds the same sealed
files byte-for-byte before the v22 method manifest and source freeze are
generated.

V15 attempts 1–5 remain append-only and are not measurement authority for v22.
...
while the lifecycle sum added that authority again. V22 removes only that term
...
rows. Its G5 rows confirm the lifecycle equation repair. V22 changes only the
...
The final `PREARM-WRAPPER-INITIALIZATION-v22.json` write is outside that bound
...
schedule, expectation, limitation, and supersession byte has settled, v22
...
child. It writes `FREEZE-VERIFICATION-v22.json` with classification
...
V22 voluntarily freezes a deliberately stricter v1-lineage design: each
...
`phase4-g5-1-product-child-lifecycle-v22`. The two copies must be byte-value
...
child, v22 verifies the complete sealed input manifest exactly once: path,
...
`g5-v22-native-clone-receipt-v1`. Its ordered `entries` bind each source master
...
inspection for a future separate API. V22 therefore removes the external
...
`g5-v22-rooted-logical-state-v1` from one query-only, autocommit, constant-row
...
V22 does not claim physical SQLite file-byte parity. The sibling
`physical_allocation_observation` remains schema
`g5-v22-physical-allocation-observation-v1`, classification
...
Each wrapper receipt uses schema `phase4-g5-1-arm-cleanup-receipt-v22` and is
repeated in order by the raw and standalone
`phase4-g5-1-work-root-lifecycle-v22` record. Started and cleaned roots equal
