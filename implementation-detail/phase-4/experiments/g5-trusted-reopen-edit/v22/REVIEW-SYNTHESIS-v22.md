# G5-1 v22 review synthesis

V21 showed one-pair children near 18.1 MiB but five-request processes at
21–22 MiB with logical owners/Q zero. V22 targets only SQLite's retained global
free-memory lifetime after closed connections; it does not alter live Store
state or product semantics.

V20 completed all 200 rows below 150 seconds and missed RSS only for one
Verified child by 81,920 bytes. V22 halves only the scrub statement batch width
to 16; all semantic, workload, population, and threshold controls are fixed.

V19 completed all 200 rows below 150 seconds but two Verified children exceeded
the 20-MiB cap. V22 keeps the authenticated batch mechanism and uses width 32
to reduce live statement memory; no row, threshold, or workload changes.

V18 completed all 200 rows in a 115.521346875-second failure path, but G4 and
G5 processes exceeded the 20-MiB per-child RSS cap. V22 changes only the common
...
20,971,520-byte cap. V22 releases free allocator pages only between rows after
...
## V16 screen and accepted v22 analyzer repair
...
V22 changes only those two analyzer predicates and the versioned child/report
...
product row or timing is promoted to v22 authority.

V22 keeps the 200-arm schedule, 56 checkpoints, half-slower calibration floor,
...
- use native APFS clone receipts, schema `g5-v22-native-clone-receipt-v1`, to
...
- emit constant-size rooted state, schema `g5-v22-rooted-logical-state-v1`,
...
`FREEZE-VERIFICATION-v22.json` is classified
...
`PRODUCT-CHILD-LIFECYCLE-v22.json` under schema
`phase4-g5-1-product-child-lifecycle-v22`. Per-arm cleanup receipts use schema
`phase4-g5-1-arm-cleanup-receipt-v22`; their ordered terminal record is likewise
duplicated as `WORK-ROOT-LIFECYCLE-v22.json` under schema
`phase4-g5-1-work-root-lifecycle-v22`.
...
`wrapper-calibration-intent-v22`, `wrapper-initialization-sample-v22`,
`wrapper-calibration-sample-v22`, and `wrapper-calibration-result-v22`. The
...
intent embeds `phase4-g5-1-wrapper-calibration-plan-v22`, retains the complete
...
`phase4-g5-1-prearm-wrapper-initialization-v22` record after lock/custody and
...
closes v22 as `PREMEASUREMENT_REVISE` before screen; no schedule or threshold
is reduced.

`DRY-RUN-INTENT-v22.json` is durably written before either calibration begins.
A single outer failure funnel then covers wrapper calibration, hash calibration,
forecasting, and sealing: it always writes `DRY-RUN-FAILED-v22.json` before a
REVISE `DRY-RUN-DISPOSITION-v22.json`, even when the wrapper result is absent.
...
V22 nevertheless voluntarily freezes the stricter v1-lineage design: each
