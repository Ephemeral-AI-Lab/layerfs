...
REPO = HERE.parents[4]
METHOD = HERE / "method"
SCHEDULE = METHOD / "SCHEDULE-v2.tsv"
EXPECTED = METHOD / "EXPECTED-OUTCOMES-v2.tsv"
INPUT_MANIFEST = METHOD / "INPUT-MANIFEST-v2.tsv"
METHOD_MANIFEST = METHOD / "METHOD-MANIFEST-v2.tsv"
SOURCE_FREEZE = METHOD / "SOURCE-FREEZE-v2.json"
LIMITATIONS = HERE / "LIMITATIONS-v2.json"
STATIC_CLOSURE = HERE / "STATIC-CLOSURE-v2.json"
...
INPUT_ROOT = REPO / f"target/phase4-g5-trusted-reopen-edit-inputs-{DATE}-v2"
SCREEN_RESULT = REPO / f"target/phase4-g5-trusted-reopen-edit-{DATE}-v2-screen"
GATE_RESULT = REPO / f"target/phase4-g5-trusted-reopen-edit-{DATE}-v2"

# Interface assumptions are deliberately isolated here. The Rust transport must
# match these constants before SOURCE-FREEZE-v2.json can be created.
G5_CHILD_BINARY = HERE / "g5-benchmark/target/release/layerfs-g5-trusted-child"
PREPARE_FLAG = "--g5-prepare-inputs"
CHILD_FLAG = "--g5-child"
ONESHOT_FLAG = "--g5-row"
CHILD_READY_SCHEMA = "phase4-g5-trusted-child-ready-v2"
CHILD_ENVELOPE_SCHEMA = "phase4-g5-trusted-child-row-v2"
CHILD_TERMINAL_SCHEMA = "phase4-g5-trusted-child-terminal-v2"
REQUEST_FIELDS = (
...
def method_source_names():
    fixed = {
        ...
        for path in (
            HERE / "PREREGISTRATION-v2.md",
            HERE / "REVIEW-SYNTHESIS-v2.md",
            HERE / "SAMPLE-COUNT-INTERPRETATION-ADDENDUM-v2.md",
            HERE / "V1-SUPERSESSION-v2.json",
            LIMITATIONS,
            HERE / "runner.py",
            PRIMARY,
            INDEPENDENT,
...
def write_input_and_method_manifests():
    ...
    freeze = {
        "schema": "phase4-g5-1-source-freeze-v2",
...
def prepare_inputs():
    ...
    if any(path.exists() for path in (INPUT_MANIFEST, METHOD_MANIFEST, SOURCE_FREEZE, DRY_RUN, STATIC_CLOSURE)):
        raise RuntimeError("v2 method/freeze evidence already exists")
    ...
    write_json(
        INPUT_ROOT / "PREPARATION-CUSTODY-v2.json",
        {
            "schema": "phase4-g5-1-input-preparation-v2",
    ...
    if completed.returncode != 0:
        raise RuntimeError("input preparation failed; preserve v2 and create v3")
...
def dry_run():
    ...
    if FULL_WRAPPER_FORECAST_NS > LIMIT_NS["gate"]:
        raise RuntimeError("v2 PREMEASUREMENT_REVISE: conservative full-wrapper forecast exceeds 120 seconds")
    generated_residue = sorted(
        str(path.relative_to(REPO)) for path in HERE.rglob("__pycache__") if path.is_dir()
    )
    value = {
        "schema": "phase4-g5-1-dry-run-v2",
        "status": "PASS",
        "measured_rows": 0,
        ...
        "sample_count_interpretation": "deliberately-stricter-v2-choice-not-unambiguous-user-minimum",
...
def release_lock(lock, result, terminal_verification=None, state="release"):
    ...
    try:
        ...
        attestation = result / "BENCHMARK-LOCK-RELEASE-ATTESTATION-v2.json"
        ...
        value = {
            "schema": "phase4-g5-1-lock-release-v2",
        ...
        write_json(result / "LOCK-RELEASE-v2.json", value)
        return value
    finally:
        os.close(lock["fd"])
        lock["fd"] = None
...
def validate_child_row(envelope):
    ...
    return {
        "schema": "phase4-g5-1-operation-v2",
...
class PersistentChild:
    def __init__(self, executable, mode, size_bytes, operation, expected_rows, result, label=None):
        ...
        self.stdout_path = result / f"children-v2/{label}.stdout"
        self.stderr_path = result / f"children-v2/{label}.stderr"
        self.time_path = result / f"time-v2/{label}.txt"
...
def run_oneshot(executable, request, result, label, g4=False):
    stdout = result / f"children-v2/{label}.stdout"
    stderr = result / f"children-v2/{label}.stderr"
    sidecar = result / f"time-v2/{label}.txt"
...
def analyze(result):
    ...
    for analyzer, name in (
        (PRIMARY, "PRIMARY-ANALYSIS-v2.json"),
        (INDEPENDENT, "INDEPENDENT-RECOMPUTATION-v2.json"),
    ):
        output = result / name
        completed = subprocess.run(
            [
                sys.executable, str(analyzer), str(result / "RAW-v2.jsonl"),
                str(result / "TIMINGS-v2.tsv"), str(SCHEDULE), str(EXPECTED), str(output),
            ],
            cwd=REPO,
            text=True,
            capture_output=True,
    ...
    write_json(
        result / "ANALYZER-AGREEMENT-v2.json",
        {
            "schema": "phase4-g5-1-analyzer-agreement-v2",
...
def run_campaign(campaign):
    ...
    try:
        ...
        work = result.parent / f"{result.name}-work-v2"
        work.mkdir(mode=0o700)
        for name in ("operands-v2", "children-v2", "time-v2"):
            (result / name).mkdir()
        fsync_dir(result)
        fsync_dir(result.parent)

        g4_copy = result / "operands-v2/frozen-g4-verified"
        g5_copy = result / "operands-v2/g5-verified-trusted"
        ...
        write_json(
            result / "OPERAND-CUSTODY-v2.json",
            {
                "schema": "phase4-g5-1-operand-custody-v2",
        ...
        write_json(
            result / "PREFLIGHT-v2.json",
            {
                "schema": "phase4-g5-1-preflight-v2",
                "status": "PASS",
                "campaign": campaign,
                "branch": BRANCH,
                "checkpoint": CHECKPOINT,
        ...
        write_json(
            result / "ENVIRONMENT-v2.json",
            {
                "schema": "phase4-g5-1-environment-v2",
        ...
        shutil.copyfile(INPUT_MANIFEST, result / "INPUT-CUSTODY-v2.tsv")
        with (result / "INPUT-CUSTODY-v2.tsv").open("rb") as handle:
            os.fsync(handle.fileno())
        ...
        write_json(result / "COMMANDS-v2.json", {"schema": "phase4-g5-1-commands-v2", "commands": commands})
        analyze(result)

        if work.parent != result.parent or not work.name.endswith("-work-v2"):
            raise RuntimeError("refusing unsafe work cleanup")
        shutil.rmtree(work)
        fsync_dir(work.parent)
        residue = [str(path) for path in result.parent.glob(f"{result.name}-work-v2")]
        write_json(
            result / "CLEANUP-v2.json",
            {
                "schema": "phase4-g5-1-cleanup-v2",
        ...
        payload_excluded = {
            "PAYLOAD-MANIFEST-v2.tsv", "MEASURED-TERMINAL-v2.json",
            "TERMINAL-VERIFICATION-v2.json", "COMPLETE-WALL-v2.json",
            "BENCHMARK-LOCK-RELEASE-ATTESTATION-v2.json", "LOCK-RELEASE-v2.json",
            "FINAL-ARTIFACT-HASHES-v2.tsv", "FINAL-READONLY-VERIFICATION-v2.json",
        }
        payload = result / "PAYLOAD-MANIFEST-v2.tsv"
        ...
        terminal = result / "MEASURED-TERMINAL-v2.json"
        write_json(
            terminal,
            {
                "schema": "phase4-g5-1-measured-terminal-v2",
                "status": "PASS",
                "campaign": campaign,
                "rows": len(timings),
        ...
        verification = result / "TERMINAL-VERIFICATION-v2.json"
        write_json(
            verification,
            {
                "schema": "phase4-g5-1-terminal-verification-v2",
                "status": "PASS",
        ...
        write_json(
            result / "COMPLETE-WALL-v2.json",
            {
                "schema": "phase4-g5-1-complete-wall-v2",
                "status": "PASS",
                "campaign": campaign,
        ...
        final = result / "FINAL-ARTIFACT-HASHES-v2.tsv"
        write_text(final, manifest_text(result, excluded={final.name, "FINAL-READONLY-VERIFICATION-v2.json"}))
        final_count = verify_manifest(result, final, "result_relative_path")
        write_json(
            result / "FINAL-READONLY-VERIFICATION-v2.json",
            {
                "schema": "phase4-g5-1-final-readonly-verification-v2",
                "status": "PASS",
        ...
        if result.exists() and not (result / "FAILED-v2.json").exists():
            write_json(
                result / "FAILED-v2.json",
                {
                    "schema": "phase4-g5-1-failure-v2",
                    "status": "REVISE",
                    "error": str(error),
