...
def recompute(raw_file, timing_file, schedule_file, expected_file):
    ...
    rows = [value for value in documents if value.get("schema") == "phase4-g5-1-operation-v4"]
    ends = [value for value in documents if value.get("schema") == "phase4-g5-trusted-child-terminal-v4"]
    semantic = [value for value in documents if value.get("schema") == "phase4-g5-trusted-semantic-v4"]
    semantic_ends = [value for value in documents if value.get("schema") == "phase4-g5-trusted-semantic-terminal-v4"]
    sentinel = [value for value in documents if value.get("schema") == "phase4-g5-1-protected-sentinel-v4"]
    ...
    if campaign == "screen":
        ...
        for value in sentinel:
            ...
            checks = (
                ...
                value.get("sequence_id") == "S07",
                value.get("executable_sha256") == "e72988fc25e96f608d0d405e157ea8e837029595ace916f066932082a736db33",
                value.get("frozen_fixture_sha256") == value.get("probe_fixture_sha256"),
                value.get("post_database_hash_semantics") == "physical-byte-parity-only-not-logical-digest",
                bool(value.get("post_database_sha256")),
                bool(value.get("post_authority_sha256")),
                bool(value.get("mutation_work_sha256")),
                bool(value.get("base_custody")),
                product.get("status") == "PASS",
                product.get("q_current") == 0,
                (product.get("transactions"), product.get("commits")) == expected,
                bool(product.get("root_id")),
                bool(product.get("transition_id")),
                route != "range" or bool(product.get("range_measurements")),
            )
            if not all(checks):
    ...
    normalized = {
        ...
        "protected_sentinel_results": sorted(
            ({
                ...
                "root_id": value.get("product", {}).get("root_id"),
                "transition_id": value.get("product", {}).get("transition_id"),
                "transactions": value.get("product", {}).get("transactions"),
                "commits": value.get("product", {}).get("commits"),
                "q_current": value.get("product", {}).get("q_current"),
    ...
    return {
        "schema": "phase4-g5-1-independent-recomputation-v4",
