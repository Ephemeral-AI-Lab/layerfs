# G5-1 v4 sample-count interpretation addendum

This append-only note corrects one sentence in `REVIEW-SYNTHESIS-v4.md`
without changing `PREREGISTRATION-v4.md` or its schedule.
...
separately inside each comparison. V4 nevertheless prospectively froze the
...
That is a deliberate v4 method choice, not an exact restatement of an
...
forecast is `<=120 s`. If it cannot, v4 is preserved as `PREMEASUREMENT_REVISE`;
only a prospectively created v4 may adopt a narrower interpretation.
