const G5_REQUEST_BYTES_V4: usize = 4_096;
...
fn g5_read_line<'a>(
    ...
    buffer: &'a mut [u8; G5_REQUEST_BYTES_V4],
...
fn g5_child() -> AnyResult<()> {
    ...
    let mut buffer = [0_u8; G5_REQUEST_BYTES_V4];
    let mut rows = 0_u64;

    writeln!(output, "{{\"status\":\"READY\",\"schema\":\"phase4-g5-trusted-child-ready-v4\",\"integrity_mode\":\"{mode_name}\",\"mode_provenance\":\"fixed-at-child-start\",\"size_bytes\":{size},\"operation\":\"{operation}\",\"expected_rows\":{expected_rows},\"full_wrapper_forecast_ns\":{forecast_ns},\"full_wrapper_limit_ns\":{limit_ns},\"custody\":\"runner-preverified-borrowed\",\"request_schema\":\"id\\troot\\titeration\\twarmup\\tvalidation\"}}")?;
    output.flush()?;
    while let Some(line) = g5_read_line(&mut input, &mut buffer)? {
        let request = g5_request(line)?;
        let source = source_path(request.root, size);
        let database = row_database_path(
            request.root,
            SELECTED_PROFILE,
            size,
            operation,
        ...
        writeln!(output, "{{\"status\":\"PASS\",\"schema\":\"phase4-g5-trusted-child-row-v4\",\"request_id\":\"{}\",\"integrity_mode\":\"{mode_name}\",\"mode_provenance\":\"fixed-at-child-start\",\"row\":{}}}", request.id, &*row)?;
        output.flush()?;
        drop(row);
        drop(session);
        drop(expectations);
    ...
    g5_terminal_q()?;
    writeln!(output, "{{\"status\":\"PASS\",\"schema\":\"phase4-g5-trusted-child-terminal-v4\",\"integrity_mode\":\"{mode_name}\",\"rows\":{rows},\"expected_rows\":{expected_rows},\"argument_owners\":0,\"request_owners\":0,\"schedule_owners\":0,\"timing_owners\":0,\"report_owners\":0,\"q_current\":0,\"rss\":\"external-high-water-only\"}}")?;
    output.flush()?;
    Ok(())
}
...
fn g5_semantic_base(
    ...
    if database.exists() || authority_path(database).exists() {
    ...
    let path_charge = store
        ._path_charge
        .as_ref()
        .map_or(0, |charge| charge.0);
    if q_current() != path_charge {
        return Err(CoreError::LengthMismatch {
            expected: path_charge,
            actual: q_current(),
        }
        .into());
    }
...
fn g5_semantic_edit(
    ...
    let path_charge = store._path_charge.as_ref().map_or(0, |charge| charge.0);
    if q_current() != path_charge {
        return Err(CoreError::LengthMismatch {
            expected: path_charge,
            actual: q_current(),
...
fn g5_emit_semantic(
    ...
    writeln!(output, "{{\"status\":\"PASS\",\"schema\":\"phase4-g5-trusted-semantic-v4\",\"case\":\"{case}\",\"integrity_mode\":\"{mode}\",\"error\":{},\"later_snapshot_error\":{},\"publication_status\":{},\"reconciliation\":{},\"before_generation\":{},\"after_generation\":{},\"before_root\":\"{}\",\"after_root\":\"{}\",\"head_unchanged\":{},\"transactions\":{},\"commits\":{},\"edit_base_complete_scrub_calls\":{},\"edit_base_complete_scrub_canonical_bytes\":{},\"trusted_assumed_equal_edges\":{},\"trusted_assumed_prior_references\":{},\"trusted_assumed_prior_raw_bytes\":{},\"verified_carry_forward\":{},\"cleanup_ok\":{},\"residue\":{},\"q_high_water\":{},\"q_current\":{}}}", G5JsonDebug(observation.error), G5JsonDebug(later_error), G5JsonDebug(observation.publication_status), G5JsonDebug(reconciliation), observation.before.0, observation.after.0, observation.before.1, observation.after.1, observation.before == observation.after, observation.transactions, observation.commits, observation.scrub_calls, observation.scrub_bytes, observation.trusted_equal_edges, observation.trusted_prior_references, observation.trusted_prior_raw_bytes, observation.verified_carry_forward, observation.cleanup_ok, residue, observation.q_high_water, observation.q_current)?;
    output.flush()?;
    Ok(())
}

fn g5_database(root: &Path, label: &str) -> PathBuf {
    root.join(format!("g5-semantic-v4-{label}.sqlite"))
}

fn g5_cleanup(database: &Path) -> AnyResult<bool> {
    remove_sqlite_image(database)?;
    Ok(database.exists()
        || authority_path(database).exists()
        || expectations_path(database).exists())
}
...
fn g5_semantic_mode_database(
    ...
    label: &str,
    mode: IntegrityMode,
) -> AnyResult<(PathBuf, Store, ObjectId, ObjectId)> {
    let database = g5_database(root, label);
    let (store, _, file, common, unrelated) = g5_semantic_base(&database)?;
    drop(store);
    ...
    Ok((database, store, if label.contains("touched") { file } else { common }, unrelated))
}
...
fn g5_semantic(case: &str, root: &Path) -> AnyResult<()> {
    ...
    match case {
        "touched-corruption" => {
            for (mode, name) in [
                ...
                let observation = g5_semantic_edit(&mut store, None, false)?;
        ...
        "unrelated-corruption" => {
            ...
            let verified_observation = g5_semantic_edit(&mut verified, None, false)?;
            ...
            let trusted_observation = g5_semantic_edit(&mut trusted, None, false)?;
        ...
        "trusted-verified-reopen" => {
            ...
            let observation = g5_semantic_edit(&mut trusted, None, false)?;
        ...
        "reconciliation" => {
            for (label, fault, reject, expected) in [
                ...
                let observation = g5_semantic_edit(&mut store, fault, reject)?;
    ...
    g5_terminal_q()?;
    writeln!(output, "{{\"status\":\"PASS\",\"schema\":\"phase4-g5-trusted-semantic-terminal-v4\",\"case\":\"{case}\",\"q_current\":0}}")?;
    output.flush()?;
    Ok(())
}
...
pub(super) fn g5_transport_main() -> AnyResult<()> {
    match g5_arg(1)? {
        "--g5-fixture" => {
            ...
            g5_terminal_q()?;
            println!("{{\"status\":\"PASS\",\"schema\":\"phase4-g5-trusted-fixture-v4\",\"size_bytes\":{size},\"q_current\":0}}" );
            Ok(())
        }
        "--g5-prepare" => {
            g5_require_args(6)?;
            ...
            g5_terminal_q()?;
            println!("{{\"status\":\"PASS\",\"schema\":\"phase4-g5-trusted-prepare-v4\",\"size_bytes\":{size},\"operation\":\"{operation}\",\"iteration\":{iteration},\"q_current\":0}}" );
            Ok(())
        }
        "--g5-child" => g5_child(),
        "--g5-semantic" => {
