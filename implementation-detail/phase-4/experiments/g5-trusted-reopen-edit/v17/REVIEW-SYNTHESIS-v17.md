...
## V16 screen and accepted v17 analyzer repair
...
V17 changes only those two analyzer predicates and the versioned child/report
...
product row or timing is promoted to v17 authority.

V17 keeps the 200-arm schedule, 56 checkpoints, half-slower calibration floor,
...
- use native APFS clone receipts, schema `g5-v17-native-clone-receipt-v1`, to
...
- emit constant-size rooted state, schema `g5-v17-rooted-logical-state-v1`,
...
`FREEZE-VERIFICATION-v17.json` is classified
...
`PRODUCT-CHILD-LIFECYCLE-v17.json` under schema
`phase4-g5-1-product-child-lifecycle-v17`. Per-arm cleanup receipts use schema
`phase4-g5-1-arm-cleanup-receipt-v17`; their ordered terminal record is likewise
duplicated as `WORK-ROOT-LIFECYCLE-v17.json` under schema
`phase4-g5-1-work-root-lifecycle-v17`.
...
`wrapper-calibration-intent-v17`, `wrapper-initialization-sample-v17`,
`wrapper-calibration-sample-v17`, and `wrapper-calibration-result-v17`. The
...
intent embeds `phase4-g5-1-wrapper-calibration-plan-v17`, retains the complete
...
`phase4-g5-1-prearm-wrapper-initialization-v17` record after lock/custody and
...
closes v17 as `PREMEASUREMENT_REVISE` before screen; no schedule or threshold
is reduced.

`DRY-RUN-INTENT-v17.json` is durably written before either calibration begins.
A single outer failure funnel then covers wrapper calibration, hash calibration,
forecasting, and sealing: it always writes `DRY-RUN-FAILED-v17.json` before a
REVISE `DRY-RUN-DISPOSITION-v17.json`, even when the wrapper result is absent.
...
V17 nevertheless voluntarily freezes the stricter v1-lineage design: each
