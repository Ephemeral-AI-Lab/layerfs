# Phase 4 G5-1 v17 preregistration
...
The candidate remains CAS + CDC + COW + canonical K64/F64 + SQLite. V17 changes
...
Any failure preserves v17. No observed row may be deleted, replaced, reordered,
or selectively rerun. A source or method repair creates v17.
...
`phase4-g5-1-wrapper-calibration-intent-v17`,
`phase4-g5-1-wrapper-initialization-sample-v17`,
`phase4-g5-1-wrapper-calibration-sample-v17`, and
`phase4-g5-1-wrapper-calibration-result-v17`.
...
`phase4-g5-1-wrapper-calibration-plan-v17` and records the complete candidate
...
must write and fsync the top-level `DRY-RUN-INTENT-v17.json` before invoking
...
write and fsync `DRY-RUN-FAILED-v17.json` first, then write and fsync a
`DRY-RUN-DISPOSITION-v17.json` with `status=REVISE` that binds the FAILED hash
and explicitly records absent optional artifacts. A success or calibrated
overrun writes and fsyncs complete `DRY-RUN-v17.json`, including both
...
`PREMEASUREMENT-REVISE-v17.json` and stops before screen. A missing dry-run or
...
V14 is byte-preserved by `V14-SUPERSESSION-v17.json`. Its zero-row passed, and
...
V17 changes the shared product timer equation and uses a fresh release. The
...
separate complete-lifecycle evidence. V17 reuses only the independently rehashed
...
are not copied, renamed, or represented as v17-generated. V17 reuses no v14
product row or timing observation. The new v17 input manifest rebinds the same sealed
files byte-for-byte before the v17 method manifest and source freeze are
generated.

V15 attempts 1–5 remain append-only and are not measurement authority for v17.
...
while the lifecycle sum added that authority again. V17 removes only that term
...
rows. Its G5 rows confirm the lifecycle equation repair. V17 changes only the
...
The final `PREARM-WRAPPER-INITIALIZATION-v17.json` write is outside that bound
...
schedule, expectation, limitation, and supersession byte has settled, v17
...
child. It writes `FREEZE-VERIFICATION-v17.json` with classification
...
V17 voluntarily freezes a deliberately stricter v1-lineage design: each
...
`phase4-g5-1-product-child-lifecycle-v17`. The two copies must be byte-value
...
child, v17 verifies the complete sealed input manifest exactly once: path,
...
`g5-v17-native-clone-receipt-v1`. Its ordered `entries` bind each source master
...
inspection for a future separate API. V17 therefore removes the external
...
`g5-v17-rooted-logical-state-v1` from one query-only, autocommit, constant-row
...
V17 does not claim physical SQLite file-byte parity. The sibling
`physical_allocation_observation` remains schema
`g5-v17-physical-allocation-observation-v1`, classification
...
Each wrapper receipt uses schema `phase4-g5-1-arm-cleanup-receipt-v17` and is
repeated in order by the raw and standalone
`phase4-g5-1-work-root-lifecycle-v17` record. Started and cleaned roots equal
