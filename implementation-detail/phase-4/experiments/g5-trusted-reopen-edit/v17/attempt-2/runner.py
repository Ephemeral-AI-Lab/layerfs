...
METHOD = HERE / "method"
SCHEDULE = METHOD / "SCHEDULE-v17.tsv"
EXPECTED = METHOD / "EXPECTED-OUTCOMES-v17.tsv"
INPUT_MANIFEST = METHOD / "INPUT-MANIFEST-v17.tsv"
METHOD_MANIFEST = METHOD / "METHOD-MANIFEST-v17.tsv"
SOURCE_FREEZE = METHOD / "SOURCE-FREEZE-v17.json"
FREEZE_VERIFICATION = HERE / "FREEZE-VERIFICATION-v17.json"
LIMITATIONS = HERE / "LIMITATIONS-v17.json"
STATIC_CLOSURE = HERE / "STATIC-CLOSURE-v17.json"
DRY_RUN = HERE / "DRY-RUN-v17.json"
DRY_RUN_INTENT = HERE / "DRY-RUN-INTENT-v17.json"
DRY_RUN_CALIBRATION_STDOUT = HERE / "DRY-RUN-CALIBRATION-v17.stdout"
DRY_RUN_CALIBRATION_STDERR = HERE / "DRY-RUN-CALIBRATION-v17.stderr"
DRY_RUN_CALIBRATION_TERMINAL = HERE / "DRY-RUN-CALIBRATION-TERMINAL-v17.json"
DRY_RUN_DISPOSITION = HERE / "DRY-RUN-DISPOSITION-v17.json"
DRY_RUN_FAILED = HERE / "DRY-RUN-FAILED-v17.json"
PREMEASUREMENT_REVISE = HERE / "PREMEASUREMENT-REVISE-v17.json"
WRAPPER_CALIBRATION_INTENT = HERE / "WRAPPER-CALIBRATION-INTENT-v17.json"
WRAPPER_CALIBRATION_RAW = HERE / "WRAPPER-CALIBRATION-RAW-v17.jsonl"
WRAPPER_CALIBRATION_RESULT = HERE / "WRAPPER-CALIBRATION-RESULT-v17.json"
...
SCREEN_RESULT = REPO / f"target/phase4-g5-trusted-reopen-edit-{DATE}-v17-screen-attempt-2"
GATE_RESULT = REPO / f"target/phase4-g5-trusted-reopen-edit-{DATE}-v17-attempt-2"
WRAPPER_CALIBRATION_ROOT = (
    REPO / f"target/phase4-g5-trusted-reopen-edit-wrapper-calibration-{DATE}-v17-attempt-2"
)

# V17 changes the product timer equation and uses a fresh hash-bound release.
G5_CHILD_BINARY = HERE / "g5-benchmark/target/release/layerfs-g5-trusted-child-v17"
...
SENTINEL_SCHEMA = "phase4-g5-1-protected-sentinel-v17"
REQUEST_FIELDS = (
...
V17_RELEASE_SHA256 = "a73d841c351f08dd2326b16647b0870b4f6c7ffed82e489aa0e9e1f28a7f3a28"
...
FORECAST_MODEL_VERSION = "phase4-g5-1-v17-fast-law-forecast-v5"
...
CLONE_RECEIPT_SCHEMA = "g5-v17-native-clone-receipt-v1"
...
PHYSICAL_ALLOCATION_SCHEMA = "g5-v17-physical-allocation-observation-v1"
...
CHILD_LIFECYCLE_SCHEMA = "phase4-g5-1-product-child-lifecycle-v17"
...
WORK_ROOT_LIFECYCLE_SCHEMA = "phase4-g5-1-work-root-lifecycle-v17"
...
def verify_repository_identity():
    ...
    verify_file(G5_CHILD_BINARY, V17_RELEASE_SHA256)
...
def method_source_names():
    fixed = {
        ...
        for path in (
            HERE / "PREREGISTRATION-v17.md",
            HERE / "REVIEW-SYNTHESIS-v17.md",
            HERE / "SAMPLE-COUNT-INTERPRETATION-ADDENDUM-v17.md",
            HERE / "FOCUSED-TEST-ATTEMPTS-v17.json",
            HERE / "PREMEASUREMENT-READINESS-AUDIT-v17.json",
            LIMITATIONS,
            HERE / "runner.py",
            PRIMARY,
            INDEPENDENT,
...
def freeze_interface_contract():
    return {
        "product_operand_version": "v17",
        "product_release_reuse": False,
        "product_release_sha256": V17_RELEASE_SHA256,
        ...
        "wrapper_calibration": {
            ...
            "intent_schema": "phase4-g5-1-wrapper-calibration-intent-v17",
            "initialization_schema": "phase4-g5-1-wrapper-initialization-sample-v17",
            "sample_schema": "phase4-g5-1-wrapper-calibration-sample-v17",
            "result_schema": "phase4-g5-1-wrapper-calibration-result-v17",
...
def write_input_and_method_manifests():
    ...
    freeze = {
        "schema": "phase4-g5-1-source-freeze-v17",
        ...
        "post_freeze_verification": {
            ...
            "schema": "phase4-g5-1-freeze-verification-v17",
...
def run_preparation_command(
    ...
    chronology = evidence_root / "CHRONOLOGY-v17.jsonl"
    ...
    write_json(
        intent,
        {
            "schema": "phase4-g5-1-preparation-command-intent-v17",
            "ordinal": ordinal,
            "label": label,
            "argv": command,
    ...
    terminal = {
        "schema": "phase4-g5-1-preparation-command-terminal-v17",
        "ordinal": ordinal,
        "label": label,
...
def prepare_inputs():
    ...
    if any(
        ...
        raise RuntimeError("v17 method/freeze evidence already exists")
    if not os.access(G5_CHILD_BINARY, os.X_OK):
        raise RuntimeError(f"hash-bound v17 child is not executable: {G5_CHILD_BINARY}")
    ...
    if future_input not in sources or missing:
        raise RuntimeError(f"v17 method source preflight mismatch: {missing}")
    write_input_and_method_manifests()
    if verify_sealed_input_manifest() != 93:
        raise RuntimeError("v17 reused input manifest cardinality mismatch")
...
def verify_dry_run(freeze):
    ...
    required = {
        "schema": "phase4-g5-1-dry-run-v17",
        "status": "PASS",
        "measured_rows": 0,
        "benchmark_child_processes_started": 0,
        "stores_opened": 0,
    ...
    intent_required = {
        "schema": "phase4-g5-1-dry-run-intent-v17",
        "status": "STARTED",
        "branch": BRANCH,
        "head": CHECKPOINT,
    ...
    if (
        ...
        or disposition.get("schema") != "phase4-g5-1-dry-run-disposition-v17"
...
def verify_freeze(require_static=False, require_dry=False):
    ...
    if require_static:
        ...
        screen_terminal = SCREEN_RESULT / "TERMINAL-VERIFICATION-v17.json"
        screen_final_manifest = SCREEN_RESULT / "FINAL-ARTIFACT-HASHES-v17.tsv"
        screen_final_verification = SCREEN_RESULT / "FINAL-READONLY-VERIFICATION-v17.json"
        screen_complete_wall = SCREEN_RESULT / "COMPLETE-WALL-v17.json"
        required = {
            "schema": "phase4-g5-1-static-closure-v17",
            "status": "PASS",
            "source_freeze_sha256": sha256(SOURCE_FREEZE),
            "tracked_diff_sha256": freeze["tracked_diff_sha256"],
...
def gate_workload_enumeration():
    ...
    return {
        "schema": "phase4-g5-1-prospective-workload-v17",
...
def hash_calibration():
    ...
    write_json(
        DRY_RUN_CALIBRATION_TERMINAL,
        {
            "schema": "phase4-g5-1-dry-run-calibration-terminal-v17",
            "status": "RETURNED",
            "return_code": completed.returncode,
    ...
    return {
        "schema": "phase4-g5-1-hash-calibration-v17",
...
def wrapper_calibration_plan():
    ...
    plan = {
        "schema": "phase4-g5-1-wrapper-calibration-plan-v17",
...
def run_wrapper_calibration(freeze, plan):
    ...
    intent = {
        "schema": "phase4-g5-1-wrapper-calibration-intent-v17",
        "status": "STARTED",
    ...
    try:
        ...
        initialization_evidence = {
            "schema": "phase4-g5-1-wrapper-initialization-evidence-v17",
        ...
        initialization = {
            "schema": "phase4-g5-1-wrapper-initialization-sample-v17",
            "status": "PASS",
        ...
        for sample_index in range(1, WRAPPER_CALIBRATION_SAMPLES + 1):
            ...
            representative = {
                "schema": "phase4-g5-1-wrapper-calibration-representative-arm-v17",
                "sample": sample_index,
                "master": master["relative_path"],
            ...
            sample = {
                "schema": "phase4-g5-1-wrapper-calibration-sample-v17",
                "status": "PASS",
                "sample": sample_index,
                "master": master["relative_path"],
                "phases_ns": phases,
        ...
        try:
            ...
            if WRAPPER_CALIBRATION_ROOT.exists():
                if (
                    ...
                    != f"phase4-g5-trusted-reopen-edit-wrapper-calibration-{DATE}-v17-attempt-2"
                ):
    ...
    result = {
        "schema": "phase4-g5-1-wrapper-calibration-result-v17",
        "status": "PASS",
        "classification": "ZeroProductWrapperCalibration",
...
def verify_wrapper_calibration(freeze):
    ...
    if (
        ...
        != "phase4-g5-1-wrapper-initialization-sample-v17"
    ...
    for sample_index, sample in enumerate(samples, start=1):
        ...
        if (
            sample.get("schema") != "phase4-g5-1-wrapper-calibration-sample-v17"
...
def dry_failure_documents(error_type, error_message, intent_sha256, collected, artifacts):
    failed = {
        "schema": "phase4-g5-1-dry-run-failed-v17",
        "status": "REVISE",
    ...
    disposition = {
        "schema": "phase4-g5-1-dry-run-disposition-v17",
        "status": "REVISE",
        "classification": "UNEXPECTED_DRY_RUN_FAILURE",
        "intent_sha256": intent_sha256,
...
def dry_run():
    ...
    if any(path.exists() for path in evidence_paths):
        raise RuntimeError("v17 dry-run evidence already exists")
    if not FREEZE_VERIFICATION.is_file():
        raise RuntimeError("v17 dry-run requires durable freeze verification")
    ...
    try:
        ...
        value = {
            ...
            "sample_count_interpretation": "deliberately-stricter-v17-choice-not-unambiguous-user-minimum",
        ...
        if status == "REVISE":
            write_json(
                PREMEASUREMENT_REVISE,
                {
                    "schema": "phase4-g5-1-premeasurement-revise-v17",
                    "status": "REVISE",
...
def release_lock(lock, result, terminal_verification=None, state="release"):
    ...
    try:
        ...
        attestation = result / "BENCHMARK-LOCK-RELEASE-ATTESTATION-v17.json"
        ...
        value = {
            "schema": "phase4-g5-1-lock-release-v17",
        ...
        write_json(result / "LOCK-RELEASE-v17.json", value)
        return value
    finally:
        os.close(lock["fd"])
        lock["fd"] = None
...
def run_semantic(executable, case, root, result, label, lifecycle=None):
    stdout = result / f"children-v17/{label}.stdout"
    stderr = result / f"children-v17/{label}.stderr"
    sidecar = result / f"time-v17/{label}.txt"
...
def validate_child_row(envelope):
    ...
    return {
        "schema": "phase4-g5-1-operation-v17",
...
class PersistentChild:
    def __init__(
        ...
        self.stdout_path = result / f"children-v17/{self.label}.stdout"
        self.stderr_path = result / f"children-v17/{self.label}.stderr"
        self.time_path = result / f"time-v17/{self.label}.txt"
...
def analyze(result):
    ...
    for analyzer, name in (
        (PRIMARY, "PRIMARY-ANALYSIS-v17.json"),
        (INDEPENDENT, "INDEPENDENT-RECOMPUTATION-v17.json"),
    ):
        output = result / name
        completed = subprocess.run(
            [
                sys.executable, str(analyzer), str(result / "RAW-v17.jsonl"),
                str(result / "TIMINGS-v17.tsv"), str(SCHEDULE), str(EXPECTED), str(output),
            ],
            cwd=REPO,
            text=True,
            capture_output=True,
    ...
    write_json(
        result / "ANALYZER-AGREEMENT-v17.json",
        {
            "schema": "phase4-g5-1-analyzer-agreement-v17",
...
def run_prearm_wrapper_initialization(result, freeze, dry, lock):
    ...
    evidence_path = result / "PREARM-WRAPPER-INITIALIZATION-EVIDENCE-v17.json"
    ...
    write_json(result / "PREARM-WRAPPER-INITIALIZATION-v17.json", value)
    if value["status"] != "PASS":
...
def ladder_prelock(campaign):
    ...
    if campaign == "gate":
        paths.extend(
            (
                SCREEN_RESULT / "TERMINAL-VERIFICATION-v17.json",
                SCREEN_RESULT / "FINAL-ARTIFACT-HASHES-v17.tsv",
                SCREEN_RESULT / "FINAL-READONLY-VERIFICATION-v17.json",
                SCREEN_RESULT / "COMPLETE-WALL-v17.json",
                STATIC_CLOSURE,
            )
        )
...
def run_campaign(campaign):
    ...
    try:
        ...
        work = result.parent / f"{result.name}-work-v17"
        work.mkdir(mode=0o700)
        work_roots = ArmWorkRootLifecycle(work)
        for name in ("operands-v17", "children-v17", "time-v17"):
            (result / name).mkdir()
        fsync_dir(result)
        fsync_dir(result.parent)

        g4_copy = result / "operands-v17/frozen-g4-verified"
        g5_copy = result / "operands-v17/g5-verified-trusted"
        ...
        write_json(
            result / "OPERAND-CUSTODY-v17.json",
            {
                "schema": "phase4-g5-1-operand-custody-v17",
        ...
        write_json(
            result / "PREFLIGHT-v17.json",
            {
                "schema": "phase4-g5-1-preflight-v17",
                "status": "PASS",
                "campaign": campaign,
                "branch": BRANCH,
                "checkpoint": CHECKPOINT,
        ...
        write_json(
            result / "ENVIRONMENT-v17.json",
            {
                "schema": "phase4-g5-1-environment-v17",
        ...
        shutil.copyfile(INPUT_MANIFEST, result / "INPUT-CUSTODY-v17.tsv")
        with (result / "INPUT-CUSTODY-v17.tsv").open("rb") as handle:
            os.fsync(handle.fileno())
                    ...
                    semantics.extend(
                        {
                            ...
                            "expectation_id": "native-semantic-v17",
                            "status": value["status"],
                            "error": value["error"],
                        }
                        for value in values
        ...
        write_json(result / "PRODUCT-CHILD-LIFECYCLE-v17.json", child_lifecycle)
        write_json(result / "WORK-ROOT-LIFECYCLE-v17.json", work_root_lifecycle)

        write_text(result / "RAW-v17.jsonl", "".join(compact(row) + "\n" for row in raw))
        timing_fields = (
        ...
        write_text(
            result / "TIMINGS-v17.tsv",
            "\t".join(timing_fields) + "\n"
            + "".join("\t".join(str(row[name]) for name in timing_fields) + "\n" for row in timings),
        )
        write_text(
            result / "SEMANTIC-FAULT-RESULTS-v17.tsv",
        ...
        write_json(result / "COMMANDS-v17.json", {"schema": "phase4-g5-1-commands-v17", "commands": commands})
        analyze(result)

        if work.parent != result.parent or not work.name.endswith("-work-v17"):
            raise RuntimeError("refusing unsafe work cleanup")
        shutil.rmtree(work)
        fsync_dir(work.parent)
        residue = [str(path) for path in result.parent.glob(f"{result.name}-work-v17")]
        write_json(
            result / "CLEANUP-v17.json",
            {
                "schema": "phase4-g5-1-cleanup-v17",
        ...
        payload_excluded = {
            "PAYLOAD-MANIFEST-v17.tsv", "MEASURED-TERMINAL-v17.json",
            "TERMINAL-VERIFICATION-v17.json", "COMPLETE-WALL-v17.json",
            "BENCHMARK-LOCK-RELEASE-ATTESTATION-v17.json", "LOCK-RELEASE-v17.json",
            "FINAL-ARTIFACT-HASHES-v17.tsv", "FINAL-READONLY-VERIFICATION-v17.json",
        }
        payload = result / "PAYLOAD-MANIFEST-v17.tsv"
        ...
        terminal = result / "MEASURED-TERMINAL-v17.json"
        write_json(
            terminal,
            {
                "schema": "phase4-g5-1-measured-terminal-v17",
                "status": "PASS",
                "campaign": campaign,
                "rows": len(timings),
                ...
                "product_child_lifecycle_sha256": sha256(
                    result / "PRODUCT-CHILD-LIFECYCLE-v17.json"
                ),
                "work_root_lifecycle_sha256": sha256(
                    result / "WORK-ROOT-LIFECYCLE-v17.json"
                ),
                "prearm_wrapper_initialization_sha256": sha256(
                    result / "PREARM-WRAPPER-INITIALIZATION-v17.json"
                ),
        ...
        verification = result / "TERMINAL-VERIFICATION-v17.json"
        write_json(
            verification,
            {
                "schema": "phase4-g5-1-terminal-verification-v17",
                "status": "PASS",
        ...
        final = result / "FINAL-ARTIFACT-HASHES-v17.tsv"
        write_text(
            final,
            manifest_text(
                result,
                excluded={final.name, "FINAL-READONLY-VERIFICATION-v17.json", "COMPLETE-WALL-v17.json"},
            ),
        )
        final_count = verify_manifest(result, final, "result_relative_path")
        write_json(
            result / "FINAL-READONLY-VERIFICATION-v17.json",
            {
                "schema": "phase4-g5-1-final-readonly-verification-v17",
                "status": "PASS",
        ...
        write_json(
            result / "COMPLETE-WALL-v17.json",
            {
                "schema": "phase4-g5-1-complete-wall-v17",
                "status": "PASS",
                "campaign": campaign,
                ...
                "terminal_self_exclusion": "COMPLETE-WALL-v17.json follows the verified final manifest",
            },
        )
        fsync_dir(result)
        ...
        if result.exists() and not (result / "FAILED-v17.json").exists():
            write_json(
                result / "FAILED-v17.json",
                {
                    "schema": "phase4-g5-1-failure-v17",
                    "status": "REVISE",
                    "error": str(error),
...
def run_self_checks():
    ...
    with tempfile.TemporaryDirectory(prefix="layerfs-g5-v17-self-check-") as temporary:
        work = pathlib.Path(temporary) / "work-v17"
        work.mkdir()
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="layerfs-g5-v17-forecast-check-") as temporary:
    ...
    assert FORECAST_MODEL_VERSION == "phase4-g5-1-v17-fast-law-forecast-v5"
