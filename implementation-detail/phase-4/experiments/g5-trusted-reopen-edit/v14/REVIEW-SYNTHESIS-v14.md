...
## V13 terminal and accepted v14 method repair

V13 honestly closed as `PREMEASUREMENT_REVISE` after a valid zero-row and
post-run verification (`106.287054720 s`) reached screen preflight. The screen
acquired its required global lock, but `verify_wrapper_calibration` rejected
the live `LOCK.exists()` as though it contradicted the historical fact that the
zero-product calibration acquired no lock. It failed before result-root
creation, product child, Store, ordinal 1, or row. The lock was released and
both release artifacts are bound by `V13-SUPERSESSION-v14.json`.

V14 changes only the wrapper-calibration live verification predicate and method
version. Historical calibration fields `global_lock_absent=true` and
`locks_acquired=0` remain exact. A later current campaign lock is irrelevant to
that historical evidence; calibration-root residue remains a live failure. The
focused check covers both absent-lock and owned-lock contexts. It retains v13's
split intent contract and top-level wrapper semantics derived from the nested
rooted state. It measures one process-first wrapper
...
unchanged v10 release and 93-file sealed input root remain named, rehashed
external operands; no v13 product row or timing is promoted to v14 authority.

V14 keeps the 200-arm schedule, 56 checkpoints, half-slower calibration floor,
thresholds, and 120-second wall. Its only authority redesign is:

- verify the complete sealed input manifest once under the global lock;
- use native APFS clone receipts, schema `g5-v14-native-clone-receipt-v1`, to
...
- emit constant-size rooted state, schema `g5-v14-rooted-logical-state-v1`,
...
`FREEZE-VERIFICATION-v14.json` is classified
...
`PRODUCT-CHILD-LIFECYCLE-v14.json` under schema
`phase4-g5-1-product-child-lifecycle-v14`. Per-arm cleanup receipts use schema
`phase4-g5-1-arm-cleanup-receipt-v14`; their ordered terminal record is likewise
duplicated as `WORK-ROOT-LIFECYCLE-v14.json` under schema
`phase4-g5-1-work-root-lifecycle-v14`.
...
`wrapper-calibration-intent-v14`, `wrapper-initialization-sample-v14`,
`wrapper-calibration-sample-v14`, and `wrapper-calibration-result-v14`. The
...
intent embeds `phase4-g5-1-wrapper-calibration-plan-v14`, retains the complete
...
`phase4-g5-1-prearm-wrapper-initialization-v14` record after lock/custody and
...
closes v14 as `PREMEASUREMENT_REVISE` before screen; no schedule or threshold
is reduced.

`DRY-RUN-INTENT-v14.json` is durably written before either calibration begins.
A single outer failure funnel then covers wrapper calibration, hash calibration,
forecasting, and sealing: it always writes `DRY-RUN-FAILED-v14.json` before a
REVISE `DRY-RUN-DISPOSITION-v14.json`, even when the wrapper result is absent.
...
V14 nevertheless voluntarily freezes the stricter v1-lineage design: each
