# Phase 4 G5-1 v14 preregistration
...
The candidate remains CAS + CDC + COW + canonical K64/F64 + SQLite. V14 changes
...
Any failure preserves v14. No observed row may be deleted, replaced, reordered,
or selectively rerun. A source or method repair creates v15.
...
`phase4-g5-1-wrapper-calibration-intent-v14`,
`phase4-g5-1-wrapper-initialization-sample-v14`,
`phase4-g5-1-wrapper-calibration-sample-v14`, and
`phase4-g5-1-wrapper-calibration-result-v14`.
...
`phase4-g5-1-wrapper-calibration-plan-v14` and records the complete candidate
...
The positive difference between 120 seconds and the forecast
...
must write and fsync the top-level `DRY-RUN-INTENT-v14.json` before invoking
...
write and fsync `DRY-RUN-FAILED-v14.json` first, then write and fsync a
`DRY-RUN-DISPOSITION-v14.json` with `status=REVISE` that binds the FAILED hash
and explicitly records absent optional artifacts. A success or calibrated
overrun writes and fsyncs complete `DRY-RUN-v14.json`, including both
...
`PREMEASUREMENT-REVISE-v14.json` and stops before screen. A missing dry-run or
...
V13 is byte-preserved and superseded by `V13-SUPERSESSION-v14.json`. Its fresh
...
V14 is a wrapper-calibration-verifier/method-only revision. It retains exact
historical `global_lock_absent=true` and zero-product `locks_acquired=0`
calibration evidence, but current lock existence is not historical evidence and
cannot invalidate later owned-lock campaign verification. The focused check
requires identical frozen calibration context to be valid both without a lock
and with a later owned campaign lock, while calibration-root residue fails in
both contexts. V14 reuses the unchanged v10
release and the independently rehashed 93-file sealed v10 input root as an
exact hash-verified external release plus sealed external inputs with `product_release_reuse=true` and
`input_reuse=true`. Their exact v10 paths and SHA-256 values remain named; they
are not copied, renamed, or represented as v14-generated. V14 reuses no v10
or v13 product row or timing observation. The new v14 input manifest rebinds the same sealed
files byte-for-byte before the v14 method manifest and source freeze are
generated.
...
The final `PREARM-WRAPPER-INITIALIZATION-v14.json` write is outside that bound
...
schedule, expectation, limitation, and supersession byte has settled, v14
...
child. It writes `FREEZE-VERIFICATION-v14.json` with classification
...
V14 voluntarily freezes a deliberately stricter v1-lineage design: each
...
`phase4-g5-1-product-child-lifecycle-v14`. The two copies must be byte-value
...
child, v14 verifies the complete sealed input manifest exactly once: path,
...
`g5-v14-native-clone-receipt-v1`. Its ordered `entries` bind each source master
...
inspection for a future separate API. V14 therefore removes the external
...
`g5-v14-rooted-logical-state-v1` from one query-only, autocommit, constant-row
...
V14 does not claim physical SQLite file-byte parity. The sibling
`physical_allocation_observation` remains schema
`g5-v14-physical-allocation-observation-v1`, classification
...
Each wrapper receipt uses schema `phase4-g5-1-arm-cleanup-receipt-v14` and is
repeated in order by the raw and standalone
`phase4-g5-1-work-root-lifecycle-v14` record. Started and cleaned roots equal
