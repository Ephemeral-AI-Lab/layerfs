...
LIFECYCLE_SCHEMA = "phase4-g5-1-product-child-lifecycle-v24"
...
CLONE_SCHEMA = "g5-v24-native-clone-receipt-v1"
CLONE_TOP = frozenset((
...
PHYSICAL_SCHEMA = "g5-v24-physical-allocation-observation-v1"
...
WORK_ROOT_SCHEMA = "phase4-g5-1-work-root-lifecycle-v24"
...
INPUT_MANIFEST = pathlib.Path(__file__).resolve().parents[1] / "method/INPUT-MANIFEST-v24.tsv"
DRY_RUN = pathlib.Path(__file__).resolve().parents[1] / "DRY-RUN-v24.json"
...
def audit_prearm(
    ...
    facts = (
        ...
        type(evidence) is dict
        and evidence.get("schema") == "phase4-g5-1-prearm-wrapper-initialization-evidence-v24",
        type(evidence) is dict
...
def recompute(raw_file, timing_file, schedule_file, expected_file):
    ...
    rows = [value for value in documents if value.get("schema") == "phase4-g5-1-operation-v24"]
    ...
    sentinel = [value for value in documents if value.get("schema") == "phase4-g5-1-protected-sentinel-v24"]
    ...
    initialization_rows = [
        ...
        if value.get("schema") == "phase4-g5-1-prearm-wrapper-initialization-v24"
    ]
    ...
    artifact = pathlib.Path(raw_file).parent / "PREARM-WRAPPER-INITIALIZATION-v24.json"
    try:
        artifact_bytes = artifact.read_bytes()
    except OSError:
        artifact_bytes = b""
    evidence_path = pathlib.Path(raw_file).parent / (
        "PREARM-WRAPPER-INITIALIZATION-EVIDENCE-v24.json"
    )
    try:
    ...
    audit_lifecycle(
        ...
        sentinel,
        campaign,
        pathlib.Path(raw_file).parent / "PRODUCT-CHILD-LIFECYCLE-v24.json",
        problems,
    )
    audit_work_roots(
        work_root_rows,
        rows,
        pathlib.Path(raw_file).parent / "WORK-ROOT-LIFECYCLE-v24.json",
        problems,
    )
    if campaign == "screen":
        counts = {}
    ...
    return {
        "schema": "phase4-g5-1-independent-recomputation-v24",
