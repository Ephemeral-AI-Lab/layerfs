...
HISTORY = HERE.parent
METHOD = HERE / "method"
SCHEDULE = METHOD / "SCHEDULE-v24.tsv"
EXPECTED = METHOD / "EXPECTED-OUTCOMES-v24.tsv"
INPUT_MANIFEST = METHOD / "INPUT-MANIFEST-v24.tsv"
METHOD_MANIFEST = METHOD / "METHOD-MANIFEST-v24.tsv"
SOURCE_FREEZE = METHOD / "SOURCE-FREEZE-v24.json"
FREEZE_VERIFICATION = HERE / "FREEZE-VERIFICATION-v24.json"
LIMITATIONS = HERE / "LIMITATIONS-v24.json"
STATIC_CLOSURE = HERE / "STATIC-CLOSURE-ATTEMPT-2-v24.json"
DRY_RUN = HERE / "DRY-RUN-v24.json"
DRY_RUN_INTENT = HERE / "DRY-RUN-INTENT-v24.json"
DRY_RUN_CALIBRATION_STDOUT = HERE / "DRY-RUN-CALIBRATION-v24.stdout"
DRY_RUN_CALIBRATION_STDERR = HERE / "DRY-RUN-CALIBRATION-v24.stderr"
DRY_RUN_CALIBRATION_TERMINAL = HERE / "DRY-RUN-CALIBRATION-TERMINAL-v24.json"
DRY_RUN_DISPOSITION = HERE / "DRY-RUN-DISPOSITION-v24.json"
DRY_RUN_FAILED = HERE / "DRY-RUN-FAILED-v24.json"
PREMEASUREMENT_REVISE = HERE / "PREMEASUREMENT-REVISE-v24.json"
WRAPPER_CALIBRATION_INTENT = HERE / "WRAPPER-CALIBRATION-INTENT-v24.json"
WRAPPER_CALIBRATION_RAW = HERE / "WRAPPER-CALIBRATION-RAW-v24.jsonl"
WRAPPER_CALIBRATION_RESULT = HERE / "WRAPPER-CALIBRATION-RESULT-v24.json"
...
SCREEN_RESULT = REPO / f"target/phase4-g5-trusted-reopen-edit-{DATE}-v24-screen-attempt-2"
GATE_RESULT = REPO / f"target/phase4-g5-trusted-reopen-edit-{DATE}-v24-attempt-2"
WRAPPER_CALIBRATION_ROOT = (
    REPO / f"target/phase4-g5-trusted-reopen-edit-wrapper-calibration-{DATE}-v24"
)

# V24 changes the product timer equation and uses a fresh hash-bound release.
G5_CHILD_BINARY = HERE / "g5-benchmark/target/release/layerfs-g5-trusted-child-v24"
...
SENTINEL_SCHEMA = "phase4-g5-1-protected-sentinel-v24"
REQUEST_FIELDS = (
...
V23_SUPERSESSION = HERE / "V23-SUPERSESSION-v24.json"
...
FORECAST_MODEL_VERSION = "phase4-g5-1-v24-fast-law-forecast-v5"
...
PHYSICAL_ALLOCATION_SCHEMA = "g5-v24-physical-allocation-observation-v1"
...
CHILD_LIFECYCLE_SCHEMA = "phase4-g5-1-product-child-lifecycle-v24"
RSS_CLASSIFICATION = "PerProductChildRetainedPeak"
SYNCHRONOUS_RSS_KIND = "synchronous-one-shot"
ARM_CLEANUP_SCHEMA = "phase4-g5-1-arm-cleanup-receipt-v24"
WORK_ROOT_LIFECYCLE_SCHEMA = "phase4-g5-1-work-root-lifecycle-v24"
...
def method_source_names():
    fixed = {
        ...
        for path in (
            HERE / "PREREGISTRATION-v24.md",
            HERE / "REVIEW-SYNTHESIS-v24.md",
            HERE / "SAMPLE-COUNT-INTERPRETATION-ADDENDUM-v24.md",
            HERE / "FOCUSED-TEST-ATTEMPTS-v24.json",
            HERE / "PREMEASUREMENT-READINESS-AUDIT-v24.json",
            LIMITATIONS,
            HERE / "runner.py",
            PRIMARY,
            INDEPENDENT,
...
def freeze_interface_contract():
    return {
        ...
        "wrapper_calibration": {
            ...
            "intent_schema": "phase4-g5-1-wrapper-calibration-intent-v24",
            "initialization_schema": "phase4-g5-1-wrapper-initialization-sample-v24",
            "sample_schema": "phase4-g5-1-wrapper-calibration-sample-v24",
            "result_schema": "phase4-g5-1-wrapper-calibration-result-v24",
...
def write_input_and_method_manifests():
    ...
    freeze = {
        "schema": "phase4-g5-1-source-freeze-v24",
        ...
        "post_freeze_verification": {
            ...
            "schema": "phase4-g5-1-freeze-verification-v24",
...
def run_preparation_command(
    ...
    chronology = evidence_root / "CHRONOLOGY-v24.jsonl"
    ...
    write_json(
        intent,
        {
            "schema": "phase4-g5-1-preparation-command-intent-v24",
            "ordinal": ordinal,
            "label": label,
            "argv": command,
    ...
    terminal = {
        "schema": "phase4-g5-1-preparation-command-terminal-v24",
        "ordinal": ordinal,
        "label": label,
...
def prepare_inputs():
    ...
    if any(
        ...
        raise RuntimeError("v24 method/freeze evidence already exists")
    if not os.access(G5_CHILD_BINARY, os.X_OK):
        raise RuntimeError(f"hash-bound v24 child is not executable: {G5_CHILD_BINARY}")
    ...
    if future_input not in sources or missing:
        raise RuntimeError(f"v24 method source preflight mismatch: {missing}")
    write_input_and_method_manifests()
    if verify_sealed_input_manifest() != 93:
        raise RuntimeError("v24 reused input manifest cardinality mismatch")
...
def verify_dry_run(freeze):
    ...
    required = {
        "schema": "phase4-g5-1-dry-run-v24",
        "status": "PASS",
        "measured_rows": 0,
        "benchmark_child_processes_started": 0,
        "stores_opened": 0,
    ...
    intent_required = {
        "schema": "phase4-g5-1-dry-run-intent-v24",
        "status": "STARTED",
        "branch": BRANCH,
        "head": CHECKPOINT,
    ...
    if (
        ...
        or disposition.get("schema") != "phase4-g5-1-dry-run-disposition-v24"
...
def verify_freeze(require_static=False, require_dry=False):
    ...
    if require_static:
        ...
        screen_terminal = SCREEN_RESULT / "TERMINAL-VERIFICATION-v24.json"
        screen_final_manifest = SCREEN_RESULT / "FINAL-ARTIFACT-HASHES-v24.tsv"
        screen_final_verification = SCREEN_RESULT / "FINAL-READONLY-VERIFICATION-v24.json"
        screen_complete_wall = SCREEN_RESULT / "COMPLETE-WALL-v24.json"
        required = {
            "schema": "phase4-g5-1-static-closure-v24",
            "status": "PASS",
            "source_freeze_sha256": sha256(SOURCE_FREEZE),
            "tracked_diff_sha256": freeze["tracked_diff_sha256"],
...
def gate_workload_enumeration():
    ...
    return {
        "schema": "phase4-g5-1-prospective-workload-v24",
...
def hash_calibration():
    ...
    write_json(
        DRY_RUN_CALIBRATION_TERMINAL,
        {
            "schema": "phase4-g5-1-dry-run-calibration-terminal-v24",
            "status": "RETURNED",
            "return_code": completed.returncode,
    ...
    return {
        "schema": "phase4-g5-1-hash-calibration-v24",
...
def wrapper_calibration_plan():
    ...
    plan = {
        "schema": "phase4-g5-1-wrapper-calibration-plan-v24",
...
def run_wrapper_calibration(freeze, plan):
    ...
    intent = {
        "schema": "phase4-g5-1-wrapper-calibration-intent-v24",
        "status": "STARTED",
    ...
    try:
        ...
        initialization_evidence = {
            "schema": "phase4-g5-1-wrapper-initialization-evidence-v24",
        ...
        initialization = {
            "schema": "phase4-g5-1-wrapper-initialization-sample-v24",
            "status": "PASS",
        ...
        for sample_index in range(1, WRAPPER_CALIBRATION_SAMPLES + 1):
            ...
            representative = {
                "schema": "phase4-g5-1-wrapper-calibration-representative-arm-v24",
                "sample": sample_index,
                "master": master["relative_path"],
            ...
            sample = {
                "schema": "phase4-g5-1-wrapper-calibration-sample-v24",
                "status": "PASS",
                "sample": sample_index,
                "master": master["relative_path"],
                "phases_ns": phases,
        ...
        try:
            ...
            if WRAPPER_CALIBRATION_ROOT.exists():
                if (
                    ...
                    != f"phase4-g5-trusted-reopen-edit-wrapper-calibration-{DATE}-v24"
                ):
    ...
    result = {
        "schema": "phase4-g5-1-wrapper-calibration-result-v24",
        "status": "PASS",
        "classification": "ZeroProductWrapperCalibration",
...
def verify_wrapper_calibration(freeze):
    ...
    if (
        ...
        != "phase4-g5-1-wrapper-initialization-sample-v24"
    ...
    for sample_index, sample in enumerate(samples, start=1):
        ...
        if (
            sample.get("schema") != "phase4-g5-1-wrapper-calibration-sample-v24"
...
def dry_failure_documents(error_type, error_message, intent_sha256, collected, artifacts):
    failed = {
        "schema": "phase4-g5-1-dry-run-failed-v24",
        "status": "REVISE",
    ...
    disposition = {
        "schema": "phase4-g5-1-dry-run-disposition-v24",
        "status": "REVISE",
        "classification": "UNEXPECTED_DRY_RUN_FAILURE",
        "intent_sha256": intent_sha256,
...
def dry_run():
    ...
    if any(path.exists() for path in evidence_paths):
        raise RuntimeError("v24 dry-run evidence already exists")
    if not FREEZE_VERIFICATION.is_file():
        raise RuntimeError("v24 dry-run requires durable freeze verification")
    ...
    try:
        ...
        value = {
            ...
            "sample_count_interpretation": "deliberately-stricter-v24-choice-not-unambiguous-user-minimum",
        ...
        if status == "REVISE":
            write_json(
                PREMEASUREMENT_REVISE,
                {
                    "schema": "phase4-g5-1-premeasurement-revise-v24",
                    "status": "REVISE",
...
def release_lock(lock, result, terminal_verification=None, state="release"):
    ...
    try:
        ...
        attestation = result / "BENCHMARK-LOCK-RELEASE-ATTESTATION-v24.json"
        ...
        value = {
            "schema": "phase4-g5-1-lock-release-v24",
        ...
        write_json(result / "LOCK-RELEASE-v24.json", value)
        return value
    finally:
        os.close(lock["fd"])
        lock["fd"] = None
...
def run_semantic(executable, case, root, result, label, lifecycle=None):
    stdout = result / f"children-v24/{label}.stdout"
    stderr = result / f"children-v24/{label}.stderr"
    sidecar = result / f"time-v24/{label}.txt"
...
def validate_child_row(envelope):
    ...
    return {
        "schema": "phase4-g5-1-operation-v24",
...
class PersistentChild:
    def __init__(
        ...
        self.stdout_path = result / f"children-v24/{self.label}.stdout"
        self.stderr_path = result / f"children-v24/{self.label}.stderr"
        self.time_path = result / f"time-v24/{self.label}.txt"
...
def analyze(result):
    ...
    for analyzer, name in (
        (PRIMARY, "PRIMARY-ANALYSIS-v24.json"),
        (INDEPENDENT, "INDEPENDENT-RECOMPUTATION-v24.json"),
    ):
        output = result / name
        completed = subprocess.run(
            [
                sys.executable, str(analyzer), str(result / "RAW-v24.jsonl"),
                str(result / "TIMINGS-v24.tsv"), str(SCHEDULE), str(EXPECTED), str(output),
            ],
            cwd=REPO,
            text=True,
            capture_output=True,
    ...
    write_json(
        result / "ANALYZER-AGREEMENT-v24.json",
        {
            "schema": "phase4-g5-1-analyzer-agreement-v24",
...
def run_prearm_wrapper_initialization(result, freeze, dry, lock):
    ...
    evidence = {
        "schema": "phase4-g5-1-prearm-wrapper-initialization-evidence-v24",
    ...
    evidence_path = result / "PREARM-WRAPPER-INITIALIZATION-EVIDENCE-v24.json"
    write_json(evidence_path, evidence)
    total_ns = max(1, time.monotonic_ns() - started)
    value = {
        **evidence,
        "schema": "phase4-g5-1-prearm-wrapper-initialization-v24",
    ...
    write_json(result / "PREARM-WRAPPER-INITIALIZATION-v24.json", value)
    if value["status"] != "PASS":
...
def ladder_prelock(campaign):
    ...
    if campaign == "gate":
        paths.extend(
            (
                SCREEN_RESULT / "TERMINAL-VERIFICATION-v24.json",
                SCREEN_RESULT / "FINAL-ARTIFACT-HASHES-v24.tsv",
                SCREEN_RESULT / "FINAL-READONLY-VERIFICATION-v24.json",
                SCREEN_RESULT / "COMPLETE-WALL-v24.json",
                STATIC_CLOSURE,
            )
        )
...
def run_campaign(campaign):
    ...
    try:
        ...
        work = result.parent / f"{result.name}-work-v24"
        work.mkdir(mode=0o700)
        work_roots = ArmWorkRootLifecycle(work)
        for name in ("operands-v24", "children-v24", "time-v24"):
            (result / name).mkdir()
        fsync_dir(result)
        fsync_dir(result.parent)

        g4_copy = result / "operands-v24/frozen-g4-verified"
        g5_copy = result / "operands-v24/g5-verified-trusted"
        ...
        write_json(
            result / "OPERAND-CUSTODY-v24.json",
            {
                "schema": "phase4-g5-1-operand-custody-v24",
        ...
        write_json(
            result / "PREFLIGHT-v24.json",
            {
                "schema": "phase4-g5-1-preflight-v24",
                "status": "PASS",
                "campaign": campaign,
                "branch": BRANCH,
                "checkpoint": CHECKPOINT,
        ...
        write_json(
            result / "DATABASE-PRECONDITION-FORECAST-v24.json",
            {
                "schema": "phase4-g5-1-database-precondition-forecast-v24",
                "status": "PASS",
                "campaign": campaign,
                "timing_claim": "CacheWarmPreconditionedNotColdReopen",
        ...
        write_json(
            result / "ENVIRONMENT-v24.json",
            {
                "schema": "phase4-g5-1-environment-v24",
        ...
        shutil.copyfile(INPUT_MANIFEST, result / "INPUT-CUSTODY-v24.tsv")
        with (result / "INPUT-CUSTODY-v24.tsv").open("rb") as handle:
            os.fsync(handle.fileno())
                    ...
                    semantics.extend(
                        {
                            ...
                            "expectation_id": "native-semantic-v24",
                            "status": value["status"],
                            "error": value["error"],
                        }
                        for value in values
        ...
        write_json(result / "PRODUCT-CHILD-LIFECYCLE-v24.json", child_lifecycle)
        write_json(result / "WORK-ROOT-LIFECYCLE-v24.json", work_root_lifecycle)

        write_text(result / "RAW-v24.jsonl", "".join(compact(row) + "\n" for row in raw))
        timing_fields = (
        ...
        write_text(
            result / "TIMINGS-v24.tsv",
            "\t".join(timing_fields) + "\n"
            + "".join("\t".join(str(row[name]) for name in timing_fields) + "\n" for row in timings),
        )
        write_text(
            result / "SEMANTIC-FAULT-RESULTS-v24.tsv",
        ...
        write_json(result / "COMMANDS-v24.json", {"schema": "phase4-g5-1-commands-v24", "commands": commands})
        analyze(result)

        if work.parent != result.parent or not work.name.endswith("-work-v24"):
            raise RuntimeError("refusing unsafe work cleanup")
        shutil.rmtree(work)
        fsync_dir(work.parent)
        residue = [str(path) for path in result.parent.glob(f"{result.name}-work-v24")]
        write_json(
            result / "CLEANUP-v24.json",
            {
                "schema": "phase4-g5-1-cleanup-v24",
        ...
        payload_excluded = {
            "PAYLOAD-MANIFEST-v24.tsv", "MEASURED-TERMINAL-v24.json",
            "TERMINAL-VERIFICATION-v24.json", "COMPLETE-WALL-v24.json",
            "BENCHMARK-LOCK-RELEASE-ATTESTATION-v24.json", "LOCK-RELEASE-v24.json",
            "FINAL-ARTIFACT-HASHES-v24.tsv", "FINAL-READONLY-VERIFICATION-v24.json",
        }
        payload = result / "PAYLOAD-MANIFEST-v24.tsv"
        ...
        terminal = result / "MEASURED-TERMINAL-v24.json"
        write_json(
            terminal,
            {
                "schema": "phase4-g5-1-measured-terminal-v24",
                "status": "PASS",
                "campaign": campaign,
                "rows": len(timings),
                ...
                "product_child_lifecycle_sha256": sha256(
                    result / "PRODUCT-CHILD-LIFECYCLE-v24.json"
                ),
                "work_root_lifecycle_sha256": sha256(
                    result / "WORK-ROOT-LIFECYCLE-v24.json"
                ),
                "prearm_wrapper_initialization_sha256": sha256(
                    result / "PREARM-WRAPPER-INITIALIZATION-v24.json"
                ),
        ...
        verification = result / "TERMINAL-VERIFICATION-v24.json"
        write_json(
            verification,
            {
                "schema": "phase4-g5-1-terminal-verification-v24",
                "status": "PASS",
        ...
        final = result / "FINAL-ARTIFACT-HASHES-v24.tsv"
        write_text(
            final,
            manifest_text(
                result,
                excluded={final.name, "FINAL-READONLY-VERIFICATION-v24.json", "COMPLETE-WALL-v24.json"},
            ),
        )
        final_count = verify_manifest(result, final, "result_relative_path")
        write_json(
            result / "FINAL-READONLY-VERIFICATION-v24.json",
            {
                "schema": "phase4-g5-1-final-readonly-verification-v24",
                "status": "PASS",
        ...
        write_json(
            result / "COMPLETE-WALL-v24.json",
            {
                "schema": "phase4-g5-1-complete-wall-v24",
                "status": "PASS",
                "campaign": campaign,
                ...
                "terminal_self_exclusion": "COMPLETE-WALL-v24.json follows the verified final manifest",
            },
        )
        fsync_dir(result)
        ...
        if result.exists() and not (result / "FAILED-v24.json").exists():
            write_json(
                result / "FAILED-v24.json",
                {
                    "schema": "phase4-g5-1-failure-v24",
                    "status": "REVISE",
                    "error": str(error),
...
def run_self_checks():
    ...
    with tempfile.TemporaryDirectory(prefix="layerfs-g5-v24-self-check-") as temporary:
        work = pathlib.Path(temporary) / "work-v24"
        work.mkdir()
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="layerfs-g5-v24-forecast-check-") as temporary:
    ...
    assert FORECAST_MODEL_VERSION == "phase4-g5-1-v24-fast-law-forecast-v5"
