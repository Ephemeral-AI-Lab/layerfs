...
def equal_work(left, right, keys=None):
    fields = list(keys) if keys else list((set(left) & set(right)) - {"status", "status_adapter", "adjacent_estimator_v12"})
...
for sequence in ESTIMATED:
    ...
    orders = arms[(sequence, control_role)]["payload"]["adjacent_estimator_v12"]["sample_order"] + arms[(sequence, candidate_role)]["payload"]["adjacent_estimator_v12"]["sample_order"]
...
report.update({"schema": "phase4-g4-independent-recomputation-v12", "status": "PASS" if not issues else "REVISE", "issues": issues, "normalized_ledger": ledger, "normalized_ledger_sha256": digest})
