...
CANDIDATE_SHA256 = "789f53068793b8aeea0d03924b1dd985cc6a0d4706c98188f2034b216ca3adcb"
SOURCE_HASHES = {
...
runner.HERE = HERE
runner.MANIFEST = HERE / "METHODOLOGY-MANIFEST-v11.tsv"
runner.TARGET = REPO / "target/phase4-g4-materialization-acceptance-20260822-v11"
runner.RESULTS = runner.TARGET / "results-v11"
runner.WORK = runner.TARGET / "work-v11"
runner.HASHES["candidate"] = CANDIDATE_SHA256
runner.BUCKET_LIMITS = {
    "lock_and_measured_preflight": 1_000_000_000,
    "private_base_and_shared_preparation": 80_000_000_000,
    "row_dispatch_and_measured_operations": 31_000_000_000,
    ...
    "payload_manifest_terminal_and_verification": 5_000_000_000,
}
if sum(runner.BUCKET_LIMITS.values()) != runner.CEILING_NS:
    raise SystemExit("v11 bucket partition must sum to the 120-second ceiling")
...
class CustodyOs:
    ...
    def open(self, path, flags, mode=0o777, *, dir_fd=None):
        ...
        content = (json.dumps({"schema": "phase4-g4-v11-lock-attestation", "state": "held", "pid": os.getpid(), "token": token}, sort_keys=True) + "\n").encode()
...
def seal_owned_lock(disposition):
    ...
    try:
        ...
        attestation = {
            "schema": "phase4-g4-v11-lock-attestation",
            "state": disposition,
            "pid": os.getpid(),
        ...
        destination = runner.RESULTS / "BENCHMARK-LOCK-RELEASE-ATTESTATION-v11.json" if disposition == "release" and runner.RESULTS.is_dir() else REPO / "target" / f"BENCHMARK-LOCK-FAILURE-ATTESTATION-v11-{lock_state['token'][:16]}.json"
...
def aggregate_role(sequence, role, aggregate_label, command, samples):
    ...
    payload["adjacent_estimator_v11"] = {
        "schema": "phase4-g4-adjacent-estimator-v11",
    ...
    runner.write_text(
        ...
        json.dumps({"schema": "phase4-g4-v11-aggregate-stderr", "role": role, "sample_order": payload["adjacent_estimator_v11"]["sample_order"]}, sort_keys=True) + "\n",
    )
...
def v11_run_capture(command, label, directory, env=None, timed=True, allow_nonzero=False):
...
runner.run_capture = v11_run_capture


def v11_write_json(path, value):
    path = Path(path)
    ...
    if path.name == "MEASURED-TERMINAL-VERIFICATION-v1.json":
        ...
        module.module.original_write_json(
            runner.RESULTS / "LOCK-RELEASE-v11.json",
            {
                "schema": "phase4-g4-v11-lock-release-v1",
                "status": value["status"],
...
runner.write_json = v11_write_json
...
if __name__ == "__main__":
    try:
        ...
        release = json.loads((runner.RESULTS / "LOCK-RELEASE-v11.json").read_text())
        if overruns or cleanup.get("declared_deleted_root") != "work-v11" or not release.get("lock_absent_after_release"):
            raise SystemExit(f"v11 wrapper closure mismatch: {overruns}")
