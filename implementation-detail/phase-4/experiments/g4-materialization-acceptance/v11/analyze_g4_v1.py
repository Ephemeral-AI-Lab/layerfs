...
def same_work(left, right, keys=None):
    selected = list(keys) if keys else sorted(set(left) & set(right) - {"status", "status_adapter", "adjacent_estimator_v11"})
...
for sequence in ESTIMATED:
    ...
    control_meta = arms[(sequence, control_role)]["payload"]["adjacent_estimator_v11"]
    candidate_meta = arms[(sequence, candidate_role)]["payload"]["adjacent_estimator_v11"]
...
report.update({"schema": "phase4-g4-primary-analysis-v11", "status": "PASS" if not issues else "REVISE", "issues": issues, "normalized_ledger": ledger, "normalized_ledger_sha256": digest})
