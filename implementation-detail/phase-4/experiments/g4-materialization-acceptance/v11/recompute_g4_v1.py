...
BASE = Path("/Users/yifanxu/Ephemeral-AI-Lab/layerfs-empty/implementation-detail/phase-4/experiments/g4-materialization-acceptance/v1/recompute_g4_v1.py")
BASE_SHA256 = "402980e4a4f441964086a37e37d2105e3ec8e78bc48b24ecdf8d7fbbb1de9bcc"
CANDIDATE = "789f53068793b8aeea0d03924b1dd985cc6a0d4706c98188f2034b216ca3adcb"
...
SOURCE_HASHES = {
    REPO / "crates/layerfs-engine/src/bin/phase4_create_edit_benchmark.rs": "10aa474bf5fe5130fde73703444f5082bfa4a41a3c82dd0d84a025bedd9485a2",
    REPO / "crates/layerfs-engine/src/bin/phase4_g3_materialization.rs": "326cf27c61915501611d5a1876392ee8b93c10038adeae3d8986c13cfc314f71",
...
HISTORY_SHA256 = "504c9e66e317d2ab518864d46b72eb64f13412cb8b5ae092abcf9d9b80850f82"
CONTROLS = {"535bfa178a8a569ea43d9f1d23808775c2349a29f9cdacddae508391a6e5e61e": "frozen-g3-v13-source-static-bound", "5d72b46d29a5b77494781f343cc6841a71879b5de426751afe744f27a033e8f5": "frozen-protected-control-source-static-bound"}
ESTIMATED = (8, 16, 17, 18, 19, 20, 22, 24, 25, 26, 27, 29, 30)
FAST_KEYS = ("root_id", "transition_id", "source_fingerprint", "actual_cdc_references", "expected_cdc_references", "expected_cdc_sequence_fingerprint", "ordered_closure_digest", "publication_status", "error", "q_current", "edit_reference_count_before", "edit_reference_count_after", "edit_count_classification", "sql_query_calls", "sql_rows_returned", "row_blob_reads", "borrowed_row_blob_reads", "borrowed_row_blob_bytes", "objects_authenticated", "canonical_bytes_authenticated", "leaf_batch_queries", "leaf_batch_references", "leaf_batch_references_max", "source_bytes_read", "source_cdc_bytes_read", "canonical_stage_source_bytes_read", "w_bytes", "d_bytes")
BUFFER_FIELDS = ("q_cdc_old_window_segment_max_bytes", "q_cdc_scan_input_segment_max_bytes", "q_cdc_old_chunk_slots_bytes", "leaf_batch_query_bytes_max", "q_report_output_bytes", "buffer_bytes")
LIMIT = 1_048_576
...
def equal_work(left, right, keys=None):
    fields = list(keys) if keys else list((set(left) & set(right)) - {"status", "status_adapter", "adjacent_estimator_v11"})
    return all(key in left and key in right and left[key] == right[key] for key in fields if not key.endswith("_ns"))
...
def parse_time(path):
    ...
    if not all((timing, rss, voluntary, involuntary)):
        raise ValueError("incomplete time evidence")
    return {
...
def expected_children():
    ...
    for sequence in range(1, 31):
        if sequence in ESTIMATED:
            ...
            quartet = ((control, 1), (candidate, 1), (candidate, 2), (control, 2)) if ESTIMATED.index(sequence) % 2 == 0 else ((candidate, 1), (control, 1), (control, 2), (candidate, 2))
            expected.extend((sequence, role, sample) for role, sample in quartet)
        else:
...
events = [json.loads(line) for line in (results / "CHRONOLOGY-v1.jsonl").read_text().splitlines() if line]
events = [event for event in events if event.get("event") == "measured-child-complete"]
if len(commands) != 76 or len(events) != 76 or [item.get("order") for item in commands] != list(range(1, 77)):
    issues.append("exact-76-child-command-chronology")
elif any(left.get(key) != right.get(key) for left, right in zip(commands, events) for key in ("order", "sequence", "role", "sample", "label", "command", "binary_sha256", "stdout_sha256", "stderr_sha256", "external")):
...
child_payloads, child_external, child_resource_evidence = {}, {}, []
for command in commands:
    order, role = command.get("order"), command.get("role")
    ...
    try:
        ...
        payload = json.loads([line for line in stdout.read_text().splitlines() if line][-1])
        ...
        child_payloads[order], child_external[order] = payload, external
        ...
        if command.get("sequence") not in ESTIMATED:
            ...
            if arm.get("payload") != payload or arm.get("external") != external or arm.get("stdout_sha256") != command.get("stdout_sha256") or arm.get("stderr_sha256") != command.get("stderr_sha256"):
                issues.append(f"child-logical-arm-binding-{order}")
    except (KeyError, OSError, IndexError, ValueError, json.JSONDecodeError):
...
observations, sample_map, estimator_ledger = [], {}, {}
for (sequence, role), arm in sorted(arms.items()):
    ...
    if sequence not in ESTIMATED:
        ...
        continue
    meta = payload.get("adjacent_estimator_v11", {})
    order = "ABBA" if ESTIMATED.index(sequence) % 2 == 0 else "BAAB"
    if (meta.get("replications_per_role"), meta.get("estimator"), meta.get("relative_limit_basis_points"), meta.get("balanced_order")) != (2, "equal-weight-arithmetic-mean", 10_500, order):
    ...
    samples = [child_payloads.get(value) for value in orders]
    ...
    if arm.get("external") != aggregate_external([child_external[value] for value in orders]):
        issues.append(f"estimator-external-aggregate-{sequence}-{role}")
    if len(samples) == 2 and not equal_work(samples[0], samples[1], FAST_KEYS if sequence in {8, 29, 30} else None):
        issues.append(f"estimator-within-role-work-parity-{sequence}-{role}")
    sample_map[(sequence, role)] = samples
...
for sequence in ESTIMATED:
    ...
    expected_roles = [control_role, candidate_role, candidate_role, control_role] if ESTIMATED.index(sequence) % 2 == 0 else [candidate_role, control_role, control_role, candidate_role]
    route_commands = [item for item in commands if item["sequence"] == sequence]
    if [item["role"] for item in route_commands] != expected_roles:
        issues.append(f"balanced-order-{sequence}")
    orders = arms[(sequence, control_role)]["payload"]["adjacent_estimator_v11"]["sample_order"] + arms[(sequence, candidate_role)]["payload"]["adjacent_estimator_v11"]["sample_order"]
    if sorted(orders) != [item["order"] for item in route_commands]:
        issues.append(f"balanced-order-{sequence}")
    for sample in range(2):
        if not equal_work(sample_map[(sequence, control_role)][sample], sample_map[(sequence, candidate_role)][sample], FAST_KEYS if sequence in {8, 29, 30} else None):
            issues.append(f"estimator-cross-role-work-parity-{sequence}-{sample + 1}")

for sequence in range(16, 28):
    if not equal_work(arms[(sequence, "g3-control")]["payload"], arms[(sequence, "s1-candidate")]["payload"]):
        issues.append(f"g3-complete-semantic-work-parity-{sequence}")
for sequence in (8, 28, 29, 30):
    if not equal_work(arms[(sequence, "protected-control")]["payload"], arms[(sequence, "protected-candidate")]["payload"], FAST_KEYS):
        issues.append(f"protected-complete-semantic-work-parity-{sequence}")

relative = {}
routes = [(sequence, "g3-control", "s1-candidate", "g3") for sequence in range(16, 28)] + [(sequence, "protected-control", "protected-candidate", "protected") for sequence in (8, 28, 29, 30)]
for sequence, control_role, candidate_role, prefix in routes:
    ...
    control_values = control.get("adjacent_estimator_v11", {}).get("samples_ns", [elapsed(sequence, control)])
    candidate_values = candidate.get("adjacent_estimator_v11", {}).get("samples_ns", [elapsed(sequence, candidate)])
...
history_path = results / "methodology-v1/PRE-EXEC-HISTORY-v10.json"
history_sha256 = digest(history_path) if history_path.is_file() else None
if results.name != "results-v11" or history_sha256 != HISTORY_SHA256:
    issues.append("v11-pre-execution-lineage")
...
for sequence, role, payload, binary in observations:
    if binary == CANDIDATE:
        ...
        if max([value.get("returned_bytes", 0) for value in payload.get("range_measurements", [])] or [0]) > LIMIT:
    ...
    elif binary in CONTROLS:
        buffer_evidence.append({"sequence": sequence, "role": role, "kind": CONTROLS[binary], "max_single_buffer_bytes": LIMIT})
    else:
...
m0, seed = arms[(14, "m0-candidate")]["payload"], arms[(7, "s1-candidate")]["payload"]
...
if any(m0.get(key) != value for key, value in durability.items()): issues.append("m0-direct-durability-counters")
if seed.get("cache_class") != "same-open-protected-seed-warm-or-unknown": issues.append("seed-cache-class")
...
if cache != {"r0_control": 2000, "r1_attribution": 1500, "r1_candidate": 1500, "r1_fresh": 1500, "m0_control": 2000, "m0_candidate": 1500}: issues.append("g4-read-cache-profile")
...
report.update({"schema": "phase4-g4-independent-recomputation-v11", "status": "PASS" if not issues else "REVISE", "issues": issues, "normalized_ledger": ledger, "normalized_ledger_sha256": digest})
