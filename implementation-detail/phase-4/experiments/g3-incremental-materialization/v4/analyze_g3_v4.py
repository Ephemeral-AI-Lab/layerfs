#!/usr/bin/env python3
"""Primary G3-v4 analysis; no code is shared with independent recomputation."""
...
def source_custody_ok(custody, results=None):
    ...
    base = (results or Path("/__g3_v4_synthetic_results__")).resolve()
    custody_root = (base / "source-custody-v4").resolve()
    structural = (
        custody.get("schema") == "phase4-g3-v4-source-custody-v1"
    ...
    for row in rows:
        ...
        expected = f"source-custody-v4/{row['path']}"
...
def validate(rows, cleanup, custody, results=None, row_cleanups=None):
    ...
    expected_cleanup_order = [(sequence, f"{sequence:02d}-{scenario}") for sequence, scenario, _ in SCHEDULE]
    if [(record.get("sequence"), record.get("label")) for record in row_cleanups] != expected_cleanup_order:
        reject("cleanup", 0, "row-order-count")
    dimensions = ("logical_bytes", "apparent_bytes", "allocated_bytes")
    for expected, record, raw_row in zip(expected_cleanup_order, row_cleanups, rows):
        sequence, label = expected
        paths = record.get("relative_paths", [])
        safe_paths = isinstance(paths, list) and paths == sorted(set(paths)) and all(isinstance(name, str) and name and not PurePosixPath(name).is_absolute() and all(part not in ("", ".", "..") for part in PurePosixPath(name).parts) for name in paths)
        ...
        if record.get("schema") != "phase4-g3-v4-row-cleanup-v1" or record.get("row_root") != f"work-v4/{label}" or not safe_paths or record.get("relative_path_count") != len(paths) or record.get("relative_path_set_sha256") != digest(paths) or record.get("enumeration_followed_symlinks") is not False or record.get("row_root_absent") is not True or record.get("deletion_method") != "enumerated-no-follow-unlink-and-rmdir" or record.get("candidate_exactness") != exactness:
            reject("cleanup", sequence, "row-record")
        pre_row, pre_work = record.get("pre_delete_row", {}), record.get("pre_delete_work", {})
        if any(type(pre_row.get(name)) is not int or pre_row.get(name) < 0 or pre_row.get(name) > 512 * MIB or pre_work.get(name) != pre_row.get(name) for name in dimensions):
            reject("cleanup", sequence, "isolated-usage")
        if any(record.get("post_delete_row", {}).get(name) != 0 for name in (*dimensions, "files", "directories", "symlinks")) or any(record.get("post_delete_work", {}).get(name) != 0 for name in (*dimensions, "files", "symlinks")):
            reject("cleanup", sequence, "post-delete")
        if results is not None and (results / record.get("row_root", "unsafe")).exists():
            reject("cleanup", sequence, "root-still-exists")
    peaks = {name: max((record.get("pre_delete_row", {}).get(name, 513 * MIB) for record in row_cleanups), default=513 * MIB) for name in dimensions}
    cumulative = {name: sum(record.get("pre_delete_row", {}).get(name, 0) for record in row_cleanups) for name in dimensions}
    if not (cleanup.get("status") == "PASS" and cleanup.get("declared_root") == "work-v4" and cleanup.get("work_root_absent") is True and cleanup.get("all_row_roots_absent") is True and cleanup.get("broad_deletion") is False and cleanup.get("row_cleanup_records") == 9 and cleanup.get("row_cleanup_labels") == [label for _, label in expected_cleanup_order] and len(cleanup.get("row_cleanup_sha256", "")) == 64 and cleanup.get("peak_equation") == "max_individual_pre_delete_row_not_cumulative_sum" and all(cleanup.get(f"peak_{name}") == peaks[name] and cleanup.get(f"cumulative_{name}") == cumulative[name] for name in dimensions) and max(peaks.values()) <= 512 * MIB):
        reject("cleanup", 0, "exact-root")
    if results is not None and (cleanup.get("row_cleanup_sha256") != file_digest(results / "ROW-CLEANUP-v4.jsonl") or (results / "work-v4").exists()):
        reject("cleanup", 0, "artifact-hash-or-work-root")
    failures = sorted(set(failures))
    ledger = {
        "schema": "phase4-g3-v4-normalized-ledger-v1", "failures": failures,
...
def fixtures():
    source_rows = [{"path": path, "sha256": f"{index + 1:x}" * 64, "size_bytes": index + 1, "source_mode": "0644", "copy_path": f"source-custody-v4/{path}", "copy_sha256": f"{index + 1:x}" * 64, "copy_size_bytes": index + 1, "copy_mode": "0400"} for index, path in enumerate(SOURCE_PATHS)]
    source_set = digest([{key: row[key] for key in ("path", "sha256", "size_bytes", "source_mode")} for row in source_rows])
    custody = {"schema": "phase4-g3-v4-source-custody-v1", "status": "PASS", "source_set_sha256": source_set, "sources": source_rows}
    rows = []
    ...
    for sequence, scenario, _ in SCHEDULE:
        label = f"{sequence:02d}-{scenario}"; paths = ["artifact"]
        usage = {"logical_bytes": sequence, "apparent_bytes": sequence + 10, "allocated_bytes": sequence + 20, "files": 1, "directories": 1, "symlinks": 0}
        row_cleanups.append({"schema": "phase4-g3-v4-row-cleanup-v1", "sequence": sequence, "label": label, "row_root": f"work-v4/{label}", "relative_paths": paths, "relative_path_count": 1, "relative_path_set_sha256": digest(paths), "enumeration_followed_symlinks": False, "pre_delete_row": usage, "pre_delete_work": dict(usage), "post_delete_row": {name: 0 for name in usage}, "post_delete_work": {**{name: 0 for name in usage}, "directories": 1}, "row_root_absent": True, "deletion_method": "enumerated-no-follow-unlink-and-rmdir", "candidate_exactness": {name: rows[sequence - 1][name] for name in ("byte_exact", "mode_exact", "temp_residue_count", "seed_residue_count", "old_or_new", "output_digest", "expected_output_digest")}})
    dimensions = ("logical_bytes", "apparent_bytes", "allocated_bytes")
    peaks = {name: max(record["pre_delete_row"][name] for record in row_cleanups) for name in dimensions}; sums = {name: sum(record["pre_delete_row"][name] for record in row_cleanups) for name in dimensions}
    cleanup = {"status": "PASS", "declared_root": "work-v4", "work_root_absent": True, "all_row_roots_absent": True, "broad_deletion": False, "row_cleanup_records": 9, "row_cleanup_labels": [row["label"] for row in row_cleanups], "row_cleanup_sha256": "d" * 64, "peak_equation": "max_individual_pre_delete_row_not_cumulative_sum", **{f"peak_{name}": peaks[name] for name in dimensions}, **{f"cumulative_{name}": sums[name] for name in dimensions}}
...
def self_check():
    ...
    prior = copy.deepcopy(rows); prior[8].update({"reconciliation_outcome": "prior", "old_or_new": "old"}); prior_cleanup = copy.deepcopy(row_cleanups); prior_cleanup[8]["candidate_exactness"].update({"old_or_new": "old"}); assert validate(prior, cleanup, custody, row_cleanups=prior_cleanup)["status"] == "PASS"
    mutations = []
    ...
    escaped = copy.deepcopy(custody); escaped["sources"][0]["copy_path"] = "source-custody-v4/../../outside"; mutations.append((rows, cleanup, escaped))
    ...
    wrong_hash = copy.deepcopy(row_cleanups); wrong_hash[0]["relative_path_set_sha256"] = "0" * 64; cleanup_mutations.append(wrong_hash)
    ...
    print(json.dumps({"status": "PASS", "mutations_rejected": len(mutations) + 5}, sort_keys=True))


def main():
    if sys.argv[1:] == ["--self-check"]: self_check(); return
    if len(sys.argv) != 2: raise SystemExit("usage: analyze_g3_v4.py RESULTS | --self-check")
    root = Path(sys.argv[1])
    rows = [json.loads(line) for line in (root / "rows-v4/G3-V4-RAW.jsonl").read_text().splitlines() if line]
    row_cleanups = [json.loads(line) for line in (root / "ROW-CLEANUP-v4.jsonl").read_text().splitlines() if line]
    report = {"schema": "phase4-g3-v4-primary-analysis-v1", **validate(rows, json.loads((root / "CLEANUP-v4.json").read_text()), json.loads((root / "SOURCE-CUSTODY-v4.json").read_text()), root, row_cleanups)}
