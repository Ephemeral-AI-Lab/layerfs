#!/usr/bin/env python3
"""Independent G3-v4 recomputation using a predicate table and fresh parser."""
...
def custody_valid(custody, results=None):
    ...
    if not all(predicates):
        return False
    result_root = (results or Path("/__g3_v4_independent_synthetic__")).resolve()
    copy_root = (result_root / "source-custody-v4").resolve()
    for source in sources:
        relative = source.get("copy_path")
        parts = PurePosixPath(relative) if isinstance(relative, str) else None
        if parts is None or parts.is_absolute() or any(part in ("", ".", "..") for part in parts.parts) or relative != f"source-custody-v4/{source['path']}":
            return False
...
def audit(rows, cleanup, custody, results=None, cleanup_rows=None):
    ...
    check("cleanup", 0, "row-order-count", [(record.get("sequence"), record.get("label")) for record in cleanup_rows] == order)
    dimensions = ("logical_bytes", "apparent_bytes", "allocated_bytes")
    for expected, record, raw in zip(order, cleanup_rows, rows):
        sequence, label = expected; names = record.get("relative_paths", [])
        paths_valid = isinstance(names, list) and names == sorted(set(names)) and all(isinstance(name, str) and name and not PurePosixPath(name).is_absolute() and all(part not in ("", ".", "..") for part in PurePosixPath(name).parts) for name in names)
        ...
        check("cleanup", sequence, "row-record", record.get("schema") == "phase4-g3-v4-row-cleanup-v1" and record.get("row_root") == f"work-v4/{label}" and paths_valid and record.get("relative_path_count") == len(names) and record.get("relative_path_set_sha256") == canonical_hash(names) and record.get("enumeration_followed_symlinks") is False and record.get("row_root_absent") is True and record.get("deletion_method") == "enumerated-no-follow-unlink-and-rmdir" and record.get("candidate_exactness") == exactness)
        before, work_before = record.get("pre_delete_row", {}), record.get("pre_delete_work", {})
        check("cleanup", sequence, "isolated-usage", all(type(before.get(key)) is int and 0 <= before[key] <= 512 * MB and work_before.get(key) == before[key] for key in dimensions))
        check("cleanup", sequence, "post-delete", all(record.get("post_delete_row", {}).get(key) == 0 for key in (*dimensions, "files", "directories", "symlinks")) and all(record.get("post_delete_work", {}).get(key) == 0 for key in (*dimensions, "files", "symlinks")))
        if results is not None: check("cleanup", sequence, "root-absent", not (results / record.get("row_root", "unsafe")).exists())
    peak = {key: max((record.get("pre_delete_row", {}).get(key, 513 * MB) for record in cleanup_rows), default=513 * MB) for key in dimensions}
    cumulative = {key: sum(record.get("pre_delete_row", {}).get(key, 0) for record in cleanup_rows) for key in dimensions}
    check("cleanup", 0, "exact-root", cleanup.get("status") == "PASS" and cleanup.get("declared_root") == "work-v4" and cleanup.get("work_root_absent") is True and cleanup.get("all_row_roots_absent") is True and cleanup.get("broad_deletion") is False and cleanup.get("row_cleanup_records") == 9 and cleanup.get("row_cleanup_labels") == [label for _, label in order] and len(cleanup.get("row_cleanup_sha256", "")) == 64 and cleanup.get("peak_equation") == "max_individual_pre_delete_row_not_cumulative_sum" and all(cleanup.get(f"peak_{key}") == peak[key] and cleanup.get(f"cumulative_{key}") == cumulative[key] for key in dimensions) and max(peak.values()) <= 512 * MB)
    if results is not None: check("cleanup", 0, "artifact-hash-work-root", cleanup.get("row_cleanup_sha256") == hash_file(results / "ROW-CLEANUP-v4.jsonl") and not (results / "work-v4").exists())
    ordered_failures = sorted(failures)
    ledger = {"schema": "phase4-g3-v4-normalized-ledger-v1", "failures": ordered_failures, "gates": {gate: not any(item.startswith(gate + ":") for item in ordered_failures) for gate in GATE_ORDER}, "source_set_sha256": source_set, "operation_total_ns": sum(row.get("operation_total_ns", 0) for row in rows), "storage": {"peak": peak, "cumulative": cumulative, "row_cleanup_records": len(cleanup_rows)}, "rows": [{key: row.get(key) for key in VIEW} for row in rows]}
...
def sample_data():
    sources = [{"path": path, "sha256": str(index + 1) * 64, "size_bytes": index + 1, "source_mode": "0644", "copy_path": f"source-custody-v4/{path}", "copy_sha256": str(index + 1) * 64, "copy_size_bytes": index + 1, "copy_mode": "0400"} for index, path in enumerate(FILES)]
    source_set = canonical_hash([{key: row[key] for key in ("path", "sha256", "size_bytes", "source_mode")} for row in sources])
    custody = {"schema": "phase4-g3-v4-source-custody-v1", "status": "PASS", "source_set_sha256": source_set, "sources": sources}
    rows = []
    ...
    for sequence, scenario, _ in PLAN:
        label = f"{sequence:02d}-{scenario}"; names = ["artifact"]
        usage = {"logical_bytes": sequence, "apparent_bytes": sequence + 10, "allocated_bytes": sequence + 20, "files": 1, "directories": 1, "symlinks": 0}
        cleanup_rows.append({"schema": "phase4-g3-v4-row-cleanup-v1", "sequence": sequence, "label": label, "row_root": f"work-v4/{label}", "relative_paths": names, "relative_path_count": 1, "relative_path_set_sha256": canonical_hash(names), "enumeration_followed_symlinks": False, "pre_delete_row": usage, "pre_delete_work": dict(usage), "post_delete_row": {key: 0 for key in usage}, "post_delete_work": {**{key: 0 for key in usage}, "directories": 1}, "row_root_absent": True, "deletion_method": "enumerated-no-follow-unlink-and-rmdir", "candidate_exactness": {key: rows[sequence - 1][key] for key in ("byte_exact", "mode_exact", "temp_residue_count", "seed_residue_count", "old_or_new", "output_digest", "expected_output_digest")}})
    dimensions = ("logical_bytes", "apparent_bytes", "allocated_bytes"); peak = {key: max(row["pre_delete_row"][key] for row in cleanup_rows) for key in dimensions}; sums = {key: sum(row["pre_delete_row"][key] for row in cleanup_rows) for key in dimensions}
    cleanup = {"status": "PASS", "declared_root": "work-v4", "work_root_absent": True, "all_row_roots_absent": True, "broad_deletion": False, "row_cleanup_records": 9, "row_cleanup_labels": [row["label"] for row in cleanup_rows], "row_cleanup_sha256": "d" * 64, "peak_equation": "max_individual_pre_delete_row_not_cumulative_sum", **{f"peak_{key}": peak[key] for key in dimensions}, **{f"cumulative_{key}": sums[key] for key in dimensions}}
...
def self_test():
    ...
    prior = copy.deepcopy(rows); prior[8].update({"reconciliation_outcome": "prior", "old_or_new": "old"}); prior_cleanup = copy.deepcopy(cleanup_rows); prior_cleanup[8]["candidate_exactness"].update({"old_or_new": "old"}); assert audit(prior, cleanup, custody, cleanup_rows=prior_cleanup)["status"] == "PASS"
    cases = []
    ...
    wrong_hash = copy.deepcopy(cleanup_rows); wrong_hash[0]["relative_path_set_sha256"] = "0" * 64; assert audit(rows, cleanup, custody, cleanup_rows=wrong_hash)["status"] == "REVISE"
    ...
    print(json.dumps({"status": "PASS", "mutations_rejected": len(cases) + 5}, sort_keys=True))


def main():
    if sys.argv[1:] == ["--self-check"]: self_test(); return
    if len(sys.argv) != 2: raise SystemExit("usage: recompute_g3_v4.py RESULTS | --self-check")
    root = Path(sys.argv[1])
    rows = [json.loads(line) for line in (root / "rows-v4/G3-V4-RAW.jsonl").read_text().splitlines() if line]
    cleanup_rows = [json.loads(line) for line in (root / "ROW-CLEANUP-v4.jsonl").read_text().splitlines() if line]
    result = {"schema": "phase4-g3-v4-independent-recomputation-v1", **audit(rows, json.loads((root / "CLEANUP-v4.json").read_text()), json.loads((root / "SOURCE-CUSTODY-v4.json").read_text()), root, cleanup_rows)}
