#!/usr/bin/env python3
"""Independent G3-v13 recomputation using a predicate table and fresh parser."""
...
REPO = HERE.parents[4]
TARGET_NAME = "phase4-g3-incremental-materialization-20260822-v13"
SOURCE_METHOD_PATH = Path("implementation-detail/phase-4/experiments/g3-incremental-materialization/v13")
MB = 1048576
RECONCILIATION_COMPARISON_BYTES = 32 * 1024
PLAN = [
...
BUILD_ARGV = ["cargo", "build", "--release", "-p", "layerfs-engine", "--bin", "phase4_create_edit_benchmark", "--offline"]
CHILD_ENV_KEYS = [
...
def frozen_plan(result_root, operand, environment):
    executable = result_root / operand["copy_path"]
    method_root = result_root / "methodology-v13"
    work_root = result_root / "work-v13"
    interpreter = environment["python_executable"]
    plan = [{"kind": "build", "label": "release-build", "timeout_seconds": 30, "command": BUILD_ARGV}]
    plan.extend((
        {"kind": "analyzer-self-check", "label": "primary-self-check", "timeout_seconds": 5, "command": [interpreter, str(method_root / "analyze_g3_v13.py"), "--self-check"]},
        {"kind": "analyzer-self-check", "label": "independent-self-check", "timeout_seconds": 5, "command": [interpreter, str(method_root / "recompute_g3_v13.py"), "--self-check"]},
    ))
    for sequence, scenario, size in PLAN:
        label = f"{sequence:02d}-{scenario}"
        plan.append({"kind": "measured-row", "label": label, "timeout_seconds": 15 if scenario == "qualified-one-byte" else 5, "command": ["/usr/bin/time", "-l", str(executable), "--g3-row", str(work_root / label), str(size), scenario]})
    plan.extend((
        {"kind": "analyzer", "label": "primary-analysis", "timeout_seconds": 5, "command": [interpreter, str(method_root / "analyze_g3_v13.py"), str(result_root)]},
        {"kind": "analyzer", "label": "independent-recomputation", "timeout_seconds": 5, "command": [interpreter, str(method_root / "recompute_g3_v13.py"), str(result_root)]},
    ))
    return plan


def evidence_authority(result_root, source_set):
    operand = json.loads((result_root / "OPERAND-CUSTODY-v13.json").read_text())
    methods = json.loads((result_root / "METHODOLOGY-CUSTODY-v13.json").read_text())
    environment_path = result_root / "ENVIRONMENT-v13.json"
    environment = json.loads(environment_path.read_text())
    commands = json.loads((result_root / "COMMANDS-v13.json").read_text())
    method_rows = methods.get("methods", [])
    executable = result_root / operand.get("copy_path", "")
    method_root = result_root / "methodology-v13"
    freeze = result_root / methods.get("premeasurement_freeze_path", "")
    child_sets = environment.get("child_environments", {})
    build = child_sets.get("build", {})
    runtime = child_sets.get("runtime", {})
    build_values = build.get("selected_environment", {})
    runtime_values = runtime.get("selected_environment", {})
    role_map = {"build": "build", "analyzer-self-check": "runtime", "measured-row": "runtime", "analyzer": "runtime"}
    common = {"LANG": "C", "LC_ALL": "C", "TZ": "UTC", "RUST_BACKTRACE": "0", "G3_SOURCE_SET_SHA256": source_set, "G3_METHODOLOGY_SET_SHA256": methods.get("methodology_set_sha256")}
    predicates = [
        operand.get("schema") == "phase4-g3-v13-operand-custody-v1",
        operand.get("status") == "PASS",
        operand.get("source_set_sha256") == source_set,
        operand.get("copy_path") == "operands-v13/phase4_create_edit_benchmark",
        operand.get("copy_mode") == "0500",
        executable.is_file() and not executable.is_symlink() and permissions(executable) == "0500" and hash_file(executable) == operand.get("sha256"),
        methods.get("schema") == "phase4-g3-v13-method-custody-v1",
        methods.get("status") == "PASS",
        [row.get("path") for row in method_rows] == METHOD_FILES,
        methods.get("methodology_set_sha256") == canonical_hash([{key: row[key] for key in ("path", "sha256", "size_bytes")} for row in method_rows]),
        all(hash_file(method_root / row["path"]) == row["sha256"] for row in method_rows),
        hash_file(method_root / "DRY-RUN-v13.json") == methods.get("dry_run_sha256"),
        methods.get("premeasurement_freeze_path") == "PREMEASUREMENT-FREEZE-v13.json",
        methods.get("premeasurement_freeze_copy_mode") == "0400",
        freeze.is_file() and not freeze.is_symlink() and permissions(freeze) == "0400" and hash_file(freeze) == methods.get("premeasurement_freeze_sha256"),
        environment.get("schema") == "phase4-g3-v13-environment-v1",
        environment.get("status") == "PASS",
        environment.get("source_set_sha256") == source_set,
        environment.get("methodology_set_sha256") == methods.get("methodology_set_sha256"),
        environment.get("executable_sha256") == operand.get("sha256"),
        environment.get("child_environment_roles") == role_map,
        set(build_values) == set(runtime_values) == set(CHILD_ENV_KEYS),
        all(build_values.get(key) == value and runtime_values.get(key) == value for key, value in common.items()),
        build_values.get("G3_EXECUTABLE_SHA256") is None,
        runtime_values.get("G3_EXECUTABLE_SHA256") == operand.get("sha256"),
        build.get("selected_environment_sha256") == canonical_hash(build_values),
        runtime.get("selected_environment_sha256") == canonical_hash(runtime_values),
        build.get("executable_sha256_applicable") is False,
        runtime.get("executable_sha256_applicable") is True,
        isinstance(environment.get("python_executable"), str) and bool(environment.get("python_executable")),
    ]
    expected = frozen_plan(result_root, operand, environment) if all(predicates) else []
    valid = all(predicates) and commands == expected
    return {"valid": valid, "executable": operand.get("sha256"), "methods": methods.get("methodology_set_sha256"), "environment": hash_file(environment_path), "row_argv": [record["command"] for record in expected[3:12]] if valid else []}


def repo_from_results(results):
    candidate = results.resolve()
    if (candidate.name, candidate.parent.name, candidate.parent.parent.name) != ("results-v13", TARGET_NAME, "target"):
        raise RuntimeError("invalid independent results namespace")
    repository = candidate.parents[2]
    if candidate != repository / "target" / TARGET_NAME / "results-v13" or not (repository / "Cargo.lock").is_file() or not (repository / "crates/layerfs-engine/Cargo.toml").is_file() or not (repository / ".git").exists():
        raise RuntimeError("independent results-to-repository binding failed")
    return repository


def self_check_context(here=HERE):
    """Identify only an exact source checkout or exact campaign method copy."""
    location = here.resolve()
    if location.name == "methodology-v13":
        results = location.parent
        repository = repo_from_results(results)
        required = repository / "target" / TARGET_NAME / "results-v13" / "methodology-v13"
        if location != required:
            raise RuntimeError("invalid independent analyzer copy location")
        return "copied", repository, results
    if location.name == "v13":
        try:
            repository = location.parents[4]
        except IndexError as error:
            raise RuntimeError("invalid independent analyzer source location") from error
        required = repository / SOURCE_METHOD_PATH
        markers = [repository / ".git", repository / "Cargo.lock", repository / "crates/layerfs-engine/Cargo.toml"]
        if location != required or any(not marker.exists() for marker in markers):
            raise RuntimeError("invalid independent analyzer source location")
        results = repository / "target" / TARGET_NAME / "results-v13"
        return "source", repository, results
    raise RuntimeError("unknown independent analyzer location")


def custody_valid(custody, results=None, repository=None):
    ...
    predicates = [
        custody.get("schema") == "phase4-g3-v13-source-custody-v1",
    ...
    if not all(predicates):
        return False
    result_root = (results or Path("/__g3_v13_independent_synthetic__")).resolve()
    repository = repository or REPO
    copy_root = (result_root / "source-custody-v13").resolve()
    for source in sources:
        relative = source.get("copy_path")
        parts = PurePosixPath(relative) if isinstance(relative, str) else None
        if parts is None or parts.is_absolute() or any(part in ("", ".", "..") for part in parts.parts) or relative != f"source-custody-v13/{source['path']}":
            return False
...
def audit(rows, cleanup, custody, results=None, cleanup_rows=None, repository=None, authority=None):
    failures = set()
    ...
    check("source_custody", 0, "source-set", custody_valid(custody, results, repository))
    source_set = custody.get("source_set_sha256")
    if authority is None:
        authority = evidence_authority(results, source_set) if results is not None else {"valid": True, "executable": "a" * 64, "methods": "b" * 64, "environment": "c" * 64, "row_argv": [["test"] for _ in PLAN]}
    check("custody", 0, "authoritative-artifacts", authority.get("valid") is True)
    ...
    for (sequence, scenario, size), row in zip(PLAN, rows):
        ...
        expected_argv = authority.get("row_argv", [])
        check("custody", sequence, "exact-operand-method-environment-command", sequence <= len(expected_argv) and row["executable_sha256"] == authority.get("executable") and row["methodology_set_sha256"] == authority.get("methods") and row["environment_sha256"] == authority.get("environment") and row["command"] == expected_argv[sequence - 1])
        ...
        if scenario == "lost-ack":
            ...
            check("publication", sequence, "reconciliation-work", all(row[key] > 0 for key in reconciliation_fields) and compared and row["reconciliation_canonical_bytes_authenticated"] >= row["output_length"] and row["reconciliation_q_high_water"] >= RECONCILIATION_COMPARISON_BYTES and row["q_high_water"] >= row["reconciliation_q_high_water"])
        else:
    ...
    check("cleanup", 0, "exact-root", cleanup.get("status") == "PASS" and cleanup.get("declared_root") == "work-v13" and cleanup.get("work_root_absent") is True and cleanup.get("all_row_roots_absent") is True and cleanup.get("broad_deletion") is False and cleanup.get("deletion_method") == method and cleanup.get("prepare_records") == cleanup.get("complete_records") == 9 and cleanup.get("row_cleanup_records") == 18 and cleanup.get("row_cleanup_labels") == labels and cleanup.get("durable_prepare_complete") is True and len(cleanup.get("row_cleanup_sha256", "")) == 64 and cleanup.get("peak_equation") == "max_individual_PREPARE_pre_delete_row_not_cumulative_sum" and all(cleanup.get(f"peak_{key}") == peak[key] and cleanup.get(f"cumulative_{key}") == cumulative[key] for key in dimensions) and max(peak.values()) <= 512 * MB)
    if results is not None: check("cleanup", 0, "artifact-hash-work-root", cleanup.get("row_cleanup_sha256") == hash_file(results / "ROW-CLEANUP-v13.jsonl") and not (results / "work-v13").exists())
    ordered_failures = sorted(failures)
    ledger = {"schema": "phase4-g3-v13-normalized-ledger-v1", "failures": ordered_failures, "gates": {gate: not any(item.startswith(gate + ":") for item in ordered_failures) for gate in GATE_ORDER}, "source_set_sha256": source_set, "operation_total_ns": sum(row.get("operation_total_ns", 0) for row in rows), "storage": {"peak": peak, "cumulative": cumulative, "row_cleanup_records": len(cleanup_rows)}, "rows": [{key: row.get(key) for key in VIEW} for row in rows]}
...
def sample_data():
    sources = [{"path": path, "sha256": str(index + 1) * 64, "size_bytes": index + 1, "source_mode": "0644", "copy_path": f"source-custody-v13/{path}", "copy_sha256": str(index + 1) * 64, "copy_size_bytes": index + 1, "copy_mode": "0400"} for index, path in enumerate(FILES)]
    source_set = canonical_hash([{key: row[key] for key in ("path", "sha256", "size_bytes", "source_mode")} for row in sources])
    custody = {"schema": "phase4-g3-v13-source-custody-v1", "status": "PASS", "source_set_sha256": source_set, "sources": sources}
    rows = []
    for seq, scenario, size in PLAN:
        ...
        if scenario == "lost-ack": row.update({"reconciliation_calls": 1, "reconciliation_sql_queries": 1, "reconciliation_sql_rows": 1, "reconciliation_blob_reads": 1, "reconciliation_canonical_bytes_authenticated": size, "reconciliation_source_bytes_compared": size, "reconciliation_q_high_water": RECONCILIATION_COMPARISON_BYTES, "q_high_water": RECONCILIATION_COMPARISON_BYTES, "destination_bytes_read": size})
        rows.append(row)
    cleanup_rows = []
    ...
    cleanup = {"status": "PASS", "declared_root": "work-v13", "work_root_absent": True, "all_row_roots_absent": True, "broad_deletion": False, "deletion_method": method, "prepare_records": 9, "complete_records": 9, "row_cleanup_records": 18, "row_cleanup_labels": [row["label"] for row in prepares], "row_cleanup_sha256": "d" * 64, "durable_prepare_complete": True, "peak_equation": "max_individual_PREPARE_pre_delete_row_not_cumulative_sum", **{f"peak_{key}": peak[key] for key in dimensions}, **{f"cumulative_{key}": sums[key] for key in dimensions}}
    return rows, cleanup, custody, cleanup_rows


def self_test():
    rows, cleanup, custody, cleanup_rows = sample_data()
    _, repository, expected_results = self_check_context()
    assert expected_results == repository / "target" / TARGET_NAME / "results-v13"
    with tempfile.TemporaryDirectory(prefix="g3-v13-independent-context-") as temporary:
        synthetic_repo = (Path(temporary) / "repo").resolve()
        (synthetic_repo / ".git").mkdir(parents=True)
        (synthetic_repo / "Cargo.lock").touch()
        (synthetic_repo / "crates/layerfs-engine").mkdir(parents=True)
        (synthetic_repo / "crates/layerfs-engine/Cargo.toml").touch()
        copied = synthetic_repo / "target" / TARGET_NAME / "results-v13" / "methodology-v13"
        copied.mkdir(parents=True)
        assert self_check_context(copied) == ("copied", synthetic_repo, copied.parent)
        rejected = [
            synthetic_repo / "target" / TARGET_NAME / "wrong-results" / "methodology-v13",
            synthetic_repo / "bad-layout/one/two/three/v13",
            synthetic_repo.parent / "target" / TARGET_NAME / "results-v13" / "methodology-v13",
        ]
        for candidate in rejected:
            try: self_check_context(candidate)
            except RuntimeError: continue
            raise AssertionError(f"invalid independent analyzer location accepted: {candidate}")
    ...
    cases = []
    for index, key, value in ((0, "source_set_sha256", "f" * 64), (0, "executable_sha256", "f" * 64), (0, "methodology_set_sha256", "f" * 64), (0, "environment_sha256", "f" * 64), (0, "command", ["wrong"]), (8, "reconciliation_outcome", "prior"), (8, "reconciliation_blob_reads", 0), (8, "reconciliation_q_high_water", RECONCILIATION_COMPARISON_BYTES - 1), (5, "route", "complete-fallback"), (1, "changed_bytes", 2), (3, "permit_consumptions", 1), (0, "operation_total_ns", 50), (0, "q_terminal", 1), (0, "mode_exact", False)):
        changed = copy.deepcopy(rows); changed[index][key] = value; cases.append((changed, cleanup, custody))
    bad_proof_claim = copy.deepcopy(rows); bad_proof_claim[1]["authority_bindings_checked"] = BOUND[:-1]; cases.append((bad_proof_claim, cleanup, custody))
...
def main():
    ...
    if len(sys.argv) != 2: raise SystemExit("usage: recompute_g3_v13.py RESULTS | --self-check")
    root = Path(sys.argv[1])
    rows = [json.loads(line) for line in (root / "rows-v13/G3-V13-RAW.jsonl").read_text().splitlines() if line]
    cleanup_rows = [json.loads(line) for line in (root / "ROW-CLEANUP-v13.jsonl").read_text().splitlines() if line]
    repository = repo_from_results(root)
    result = {"schema": "phase4-g3-v13-independent-recomputation-v1", **audit(rows, json.loads((root / "CLEANUP-v13.json").read_text()), json.loads((root / "SOURCE-CUSTODY-v13.json").read_text()), root, cleanup_rows, repository)}
