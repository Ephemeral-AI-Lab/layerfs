#!/usr/bin/env python3
"""Primary G3-v13 analysis; no code is shared with independent recomputation."""
...
def expected_commands(results, operand, environment):
    binary = results / operand["copy_path"]
    methods = results / "methodology-v13"
    work = results / "work-v13"
    python = environment["python_executable"]
    commands = [{"kind": "build", "label": "release-build", "timeout_seconds": 30, "command": BUILD_COMMAND}]
    commands += [
    ...
    for sequence, scenario, size in SCHEDULE:
        ...
        commands.append({"kind": "measured-row", "label": label, "timeout_seconds": 15 if scenario == "qualified-one-byte" else 5, "command": ["/usr/bin/time", "-l", str(binary), "--g3-row", str(work / label), str(size), scenario]})
    commands += [
...
def authoritative_custody(results, source_set):
    operand = json.loads((results / "OPERAND-CUSTODY-v13.json").read_text())
    methods = json.loads((results / "METHODOLOGY-CUSTODY-v13.json").read_text())
    ...
    freeze = results / methods.get("premeasurement_freeze_path", "")
    method_root = results / "methodology-v13"
    ...
    roles = {"build": "build", "analyzer-self-check": "runtime", "measured-row": "runtime", "analyzer": "runtime"}
    fixed = {"LANG": "C", "LC_ALL": "C", "TZ": "UTC", "RUST_BACKTRACE": "0", "G3_SOURCE_SET_SHA256": source_set, "G3_METHODOLOGY_SET_SHA256": methods.get("methodology_set_sha256")}
    valid = (
        operand.get("schema") == "phase4-g3-v13-operand-custody-v1"
        and operand.get("status") == "PASS"
        and operand.get("source_set_sha256") == source_set
        and operand.get("copy_path") == "operands-v13/phase4_create_edit_benchmark"
        and operand.get("copy_mode") == "0500"
        and binary.is_file() and not binary.is_symlink() and file_mode(binary) == "0500" and file_digest(binary) == operand.get("sha256")
        and methods.get("schema") == "phase4-g3-v13-method-custody-v1"
        and methods.get("status") == "PASS"
        and [row.get("path") for row in method_rows] == METHOD_NAMES
        and methods.get("methodology_set_sha256") == digest([{key: row[key] for key in ("path", "sha256", "size_bytes")} for row in method_rows])
        and all(file_digest(method_root / row["path"]) == row["sha256"] for row in method_rows)
        and file_digest(method_root / "DRY-RUN-v13.json") == methods.get("dry_run_sha256")
        and methods.get("premeasurement_freeze_path") == "PREMEASUREMENT-FREEZE-v13.json"
        and methods.get("premeasurement_freeze_copy_mode") == "0400"
        and freeze.is_file() and not freeze.is_symlink() and file_mode(freeze) == "0400" and file_digest(freeze) == methods.get("premeasurement_freeze_sha256")
        and environment.get("schema") == "phase4-g3-v13-environment-v1" and environment.get("status") == "PASS"
        and environment.get("source_set_sha256") == source_set
        and environment.get("methodology_set_sha256") == methods.get("methodology_set_sha256")
        and environment.get("executable_sha256") == operand.get("sha256")
        and environment.get("child_environment_roles") == roles
        and set(build_selected) == set(runtime_selected) == set(CONTROLLED_ENVIRONMENT)
        and all(build_selected.get(name) == value and runtime_selected.get(name) == value for name, value in fixed.items())
        and build_selected.get("G3_EXECUTABLE_SHA256") is None
        and runtime_selected.get("G3_EXECUTABLE_SHA256") == operand.get("sha256")
        and build.get("selected_environment_sha256") == digest(build_selected)
        and runtime.get("selected_environment_sha256") == digest(runtime_selected)
    ...
    expected = expected_commands(results, operand, environment) if valid else []
    valid = valid and commands == expected
    return {"valid": valid, "executable_sha256": operand.get("sha256"), "methodology_set_sha256": methods.get("methodology_set_sha256"), "environment_sha256": file_digest(environment_path), "measured_commands": [row["command"] for row in expected[3:12]] if valid else []}


def repo_from_results(results):
    resolved = results.resolve()
    if resolved.name != "results-v13" or resolved.parent.name != TARGET_NAME or resolved.parent.parent.name != "target":
        raise RuntimeError("unexpected G3-v13 results namespace")
    repo = resolved.parents[2]
    expected = repo / "target" / TARGET_NAME / "results-v13"
    markers = (repo / "Cargo.lock", repo / "crates/layerfs-engine/Cargo.toml", repo / ".git")
    if resolved != expected or not all(path.exists() for path in markers):
        raise RuntimeError("results root does not identify the expected repository")
    return repo


def self_check_context(here=HERE):
    """Resolve only the exact source or frozen campaign-copy location."""
    location = here.resolve()
    if location.name == "methodology-v13":
        results = location.parent
        repository = repo_from_results(results)
        expected = repository / "target" / TARGET_NAME / "results-v13" / "methodology-v13"
        if location != expected:
            raise RuntimeError("malformed copied analyzer location")
        return {"mode": "copied", "repository": repository, "results": results}
    if location.name == "v13":
        try:
            repository = location.parents[4]
        except IndexError as error:
            raise RuntimeError("malformed source analyzer location") from error
        markers = (repository / "Cargo.lock", repository / "crates/layerfs-engine/Cargo.toml", repository / ".git")
        if location != repository / SOURCE_METHOD_PATH or not all(path.exists() for path in markers):
            raise RuntimeError("malformed source analyzer location")
        results = repository / "target" / TARGET_NAME / "results-v13"
        return {"mode": "source", "repository": repository, "results": results}
    raise RuntimeError("unrecognized analyzer execution location")
...
def source_custody_ok(custody, results=None, repository=None):
    rows = custody.get("sources", [])
    base = (results or Path("/__g3_v13_synthetic_results__")).resolve()
    repository = repository or REPO
    custody_root = (base / "source-custody-v13").resolve()
    structural = (
    ...
    for row in rows:
        ...
        expected = f"source-custody-v13/{row['path']}"
        ...
        if custody_root not in copy.resolve().parents or copy.resolve() == (repository / row["path"]).resolve():
            return False
...
def validate(rows, cleanup, custody, results=None, row_cleanups=None, repository=None, authoritative=None):
    failures = []
    ...
    if not source_custody_ok(custody, results, repository):
        reject("source_custody", 0, "source-set")
    source_set = custody.get("source_set_sha256")
    if authoritative is None:
        authoritative = authoritative_custody(results, source_set) if results is not None else {"valid": True, "executable_sha256": "a" * 64, "methodology_set_sha256": "b" * 64, "environment_sha256": "c" * 64, "measured_commands": [["test"] for _ in SCHEDULE]}
    if not authoritative.get("valid"):
        reject("custody", 0, "authoritative-artifacts")
    ...
    for (sequence, scenario, size), row in zip(SCHEDULE, rows):
        ...
        if scenario == "lost-ack":
            ...
            if any(row[name] <= 0 for name in reconciliation_fields) or not comparison_exact or row["reconciliation_canonical_bytes_authenticated"] < row["output_length"] or row["reconciliation_q_high_water"] < RECONCILIATION_COMPARISON_BYTES or row["q_high_water"] < row["reconciliation_q_high_water"]:
        ...
        expected_commands = authoritative.get("measured_commands", [])
        if sequence > len(expected_commands) or row["executable_sha256"] != authoritative.get("executable_sha256") or row["methodology_set_sha256"] != authoritative.get("methodology_set_sha256") or row["environment_sha256"] != authoritative.get("environment_sha256") or row["command"] != expected_commands[sequence - 1]:
            reject("custody", sequence, "exact-operand-method-environment-command")
    ...
    if not (cleanup.get("status") == "PASS" and cleanup.get("declared_root") == "work-v13" and cleanup.get("work_root_absent") is True and cleanup.get("all_row_roots_absent") is True and cleanup.get("broad_deletion") is False and cleanup.get("deletion_method") == method and cleanup.get("prepare_records") == 9 and cleanup.get("complete_records") == 9 and cleanup.get("row_cleanup_records") == 18 and cleanup.get("row_cleanup_labels") == labels and cleanup.get("durable_prepare_complete") is True and len(cleanup.get("row_cleanup_sha256", "")) == 64 and cleanup.get("peak_equation") == "max_individual_PREPARE_pre_delete_row_not_cumulative_sum" and all(cleanup.get(f"peak_{name}") == peaks[name] and cleanup.get(f"cumulative_{name}") == cumulative[name] for name in dimensions) and max(peaks.values()) <= 512 * MIB):
        reject("cleanup", 0, "exact-root")
    if results is not None and (cleanup.get("row_cleanup_sha256") != file_digest(results / "ROW-CLEANUP-v13.jsonl") or (results / "work-v13").exists()):
        reject("cleanup", 0, "artifact-hash-or-work-root")
    failures = sorted(set(failures))
    ledger = {
        "schema": "phase4-g3-v13-normalized-ledger-v1", "failures": failures,
...
def fixtures():
    source_rows = [{"path": path, "sha256": f"{index + 1:x}" * 64, "size_bytes": index + 1, "source_mode": "0644", "copy_path": f"source-custody-v13/{path}", "copy_sha256": f"{index + 1:x}" * 64, "copy_size_bytes": index + 1, "copy_mode": "0400"} for index, path in enumerate(SOURCE_PATHS)]
    source_set = digest([{key: row[key] for key in ("path", "sha256", "size_bytes", "source_mode")} for row in source_rows])
    custody = {"schema": "phase4-g3-v13-source-custody-v1", "status": "PASS", "source_set_sha256": source_set, "sources": source_rows}
    rows = []
    for sequence, scenario, size in SCHEDULE:
...
def self_check():
    ...
    assert context["results"] == context["repository"] / "target" / TARGET_NAME / "results-v13"
    with tempfile.TemporaryDirectory(prefix="g3-v13-primary-context-") as temporary:
        ...
        simulated = self_check_context(copied)
        assert simulated == {"mode": "copied", "repository": synthetic_repo, "results": copied.parent}
        malformed = (
            synthetic_repo / "target" / "wrong-target" / "results-v13" / "methodology-v13",
            synthetic_repo / "bad-layout/one/two/three/v13",
            synthetic_repo.parent / "target" / TARGET_NAME / "results-v13" / "methodology-v13",
        )
        for bad_location in malformed:
            try: self_check_context(bad_location)
            except RuntimeError: pass
            else: raise AssertionError(f"malformed analyzer location accepted: {bad_location}")
    ...
    mutations = []
    for index, key, value in ((0, "source_set_sha256", "0" * 64), (0, "executable_sha256", "0" * 64), (0, "methodology_set_sha256", "0" * 64), (0, "environment_sha256", "0" * 64), (0, "command", ["wrong"]), (8, "old_or_new", "old"), (8, "reconciliation_source_bytes_compared", 0), (8, "reconciliation_q_high_water", RECONCILIATION_COMPARISON_BYTES - 1), (5, "qualification_reason", "count-change"), (1, "patch_bytes", 2), (3, "fallback_calls", 0), (0, "operation_total_ns", 99), (0, "q_terminal", 1), (0, "byte_exact", False)):
        changed = copy.deepcopy(rows); changed[index][key] = value; mutations.append((changed, cleanup, custody))
    bad_proof_claim = copy.deepcopy(rows); bad_proof_claim[1]["authority_bindings_checked"] = BINDINGS[:-1]; mutations.append((bad_proof_claim, cleanup, custody))
    ...
    escaped = copy.deepcopy(custody); escaped["sources"][0]["copy_path"] = "source-custody-v13/../../outside"; mutations.append((rows, cleanup, escaped))
...
def main():
    ...
    if len(sys.argv) != 2: raise SystemExit("usage: analyze_g3_v13.py RESULTS | --self-check")
    root = Path(sys.argv[1])
    rows = [json.loads(line) for line in (root / "rows-v13/G3-V13-RAW.jsonl").read_text().splitlines() if line]
    row_cleanups = [json.loads(line) for line in (root / "ROW-CLEANUP-v13.jsonl").read_text().splitlines() if line]
    repository = repo_from_results(root)
    report = {"schema": "phase4-g3-v13-primary-analysis-v1", **validate(rows, json.loads((root / "CLEANUP-v13.json").read_text()), json.loads((root / "SOURCE-CUSTODY-v13.json").read_text()), root, row_cleanups, repository)}
