# Prospective G3-v8 incremental materialization contract

Status: **frozen before build or measurement**

v8 is the methodology-consistency repair after zero-row v7's audit. It freezes the
...
parent-synced. v8 therefore claims crash durability for these evidence writes,
...
`CLEANUP-v8.json` requires 18 alternating events—nine exact PREPARE/COMPLETE
...
The residual stat/unlink micro-race is an ordinary POSIX limitation. v8 claims
...
`source-custody-v8/<repository-relative-source-path>`. Its resolved path must
remain inside `results-v8/source-custody-v8`, be a regular non-symlink with a
...
`target/phase4-g3-incremental-materialization-20260822-v8/results-v8`, guarded
...
- Lost acknowledgement accepts exactly either (`reconciliation_outcome` =
  ...
  value passes. The deterministic v8 injection may yield `target`/`new`.
...
- Clone failure is a complete fallback reason and consumes zero permits. v8 has
...
No measured campaign is authorized by the dry run. A fresh v9 is required for
any pre-execution defect discovered after v8 is frozen.
