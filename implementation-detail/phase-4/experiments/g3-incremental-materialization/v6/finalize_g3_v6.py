#!/usr/bin/env python3
"""Finalize and seal a completed G3-v6 campaign after parent static closure."""
...
MANIFEST = RESULTS / "PAYLOAD-MANIFEST-v6.tsv"
TERMINAL = RESULTS / "TERMINAL-v6.json"
VERIFICATION = RESULTS / "TERMINAL-VERIFICATION-v6.txt"
STATIC = RESULTS / "STATIC-CLOSURE-v6.json"
...
METHOD_NAMES = [
    ...
    "COUNTER-DICTIONARY-v6.md", "run_g3_v6.py", "analyze_g3_v6.py",
    "recompute_g3_v6.py", "finalize_g3_v6.py",
]
G2_ROOT = REPO / "target/phase4-g2-materialization-decomposition-20260822-v6/results-v6"
G2 = {
    "payload_manifest_sha256": (G2_ROOT / "PAYLOAD-MANIFEST-v6.tsv", "12f74b88188c1a22babe129c4b1d5d0e1889ba55d2cf0046ae55af6803709399"),
    "terminal_sha256": (G2_ROOT / "TERMINAL-v6.json", "09a5948a2c6a31c55811d50459c24cf72c4d2e3ff61ea5773754bf5c6c1a60a2"),
    "terminal_verification_sha256": (G2_ROOT / "TERMINAL-VERIFICATION-v6.txt", "41447453a34b1933850e6e090a2bc59628d58f7d585e7c394e937cfe03250af0"),
    "raw_sha256": (G2_ROOT / "rows-v6/G2-V6-RAW.jsonl", "c64a4f7b4d1a831fd7406251f0de2ab44cfbf390d07188d55298fdbbfefb0eeb"),
}
G2_LEDGER_SHA256 = "5de0586cdcb80932b503458c0b74e1983b3b2b5179adc6ba5ed4480aa7af33b9"
EXCLUDED = {"PAYLOAD-MANIFEST-v6.tsv", "TERMINAL-v6.json", "TERMINAL-VERIFICATION-v6.txt"}
...
def exact_source_copy(row, result_root=RESULTS):
    ...
    expected = f"source-custody-v6/{row.get('path')}"
...
def verify_g2():
    ...
    for name, (path, expected) in G2.items():
        if not path.is_file() or sha256(path) != expected:
            raise RuntimeError(f"G2-v6 custody mismatch: {name}")
        observed[name] = expected
    if read_json(G2["terminal_sha256"][0]).get("status") != "PASS":
        raise RuntimeError("G2-v6 terminal is not PASS")
    primary = read_json(G2_ROOT / "G2-V6-ANALYSIS.json")
    independent = read_json(G2_ROOT / "G2-V6-INDEPENDENT-RECOMPUTATION.json")
    if canonical_hash(primary.get("normalized_ledger")) != G2_LEDGER_SHA256 or canonical_hash(independent.get("normalized_ledger")) != G2_LEDGER_SHA256 or primary.get("normalized_ledger") != independent.get("normalized_ledger"):
        raise RuntimeError("G2-v6 normalized ledger custody mismatch")
...
def verify_static(source_set):
    ...
    if closure.get("schema") != "phase4-g3-v6-static-closure-v1" or closure.get("status") != "PASS" or closure.get("source_set_sha256") != source_set or closure.get("candidate_retained") is not True or [row.get("label") for row in commands] != REQUIRED_STATIC_LABELS:
...
def verify_row_cleanup(cleanup, records, raw_rows, results=None):
    ...
    expected_cleanup = cleanup.get("status") == "PASS" and cleanup.get("declared_root") == "work-v6" and cleanup.get("deletion_method") == method and cleanup.get("prepare_records") == cleanup.get("complete_records") == 9 and cleanup.get("row_cleanup_records") == 18 and cleanup.get("row_cleanup_labels") == ROW_LABELS and cleanup.get("all_row_roots_absent") is True and cleanup.get("work_root_absent") is True and cleanup.get("broad_deletion") is False and cleanup.get("durable_prepare_complete") is True and cleanup.get("peak_equation") == "max_individual_PREPARE_pre_delete_row_not_cumulative_sum" and all(cleanup.get(f"peak_{key}") == peaks[key] and cleanup.get(f"cumulative_{key}") == cumulative[key] for key in dimensions) and max(peaks.values()) <= 512 * 1024 * 1024
    if not expected_cleanup or (results is not None and (cleanup.get("row_cleanup_sha256") != sha256(results / "ROW-CLEANUP-v6.jsonl") or (results / "work-v6").exists())):
...
def verify_campaign_inputs():
    ...
    if any(path.exists() for path in (MANIFEST, TERMINAL, VERIFICATION)):
        raise RuntimeError("v6 final artifacts already exist")
    if (RESULTS / "FAILURE-v6.json").exists():
        raise RuntimeError("failed/revised campaign cannot be finalized")
    campaign = read_json(RESULTS / "CAMPAIGN-v6.json")
    primary = read_json(RESULTS / "G3-PRIMARY-ANALYSIS-v6.json")
    independent = read_json(RESULTS / "G3-INDEPENDENT-RECOMPUTATION-v6.json")
    source = read_json(RESULTS / "SOURCE-CUSTODY-v6.json")
    operand = read_json(RESULTS / "OPERAND-CUSTODY-v6.json")
    methods = read_json(RESULTS / "METHODOLOGY-CUSTODY-v6.json")
    cleanup = read_json(RESULTS / "CLEANUP-v6.json")
    raw = RESULTS / "rows-v6/G3-V6-RAW.jsonl"
    raw_rows = [json.loads(line) for line in raw.read_text().splitlines() if line]
    row_cleanup_path = RESULTS / "ROW-CLEANUP-v6.jsonl"
    row_cleanups = [json.loads(line) for line in row_cleanup_path.read_text().splitlines() if line]
    commands = json.loads((RESULTS / "COMMANDS-v6.json").read_text())
    chronology = [json.loads(line) for line in (RESULTS / "CHRONOLOGY-v6.jsonl").read_text().splitlines() if line]
    ...
    for row in method_rows:
        if sha256(HERE / row["path"]) != row["sha256"] or sha256(RESULTS / "methodology-v6" / row["path"]) != row["sha256"]:
            raise RuntimeError(f"method changed: {row['path']}")
    if sha256(DRY_RUN) != methods.get("dry_run_sha256") or sha256(RESULTS / "methodology-v6/DRY-RUN-v6.json") != methods.get("dry_run_sha256"):
        raise RuntimeError("dry-run custody mismatch")
    if cleanup.get("status") != "PASS" or cleanup.get("declared_root") != "work-v6" or cleanup.get("work_root_absent") is not True or cleanup.get("broad_deletion") is not False or (RESULTS / "work-v6").exists():
        raise RuntimeError("cleanup mismatch")
    hashes = {"raw_sha256": raw, "row_cleanup_sha256": row_cleanup_path, "primary_sha256": RESULTS / "G3-PRIMARY-ANALYSIS-v6.json", "independent_sha256": RESULTS / "G3-INDEPENDENT-RECOMPUTATION-v6.json", "cleanup_sha256": RESULTS / "CLEANUP-v6.json", "source_custody_sha256": RESULTS / "SOURCE-CUSTODY-v6.json", "binary_custody_sha256": RESULTS / "OPERAND-CUSTODY-v6.json"}
...
def finalize():
    ...
    terminal = {
        "schema": "phase4-g3-v6-terminal-v1", "status": "PASS",
        ...
        "campaign_sha256": sha256(RESULTS / "CAMPAIGN-v6.json"),
        "cleanup_sha256": sha256(RESULTS / "CLEANUP-v6.json"),
        ...
        "methodology_custody_sha256": sha256(RESULTS / "METHODOLOGY-CUSTODY-v6.json"),
    ...
    seal_existing()
    snapshot = {"schema": "phase4-g3-v6-terminal-verification-v1", "status": "PASS", "disposition": terminal["disposition"], **final_verification(len(manifest_rows), g2, terminal_hash, manifest_hash, source["source_set_sha256"], False)}
...
def self_check():
    assert HERE.parents[4] == REPO and EXCLUDED == {"PAYLOAD-MANIFEST-v6.tsv", "TERMINAL-v6.json", "TERMINAL-VERIFICATION-v6.txt"}
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        root = Path(directory) / "results-v6"; copied = root / "source-custody-v6/Cargo.lock"; copied.parent.mkdir(parents=True); copied.write_bytes((REPO / "Cargo.lock").read_bytes()); copied.chmod(0o400)
        source_hash = sha256(REPO / "Cargo.lock")
        record = {"path": "Cargo.lock", "sha256": source_hash, "size_bytes": (REPO / "Cargo.lock").stat().st_size, "source_mode": "0644", "copy_path": "source-custody-v6/Cargo.lock", "copy_sha256": source_hash, "copy_size_bytes": copied.stat().st_size, "copy_mode": "0400"}
        exact_source_copy(record, root)
        mutations = []
        missing = dict(record); missing.pop("copy_path"); mutations.append(missing)
        escaped = dict(record, copy_path="source-custody-v6/../../outside"); mutations.append(escaped)
    ...
    print(json.dumps({"status": "PASS", "static_commands": 6, "source_copy_mutations_rejected": 3, "row_cleanup_mutations_rejected": 7, "campaign_children_invoked": 0, "finalized": False}, sort_keys=True))
...
def main():
    ...
    else: raise SystemExit("refusing: use --self-check or --execute after PASS STATIC-CLOSURE-v6.json")
