#!/usr/bin/env python3
"""Fresh one-shot G3-v6 runner with complete source/build custody."""
...
REPO = HERE.parents[4]
TARGET = REPO / "target/phase4-g3-incremental-materialization-20260822-v6"
RESULTS = TARGET / "results-v6"
LOCK = REPO / "target/phase4-g3-incremental-materialization-20260822-v6.lock"
WORK = RESULTS / "work-v6"
...
METHOD_NAMES = (
    "PROSPECTIVE-G3-INCREMENTAL-MATERIALIZATION-v6.md",
    "COUNTER-DICTIONARY-v6.md",
    "run_g3_v6.py",
    "analyze_g3_v6.py",
    "recompute_g3_v6.py",
    "finalize_g3_v6.py",
...
def preflight():
    if Path.cwd().resolve() != REPO or HERE.parents[4] != REPO:
        raise RuntimeError("run from repository root; invalid v6 REPO derivation")
...
def fresh():
    if TARGET.exists() or LOCK.exists():
        raise RuntimeError("G3-v6 result root or lock already exists")
...
def chronology(event, started, **fields):
    ...
    durable_append_text(RESULTS / "CHRONOLOGY-v6.jsonl", json.dumps(record, sort_keys=True, separators=(",", ":")) + "\n", label="chronology")


def remaining(started, requested):
    seconds = (GLOBAL_NS - (time.monotonic_ns() - started)) / 1_000_000_000
    if seconds <= 0:
        raise TimeoutError("G3-v6 global ceiling exhausted")
...
def freeze_sources():
    destination = RESULTS / "source-custody-v6"
    destination.mkdir()
    records = []
    ...
    custody = {"schema": "phase4-g3-v6-source-custody-v1", "status": "PASS", "source_set_sha256": canonical_hash(identity), "sources": records}
    (RESULTS / "SOURCE-CUSTODY-v6.json").write_text(json.dumps(custody, indent=2, sort_keys=True) + "\n")
...
def verify_sources(custody, result_root=RESULTS):
    ...
    custody_root = (result_root / "source-custody-v6").resolve()
    for record in custody["sources"]:
        expected_copy = f"source-custody-v6/{record['path']}"
...
def freeze_methods():
    destination = RESULTS / "methodology-v6"
    destination.mkdir()
    records = []
    ...
    custody = {"schema": "phase4-g3-v6-method-custody-v1", "status": "PASS", "methodology_set_sha256": canonical_hash(records), "methods": records, "dry_run_sha256": sha256(DRY_RUN)}
    (RESULTS / "METHODOLOGY-CUSTODY-v6.json").write_text(json.dumps(custody, indent=2, sort_keys=True) + "\n")
    return custody, destination


def verify_methods(custody):
    for record in custody["methods"]:
        if sha256(HERE / record["path"]) != record["sha256"] or sha256(RESULTS / "methodology-v6" / record["path"]) != record["sha256"]:
...
def plan(binary, methods):
    ...
    result += [
        {"kind": "analyzer-self-check", "label": "primary-self-check", "timeout_seconds": 5, "command": [sys.executable, str(methods / "analyze_g3_v6.py"), "--self-check"]},
        {"kind": "analyzer-self-check", "label": "independent-self-check", "timeout_seconds": 5, "command": [sys.executable, str(methods / "recompute_g3_v6.py"), "--self-check"]},
    ]
    for row in rows():
    ...
    result += [
        {"kind": "analyzer", "label": "primary-analysis", "timeout_seconds": 5, "command": [sys.executable, str(methods / "analyze_g3_v6.py"), str(RESULTS)]},
        {"kind": "analyzer", "label": "independent-recomputation", "timeout_seconds": 5, "command": [sys.executable, str(methods / "recompute_g3_v6.py"), str(RESULTS)]},
    ]
    return result
...
def retire_row(specification, row, storage, work_fd):
    ...
    try:
        ...
        durable_append(RESULTS / "ROW-CLEANUP-v6.jsonl", complete)
...
def finalize_cleanup(storage, results_fd, work_fd):
    records = [json.loads(line) for line in (RESULTS / "ROW-CLEANUP-v6.jsonl").read_text().splitlines() if line]
    ...
    os.rmdir("work-v6", dir_fd=results_fd)
...
def campaign():
    ...
    try:
        ...
        planned_binary = RESULTS / "operands-v6" / BINARY.name
        commands = plan(planned_binary, methods)
        (RESULTS / "COMMANDS-v6.json").write_text(json.dumps(commands, indent=2, sort_keys=True) + "\n")
        _, _, build_stdout, build_stderr = child(commands[0]["command"], commands[0]["label"], commands[0]["kind"], 30, RESULTS / "build-v6", env, started)
        ...
        for command in commands[1:3]:
            stdout, _, _, _ = child(command["command"], command["label"], command["kind"], command["timeout_seconds"], RESULTS / "self-check-v6", env, started)
        ...
        results_fd = os.open(RESULTS, DIR_FLAGS); os.mkdir("work-v6", dir_fd=results_fd); work_fd = os.open("work-v6", DIR_FLAGS, dir_fd=results_fd)
        row_dir = RESULTS / "rows-v6"; row_dir.mkdir(); fsync_directory(RESULTS); raw = row_dir / "G3-V6-RAW.jsonl"; durable_write_new(raw, "", label="raw-create"); durable_write_new(RESULTS / "ROW-CLEANUP-v6.jsonl", "", label="cleanup-log-create")
        storage = {"schema": "phase4-g3-v6-runner-storage-v1", "ceiling_bytes": STORAGE, "samples": []}
        operation_sum = 0
        ...
        (RESULTS / "STORAGE-v6.json").write_text(json.dumps(storage, indent=2, sort_keys=True) + "\n")
        ...
        reports = []
        for command, filename in zip(commands[12:], ("G3-PRIMARY-ANALYSIS-v6.json", "G3-INDEPENDENT-RECOMPUTATION-v6.json")):
            stdout, _, _, _ = child(command["command"], command["label"], command["kind"], command["timeout_seconds"], RESULTS / "analysis-children-v6", env, started)
        ...
        record = {"schema": "phase4-g3-v6-campaign-v1", "status": "PASS", "disposition": "G3_V6_CAMPAIGN_PASS_STATIC_CLOSURE_REQUIRED", "rows": 9, "rows_rerun": 0, "build_command": BUILD_COMMAND, "build_invocations": 1, "build_ceiling_seconds": 30, "operation_total_ns": operation_sum, "operation_sum_ceiling_ns": SUM_NS, "per_operation_ceiling_ns": OP_NS, "global_elapsed_ns": elapsed, "global_ceiling_ns": GLOBAL_NS, "source_set_sha256": source_custody["source_set_sha256"], "executable_sha256": binary_custody["sha256"], "methodology_set_sha256": method_custody["methodology_set_sha256"], "raw_sha256": sha256(raw), "row_cleanup_sha256": sha256(RESULTS / "ROW-CLEANUP-v6.jsonl"), "primary_sha256": sha256(RESULTS / "G3-PRIMARY-ANALYSIS-v6.json"), "independent_sha256": sha256(RESULTS / "G3-INDEPENDENT-RECOMPUTATION-v6.json"), "normalized_ledger_sha256": reports[0]["normalized_ledger_sha256"], "cleanup_sha256": sha256(RESULTS / "CLEANUP-v6.json"), "source_custody_sha256": sha256(RESULTS / "SOURCE-CUSTODY-v6.json"), "binary_custody_sha256": sha256(RESULTS / "OPERAND-CUSTODY-v6.json"), "static_closure_required": True, "finalizer_required": True, "lock_removed_after_campaign": True}
        (RESULTS / "CAMPAIGN-v6.json").write_text(json.dumps(record, indent=2, sort_keys=True) + "\n")
        chronology("campaign-complete", started, status="PASS", rows=9)
        print(json.dumps(record, sort_keys=True, separators=(",", ":")))
    except Exception as error:
        if RESULTS.is_dir():
            failure = {"schema": "phase4-g3-v6-failure-v1", "status": "REVISE", "reason": f"{type(error).__name__}: {error}", "source_set_sha256": source_custody.get("source_set_sha256") if source_custody else None, "global_elapsed_ns": time.monotonic_ns() - started, "result_root_preserved": True}
            durable_write_new(RESULTS / "FAILURE-v6.json", json.dumps(failure, indent=2, sort_keys=True) + "\n", label="failure")
        raise
    finally:
...
def self_check():
    ...
    assert "finalize_g3_v6.py" in METHOD_NAMES
    with tempfile.TemporaryDirectory() as directory:
        result_root = Path(directory) / "results-v6"; records = []
        for relative in SOURCE_PATHS:
            source, copy = REPO / relative, result_root / "source-custody-v6" / relative
            copy.parent.mkdir(parents=True, exist_ok=True); shutil.copyfile(source, copy); copy.chmod(0o400)
            records.append({"path": relative, "sha256": sha256(source), "size_bytes": source.stat().st_size, "source_mode": mode(source), "copy_path": f"source-custody-v6/{relative}", "copy_sha256": sha256(copy), "copy_size_bytes": copy.stat().st_size, "copy_mode": mode(copy)})
        custody = {"sources": records}; verify_sources(custody, result_root)
        mutations = []
        for field, value in (("copy_path", None), ("copy_path", "source-custody-v6/../../outside"), ("copy_size_bytes", records[0]["copy_size_bytes"] + 1)):
