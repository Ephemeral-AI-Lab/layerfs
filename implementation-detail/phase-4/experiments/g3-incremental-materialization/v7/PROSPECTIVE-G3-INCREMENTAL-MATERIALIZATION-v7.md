# Prospective G3-v7 incremental materialization contract

Status: **frozen before build or measurement**

v7 is the finalizer-anchor-only repair after zero-row v6's audit. It freezes the
...
parent-synced. v7 therefore claims crash durability for these evidence writes,
...
`CLEANUP-v7.json` requires 18 alternating events—nine exact PREPARE/COMPLETE
...
The residual stat/unlink micro-race is an ordinary POSIX limitation. v7 claims
...
`source-custody-v7/<repository-relative-source-path>`. Its resolved path must
remain inside `results-v7/source-custody-v7`, be a regular non-symlink with a
...
`target/phase4-g3-incremental-materialization-20260822-v7/results-v7`, guarded
...
- Lost acknowledgement accepts exactly either (`reconciliation_outcome` =
  ...
  value passes. The deterministic v7 injection may yield `target`/`new`.
...
- Clone failure is a complete fallback reason and consumes zero permits. v7 has
...
No measured campaign is authorized by the dry run. A fresh v8 is required for
any pre-execution defect discovered after v7 is frozen.
