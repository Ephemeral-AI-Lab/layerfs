#!/usr/bin/env python3
"""Primary G3-v7 analysis; no code is shared with independent recomputation."""
...
def source_custody_ok(custody, results=None):
    ...
    base = (results or Path("/__g3_v7_synthetic_results__")).resolve()
    custody_root = (base / "source-custody-v7").resolve()
    structural = (
    ...
    for row in rows:
        ...
        expected = f"source-custody-v7/{row['path']}"
...
def validate(rows, cleanup, custody, results=None, row_cleanups=None):
    ...
    if not (cleanup.get("status") == "PASS" and cleanup.get("declared_root") == "work-v7" and cleanup.get("work_root_absent") is True and cleanup.get("all_row_roots_absent") is True and cleanup.get("broad_deletion") is False and cleanup.get("deletion_method") == method and cleanup.get("prepare_records") == 9 and cleanup.get("complete_records") == 9 and cleanup.get("row_cleanup_records") == 18 and cleanup.get("row_cleanup_labels") == labels and cleanup.get("durable_prepare_complete") is True and len(cleanup.get("row_cleanup_sha256", "")) == 64 and cleanup.get("peak_equation") == "max_individual_PREPARE_pre_delete_row_not_cumulative_sum" and all(cleanup.get(f"peak_{name}") == peaks[name] and cleanup.get(f"cumulative_{name}") == cumulative[name] for name in dimensions) and max(peaks.values()) <= 512 * MIB):
        reject("cleanup", 0, "exact-root")
    if results is not None and (cleanup.get("row_cleanup_sha256") != file_digest(results / "ROW-CLEANUP-v7.jsonl") or (results / "work-v7").exists()):
        reject("cleanup", 0, "artifact-hash-or-work-root")
    failures = sorted(set(failures))
    ledger = {
        "schema": "phase4-g3-v7-normalized-ledger-v1", "failures": failures,
...
def fixtures():
    source_rows = [{"path": path, "sha256": f"{index + 1:x}" * 64, "size_bytes": index + 1, "source_mode": "0644", "copy_path": f"source-custody-v7/{path}", "copy_sha256": f"{index + 1:x}" * 64, "copy_size_bytes": index + 1, "copy_mode": "0400"} for index, path in enumerate(SOURCE_PATHS)]
    source_set = digest([{key: row[key] for key in ("path", "sha256", "size_bytes", "source_mode")} for row in source_rows])
    custody = {"schema": "phase4-g3-v7-source-custody-v1", "status": "PASS", "source_set_sha256": source_set, "sources": source_rows}
    rows = []
...
def self_check():
    ...
    escaped = copy.deepcopy(custody); escaped["sources"][0]["copy_path"] = "source-custody-v7/../../outside"; mutations.append((rows, cleanup, escaped))
...
def main():
    ...
    if len(sys.argv) != 2: raise SystemExit("usage: analyze_g3_v7.py RESULTS | --self-check")
    root = Path(sys.argv[1])
    rows = [json.loads(line) for line in (root / "rows-v7/G3-V7-RAW.jsonl").read_text().splitlines() if line]
    row_cleanups = [json.loads(line) for line in (root / "ROW-CLEANUP-v7.jsonl").read_text().splitlines() if line]
    report = {"schema": "phase4-g3-v7-primary-analysis-v1", **validate(rows, json.loads((root / "CLEANUP-v7.json").read_text()), json.loads((root / "SOURCE-CUSTODY-v7.json").read_text()), root, row_cleanups)}
