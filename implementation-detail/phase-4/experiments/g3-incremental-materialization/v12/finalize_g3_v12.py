#!/usr/bin/env python3
"""Finalize and seal a completed G3-v12 campaign after parent static closure."""
...
MANIFEST = RESULTS / "PAYLOAD-MANIFEST-v12.tsv"
TERMINAL = RESULTS / "TERMINAL-v12.json"
VERIFICATION = RESULTS / "TERMINAL-VERIFICATION-v12.txt"
STATIC = RESULTS / "STATIC-CLOSURE-v12.json"
...
METHOD_NAMES = [
    ...
    "COUNTER-DICTIONARY-v12.md", "run_g3_v12.py", "analyze_g3_v12.py",
    "recompute_g3_v12.py", "finalize_g3_v12.py",
]
...
EXCLUDED = {"PAYLOAD-MANIFEST-v12.tsv", "TERMINAL-v12.json", "TERMINAL-VERIFICATION-v12.txt"}
...
def exact_source_copy(row, result_root=RESULTS):
    ...
    expected = f"source-custody-v12/{row.get('path')}"
...
def verify_static(source_set):
    ...
    if closure.get("schema") != "phase4-g3-v12-static-closure-v1" or closure.get("status") != "PASS" or closure.get("source_set_sha256") != source_set or closure.get("candidate_retained") is not True or [row.get("label") for row in commands] != REQUIRED_STATIC_LABELS:
    ...
    if FOCUSED_G3_TEST_RESULT not in focused_stdout.read_bytes():
        raise RuntimeError("focused G3 static closure did not prove exactly 15 passed and 0 failed")
    return closure
...
def verify_row_cleanup(cleanup, records, raw_rows, results=None):
    ...
    expected_cleanup = cleanup.get("status") == "PASS" and cleanup.get("declared_root") == "work-v12" and cleanup.get("deletion_method") == method and cleanup.get("prepare_records") == cleanup.get("complete_records") == 9 and cleanup.get("row_cleanup_records") == 18 and cleanup.get("row_cleanup_labels") == ROW_LABELS and cleanup.get("all_row_roots_absent") is True and cleanup.get("work_root_absent") is True and cleanup.get("broad_deletion") is False and cleanup.get("durable_prepare_complete") is True and cleanup.get("peak_equation") == "max_individual_PREPARE_pre_delete_row_not_cumulative_sum" and all(cleanup.get(f"peak_{key}") == peaks[key] and cleanup.get(f"cumulative_{key}") == cumulative[key] for key in dimensions) and max(peaks.values()) <= 512 * 1024 * 1024
    if not expected_cleanup or (results is not None and (cleanup.get("row_cleanup_sha256") != sha256(results / "ROW-CLEANUP-v12.jsonl") or (results / "work-v12").exists())):
...
def verify_campaign_inputs():
    ...
    if any(path.exists() for path in (MANIFEST, TERMINAL, VERIFICATION)):
        raise RuntimeError("v12 final artifacts already exist")
    if (RESULTS / "FAILURE-v12.json").exists():
        raise RuntimeError("failed/revised campaign cannot be finalized")
    campaign = read_json(RESULTS / "CAMPAIGN-v12.json")
    primary = read_json(RESULTS / "G3-PRIMARY-ANALYSIS-v12.json")
    independent = read_json(RESULTS / "G3-INDEPENDENT-RECOMPUTATION-v12.json")
    source = read_json(RESULTS / "SOURCE-CUSTODY-v12.json")
    operand = read_json(RESULTS / "OPERAND-CUSTODY-v12.json")
    methods = read_json(RESULTS / "METHODOLOGY-CUSTODY-v12.json")
    cleanup = read_json(RESULTS / "CLEANUP-v12.json")
    raw = RESULTS / "rows-v12/G3-V12-RAW.jsonl"
    raw_rows = [json.loads(line) for line in raw.read_text().splitlines() if line]
    row_cleanup_path = RESULTS / "ROW-CLEANUP-v12.jsonl"
    row_cleanups = [json.loads(line) for line in row_cleanup_path.read_text().splitlines() if line]
    commands = json.loads((RESULTS / "COMMANDS-v12.json").read_text())
    chronology = [json.loads(line) for line in (RESULTS / "CHRONOLOGY-v12.jsonl").read_text().splitlines() if line]
    ...
    for row in method_rows:
        if sha256(HERE / row["path"]) != row["sha256"] or sha256(RESULTS / "methodology-v12" / row["path"]) != row["sha256"]:
            raise RuntimeError(f"method changed: {row['path']}")
    if sha256(DRY_RUN) != methods.get("dry_run_sha256") or sha256(RESULTS / "methodology-v12/DRY-RUN-v12.json") != methods.get("dry_run_sha256"):
        raise RuntimeError("dry-run custody mismatch")
    if cleanup.get("status") != "PASS" or cleanup.get("declared_root") != "work-v12" or cleanup.get("work_root_absent") is not True or cleanup.get("broad_deletion") is not False or (RESULTS / "work-v12").exists():
        raise RuntimeError("cleanup mismatch")
    hashes = {"raw_sha256": raw, "row_cleanup_sha256": row_cleanup_path, "primary_sha256": RESULTS / "G3-PRIMARY-ANALYSIS-v12.json", "independent_sha256": RESULTS / "G3-INDEPENDENT-RECOMPUTATION-v12.json", "cleanup_sha256": RESULTS / "CLEANUP-v12.json", "source_custody_sha256": RESULTS / "SOURCE-CUSTODY-v12.json", "binary_custody_sha256": RESULTS / "OPERAND-CUSTODY-v12.json"}
...
def finalize():
    ...
    terminal = {
        "schema": "phase4-g3-v12-terminal-v1", "status": "PASS",
        ...
        "campaign_sha256": sha256(RESULTS / "CAMPAIGN-v12.json"),
        "cleanup_sha256": sha256(RESULTS / "CLEANUP-v12.json"),
        "static_closure_sha256": sha256(STATIC),
        "source_custody_sha256": sha256(RESULTS / "SOURCE-CUSTODY-v12.json"),
        "binary_custody_sha256": sha256(RESULTS / "OPERAND-CUSTODY-v12.json"),
        "methodology_custody_sha256": sha256(RESULTS / "METHODOLOGY-CUSTODY-v12.json"),
    ...
    seal_existing()
    snapshot = {"schema": "phase4-g3-v12-terminal-verification-v1", "status": "PASS", "disposition": terminal["disposition"], **final_verification(len(manifest_rows), g2, terminal_hash, manifest_hash, source["source_set_sha256"], False)}
...
def self_check():
    assert HERE.parents[4] == REPO and EXCLUDED == {"PAYLOAD-MANIFEST-v12.tsv", "TERMINAL-v12.json", "TERMINAL-VERIFICATION-v12.txt"}
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        root = Path(directory) / "results-v12"; copied = root / "source-custody-v12/Cargo.lock"; copied.parent.mkdir(parents=True); copied.write_bytes((REPO / "Cargo.lock").read_bytes()); copied.chmod(0o400)
        source_hash = sha256(REPO / "Cargo.lock")
        record = {"path": "Cargo.lock", "sha256": source_hash, "size_bytes": (REPO / "Cargo.lock").stat().st_size, "source_mode": "0644", "copy_path": "source-custody-v12/Cargo.lock", "copy_sha256": source_hash, "copy_size_bytes": copied.stat().st_size, "copy_mode": "0400"}
        exact_source_copy(record, root)
        mutations = []
        missing = dict(record); missing.pop("copy_path"); mutations.append(missing)
        escaped = dict(record, copy_path="source-custody-v12/../../outside"); mutations.append(escaped)
...
def main():
    ...
    else: raise SystemExit("refusing: use --self-check or --execute after PASS STATIC-CLOSURE-v12.json")
