#!/usr/bin/env python3
"""Primary G3-v3 analysis; no code is shared with independent recomputation."""
...
def source_custody_ok(custody, results=None):
    ...
    base = (results or Path("/__g3_v3_synthetic_results__")).resolve()
    custody_root = (base / "source-custody-v3").resolve()
    structural = (
    ...
    for row in rows:
        ...
        expected = f"source-custody-v3/{row['path']}"
...
def validate(rows, cleanup, custody, results=None):
    failures = []
    ...
    if sum(row.get("operation_total_ns", 5_000_000_000) for row in rows) >= 20_000_000_000:
        reject("timers", 0, "sum")
    if not (cleanup.get("status") == "PASS" and cleanup.get("declared_root") == "work-v3" and cleanup.get("work_root_absent") is True and cleanup.get("broad_deletion") is False and max(cleanup.get("peak_logical_bytes", 513 * MIB), cleanup.get("peak_apparent_bytes", 513 * MIB), cleanup.get("peak_allocated_bytes", 513 * MIB)) <= 512 * MIB):
        reject("cleanup", 0, "exact-root")
    failures = sorted(set(failures))
    ledger = {
        "schema": "phase4-g3-v3-normalized-ledger-v1", "failures": failures,
...
def fixtures():
    source_rows = [{"path": path, "sha256": f"{index + 1:x}" * 64, "size_bytes": index + 1, "source_mode": "0644", "copy_path": f"source-custody-v3/{path}", "copy_sha256": f"{index + 1:x}" * 64, "copy_size_bytes": index + 1, "copy_mode": "0400"} for index, path in enumerate(SOURCE_PATHS)]
    source_set = digest([{key: row[key] for key in ("path", "sha256", "size_bytes", "source_mode")} for row in source_rows])
    custody = {"schema": "phase4-g3-v3-source-custody-v1", "status": "PASS", "source_set_sha256": source_set, "sources": source_rows}
    rows = []
...
def self_check():
    ...
    escaped = copy.deepcopy(custody); escaped["sources"][0]["copy_path"] = "source-custody-v3/../../outside"; mutations.append((rows, cleanup, escaped))
...
def main():
    ...
    if len(sys.argv) != 2: raise SystemExit("usage: analyze_g3_v3.py RESULTS | --self-check")
    root = Path(sys.argv[1])
    rows = [json.loads(line) for line in (root / "rows-v3/G3-V3-RAW.jsonl").read_text().splitlines() if line]
    report = {"schema": "phase4-g3-v3-primary-analysis-v1", **validate(rows, json.loads((root / "CLEANUP-v3.json").read_text()), json.loads((root / "SOURCE-CUSTODY-v3.json").read_text()), root)}
