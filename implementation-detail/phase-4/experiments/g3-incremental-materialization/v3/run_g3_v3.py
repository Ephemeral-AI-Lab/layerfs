#!/usr/bin/env python3
"""Fresh one-shot G3-v3 runner with complete source/build custody."""
...
REPO = HERE.parents[4]
TARGET = REPO / "target/phase4-g3-incremental-materialization-20260822-v3"
RESULTS = TARGET / "results-v3"
LOCK = REPO / "target/phase4-g3-incremental-materialization-20260822-v3.lock"
WORK = RESULTS / "work-v3"
...
METHOD_NAMES = (
    "PROSPECTIVE-G3-INCREMENTAL-MATERIALIZATION-v3.md",
    "COUNTER-DICTIONARY-v3.md",
    "run_g3_v3.py",
    "analyze_g3_v3.py",
    "recompute_g3_v3.py",
    "finalize_g3_v3.py",
...
def preflight():
    if Path.cwd().resolve() != REPO or HERE.parents[4] != REPO:
        raise RuntimeError("run from repository root; invalid v3 REPO derivation")
...
def fresh():
    if TARGET.exists() or LOCK.exists():
        raise RuntimeError("G3-v3 result root or lock already exists")
...
def chronology(event, started, **fields):
    ...
    with (RESULTS / "CHRONOLOGY-v3.jsonl").open("a") as handle:
...
def remaining(started, requested):
    ...
    if seconds <= 0:
        raise TimeoutError("G3-v3 global ceiling exhausted")
...
def freeze_sources():
    destination = RESULTS / "source-custody-v3"
    destination.mkdir()
    records = []
    ...
    custody = {"schema": "phase4-g3-v3-source-custody-v1", "status": "PASS", "source_set_sha256": canonical_hash(identity), "sources": records}
    (RESULTS / "SOURCE-CUSTODY-v3.json").write_text(json.dumps(custody, indent=2, sort_keys=True) + "\n")
...
def verify_sources(custody, result_root=RESULTS):
    ...
    custody_root = (result_root / "source-custody-v3").resolve()
    for record in custody["sources"]:
        expected_copy = f"source-custody-v3/{record['path']}"
...
def freeze_methods():
    destination = RESULTS / "methodology-v3"
    destination.mkdir()
    records = []
    ...
    custody = {"schema": "phase4-g3-v3-method-custody-v1", "status": "PASS", "methodology_set_sha256": canonical_hash(records), "methods": records, "dry_run_sha256": sha256(DRY_RUN)}
    (RESULTS / "METHODOLOGY-CUSTODY-v3.json").write_text(json.dumps(custody, indent=2, sort_keys=True) + "\n")
    return custody, destination


def verify_methods(custody):
    for record in custody["methods"]:
        if sha256(HERE / record["path"]) != record["sha256"] or sha256(RESULTS / "methodology-v3" / record["path"]) != record["sha256"]:
...
def plan(binary, methods):
    ...
    result += [
        {"kind": "analyzer-self-check", "label": "primary-self-check", "timeout_seconds": 5, "command": [sys.executable, str(methods / "analyze_g3_v3.py"), "--self-check"]},
        {"kind": "analyzer-self-check", "label": "independent-self-check", "timeout_seconds": 5, "command": [sys.executable, str(methods / "recompute_g3_v3.py"), "--self-check"]},
    ]
    for row in rows():
    ...
    result += [
        {"kind": "analyzer", "label": "primary-analysis", "timeout_seconds": 5, "command": [sys.executable, str(methods / "analyze_g3_v3.py"), str(RESULTS)]},
        {"kind": "analyzer", "label": "independent-recomputation", "timeout_seconds": 5, "command": [sys.executable, str(methods / "recompute_g3_v3.py"), str(RESULTS)]},
    ]
    return result
...
def cleanup(storage):
    if WORK != RESULTS / "work-v3" or WORK.parent.resolve() != RESULTS.resolve() or not WORK.is_dir() or WORK.is_symlink():
    ...
    record = {"schema": "phase4-g3-v3-cleanup-v1", "status": "PASS" if not WORK.exists() and max(peaks.values()) <= STORAGE else "REVISE", "declared_root": "work-v3", "broad_deletion": False, "deletion_method": "enumerated-no-follow-unlink-and-rmdir", "deleted_entries": len(names), "deleted_path_set_sha256": canonical_hash(names), "work_root_absent": not WORK.exists(), "storage_ceiling_bytes": STORAGE, "peak_logical_bytes": peaks["logical_bytes"], "peak_apparent_bytes": peaks["apparent_bytes"], "peak_allocated_bytes": peaks["allocated_bytes"]}
    (RESULTS / "CLEANUP-v3.json").write_text(json.dumps(record, indent=2, sort_keys=True) + "\n")
    return record


def campaign():
    fresh()
    if os.environ.get("G3_V3_EXECUTE") != TOKEN:
        raise RuntimeError("set exact G3_V3_EXECUTE token")
    ...
    try:
        ...
        planned_binary = RESULTS / "operands-v3" / BINARY.name
        commands = plan(planned_binary, methods)
        (RESULTS / "COMMANDS-v3.json").write_text(json.dumps(commands, indent=2, sort_keys=True) + "\n")
        _, _, build_stdout, build_stderr = child(commands[0]["command"], commands[0]["label"], commands[0]["kind"], 30, RESULTS / "build-v3", env, started)
        ...
        for command in commands[1:3]:
            stdout, _, _, _ = child(command["command"], command["label"], command["kind"], command["timeout_seconds"], RESULTS / "self-check-v3", env, started)
        ...
        WORK.mkdir(); row_dir = RESULTS / "rows-v3"; row_dir.mkdir(); raw = row_dir / "G3-V3-RAW.jsonl"; raw.open("x").close()
        storage = {"schema": "phase4-g3-v3-runner-storage-v1", "ceiling_bytes": STORAGE, "samples": []}
        operation_sum = 0
        ...
        (RESULTS / "STORAGE-v3.json").write_text(json.dumps(storage, indent=2, sort_keys=True) + "\n")
        if cleanup(storage)["status"] != "PASS": raise RuntimeError("cleanup failed")
        reports = []
        for command, filename in zip(commands[12:], ("G3-PRIMARY-ANALYSIS-v3.json", "G3-INDEPENDENT-RECOMPUTATION-v3.json")):
            stdout, _, _, _ = child(command["command"], command["label"], command["kind"], command["timeout_seconds"], RESULTS / "analysis-children-v3", env, started)
        ...
        record = {"schema": "phase4-g3-v3-campaign-v1", "status": "PASS", "disposition": "G3_V3_CAMPAIGN_PASS_STATIC_CLOSURE_REQUIRED", "rows": 9, "rows_rerun": 0, "build_command": BUILD_COMMAND, "build_invocations": 1, "build_ceiling_seconds": 30, "operation_total_ns": operation_sum, "operation_sum_ceiling_ns": SUM_NS, "per_operation_ceiling_ns": OP_NS, "global_elapsed_ns": elapsed, "global_ceiling_ns": GLOBAL_NS, "source_set_sha256": source_custody["source_set_sha256"], "executable_sha256": binary_custody["sha256"], "methodology_set_sha256": method_custody["methodology_set_sha256"], "raw_sha256": sha256(raw), "primary_sha256": sha256(RESULTS / "G3-PRIMARY-ANALYSIS-v3.json"), "independent_sha256": sha256(RESULTS / "G3-INDEPENDENT-RECOMPUTATION-v3.json"), "normalized_ledger_sha256": reports[0]["normalized_ledger_sha256"], "cleanup_sha256": sha256(RESULTS / "CLEANUP-v3.json"), "source_custody_sha256": sha256(RESULTS / "SOURCE-CUSTODY-v3.json"), "binary_custody_sha256": sha256(RESULTS / "OPERAND-CUSTODY-v3.json"), "static_closure_required": True, "finalizer_required": True, "lock_removed_after_campaign": True}
        (RESULTS / "CAMPAIGN-v3.json").write_text(json.dumps(record, indent=2, sort_keys=True) + "\n")
        chronology("campaign-complete", started, status="PASS", rows=9)
        print(json.dumps(record, sort_keys=True, separators=(",", ":")))
    except Exception as error:
        if RESULTS.is_dir():
            failure = {"schema": "phase4-g3-v3-failure-v1", "status": "REVISE", "reason": f"{type(error).__name__}: {error}", "source_set_sha256": source_custody.get("source_set_sha256") if source_custody else None, "global_elapsed_ns": time.monotonic_ns() - started, "result_root_preserved": True}
            (RESULTS / "FAILURE-v3.json").write_text(json.dumps(failure, indent=2, sort_keys=True) + "\n")
        raise
    finally:
...
def self_check():
    ...
    assert "finalize_g3_v3.py" in METHOD_NAMES
    with tempfile.TemporaryDirectory() as directory:
        result_root = Path(directory) / "results-v3"; records = []
        for relative in SOURCE_PATHS:
            source, copy = REPO / relative, result_root / "source-custody-v3" / relative
            copy.parent.mkdir(parents=True, exist_ok=True); shutil.copyfile(source, copy); copy.chmod(0o400)
            records.append({"path": relative, "sha256": sha256(source), "size_bytes": source.stat().st_size, "source_mode": mode(source), "copy_path": f"source-custody-v3/{relative}", "copy_sha256": sha256(copy), "copy_size_bytes": copy.stat().st_size, "copy_mode": mode(copy)})
        custody = {"sources": records}; verify_sources(custody, result_root)
        mutations = []
        for field, value in (("copy_path", None), ("copy_path", "source-custody-v3/../../outside"), ("copy_size_bytes", records[0]["copy_size_bytes"] + 1)):
