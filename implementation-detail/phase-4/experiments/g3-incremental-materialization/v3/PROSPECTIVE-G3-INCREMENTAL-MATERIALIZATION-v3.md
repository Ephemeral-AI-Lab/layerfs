# Prospective G3-v3 incremental materialization contract

Status: **frozen before build or measurement**

v3 is the fresh pre-execution custody repair after zero-row v2. It freezes the
final repaired candidate bytes and independently proves each complete source
copy. This is still the same authorized Attempt-B protected-seed/clone-patch
mechanism and the same one variable; it is not a second mechanism. The trust
model, base JSON fields, six reconciliation counters, nine-row sizes/order,
resource, storage, and timing gates remain unchanged. Neither v1 nor v2 has a
result root or measured row, and no historical row is reused.
...
`source-custody-v3/<repository-relative-source-path>`. Its resolved path must
remain inside `results-v3/source-custody-v3`, be a regular non-symlink with a
...
`target/phase4-g3-incremental-materialization-20260822-v3/results-v3`, guarded
...
- Lost acknowledgement accepts exactly either (`reconciliation_outcome` =
  ...
  value passes. The deterministic v3 injection may yield `target`/`new`.
...
- Clone failure is a complete fallback reason and consumes zero permits. v3 has
...
No measured campaign is authorized by the dry run. A fresh v4 is required for
any pre-execution defect discovered after v3 is frozen.
