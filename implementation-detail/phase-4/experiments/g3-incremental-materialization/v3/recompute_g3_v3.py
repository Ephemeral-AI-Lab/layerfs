#!/usr/bin/env python3
"""Independent G3-v3 recomputation using a predicate table and fresh parser."""
...
def custody_valid(custody, results=None):
    ...
    predicates = [
        custody.get("schema") == "phase4-g3-v3-source-custody-v1",
    ...
    if not all(predicates):
        return False
    result_root = (results or Path("/__g3_v3_independent_synthetic__")).resolve()
    copy_root = (result_root / "source-custody-v3").resolve()
    for source in sources:
        relative = source.get("copy_path")
        parts = PurePosixPath(relative) if isinstance(relative, str) else None
        if parts is None or parts.is_absolute() or any(part in ("", ".", "..") for part in parts.parts) or relative != f"source-custody-v3/{source['path']}":
            return False
...
def audit(rows, cleanup, custody, results=None):
    failures = set()
    ...
    check("cleanup", 0, "exact-root", cleanup.get("status") == "PASS" and cleanup.get("declared_root") == "work-v3" and cleanup.get("work_root_absent") is True and cleanup.get("broad_deletion") is False and max(cleanup.get("peak_logical_bytes", 513 * MB), cleanup.get("peak_apparent_bytes", 513 * MB), cleanup.get("peak_allocated_bytes", 513 * MB)) <= 512 * MB)
    ordered_failures = sorted(failures)
    ledger = {"schema": "phase4-g3-v3-normalized-ledger-v1", "failures": ordered_failures, "gates": {gate: not any(item.startswith(gate + ":") for item in ordered_failures) for gate in GATE_ORDER}, "source_set_sha256": source_set, "operation_total_ns": sum(row.get("operation_total_ns", 0) for row in rows), "rows": [{key: row.get(key) for key in VIEW} for row in rows]}
...
def sample_data():
    sources = [{"path": path, "sha256": str(index + 1) * 64, "size_bytes": index + 1, "source_mode": "0644", "copy_path": f"source-custody-v3/{path}", "copy_sha256": str(index + 1) * 64, "copy_size_bytes": index + 1, "copy_mode": "0400"} for index, path in enumerate(FILES)]
    source_set = canonical_hash([{key: row[key] for key in ("path", "sha256", "size_bytes", "source_mode")} for row in sources])
    custody = {"schema": "phase4-g3-v3-source-custody-v1", "status": "PASS", "source_set_sha256": source_set, "sources": sources}
    rows = []
    for seq, scenario, size in PLAN:
        ...
        rows.append(row)
    cleanup = {"status": "PASS", "declared_root": "work-v3", "work_root_absent": True, "broad_deletion": False, "peak_logical_bytes": 1, "peak_apparent_bytes": 1, "peak_allocated_bytes": 1}
...
def main():
    ...
    if len(sys.argv) != 2: raise SystemExit("usage: recompute_g3_v3.py RESULTS | --self-check")
    root = Path(sys.argv[1])
    rows = [json.loads(line) for line in (root / "rows-v3/G3-V3-RAW.jsonl").read_text().splitlines() if line]
    result = {"schema": "phase4-g3-v3-independent-recomputation-v1", **audit(rows, json.loads((root / "CLEANUP-v3.json").read_text()), json.loads((root / "SOURCE-CUSTODY-v3.json").read_text()), root)}
