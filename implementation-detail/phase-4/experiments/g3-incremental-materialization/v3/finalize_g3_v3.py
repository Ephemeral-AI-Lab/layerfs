#!/usr/bin/env python3
"""Finalize and seal a completed G3-v3 campaign after parent static closure."""
...
TERMINAL = RESULTS / "TERMINAL-v3.json"
VERIFICATION = RESULTS / "TERMINAL-VERIFICATION-v3.txt"
STATIC = RESULTS / "STATIC-CLOSURE-v3.json"
...
METHOD_NAMES = [
    ...
    "COUNTER-DICTIONARY-v3.md", "run_g3_v3.py", "analyze_g3_v3.py",
    "recompute_g3_v3.py", "finalize_g3_v3.py",
]
...
EXCLUDED = {"PAYLOAD-MANIFEST-v3.tsv", "TERMINAL-v3.json", "TERMINAL-VERIFICATION-v3.txt"}
...
def exact_source_copy(row, result_root=RESULTS):
    ...
    expected = f"source-custody-v3/{row.get('path')}"
...
def verify_static(source_set):
    ...
    if closure.get("schema") != "phase4-g3-v3-static-closure-v1" or closure.get("status") != "PASS" or closure.get("source_set_sha256") != source_set or closure.get("candidate_retained") is not True or [row.get("label") for row in commands] != REQUIRED_STATIC_LABELS:
...
def verify_campaign_inputs():
    ...
    if any(path.exists() for path in (MANIFEST, TERMINAL, VERIFICATION)):
        raise RuntimeError("v3 final artifacts already exist")
    if (RESULTS / "FAILURE-v3.json").exists():
        raise RuntimeError("failed/revised campaign cannot be finalized")
    campaign = read_json(RESULTS / "CAMPAIGN-v3.json")
    primary = read_json(RESULTS / "G3-PRIMARY-ANALYSIS-v3.json")
    independent = read_json(RESULTS / "G3-INDEPENDENT-RECOMPUTATION-v3.json")
    source = read_json(RESULTS / "SOURCE-CUSTODY-v3.json")
    operand = read_json(RESULTS / "OPERAND-CUSTODY-v3.json")
    methods = read_json(RESULTS / "METHODOLOGY-CUSTODY-v3.json")
    cleanup = read_json(RESULTS / "CLEANUP-v3.json")
    raw = RESULTS / "rows-v3/G3-V3-RAW.jsonl"
    commands = json.loads((RESULTS / "COMMANDS-v3.json").read_text())
    chronology = [json.loads(line) for line in (RESULTS / "CHRONOLOGY-v3.jsonl").read_text().splitlines() if line]
    ...
    for row in method_rows:
        if sha256(HERE / row["path"]) != row["sha256"] or sha256(RESULTS / "methodology-v3" / row["path"]) != row["sha256"]:
            raise RuntimeError(f"method changed: {row['path']}")
    if sha256(DRY_RUN) != methods.get("dry_run_sha256") or sha256(RESULTS / "methodology-v3/DRY-RUN-v3.json") != methods.get("dry_run_sha256"):
        raise RuntimeError("dry-run custody mismatch")
    if cleanup.get("status") != "PASS" or cleanup.get("declared_root") != "work-v3" or cleanup.get("work_root_absent") is not True or cleanup.get("broad_deletion") is not False or (RESULTS / "work-v3").exists():
        raise RuntimeError("cleanup mismatch")
    hashes = {"raw_sha256": raw, "primary_sha256": RESULTS / "G3-PRIMARY-ANALYSIS-v3.json", "independent_sha256": RESULTS / "G3-INDEPENDENT-RECOMPUTATION-v3.json", "cleanup_sha256": RESULTS / "CLEANUP-v3.json", "source_custody_sha256": RESULTS / "SOURCE-CUSTODY-v3.json", "binary_custody_sha256": RESULTS / "OPERAND-CUSTODY-v3.json"}
...
def finalize():
    ...
    terminal = {
        "schema": "phase4-g3-v3-terminal-v1", "status": "PASS",
        ...
        "campaign_sha256": sha256(RESULTS / "CAMPAIGN-v3.json"),
        "cleanup_sha256": sha256(RESULTS / "CLEANUP-v3.json"),
        "static_closure_sha256": sha256(STATIC),
        "source_custody_sha256": sha256(RESULTS / "SOURCE-CUSTODY-v3.json"),
        "binary_custody_sha256": sha256(RESULTS / "OPERAND-CUSTODY-v3.json"),
        "methodology_custody_sha256": sha256(RESULTS / "METHODOLOGY-CUSTODY-v3.json"),
    ...
    seal_existing()
    snapshot = {"schema": "phase4-g3-v3-terminal-verification-v1", "status": "PASS", "disposition": terminal["disposition"], **final_verification(len(manifest_rows), g2, terminal_hash, manifest_hash, source["source_set_sha256"], False)}
...
def self_check():
    assert HERE.parents[4] == REPO and EXCLUDED == {"PAYLOAD-MANIFEST-v3.tsv", "TERMINAL-v3.json", "TERMINAL-VERIFICATION-v3.txt"}
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        root = Path(directory) / "results-v3"; copied = root / "source-custody-v3/Cargo.lock"; copied.parent.mkdir(parents=True); copied.write_bytes((REPO / "Cargo.lock").read_bytes()); copied.chmod(0o400)
        source_hash = sha256(REPO / "Cargo.lock")
        record = {"path": "Cargo.lock", "sha256": source_hash, "size_bytes": (REPO / "Cargo.lock").stat().st_size, "source_mode": "0644", "copy_path": "source-custody-v3/Cargo.lock", "copy_sha256": source_hash, "copy_size_bytes": copied.stat().st_size, "copy_mode": "0400"}
        exact_source_copy(record, root)
        mutations = []
        missing = dict(record); missing.pop("copy_path"); mutations.append(missing)
        escaped = dict(record, copy_path="source-custody-v3/../../outside"); mutations.append(escaped)
    ...
    print(json.dumps({"status": "PASS", "static_commands": 6, "source_copy_mutations_rejected": 3, "campaign_children_invoked": 0, "finalized": False}, sort_keys=True))
...
def main():
    ...
    else: raise SystemExit("refusing: use --self-check or --execute after PASS STATIC-CLOSURE-v3.json")
