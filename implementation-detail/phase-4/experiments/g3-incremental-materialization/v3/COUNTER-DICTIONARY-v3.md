# G3-v3 direct-counter dictionary

Status: **frozen before build or measurement**

The candidate command and all v1 flat JSON fields remain. v3 adds six explicit
reconciliation counters, complete build provenance, and repaired gate
expressions. This file restates the controlling equations so v3 analysis is
self-contained.
...
behavior. Runner-added evidence uses v3 names and adds `source_set_sha256`.
...
reconciliation. v3 adds these non-negative observed counters:
...
canonical digest in `SOURCE-CUSTODY-v3.json`. That record contains exactly the
four ordered source paths from the v3 preregistration, original/copy SHA-256,
...
contains the v3 preregistration, dictionary, runner, primary analyzer,
independent analyzer, and post-static finalizer; the dry run is separately
hashed and copied.

For each source, `copy_path` is exactly
`source-custody-v3/<repository-relative-source-path>`, is relative with no empty,
...
Only the fresh exact `results-v3/work-v3` tree may be removed, by enumerated
...
After a PASS campaign, the parent creates `STATIC-CLOSURE-v3.json` with schema
`phase4-g3-v3-static-closure-v1`, the frozen `source_set_sha256`,
...
stdout/stderr path, SHA-256, and size. `finalize_g3_v3.py` refuses incomplete or
