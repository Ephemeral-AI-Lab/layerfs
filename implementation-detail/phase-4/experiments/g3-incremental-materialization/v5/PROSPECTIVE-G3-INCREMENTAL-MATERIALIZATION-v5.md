# Prospective G3-v5 incremental materialization contract

Status: **frozen before build or measurement**

v5 is the cleanup-evidence-only repair after zero-row v4's final audit. It
freezes the same final candidate bytes, source-copy proof, Attempt-B mechanism,
routes, counters, nine-row schedule, limits, and acceptance gates. No historical
row is reused.
...
`CLEANUP-v5.json` requires 18 alternating events—nine exact PREPARE/COMPLETE
...
The residual stat/unlink micro-race is an ordinary POSIX limitation. v5 claims
...
`source-custody-v5/<repository-relative-source-path>`. Its resolved path must
remain inside `results-v5/source-custody-v5`, be a regular non-symlink with a
...
`target/phase4-g3-incremental-materialization-20260822-v5/results-v5`, guarded
...
- Lost acknowledgement accepts exactly either (`reconciliation_outcome` =
  ...
  value passes. The deterministic v5 injection may yield `target`/`new`.
...
- Clone failure is a complete fallback reason and consumes zero permits. v5 has
...
No measured campaign is authorized by the dry run. A fresh v6 is required for
any pre-execution defect discovered after v5 is frozen.
