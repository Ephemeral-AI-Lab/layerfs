# G3-v5 direct-counter dictionary

Status: **frozen before build or measurement**

The candidate command and all v1 flat JSON fields remain. v5 adds six explicit
reconciliation counters, complete build provenance, and repaired gate
expressions. This file restates the controlling equations so v5 analysis is
self-contained.
...
behavior. Runner-added evidence uses v5 names and adds `source_set_sha256`.
...
reconciliation. v5 adds these non-negative observed counters:
...
canonical digest in `SOURCE-CUSTODY-v5.json`. That record contains exactly the
four ordered source paths from the v5 preregistration, original/copy SHA-256,
...
contains the v5 preregistration, dictionary, runner, primary analyzer,
independent analyzer, and post-static finalizer; the dry run is separately
hashed and copied.

For each source, `copy_path` is exactly
`source-custody-v5/<repository-relative-source-path>`, is relative with no empty,
...
Only the current exact `results-v5/work-v5/<label>` row root may be removed.
`ROW-CLEANUP-v5.jsonl` has 18 fsynced alternating events. PREPARE binds the
...
After a PASS campaign, the parent creates `STATIC-CLOSURE-v5.json` with schema
`phase4-g3-v5-static-closure-v1`, the frozen `source_set_sha256`,
...
stdout/stderr path, SHA-256, and size. `finalize_g3_v5.py` refuses incomplete or
