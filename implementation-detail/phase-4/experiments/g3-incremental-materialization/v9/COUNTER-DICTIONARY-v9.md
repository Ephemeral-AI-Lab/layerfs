# G3-v9 direct-counter dictionary

Status: **frozen before build or measurement**

The candidate command and all v1 flat JSON fields remain. v9 adds six explicit
reconciliation counters, complete build provenance, and repaired gate
expressions. This file restates the controlling equations so v9 analysis is
self-contained.
...
behavior. Runner-added evidence uses v9 names and adds `source_set_sha256`.
...
reconciliation. v9 adds these non-negative observed counters:
...
canonical digest in `SOURCE-CUSTODY-v9.json`. That record contains exactly the
four ordered source paths from the v9 preregistration, original/copy SHA-256,
...
contains the v9 preregistration, dictionary, runner, primary analyzer,
independent analyzer, and post-static finalizer; the dry run is separately
hashed and copied.

For each source, `copy_path` is exactly
`source-custody-v9/<repository-relative-source-path>`, is relative with no empty,
...
Only the current exact `results-v9/work-v9/<label>` row root may be removed.
`ROW-CLEANUP-v9.jsonl` has 18 fsynced alternating events. PREPARE binds the
...
append with parent sync on first creation. `FAILURE-v9.json` uses exclusive
...
Copied analyzers never derive campaign custody from their own method-copy path.
They resolve the supplied `results-v9`, require the exact v9 target namespace
under a `target` directory, derive the repository as `results.parents[2]`, and
require the repository markers before checking original/copy source bytes. The
source self-check accepts the exact expected shape and rejects the former
one-level-high derivation.

## Static closure and finalization

After a PASS campaign, the parent creates `STATIC-CLOSURE-v9.json` with schema
`phase4-g3-v9-static-closure-v1`, the frozen `source_set_sha256`,
...
stdout/stderr path, SHA-256, and size. `finalize_g3_v9.py` refuses incomplete or
