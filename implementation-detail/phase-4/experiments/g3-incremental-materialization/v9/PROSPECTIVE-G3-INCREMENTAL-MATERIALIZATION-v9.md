# Prospective G3-v9 incremental materialization contract

Status: **frozen before build or measurement**

v9 is the analyzer-relocation repair after completed-row v8. It freezes the
same candidate bytes, source-copy proof, evidence durability,
...
## One protocol change: repository derivation from results custody

Primary and independent analyzers derive the campaign repository only from the
supplied, resolved results root. It must be exactly
`<repo>/target/phase4-g3-incremental-materialization-20260822-v9/results-v9`,
...
Source-tree self-checks may use their source-file derivation, but both analyzers
explicitly validate the expected relocated results shape and reject a synthetic
one-level-high path. Actual campaign custody always receives the repository from
`repo_from_results`, never from the copied analyzer's `HERE`.

## One protocol change: exact sealed G2-v5 dependency
...
parent-synced. v9 therefore claims crash durability for these evidence writes,
...
`CLEANUP-v9.json` requires 18 alternating events—nine exact PREPARE/COMPLETE
...
The residual stat/unlink micro-race is an ordinary POSIX limitation. v9 claims
...
`source-custody-v9/<repository-relative-source-path>`. Its resolved path must
remain inside `results-v9/source-custody-v9`, be a regular non-symlink with a
...
`target/phase4-g3-incremental-materialization-20260822-v9/results-v9`, guarded
...
- Lost acknowledgement accepts exactly either (`reconciliation_outcome` =
  ...
  value passes. The deterministic v9 injection may yield `target`/`new`.
...
- Clone failure is a complete fallback reason and consumes zero permits. v9 has
...
No measured campaign is authorized by the dry run. A fresh v10 is required for
any pre-execution defect discovered after v9 is frozen.
