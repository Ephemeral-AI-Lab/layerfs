#!/usr/bin/env python3
"""Independent G3-v9 recomputation using a predicate table and fresh parser."""
...
REPO = HERE.parents[4]
TARGET_NAME = "phase4-g3-incremental-materialization-20260822-v9"
MB = 1048576
PLAN = [
...
def repo_from_results(results):
    ...
    if (candidate.name, candidate.parent.name, candidate.parent.parent.name) != ("results-v9", TARGET_NAME, "target"):
        raise RuntimeError("invalid independent results namespace")
    repository = candidate.parents[2]
    if candidate != repository / "target" / TARGET_NAME / "results-v9" or not (repository / "Cargo.lock").is_file() or not (repository / "crates/layerfs-engine/Cargo.toml").is_file() or not (repository / ".git").exists():
...
def custody_valid(custody, results=None, repository=None):
    ...
    predicates = [
        custody.get("schema") == "phase4-g3-v9-source-custody-v1",
    ...
    if not all(predicates):
        return False
    result_root = (results or Path("/__g3_v9_independent_synthetic__")).resolve()
    repository = repository or REPO
    copy_root = (result_root / "source-custody-v9").resolve()
    for source in sources:
        relative = source.get("copy_path")
        parts = PurePosixPath(relative) if isinstance(relative, str) else None
        if parts is None or parts.is_absolute() or any(part in ("", ".", "..") for part in parts.parts) or relative != f"source-custody-v9/{source['path']}":
            return False
...
def audit(rows, cleanup, custody, results=None, cleanup_rows=None, repository=None):
    ...
    check("cleanup", 0, "exact-root", cleanup.get("status") == "PASS" and cleanup.get("declared_root") == "work-v9" and cleanup.get("work_root_absent") is True and cleanup.get("all_row_roots_absent") is True and cleanup.get("broad_deletion") is False and cleanup.get("deletion_method") == method and cleanup.get("prepare_records") == cleanup.get("complete_records") == 9 and cleanup.get("row_cleanup_records") == 18 and cleanup.get("row_cleanup_labels") == labels and cleanup.get("durable_prepare_complete") is True and len(cleanup.get("row_cleanup_sha256", "")) == 64 and cleanup.get("peak_equation") == "max_individual_PREPARE_pre_delete_row_not_cumulative_sum" and all(cleanup.get(f"peak_{key}") == peak[key] and cleanup.get(f"cumulative_{key}") == cumulative[key] for key in dimensions) and max(peak.values()) <= 512 * MB)
    if results is not None: check("cleanup", 0, "artifact-hash-work-root", cleanup.get("row_cleanup_sha256") == hash_file(results / "ROW-CLEANUP-v9.jsonl") and not (results / "work-v9").exists())
    ordered_failures = sorted(failures)
    ledger = {"schema": "phase4-g3-v9-normalized-ledger-v1", "failures": ordered_failures, "gates": {gate: not any(item.startswith(gate + ":") for item in ordered_failures) for gate in GATE_ORDER}, "source_set_sha256": source_set, "operation_total_ns": sum(row.get("operation_total_ns", 0) for row in rows), "storage": {"peak": peak, "cumulative": cumulative, "row_cleanup_records": len(cleanup_rows)}, "rows": [{key: row.get(key) for key in VIEW} for row in rows]}
...
def sample_data():
    sources = [{"path": path, "sha256": str(index + 1) * 64, "size_bytes": index + 1, "source_mode": "0644", "copy_path": f"source-custody-v9/{path}", "copy_sha256": str(index + 1) * 64, "copy_size_bytes": index + 1, "copy_mode": "0400"} for index, path in enumerate(FILES)]
    source_set = canonical_hash([{key: row[key] for key in ("path", "sha256", "size_bytes", "source_mode")} for row in sources])
    custody = {"schema": "phase4-g3-v9-source-custody-v1", "status": "PASS", "source_set_sha256": source_set, "sources": sources}
    rows = []
...
def main():
    ...
    if len(sys.argv) != 2: raise SystemExit("usage: recompute_g3_v9.py RESULTS | --self-check")
    root = Path(sys.argv[1])
    rows = [json.loads(line) for line in (root / "rows-v9/G3-V9-RAW.jsonl").read_text().splitlines() if line]
    cleanup_rows = [json.loads(line) for line in (root / "ROW-CLEANUP-v9.jsonl").read_text().splitlines() if line]
    repository = repo_from_results(root)
    result = {"schema": "phase4-g3-v9-independent-recomputation-v1", **audit(rows, json.loads((root / "CLEANUP-v9.json").read_text()), json.loads((root / "SOURCE-CUSTODY-v9.json").read_text()), root, cleanup_rows, repository)}
