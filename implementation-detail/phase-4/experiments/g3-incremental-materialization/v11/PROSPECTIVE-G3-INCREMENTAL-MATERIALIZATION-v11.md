# Prospective G3-v11 incremental materialization contract

Status: **frozen before build or measurement**

v11 is the fresh campaign required after the v10 campaign passed but workspace
static closure failed. It freezes the same G3 module and main benchmark bytes,
source-copy proof, evidence durability, descriptor-anchored cleanup, Attempt-B
mechanism, routes, counters, nine-row schedule, limits, analyzer relocation,
and sealed G2-v5 gates. No historical row is reused.

## Only changed source/build variable: explicit Cargo binary topology

v10's focused G3 tests passed 9/9, but `cargo test --workspace --offline
--all-targets` auto-discovered `phase4_g3_materialization.rs` as a standalone
binary. That file is a module of `phase4_create_edit_benchmark.rs`, so its
parent-scope imports and visibility cannot compile independently. v11 changes
only `crates/layerfs-engine/Cargo.toml`: package `autobins = false` plus one
explicit binary named `phase4_create_edit_benchmark` at
...
`<repo>/target/phase4-g3-incremental-materialization-20260822-v11/results-v11/methodology-v11`,
...
the copied branch without creating the v11 result namespace or invoking a
...
It must be exactly
`<repo>/target/phase4-g3-incremental-materialization-20260822-v11/results-v11`,
...
parent-synced. v11 therefore claims crash durability for these evidence writes,
...
`CLEANUP-v11.json` requires 18 alternating events—nine exact PREPARE/COMPLETE
...
The residual stat/unlink micro-race is an ordinary POSIX limitation. v11 claims
...
| `crates/layerfs-engine/src/bin/phase4_g3_materialization.rs` | `5ecc718a3e0595b79910d1bc12353318dd6d55ffff2d2c856118a7cfc14691e8` |
| canonical source set | `45a08ba60b02316bc803cca69871e773751009bb9f9196fb9c03e8c7ad705821` |

Every source record must have the exact relative traversal-free `copy_path`
`source-custody-v11/<repository-relative-source-path>`. Its resolved path must
remain inside `results-v11/source-custody-v11`, be a regular non-symlink with a
...
`target/phase4-g3-incremental-materialization-20260822-v11/results-v11`, guarded
...
- Lost acknowledgement accepts exactly either (`reconciliation_outcome` =
  ...
  value passes. The deterministic v11 injection may yield `target`/`new`.
...
- Clone failure is a complete fallback reason and consumes zero permits. v11 has
...
No measured campaign is authorized by the dry run. A fresh v12 is required for
any pre-execution defect discovered after v11 is frozen.
