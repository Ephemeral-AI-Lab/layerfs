#!/usr/bin/env python3
"""Fresh one-shot G3-v11 runner with complete source/build custody."""
...
def preflight():
    if Path.cwd().resolve() != REPO or HERE.parents[4] != REPO:
        raise RuntimeError("run from repository root; invalid v11 REPO derivation")
...
def fresh():
    if TARGET.exists() or LOCK.exists():
        raise RuntimeError("G3-v11 result root or lock already exists")
...
def chronology(event, started, **fields):
    ...
    durable_append_text(RESULTS / "CHRONOLOGY-v11.jsonl", json.dumps(record, sort_keys=True, separators=(",", ":")) + "\n", label="chronology")


def remaining(started, requested):
    seconds = (GLOBAL_NS - (time.monotonic_ns() - started)) / 1_000_000_000
    if seconds <= 0:
        raise TimeoutError("G3-v11 global ceiling exhausted")
...
def freeze_sources():
    destination = RESULTS / "source-custody-v11"
    destination.mkdir()
    records = []
    ...
    custody = {"schema": "phase4-g3-v11-source-custody-v1", "status": "PASS", "source_set_sha256": canonical_hash(identity), "sources": records}
    (RESULTS / "SOURCE-CUSTODY-v11.json").write_text(json.dumps(custody, indent=2, sort_keys=True) + "\n")
...
def verify_sources(custody, result_root=RESULTS):
    ...
    for record in custody["sources"]:
        expected_copy = f"source-custody-v11/{record['path']}"
...
def freeze_methods():
    destination = RESULTS / "methodology-v11"
    destination.mkdir()
    records = []
    ...
    custody = {"schema": "phase4-g3-v11-method-custody-v1", "status": "PASS", "methodology_set_sha256": canonical_hash(records), "methods": records, "dry_run_sha256": sha256(DRY_RUN)}
    (RESULTS / "METHODOLOGY-CUSTODY-v11.json").write_text(json.dumps(custody, indent=2, sort_keys=True) + "\n")
    return custody, destination


def verify_methods(custody):
    for record in custody["methods"]:
        if sha256(HERE / record["path"]) != record["sha256"] or sha256(RESULTS / "methodology-v11" / record["path"]) != record["sha256"]:
...
def plan(binary, methods):
    ...
    result += [
        {"kind": "analyzer-self-check", "label": "primary-self-check", "timeout_seconds": 5, "command": [sys.executable, str(methods / "analyze_g3_v11.py"), "--self-check"]},
        {"kind": "analyzer-self-check", "label": "independent-self-check", "timeout_seconds": 5, "command": [sys.executable, str(methods / "recompute_g3_v11.py"), "--self-check"]},
    ]
    for row in rows():
    ...
    result += [
        {"kind": "analyzer", "label": "primary-analysis", "timeout_seconds": 5, "command": [sys.executable, str(methods / "analyze_g3_v11.py"), str(RESULTS)]},
        {"kind": "analyzer", "label": "independent-recomputation", "timeout_seconds": 5, "command": [sys.executable, str(methods / "recompute_g3_v11.py"), str(RESULTS)]},
    ]
    return result
...
def retire_row(specification, row, storage, work_fd):
    ...
    try:
        ...
        durable_append(RESULTS / "ROW-CLEANUP-v11.jsonl", complete)
...
def finalize_cleanup(storage, results_fd, work_fd):
    records = [json.loads(line) for line in (RESULTS / "ROW-CLEANUP-v11.jsonl").read_text().splitlines() if line]
    ...
    os.rmdir("work-v11", dir_fd=results_fd)
...
def campaign():
    ...
    try:
        ...
        planned_binary = RESULTS / "operands-v11" / BINARY.name
        commands = plan(planned_binary, methods)
        (RESULTS / "COMMANDS-v11.json").write_text(json.dumps(commands, indent=2, sort_keys=True) + "\n")
        _, _, build_stdout, build_stderr = child(commands[0]["command"], commands[0]["label"], commands[0]["kind"], 30, RESULTS / "build-v11", env, started)
        ...
        for command in commands[1:3]:
            stdout, _, _, _ = child(command["command"], command["label"], command["kind"], command["timeout_seconds"], RESULTS / "self-check-v11", env, started)
        ...
        results_fd = os.open(RESULTS, DIR_FLAGS); os.mkdir("work-v11", dir_fd=results_fd); work_fd = os.open("work-v11", DIR_FLAGS, dir_fd=results_fd)
        row_dir = RESULTS / "rows-v11"; row_dir.mkdir(); fsync_directory(RESULTS); raw = row_dir / "G3-V11-RAW.jsonl"; durable_write_new(raw, "", label="raw-create"); durable_write_new(RESULTS / "ROW-CLEANUP-v11.jsonl", "", label="cleanup-log-create")
        storage = {"schema": "phase4-g3-v11-runner-storage-v1", "ceiling_bytes": STORAGE, "samples": []}
        operation_sum = 0
        ...
        (RESULTS / "STORAGE-v11.json").write_text(json.dumps(storage, indent=2, sort_keys=True) + "\n")
        ...
        reports = []
        for command, filename in zip(commands[12:], ("G3-PRIMARY-ANALYSIS-v11.json", "G3-INDEPENDENT-RECOMPUTATION-v11.json")):
            stdout, _, _, _ = child(command["command"], command["label"], command["kind"], command["timeout_seconds"], RESULTS / "analysis-children-v11", env, started)
        ...
        record = {"schema": "phase4-g3-v11-campaign-v1", "status": "PASS", "disposition": "G3_V11_CAMPAIGN_PASS_STATIC_CLOSURE_REQUIRED", "rows": 9, "rows_rerun": 0, "build_command": BUILD_COMMAND, "build_invocations": 1, "build_ceiling_seconds": 30, "operation_total_ns": operation_sum, "operation_sum_ceiling_ns": SUM_NS, "per_operation_ceiling_ns": OP_NS, "global_elapsed_ns": elapsed, "global_ceiling_ns": GLOBAL_NS, "source_set_sha256": source_custody["source_set_sha256"], "executable_sha256": binary_custody["sha256"], "methodology_set_sha256": method_custody["methodology_set_sha256"], "raw_sha256": sha256(raw), "row_cleanup_sha256": sha256(RESULTS / "ROW-CLEANUP-v11.jsonl"), "primary_sha256": sha256(RESULTS / "G3-PRIMARY-ANALYSIS-v11.json"), "independent_sha256": sha256(RESULTS / "G3-INDEPENDENT-RECOMPUTATION-v11.json"), "normalized_ledger_sha256": reports[0]["normalized_ledger_sha256"], "cleanup_sha256": sha256(RESULTS / "CLEANUP-v11.json"), "source_custody_sha256": sha256(RESULTS / "SOURCE-CUSTODY-v11.json"), "binary_custody_sha256": sha256(RESULTS / "OPERAND-CUSTODY-v11.json"), "static_closure_required": True, "finalizer_required": True, "lock_removed_after_campaign": True}
        (RESULTS / "CAMPAIGN-v11.json").write_text(json.dumps(record, indent=2, sort_keys=True) + "\n")
        chronology("campaign-complete", started, status="PASS", rows=9)
        print(json.dumps(record, sort_keys=True, separators=(",", ":")))
    except Exception as error:
        if RESULTS.is_dir():
            failure = {"schema": "phase4-g3-v11-failure-v1", "status": "REVISE", "reason": f"{type(error).__name__}: {error}", "source_set_sha256": source_custody.get("source_set_sha256") if source_custody else None, "global_elapsed_ns": time.monotonic_ns() - started, "result_root_preserved": True}
            durable_write_new(RESULTS / "FAILURE-v11.json", json.dumps(failure, indent=2, sort_keys=True) + "\n", label="failure")
        raise
    finally:
...
def self_check():
    ...
    assert "finalize_g3_v11.py" in METHOD_NAMES
    forbidden = ("G2-" + "v11", "G2-" + "V11", "decomposition-20260822-" + "v11")
    forbidden_hits = {name: token for name in METHOD_NAMES for token in forbidden if token in (HERE / name).read_text()}
    assert not forbidden_hits
    with tempfile.TemporaryDirectory() as directory:
        result_root = Path(directory) / "results-v11"; records = []
        for relative in SOURCE_PATHS:
            source, copy = REPO / relative, result_root / "source-custody-v11" / relative
            copy.parent.mkdir(parents=True, exist_ok=True); shutil.copyfile(source, copy); copy.chmod(0o400)
            records.append({"path": relative, "sha256": sha256(source), "size_bytes": source.stat().st_size, "source_mode": mode(source), "copy_path": f"source-custody-v11/{relative}", "copy_sha256": sha256(copy), "copy_size_bytes": copy.stat().st_size, "copy_mode": mode(copy)})
        custody = {"sources": records}; verify_sources(custody, result_root)
        mutations = []
        for field, value in (("copy_path", None), ("copy_path", "source-custody-v11/../../outside"), ("copy_size_bytes", records[0]["copy_size_bytes"] + 1)):
