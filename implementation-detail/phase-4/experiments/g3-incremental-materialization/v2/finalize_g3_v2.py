#!/usr/bin/env python3
"""Finalize and seal a completed G3-v2 campaign after parent static closure."""
...
TERMINAL = RESULTS / "TERMINAL-v2.json"
VERIFICATION = RESULTS / "TERMINAL-VERIFICATION-v2.txt"
STATIC = RESULTS / "STATIC-CLOSURE-v2.json"
...
METHOD_NAMES = [
    ...
    "COUNTER-DICTIONARY-v2.md", "run_g3_v2.py", "analyze_g3_v2.py",
    "recompute_g3_v2.py", "finalize_g3_v2.py",
]
...
EXCLUDED = {"PAYLOAD-MANIFEST-v2.tsv", "TERMINAL-v2.json", "TERMINAL-VERIFICATION-v2.txt"}
...
def verify_static(source_set):
    ...
    if closure.get("schema") != "phase4-g3-v2-static-closure-v1" or closure.get("status") != "PASS" or closure.get("source_set_sha256") != source_set or closure.get("candidate_retained") is not True or [row.get("label") for row in commands] != REQUIRED_STATIC_LABELS:
...
def verify_campaign_inputs():
    ...
    if any(path.exists() for path in (MANIFEST, TERMINAL, VERIFICATION)):
        raise RuntimeError("v2 final artifacts already exist")
    if (RESULTS / "FAILURE-v2.json").exists():
        raise RuntimeError("failed/revised campaign cannot be finalized")
    campaign = read_json(RESULTS / "CAMPAIGN-v2.json")
    primary = read_json(RESULTS / "G3-PRIMARY-ANALYSIS-v2.json")
    independent = read_json(RESULTS / "G3-INDEPENDENT-RECOMPUTATION-v2.json")
    source = read_json(RESULTS / "SOURCE-CUSTODY-v2.json")
    operand = read_json(RESULTS / "OPERAND-CUSTODY-v2.json")
    methods = read_json(RESULTS / "METHODOLOGY-CUSTODY-v2.json")
    cleanup = read_json(RESULTS / "CLEANUP-v2.json")
    raw = RESULTS / "rows-v2/G3-V2-RAW.jsonl"
    commands = json.loads((RESULTS / "COMMANDS-v2.json").read_text())
    chronology = [json.loads(line) for line in (RESULTS / "CHRONOLOGY-v2.jsonl").read_text().splitlines() if line]
    ...
    for row in source["sources"]:
        original, copy = REPO / row["path"], RESULTS / row["copy_path"]
        if row.get("source_mode") != "0644" or row.get("copy_mode") != "0400" or mode(original) != "0644" or sha256(original) != row["sha256"] or original.stat().st_size != row["size_bytes"] or sha256(copy) != row["copy_sha256"] or row["sha256"] != row["copy_sha256"] or mode(copy) != "0400":
            raise RuntimeError(f"source bytes/mode changed: {row['path']}")
    ...
    for row in method_rows:
        if sha256(HERE / row["path"]) != row["sha256"] or sha256(RESULTS / "methodology-v2" / row["path"]) != row["sha256"]:
            raise RuntimeError(f"method changed: {row['path']}")
    if sha256(DRY_RUN) != methods.get("dry_run_sha256") or sha256(RESULTS / "methodology-v2/DRY-RUN-v2.json") != methods.get("dry_run_sha256"):
        raise RuntimeError("dry-run custody mismatch")
    if cleanup.get("status") != "PASS" or cleanup.get("declared_root") != "work-v2" or cleanup.get("work_root_absent") is not True or cleanup.get("broad_deletion") is not False or (RESULTS / "work-v2").exists():
        raise RuntimeError("cleanup mismatch")
    hashes = {"raw_sha256": raw, "primary_sha256": RESULTS / "G3-PRIMARY-ANALYSIS-v2.json", "independent_sha256": RESULTS / "G3-INDEPENDENT-RECOMPUTATION-v2.json", "cleanup_sha256": RESULTS / "CLEANUP-v2.json", "source_custody_sha256": RESULTS / "SOURCE-CUSTODY-v2.json", "binary_custody_sha256": RESULTS / "OPERAND-CUSTODY-v2.json"}
...
def finalize():
    ...
    terminal = {
        "schema": "phase4-g3-v2-terminal-v1", "status": "PASS",
        ...
        "campaign_sha256": sha256(RESULTS / "CAMPAIGN-v2.json"),
        "cleanup_sha256": sha256(RESULTS / "CLEANUP-v2.json"),
        ...
        "methodology_custody_sha256": sha256(RESULTS / "METHODOLOGY-CUSTODY-v2.json"),
    ...
    seal_existing()
    snapshot = {"schema": "phase4-g3-v2-terminal-verification-v1", "status": "PASS", "disposition": terminal["disposition"], **final_verification(len(manifest_rows), g2, terminal_hash, manifest_hash, source["source_set_sha256"], False)}
...
def self_check():
    assert HERE.parents[4] == REPO and EXCLUDED == {"PAYLOAD-MANIFEST-v2.tsv", "TERMINAL-v2.json", "TERMINAL-VERIFICATION-v2.txt"}
    ...
    print(json.dumps({"status": "PASS", "static_commands": 6, "campaign_children_invoked": 0, "finalized": False}, sort_keys=True))
...
def main():
    ...
    else: raise SystemExit("refusing: use --self-check or --execute after PASS STATIC-CLOSURE-v2.json")
