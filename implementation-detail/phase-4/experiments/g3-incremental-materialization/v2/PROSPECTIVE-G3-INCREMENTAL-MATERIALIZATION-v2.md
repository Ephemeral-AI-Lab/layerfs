# Prospective G3-v2 incremental materialization contract

Status: **frozen before build or measurement**

v2 repairs both the zero-row v1 custody protocol and pre-measurement candidate
defects found by the independent post-implementation audit. It freezes the
repaired candidate bytes and their complete source custody together. This is
still the same authorized Attempt-B protected-seed/clone-patch mechanism and
the same one variable; it is not a second mechanism. The trust model, v1 base
JSON fields, nine-row sizes/order, resource, storage, and timing gates remain;
v2 adds only the separately charged reconciliation counters required by the
audit. No v1 result root exists and no v1 row is reused.
...
`target/phase4-g3-incremental-materialization-20260822-v2/results-v2`, guarded
...
- Lost acknowledgement accepts exactly either (`reconciliation_outcome` =
  ...
  value passes. The deterministic v2 injection may yield `target`/`new`.
...
- Clone failure is a complete fallback reason and consumes zero permits. v2 has
...
No measured campaign is authorized by the dry run. A fresh v3 is required for
any pre-execution defect discovered after v2 is frozen.
