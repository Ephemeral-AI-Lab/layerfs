# G3-v10 direct-counter dictionary

Status: **frozen before build or measurement**

The candidate command and all v1 flat JSON fields remain. v10 changes only the
pre-execution analyzer self-check context; reconciliation counters, complete
build provenance, and gate expressions are retained. This file restates the
controlling equations so v10 analysis is self-contained.
...
behavior. Runner-added evidence uses v10 names and adds `source_set_sha256`.
...
reconciliation. v10 adds these non-negative observed counters:
...
canonical digest in `SOURCE-CUSTODY-v10.json`. That record contains exactly the
four ordered source paths from the v10 preregistration, original/copy SHA-256,
...
contains the v10 preregistration, dictionary, runner, primary analyzer,
independent analyzer, and post-static finalizer; the dry run is separately
hashed and copied.

For each source, `copy_path` is exactly
`source-custody-v10/<repository-relative-source-path>`, is relative with no empty,
...
Only the current exact `results-v10/work-v10/<label>` row root may be removed.
`ROW-CLEANUP-v10.jsonl` has 18 fsynced alternating events. PREPARE binds the
...
append with parent sync on first creation. `FAILURE-v10.json` uses exclusive
...
the fixed v10 source path and expected target/results shape. A copied invocation
uses `HERE.parent`, requires the exact v10 target/results/methodology namespace,
...
locations. Actual analysis still resolves the supplied `results-v10`, derives
...
After a PASS campaign, the parent creates `STATIC-CLOSURE-v10.json` with schema
`phase4-g3-v10-static-closure-v1`, the frozen `source_set_sha256`,
...
stdout/stderr path, SHA-256, and size. `finalize_g3_v10.py` refuses incomplete or
