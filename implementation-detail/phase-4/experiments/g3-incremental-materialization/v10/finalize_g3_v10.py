#!/usr/bin/env python3
"""Finalize and seal a completed G3-v10 campaign after parent static closure."""
...
MANIFEST = RESULTS / "PAYLOAD-MANIFEST-v10.tsv"
TERMINAL = RESULTS / "TERMINAL-v10.json"
VERIFICATION = RESULTS / "TERMINAL-VERIFICATION-v10.txt"
STATIC = RESULTS / "STATIC-CLOSURE-v10.json"
...
METHOD_NAMES = [
    ...
    "COUNTER-DICTIONARY-v10.md", "run_g3_v10.py", "analyze_g3_v10.py",
    "recompute_g3_v10.py", "finalize_g3_v10.py",
]
...
EXCLUDED = {"PAYLOAD-MANIFEST-v10.tsv", "TERMINAL-v10.json", "TERMINAL-VERIFICATION-v10.txt"}
...
def exact_source_copy(row, result_root=RESULTS):
    ...
    expected = f"source-custody-v10/{row.get('path')}"
...
def verify_static(source_set):
    ...
    if closure.get("schema") != "phase4-g3-v10-static-closure-v1" or closure.get("status") != "PASS" or closure.get("source_set_sha256") != source_set or closure.get("candidate_retained") is not True or [row.get("label") for row in commands] != REQUIRED_STATIC_LABELS:
...
def verify_row_cleanup(cleanup, records, raw_rows, results=None):
    ...
    expected_cleanup = cleanup.get("status") == "PASS" and cleanup.get("declared_root") == "work-v10" and cleanup.get("deletion_method") == method and cleanup.get("prepare_records") == cleanup.get("complete_records") == 9 and cleanup.get("row_cleanup_records") == 18 and cleanup.get("row_cleanup_labels") == ROW_LABELS and cleanup.get("all_row_roots_absent") is True and cleanup.get("work_root_absent") is True and cleanup.get("broad_deletion") is False and cleanup.get("durable_prepare_complete") is True and cleanup.get("peak_equation") == "max_individual_PREPARE_pre_delete_row_not_cumulative_sum" and all(cleanup.get(f"peak_{key}") == peaks[key] and cleanup.get(f"cumulative_{key}") == cumulative[key] for key in dimensions) and max(peaks.values()) <= 512 * 1024 * 1024
    if not expected_cleanup or (results is not None and (cleanup.get("row_cleanup_sha256") != sha256(results / "ROW-CLEANUP-v10.jsonl") or (results / "work-v10").exists())):
...
def verify_campaign_inputs():
    ...
    if any(path.exists() for path in (MANIFEST, TERMINAL, VERIFICATION)):
        raise RuntimeError("v10 final artifacts already exist")
    if (RESULTS / "FAILURE-v10.json").exists():
        raise RuntimeError("failed/revised campaign cannot be finalized")
    campaign = read_json(RESULTS / "CAMPAIGN-v10.json")
    primary = read_json(RESULTS / "G3-PRIMARY-ANALYSIS-v10.json")
    independent = read_json(RESULTS / "G3-INDEPENDENT-RECOMPUTATION-v10.json")
    source = read_json(RESULTS / "SOURCE-CUSTODY-v10.json")
    operand = read_json(RESULTS / "OPERAND-CUSTODY-v10.json")
    methods = read_json(RESULTS / "METHODOLOGY-CUSTODY-v10.json")
    cleanup = read_json(RESULTS / "CLEANUP-v10.json")
    raw = RESULTS / "rows-v10/G3-V10-RAW.jsonl"
    raw_rows = [json.loads(line) for line in raw.read_text().splitlines() if line]
    row_cleanup_path = RESULTS / "ROW-CLEANUP-v10.jsonl"
    row_cleanups = [json.loads(line) for line in row_cleanup_path.read_text().splitlines() if line]
    commands = json.loads((RESULTS / "COMMANDS-v10.json").read_text())
    chronology = [json.loads(line) for line in (RESULTS / "CHRONOLOGY-v10.jsonl").read_text().splitlines() if line]
    ...
    for row in method_rows:
        if sha256(HERE / row["path"]) != row["sha256"] or sha256(RESULTS / "methodology-v10" / row["path"]) != row["sha256"]:
            raise RuntimeError(f"method changed: {row['path']}")
    if sha256(DRY_RUN) != methods.get("dry_run_sha256") or sha256(RESULTS / "methodology-v10/DRY-RUN-v10.json") != methods.get("dry_run_sha256"):
        raise RuntimeError("dry-run custody mismatch")
    if cleanup.get("status") != "PASS" or cleanup.get("declared_root") != "work-v10" or cleanup.get("work_root_absent") is not True or cleanup.get("broad_deletion") is not False or (RESULTS / "work-v10").exists():
        raise RuntimeError("cleanup mismatch")
    hashes = {"raw_sha256": raw, "row_cleanup_sha256": row_cleanup_path, "primary_sha256": RESULTS / "G3-PRIMARY-ANALYSIS-v10.json", "independent_sha256": RESULTS / "G3-INDEPENDENT-RECOMPUTATION-v10.json", "cleanup_sha256": RESULTS / "CLEANUP-v10.json", "source_custody_sha256": RESULTS / "SOURCE-CUSTODY-v10.json", "binary_custody_sha256": RESULTS / "OPERAND-CUSTODY-v10.json"}
...
def finalize():
    ...
    terminal = {
        "schema": "phase4-g3-v10-terminal-v1", "status": "PASS",
        ...
        "campaign_sha256": sha256(RESULTS / "CAMPAIGN-v10.json"),
        "cleanup_sha256": sha256(RESULTS / "CLEANUP-v10.json"),
        "static_closure_sha256": sha256(STATIC),
        "source_custody_sha256": sha256(RESULTS / "SOURCE-CUSTODY-v10.json"),
        "binary_custody_sha256": sha256(RESULTS / "OPERAND-CUSTODY-v10.json"),
        "methodology_custody_sha256": sha256(RESULTS / "METHODOLOGY-CUSTODY-v10.json"),
    ...
    seal_existing()
    snapshot = {"schema": "phase4-g3-v10-terminal-verification-v1", "status": "PASS", "disposition": terminal["disposition"], **final_verification(len(manifest_rows), g2, terminal_hash, manifest_hash, source["source_set_sha256"], False)}
...
def self_check():
    assert HERE.parents[4] == REPO and EXCLUDED == {"PAYLOAD-MANIFEST-v10.tsv", "TERMINAL-v10.json", "TERMINAL-VERIFICATION-v10.txt"}
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        root = Path(directory) / "results-v10"; copied = root / "source-custody-v10/Cargo.lock"; copied.parent.mkdir(parents=True); copied.write_bytes((REPO / "Cargo.lock").read_bytes()); copied.chmod(0o400)
        source_hash = sha256(REPO / "Cargo.lock")
        record = {"path": "Cargo.lock", "sha256": source_hash, "size_bytes": (REPO / "Cargo.lock").stat().st_size, "source_mode": "0644", "copy_path": "source-custody-v10/Cargo.lock", "copy_sha256": source_hash, "copy_size_bytes": copied.stat().st_size, "copy_mode": "0400"}
        exact_source_copy(record, root)
        mutations = []
        missing = dict(record); missing.pop("copy_path"); mutations.append(missing)
        escaped = dict(record, copy_path="source-custody-v10/../../outside"); mutations.append(escaped)
...
def main():
    ...
    else: raise SystemExit("refusing: use --self-check or --execute after PASS STATIC-CLOSURE-v10.json")
