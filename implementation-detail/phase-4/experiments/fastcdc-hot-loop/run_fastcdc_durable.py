...
CANDIDATE = TARGET_ROOT / "operands-v1/phase4_create_edit_benchmark-fastcdc-exact-hot-loop"
ANALYZER = HERE / "analyze_fastcdc_durable.py"
...
def remaining(global_started_epoch_ns):
    return 119.0 - (time.time_ns() - global_started_epoch_ns) / 1_000_000_000


def run(command, label, global_started_epoch_ns, env=None, allow_analysis_failure=False):
    budget = remaining(global_started_epoch_ns)
    if budget <= 1:
...
def main():
    ...
    screen_clock = json.loads((SCREEN / "SCREEN-FINAL-CLOCK-v1.json").read_text())
    global_started = screen_clock["global_benchmark_started_epoch_ns"]
    ...
    try:
        ...
        for offset, (arm, executable) in enumerate((("A", CONTROL), ("B", CANDIDATE))):
            ...
            run([executable, "--fast-prepare", master_root, "104857600", "write", str(iteration)],
                f"prepare-master-{arm}", global_started)
        ...
        for spec in rows:
            ...
            completed = run(command, f"row-{spec['label']}", global_started, env)
        ...
        analyzed = run([sys.executable, ANALYZER, ROOT], "durable-analyzer", global_started,
                       allow_analysis_failure=True)
        if not analyzed.stdout.strip():
            raise RuntimeError("durable analyzer produced no disposition")
        clock = {"global_benchmark_started_epoch_ns": global_started,
                 "global_elapsed_ns_before_manifest": time.time_ns() - global_started,
                 "ceiling_ns": 119_000_000_000,
                 "within_ceiling_before_manifest": remaining(global_started) > 0}
        (ROOT / "DURABLE-CLOCK-v1.json").write_text(json.dumps(clock, indent=2, sort_keys=True) + "\n")
        manifest, entries = write_manifest()
        final = {"global_benchmark_elapsed_ns": time.time_ns() - global_started,
                 "global_benchmark_ceiling_ns": 119_000_000_000,
                 "within_global_ceiling": remaining(global_started) > 0,
        ...
        return 0 if final["within_global_ceiling"] else 124
    finally:
        if LOCK.exists():
            LOCK.rmdir()
