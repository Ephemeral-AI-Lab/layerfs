...
FREEZE = HERE / "method/SOURCE-FREEZE-v1.json"
INPUT_MANIFEST = HERE / "method/INPUT-MANIFEST-v1.json"
...
RESULT_NAMES = {"screen": "phase4-g5-warm-projection-v1-screen", "gate": "phase4-g5-warm-projection-v1-gate"}
AUTHORITATIVE = (
    HERE / "runner.py", PRIMARY, INDEPENDENT, HERE / "PREREGISTRATION-v1.md",
    HERE / "LIMITATIONS-v1.md", HERE / "method/SCHEDULE-v1.tsv",
    HERE / "method/EXPECTED-OUTCOMES-v1.tsv", HERE / "FOCUSED-TEST-ATTEMPTS-v1.json",
    INPUT_MANIFEST,
...
def prepare_inputs(executable, input_root):
    ...
    try:
        ...
        manifest = {"schema": "phase4-g5-2-input-manifest-v1", "status": "PASS", "executable_sha256": sha256(executable), "preparation_timing": "outside-campaign", "inputs": records}
...
def freeze(executable):
    ...
    receipt = {
        "schema": "phase4-g5-2-source-freeze-v1", "status": "PASS",
...
def forecast():
    ...
    receipt = {
        "schema": "phase4-g5-2-zero-row-forecast-v1", "status": "PASS" if total <= LIMITS["gate"] else "REVISE",
...
def acquire_lock(intent):
    ...
    try:
        os.write(descriptor, (compact(intent) + "\n").encode())
        os.fsync(descriptor)
        os.close(descriptor)
        descriptor = -1
...
def begin_process_evidence(process_root, command):
    ...
    write_json(process_root / "COMMAND.json", {"schema": "phase4-g5-2-process-command-v1", "argv": list(command)})


def persist_process_evidence(process_root, stdout, stderr, returncode, timed_out, rss_text):
    process_root = pathlib.Path(process_root)
    ...
    write_json(process_root / "RETURN.json", {"schema": "phase4-g5-2-process-return-v1", "returncode": returncode, "timed_out": timed_out})
    write_text(process_root / "RSS.txt", rss_text)
    try:
        parsed = json.loads(text_value(stdout))
        parsed_evidence = {"schema": "phase4-g5-2-parsed-product-receipt-v1", "status": "Available", "receipt": parsed}
    except Exception as error:
        parsed = None
        parsed_evidence = {"schema": "phase4-g5-2-parsed-product-receipt-v1", "status": "Unavailable", "error": f"{type(error).__name__}: {error}"}
    ...
    evidence = {"schema": "phase4-g5-2-lossless-process-evidence-v1", "status": "PASS" if returncode == 0 and not timed_out and parsed is not None and rss is not None else "REVISE", "returncode": returncode, "timed_out": timed_out, "maximum_resident_set_size": rss, "artifacts": [{"name": name, "bytes": (process_root / name).stat().st_size, "sha256": sha256(process_root / name)} for name in files]}
...
def campaign(phase):
    ...
    intent = {"schema": "phase4-g5-2-lock-intent-v1", "phase": phase, "result": str(result), "freeze_sha256": sha256(FREEZE), "pid": os.getpid()}
    ...
    try:
        ...
        for ordinal, mode in enumerate(CAMPAIGNS[phase], 1):
            ...
            try:
                ...
                clone = {"schema": "phase4-g5-2-clone-receipt-v1", "status": "PASS", "method": "APFSCloneCpC", "inventory_equal": source_inventory["sha256"] == attempt_inventory["sha256"], "source_inventory_sha256": source_inventory["sha256"], "attempt_inventory_sha256": attempt_inventory["sha256"]}
                ...
                write_json(process_root / "CLONE.json", {"schema": "phase4-g5-2-clone-receipt-v1", "status": "REVISE", "method": "APFSCloneCpC", "inventory_equal": False, "error": f"{type(clone_error).__name__}: {clone_error}"})
            ...
            evidence, product = persist_process_evidence(process_root, stdout, stderr, returncode, timed_out, rss_text)
            ...
            fsync_dir(result)
        envelope = {
            "schema": "phase4-g5-2-harness-row-v1", "status": "PASS", "phase": phase,
        ...
        preliminary_raw = result / "RAW-PRELIMINARY-v1.jsonl"
        write_text(preliminary_raw, compact(envelope) + "\n")
        preliminary_primary = result / "PRIMARY-PRELIMINARY-v1.json"
        preliminary_independent = result / "INDEPENDENT-PRELIMINARY-v1.json"
        ...
        release_lock()
        lock_held = False
        write_json(result / "LOCK-RELEASE-v1.json", {"schema": "phase4-g5-2-lock-release-v1", "status": "PASS", "lock_absent": not LOCK.exists(), "phase": phase})
        complete_wall = time.monotonic_ns() - started
        terminal = {"schema": "phase4-g5-2-terminal-v1", "status": "PASS" if complete_wall <= LIMITS[phase] else "REVISE", "phase": phase, "complete_wall_ns": complete_wall, "limit_ns": LIMITS[phase], "wall_scope": "CustodyCloneThreeProductsCleanupOneAnalyzerPairLockReleaseAndReceipt", "lock_released": True, "product_processes": 3, "product_rows": 3, "primary_decision_rows": 1, "preliminary_analyzer_agreement": True, "terminal_fixture_roots": 0}
        write_json(result / "TERMINAL-v1.json", terminal)
        envelope["analysis_stage"] = "final"
        envelope["terminal"] = terminal
        envelope["terminal_sha256"] = sha256(result / "TERMINAL-v1.json")
        raw = result / "RAW-v1.jsonl"
        write_text(raw, compact(envelope) + "\n")
        primary_path, independent_path = result / "PRIMARY-v1.json", result / "INDEPENDENT-v1.json"
        ...
        write_json(result / "FINAL-DECISION-v1.json", {"schema": "phase4-g5-2-final-decision-v1", "status": "PASS", "terminal_sha256": envelope["terminal_sha256"], "primary_sha256": sha256(primary_path), "independent_sha256": sha256(independent_path), "normalized_sha256": primary["normalized_sha256"]})
    except Exception as error:
        failure = error
    finally:
...
def synthetic_row():
    ...
    for mode, population in (("self-check", [1, 0]), ("screen-count", [1, 1]), ("screen", [2, 2])):
        ...
        exact_sparse = [
            ...
            for ordinal in range(1, exact + 1)
        ]
        latest_sparse = [
            {"plan": "Ranges", "parent_length": 10, "target_length": 10, "range_count": 1, "wall_ns": 5_000_000, "contention": False, "policy": "LatestFollowing", "ordinal": ordinal}
            for ordinal in range(min(latest, 2) + 1)
        ]
        value.update({
            ...
            "build_evidence": (
                ...
                + [
                    {"plan": "FullFallback", "parent_length": 10, "target_length": 11, "range_count": 0, "wall_ns": 300_000_000, "contention": False, "policy": "ExactEveryRoot", "ordinal": 4_294_967_295},
                    {"plan": "FullFallback", "parent_length": 11, "target_length": 12, "range_count": 0, "wall_ns": 500_000_000, "contention": True, "policy": "LatestFollowing", "ordinal": 4_294_967_295},
                ]
            ),
        })
        value["mode"] = mode
        records.append({"mode": mode, "product": value, "maximum_resident_set_size": 20_000_000, "clone": {"method": "APFSCloneCpC", "inventory_equal": True}})
    return {"schema": "phase4-g5-2-harness-row-v1", "status": "PASS", "phase": "screen", "analysis_stage": "preliminary", "product_processes": 3, "products": records, "maximum_resident_set_size": 20_000_000, "evidence_assembly_elapsed_ns": 1_000_000_000, "cache_state": "WarmUnknownPreparedFixtureAPFSClone", "cold_reopen_claim": False}
...
def self_check():
    with tempfile.TemporaryDirectory() as directory:
        ...
        for label, mutation in cases:
            ...
            if label == "policy-forgery":
                ...
                forged = {"plan": "Ranges", "parent_length": 10, "target_length": 10, "range_count": 1, "wall_ns": 5_000_000, "contention": False, "policy": "ExactEveryRoot", "ordinal": 3}
            ...
            if label == "terminal-forgery":
                terminal = {"schema": "phase4-g5-2-terminal-v1", "status": "PASS", "complete_wall_ns": LIMITS["screen"] + 1, "limit_ns": LIMITS["screen"], "lock_released": True, "terminal_fixture_roots": 0}
                row.update({"analysis_stage": "final", "terminal": terminal, "terminal_sha256": hashlib.sha256((compact(terminal) + "\n").encode()).hexdigest()})
            if label == "valid-final":
                terminal = {"schema": "phase4-g5-2-terminal-v1", "status": "PASS", "complete_wall_ns": 1_000_000_000, "limit_ns": LIMITS["screen"], "lock_released": True, "terminal_fixture_roots": 0}
                row.update({"analysis_stage": "final", "terminal": terminal, "terminal_sha256": hashlib.sha256((compact(terminal) + "\n").encode()).hexdigest()})
        ...
        write_json(process_root / "CLONE.json", {"schema": "phase4-g5-2-clone-receipt-v1", "status": "PASS", "method": "APFSCloneCpC", "inventory_equal": True})
    ...
    result = {"schema": "phase4-g5-2-runner-self-check-v1", "status": "PASS", "checks": len(cases) + 1, "product_processes": 0, "product_rows": 0, "partial_process_evidence_retained": True}
