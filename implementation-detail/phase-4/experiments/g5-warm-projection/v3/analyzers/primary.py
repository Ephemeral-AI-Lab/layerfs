...
def inspect(record, failures, authority, evidence_root):
    ...
    need([p.get("foreground_transactions"), p.get("foreground_commits"), p.get("reader_commit_autocommit"), p.get("reader_commit_scope_live"), p.get("foreground_commit_primary_code"), p.get("foreground_commit_extended_code")] == [1, 1, 1, 0, 0, 0] and p.get("contention_intervals_overlap") is True, "foreground-contention-tuple")
    ...
    need(record.get("clone", {}).get("method") == "APFSCloneCpC" and record.get("clone", {}).get("inventory_equal") is True and record.get("clone", {}).get("source_sealed_reverified") is True and record.get("clone", {}).get("private_attempt_permissions") == "WritableAfterExactCloneInventory", "clone/sealed-source")
