...
def attempt_clone_receipts_valid(clone, expected_attempt, process_evidence=None):
    ...
    settlement = clone.get("clone_settlement", {})
    settlement_ok = settlement.get("classification") == "APFSCloneDurablySettledBeforeInventory" and type(settlement.get("regular_file_fsyncs")) is int and settlement["regular_file_fsyncs"] > 0 and type(settlement.get("directory_fsyncs")) is int and settlement["directory_fsyncs"] > 0 and [settlement.get("result_parent_fsyncs"), settlement.get("inventory_observations"), settlement.get("sleep_calls"), settlement.get("retry_loops")] == [1, 1, 0, 0]
    physical = clone.get("physical_allocation_observations", {})
    limit = physical.get("limit_bytes")
    logical_ok = clone.get("custody_classification") == "LogicalCustodyHashPhysicalAllocationObservedSeparately" and clone.get("logical_inventory_equal") is True and clone.get("source_inventory_sha256") == clone.get("attempt_inventory_sha256") and isinstance(clone.get("source_inventory_sha256"), str) and len(clone["source_inventory_sha256"]) == 64 and type(limit) is int and limit == 10_000_000 and all(type(physical.get(name)) is int and 0 <= physical[name] <= limit for name in ("source_apparent_bytes", "source_allocated_bytes", "attempt_apparent_bytes", "attempt_allocated_bytes")) and clone.get("physical_allocation_within_bound") is True
    ...
    return logical_ok and settlement_ok and len(paths) == len(entries) == len(set(paths)) and all(isinstance(path, str) and not pathlib.PurePath(path).is_absolute() and ".." not in pathlib.PurePath(path).parts for path in paths) and modes and sum(row.get("kind") == "authority" for row in entries) == 1 and permissions.get("status") == "PASS" and permissions.get("classification") == "Directories0755Ordinary0644AuthoritySidecar0600NoSymlinks" and permissions.get("authority_files") == 1 and permissions.get("symlinks") == 0 and permissions.get("map_sha256") == permission_hash == clone.get("private_permission_map_sha256") and rebind.get("status") == "PASS" and rebind.get("scope") == "SealedCloneDirectoryPathRebindOnly" and rebind.get("field") == "directory" and rebind.get("changed_fields") == 1 and rebind.get("all_other_tsv_fields_byte_identical") is True and rebind.get("new_value") == expected_directory and rebind.get("old_value") != expected_directory and clone.get("rebind_receipt_sha256") == rebind_hash and evidence_ok
def pct(v, p):
    v = sorted(v)
