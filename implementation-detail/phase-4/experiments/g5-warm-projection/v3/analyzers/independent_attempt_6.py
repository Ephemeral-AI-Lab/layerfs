...
def private_clone_is_exact(clone, attempt, evidence=None):
    ...
    settled = clone.get("clone_settlement", {})
    settled_ok = settled.get("classification") == "APFSCloneDurablySettledBeforeInventory" and all(type(settled.get(name)) is int and settled[name] > 0 for name in ("regular_file_fsyncs", "directory_fsyncs")) and tuple(settled.get(name) for name in ("result_parent_fsyncs", "inventory_observations", "sleep_calls", "retry_loops")) == (1, 1, 0, 0)
    allocation = clone.get("physical_allocation_observations", {})
    cap = allocation.get("limit_bytes")
    custody_ok = clone.get("custody_classification") == "LogicalCustodyHashPhysicalAllocationObservedSeparately" and clone.get("logical_inventory_equal") is True and clone.get("source_inventory_sha256") == clone.get("attempt_inventory_sha256") and type(clone.get("source_inventory_sha256")) is str and len(clone["source_inventory_sha256"]) == 64 and type(cap) is int and cap == 10_000_000 and all(type(allocation.get(key)) is int and 0 <= allocation[key] <= cap for key in ("source_apparent_bytes", "source_allocated_bytes", "attempt_apparent_bytes", "attempt_allocated_bytes")) and clone.get("physical_allocation_within_bound") is True
