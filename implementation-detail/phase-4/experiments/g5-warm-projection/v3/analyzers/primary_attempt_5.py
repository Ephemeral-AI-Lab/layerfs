...
def attempt_clone_receipts_valid(clone, expected_attempt, process_evidence=None):
    ...
    return settlement_ok and len(paths) == len(entries) == len(set(paths)) and all(isinstance(path, str) and not pathlib.PurePath(path).is_absolute() and ".." not in pathlib.PurePath(path).parts for path in paths) and modes and sum(row.get("kind") == "authority" for row in entries) == 1 and permissions.get("status") == "PASS" and permissions.get("classification") == "Directories0755Ordinary0644AuthoritySidecar0600NoSymlinks" and permissions.get("authority_files") == 1 and permissions.get("symlinks") == 0 and permissions.get("map_sha256") == permission_hash == clone.get("private_permission_map_sha256") and rebind.get("status") == "PASS" and rebind.get("scope") == "SealedCloneDirectoryPathRebindOnly" and rebind.get("field") == "directory" and rebind.get("changed_fields") == 1 and rebind.get("all_other_tsv_fields_byte_identical") is True and rebind.get("new_value") == expected_directory and rebind.get("old_value") != expected_directory and clone.get("rebind_receipt_sha256") == rebind_hash and evidence_ok
def pct(v, p):
    v = sorted(v)
