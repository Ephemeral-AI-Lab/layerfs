...
def audit(record, method, evidence_root):
    ...
    check(tuple(p.get(k) for k in ("foreground_transactions", "foreground_commits", "reader_commit_autocommit", "reader_commit_scope_live", "foreground_commit_primary_code", "foreground_commit_extended_code")) == (1, 1, 1, 0, 0, 0) and p.get("contention_intervals_overlap") is True, "foreground-contention-tuple")
    ...
    check(record.get("clone", {}).get("method") == "APFSCloneCpC" and record.get("clone", {}).get("inventory_equal") is True and record.get("clone", {}).get("source_sealed_reverified") is True and record.get("clone", {}).get("private_attempt_permissions") == "WritableAfterExactCloneInventory", "clone/sealed-source")
