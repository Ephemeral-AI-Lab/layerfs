...
EXECUTION_PRIMARY = HERE / "analyzers/primary_attempt_5.py"
EXECUTION_INDEPENDENT = HERE / "analyzers/independent_attempt_5.py"
...
ATTEMPT_5_BINDING = HERE / "ATTEMPT-5-METHOD-BINDING-v3.json"
...
RESULT_NAMES = {"screen": "phase4-g5-warm-projection-v3-screen-attempt-4", "gate": "phase4-g5-warm-projection-v3-gate-attempt-5"}
AUTHORITATIVE = (
...
def verify_attempt_5_binding():
    binding = load_json(ATTEMPT_5_BINDING)
    expected = {
        "runner_attempt_5_sha256": sha256(pathlib.Path(__file__)),
        "frozen_runner_sha256": sha256(HERE / "runner.py"),
        "runner_attempt_4_sha256": sha256(HERE / "runner_attempt_4.py"),
        "attempt_4_binding_sha256": sha256(HERE / "ATTEMPT-4-METHOD-BINDING-v3.json"),
        "gate_attempt_4_disposition_sha256": sha256(HERE / "GATE-ATTEMPT-4-v3.json"),
        ...
        "primary_attempt_5_sha256": sha256(EXECUTION_PRIMARY),
        "independent_attempt_5_sha256": sha256(EXECUTION_INDEPENDENT),
    }
    if binding.get("status") != "PROSPECTIVE_BEFORE_GATE_ATTEMPT_5" or binding.get("scope") != "APFSCloneDurablySettledBeforeInventoryOnly" or binding.get("hashes") != expected or binding.get("screen_limit_ns") != LIMITS["screen"] or binding.get("gate_limit_ns") != LIMITS["gate"] or binding.get("mode_populations") != MODE_POPULATIONS or any(binding.get(name) is not False for name in ("threshold_change", "population_change", "product_change", "method_change")):
        raise RuntimeError("attempt-5 method binding mismatch")
    return binding
...
def bind_synthetic_evidence(case_root, row):
    for record in (*row["products"], *row.get("fault_runs", [])):
        ...
        record["clone"].update({"private_permission_receipt": permission, "private_permission_map_sha256": permission["map_sha256"], "rebind_receipt": rebind, "rebind_receipt_sha256": sha256(process_root / "REBIND.json"), "clone_settlement": settlement})
...
def self_check():
    ...
    result = {"schema": "phase4-g5-2-runner-self-check-v3-attempt-5", "status": "PASS", "checks": len(cases) + 12, "product_processes": 0, "product_rows": 0, "partial_process_evidence_retained": True, "directory_rebind_exact": True, "private_permission_map_exact": True, "contention_overlap_field_exact": True, "clone_settlement_exact": True}
...
def main():
    ...
    verify_attempt_5_binding()
    if args.action == "screen":
        raise RuntimeError("attempt 5 is gate-only and reuses attempt-4 screen PASS")
