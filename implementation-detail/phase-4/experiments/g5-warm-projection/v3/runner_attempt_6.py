...
EXECUTION_PRIMARY = HERE / "analyzers/primary_attempt_6.py"
EXECUTION_INDEPENDENT = HERE / "analyzers/independent_attempt_6.py"
...
ATTEMPT_6_BINDING = HERE / "ATTEMPT-6-METHOD-BINDING-v3.json"
...
RESULT_NAMES = {"screen": "phase4-g5-warm-projection-v3-screen-attempt-4", "gate": "phase4-g5-warm-projection-v3-gate-attempt-6"}
AUTHORITATIVE = (
...
def durably_settle_clone(root, result_parent):
    root, result_parent = pathlib.Path(root), pathlib.Path(result_parent)
    ...
    for path in sorted(directories, key=lambda value: len(value.parts), reverse=True):
        fsync_dir(path)
    fsync_dir(result_parent)
    return {
        "classification": "APFSCloneDurablySettledBeforeInventory",
        "regular_file_fsyncs": len(files), "directory_fsyncs": len(directories),
        "result_parent_fsyncs": 1, "inventory_observations": 1,
        "sleep_calls": 0, "retry_loops": 0,
    }
...
def logical_inventory_binding(value):
    entries = [({key: row[key] for key in ("path", "kind", "mode", "bytes", "sha256") if key in row}) for row in value["entries"]]
    payload = {"entries": entries, "embedded_token_edit_entries": value.get("embedded_token_edit_entries", [])}
    return {"classification": "LogicalCustodyHashPhysicalAllocationObservedSeparately", "sha256": hashlib.sha256(compact(payload).encode()).hexdigest(), "apparent_bytes": value["apparent_bytes"], "allocated_bytes_observed": value["allocated_bytes"]}


def verify_sealed_fixture(root, expected_inventory=None):
    root = pathlib.Path(root)
    observed = inventory(root)
    logical = logical_inventory_binding(observed)
    expected_logical = logical_inventory_binding(expected_inventory) if expected_inventory is not None else logical
    limit = load_json(METHOD_CONTRACT)["compact_fixture_limits"]["max_input_root_bytes"]
    if root.stat().st_mode & 0o7777 != 0o555 or any(row["mode"] != (0o444 if row["kind"] == "file" else 0o555) for row in observed["entries"]) or logical["sha256"] != expected_logical["sha256"] or any(type(logical[name]) is not int or logical[name] < 0 or logical[name] > limit for name in ("apparent_bytes", "allocated_bytes_observed")):
        raise RuntimeError("sealed fixture logical custody/mode/allocation bound mismatch")
    observed["logical_custody"] = logical
    return observed
...
def verify_sealed_manifest_inputs(manifest):
    ...
    for mode in modes:
        ...
        current.append(verify_sealed_fixture(root, record["inventory"]))
    limit = load_json(METHOD_CONTRACT)["compact_fixture_limits"]["max_input_root_bytes"]
    if len(set(roots)) != 1 or roots[0].stat().st_mode & 0o7777 != 0o555 or sum(row["apparent_bytes"] for row in current) > limit or sum(row["allocated_bytes"] for row in current) > limit:
        raise RuntimeError("sealed input root mismatch")
    return {"classification": "LogicalCustodyHashPhysicalAllocationObservedSeparately", "apparent_bytes": sum(row["apparent_bytes"] for row in current), "allocated_bytes_observed": sum(row["allocated_bytes"] for row in current), "limit_bytes": limit}
...
def verify_attempt_6_binding():
    binding = load_json(ATTEMPT_6_BINDING)
    expected = {
        "runner_attempt_6_sha256": sha256(pathlib.Path(__file__)),
        "frozen_runner_sha256": sha256(HERE / "runner.py"),
        "runner_attempt_5_sha256": sha256(HERE / "runner_attempt_5.py"),
        "attempt_5_binding_sha256": sha256(HERE / "ATTEMPT-5-METHOD-BINDING-v3.json"),
        "gate_attempt_5_disposition_sha256": sha256(HERE / "GATE-ATTEMPT-5-v3.json"),
        "screen_attempt_4_terminal_sha256": sha256(TARGET / "phase4-g5-warm-projection-v3-screen-attempt-4/TERMINAL-v3.json"),
        "screen_attempt_4_final_decision_sha256": sha256(TARGET / "phase4-g5-warm-projection-v3-screen-attempt-4/FINAL-DECISION-v3.json"),
        "screen_attempt_4_primary_sha256": sha256(TARGET / "phase4-g5-warm-projection-v3-screen-attempt-4/PRIMARY-v3.json"),
        "screen_attempt_4_independent_sha256": sha256(TARGET / "phase4-g5-warm-projection-v3-screen-attempt-4/INDEPENDENT-v3.json"),
        ...
        "primary_attempt_6_sha256": sha256(EXECUTION_PRIMARY),
        "independent_attempt_6_sha256": sha256(EXECUTION_INDEPENDENT),
    }
    if binding.get("status") != "PROSPECTIVE_BEFORE_GATE_ATTEMPT_6" or binding.get("scope") != "LogicalCustodyHashPhysicalAllocationObservedSeparatelyOnly" or binding.get("hashes") != expected or binding.get("screen_limit_ns") != LIMITS["screen"] or binding.get("gate_limit_ns") != LIMITS["gate"] or binding.get("mode_populations") != MODE_POPULATIONS or any(binding.get(name) is not False for name in ("threshold_change", "population_change", "product_change", "source_change", "method_decision_change")):
        raise RuntimeError("attempt-6 method binding mismatch")
    return binding
...
def campaign(phase):
    ...
    try:
        ...
        for ordinal, mode in enumerate(CAMPAIGNS[phase], 1):
            ...
            settlement = None
            try:
                source_inventory = verify_sealed_fixture(source, manifest["inputs"][mode]["inventory"])
                subprocess.run(["/bin/cp", "-cR", str(source), str(attempt)], check=True, timeout=10)
                settlement = durably_settle_clone(attempt, result)
                attempt_inventory = inventory(attempt)
                source_logical, attempt_logical = logical_inventory_binding(source_inventory), logical_inventory_binding(attempt_inventory)
                allocation_limit = load_json(METHOD_CONTRACT)["compact_fixture_limits"]["max_input_root_bytes"]
                physical = {"source_apparent_bytes": source_inventory["apparent_bytes"], "source_allocated_bytes": source_inventory["allocated_bytes"], "attempt_apparent_bytes": attempt_inventory["apparent_bytes"], "attempt_allocated_bytes": attempt_inventory["allocated_bytes"], "limit_bytes": allocation_limit}
                clone = {"schema": "phase4-g5-2-clone-receipt-v3", "status": "PASS", "method": "APFSCloneCpC", "custody_classification": "LogicalCustodyHashPhysicalAllocationObservedSeparately", "inventory_equal": source_logical["sha256"] == attempt_logical["sha256"], "logical_inventory_equal": source_logical["sha256"] == attempt_logical["sha256"], "source_inventory_sha256": source_logical["sha256"], "attempt_inventory_sha256": attempt_logical["sha256"], "physical_allocation_observations": physical, "physical_allocation_within_bound": all(type(value) is int and 0 <= value <= allocation_limit for key, value in physical.items() if key != "limit_bytes"), "source_sealed_reverified": True, "private_attempt_permissions": "WritableAfterExactCloneInventory", "clone_settlement": settlement}
                if not clone["inventory_equal"] or not clone["physical_allocation_within_bound"]:
                ...
                write_json(process_root / "CLONE.json", {"schema": "phase4-g5-2-clone-receipt-v3", "status": "REVISE", "method": "APFSCloneCpC", "inventory_equal": False, "clone_settlement": settlement, "error": f"{type(clone_error).__name__}: {clone_error}"})
        ...
        for ordinal, (fault_case, mode) in enumerate(FAULT_RUN_MODES.items(), 1):
            ...
            settlement = None
            try:
                source_inventory = verify_sealed_fixture(fault_source, manifest["inputs"]["self-check"]["inventory"])
...
def bind_synthetic_evidence(case_root, row):
    for record in (*row["products"], *row.get("fault_runs", [])):
        ...
        settlement = {"classification": "APFSCloneDurablySettledBeforeInventory", "regular_file_fsyncs": 1, "directory_fsyncs": 1, "result_parent_fsyncs": 1, "inventory_observations": 1, "sleep_calls": 0, "retry_loops": 0}
        physical = {"source_apparent_bytes": 1, "source_allocated_bytes": 4096, "attempt_apparent_bytes": 1, "attempt_allocated_bytes": 4096, "limit_bytes": 10_000_000}
        record["clone"].update({"private_permission_receipt": permission, "private_permission_map_sha256": permission["map_sha256"], "rebind_receipt": rebind, "rebind_receipt_sha256": sha256(process_root / "REBIND.json"), "clone_settlement": settlement, "custody_classification": "LogicalCustodyHashPhysicalAllocationObservedSeparately", "logical_inventory_equal": True, "source_inventory_sha256": "c" * 64, "attempt_inventory_sha256": "c" * 64, "physical_allocation_observations": physical, "physical_allocation_within_bound": True})
...
def self_check():
    with tempfile.TemporaryDirectory() as directory:
        ...
        for label, mutation in cases:
            ...
            if label == "physical-custody":
                record = row["products"][-1]
                record["clone"]["physical_allocation_within_bound"] = False
                write_json(case_root / record["process_evidence_path"] / "CLONE.json", {"schema": "phase4-g5-2-clone-receipt-v3", "status": "PASS", **record["clone"]})
                resign_synthetic_artifact(case_root, record, "CLONE.json")
    ...
    result = {"schema": "phase4-g5-2-runner-self-check-v3-attempt-6", "status": "PASS", "checks": len(cases) + 13, "product_processes": 0, "product_rows": 0, "partial_process_evidence_retained": True, "directory_rebind_exact": True, "private_permission_map_exact": True, "contention_overlap_field_exact": True, "clone_settlement_exact": True, "logical_custody_physical_observation_separated": True}
...
def main():
    ...
    verify_attempt_6_binding()
    if args.action not in ("self-check", "gate"):
        raise RuntimeError("attempt 6 is gate-only and reuses attempt-4 screen PASS")
