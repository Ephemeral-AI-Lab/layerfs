...
G5_1_V26_TERMINAL = HERE.parents[1] / "g5-trusted-reopen-edit/v26/G5-1-TERMINAL-AUDIT-v26.json"
...
RESULT_NAMES = {"screen": "phase4-g5-warm-projection-v3-screen", "gate": "phase4-g5-warm-projection-v3-gate"}
AUTHORITATIVE = (
    ...
    HERE / "READINESS-PLAN-v3.md", METHOD_CONTRACT, SOURCE_FAULT_PROOFS, FOCUSED_FAULT_EXECUTION, FINAL_SOURCE_BINDING, V2_DISPOSITION,
    V3_READINESS, G5_1_V26_TERMINAL, INPUT_MANIFEST,
...
def inventory(root):
    ...
    if len(descriptors) == 1:
        try:
            ...
            embedded_too_large = True
    if len(descriptors) != 1 or len(files) > limits["max_files"] or sum(row["bytes"] for row in files) > limits["max_aggregate_bytes"] or descriptors[0].stat().st_size > limits["max_token_or_edit_bytes"] or embedded_too_large or any(row["bytes"] > limits["max_token_or_edit_bytes"] for row in files if row["path"].endswith((".edge-token", ".edit"))):
        raise RuntimeError("compact fixture count/size/aggregate bound exceeded")
    return {"schema": "phase4-g5-2-compact-fixture-inventory-v3", "classification": "ParentLatestTokenEditNoPerRevisionSources", "entries": rows, "embedded_token_edit_entries": embedded, "embedded_token_edit_aggregate_bytes": sum(item["bytes"] for item in embedded), "sha256": hashlib.sha256(compact({"entries": rows, "embedded_token_edit_entries": embedded}).encode()).hexdigest()}
...
def require_promotion_authorized():
    ...
    g5_1 = load_json(G5_1_V26_TERMINAL) if G5_1_V26_TERMINAL.is_file() else {}
    if (
...
def prepare_inputs(executable, input_root):
    ...
    try:
        ...
        manifest = {"schema": "phase4-g5-2-input-manifest-v3", "status": "PASS", "executable_sha256": sha256(executable), "preparation_timing": "outside-campaign", "inputs": records}
    ...
    try:
        ...
        for ordinal, mode in enumerate(CAMPAIGNS[phase], 1):
            ...
            try:
                ...
                clone = {"schema": "phase4-g5-2-clone-receipt-v3", "status": "PASS", "method": "APFSCloneCpC", "inventory_equal": source_inventory["sha256"] == attempt_inventory["sha256"], "source_inventory_sha256": source_inventory["sha256"], "attempt_inventory_sha256": attempt_inventory["sha256"]}
...
def synthetic_row():
    product = {
        "schema": PRODUCT_SCHEMA, "status": "PASS", "worker_count": 1,
    ...
    for mode, population in (("self-check", [1, 0]), ("screen-count", [1, 1]), ("screen", [2, 2])):
        ...
        value["mode"] = mode
        records.append({"mode": mode, "attempt_root": f"/synthetic/{mode}", "product": value, "maximum_resident_set_size": 20_000_000, "clone": {"method": "APFSCloneCpC", "inventory_equal": True}, "process_evidence_path": f"PROCESS-{len(records)+1:02d}-{mode}", "process_evidence_sha256": "0" * 64})
    fault_runs = []
...
def self_check():
    with tempfile.TemporaryDirectory() as directory:
        ...
        cases = [("valid", None), ("valid-final", None), ("product-schema", ("product", "schema", "phase4-g5-projection-suite-v1")), ("pending", ("product", "max_pending", 2)), ("commit", ("product", "sqlite_commits", 1)), ("contention-tuple", ("product", "reader_commit_autocommit", 0)), ("barrier-scope", ("product", "reader_barrier_scope_live", 0)), ("commit-scope", ("product", "reader_commit_scope_live", 1)), ("t0-t4", ("product", "end_to_end_worker_start_t3_ns", 2)), ("canonical-transaction", ("product", "end_to_end_canonical_transactions", 0)), ("descriptor", ("product", "terminal_descriptor_classification", "Forged")), ("storage", ("product", "terminal_storage_allocated_bytes", -1)), ("root", ("product", "projected_root", "wrong")), ("rss", (None, "maximum_resident_set_size", 40_000_000)), ("terminal", ("product", "terminal_workers", 1)), ("population", ("product", "exact_every_root_population", 3)), ("claim", (None, "cold_reopen_claim", True)), ("plural-fault-receipts", None), ("fault-mode-skipped", None), ("fault-matrix-case-mismatch", None), ("forged-source-proof", None), ("stdout-mismatch", None), ("command-mismatch", None), ("policy-forgery", None), ("policy-route-constant", None), ("route-forgery", None), ("terminal-forgery", None), ("nested-artifact-forgery", None)]
        ...
        if any("schedule_sha256" not in path.read_text() or "fault_matrix_sha256" not in path.read_text() for path in (pathlib.Path(__file__), PRIMARY, INDEPENDENT)):
        ...
        if binding.get("status") != "PASS" or binding.get("source", {}).get("sha256") != sha256(REPO / "crates/layerfs-engine/src/bin/phase4_g3_materialization.rs") or binding.get("executable", {}).get("sha256") != sha256(REPO / "target/debug/phase4_create_edit_benchmark") or binding.get("focused_tests", {}).get("passed") != 24:
    ...
    result = {"schema": "phase4-g5-2-runner-self-check-v3", "status": "PASS", "checks": len(cases) + 8, "product_processes": 0, "product_rows": 0, "partial_process_evidence_retained": True}
