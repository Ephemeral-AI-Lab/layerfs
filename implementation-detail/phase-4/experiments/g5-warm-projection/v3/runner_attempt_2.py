...
ATTEMPT_2_BINDING = HERE / "ATTEMPT-2-METHOD-BINDING-v3.json"
...
RESULT_NAMES = {"screen": "phase4-g5-warm-projection-v3-screen-attempt-2", "gate": "phase4-g5-warm-projection-v3-gate-attempt-2"}
AUTHORITATIVE = (
...
def verify_attempt_2_binding():
    binding = load_json(ATTEMPT_2_BINDING)
    expected = {
    ...
    try:
        ...
        for ordinal, mode in enumerate(CAMPAIGNS[phase], 1):
            ...
            try:
                ...
                clone.update({"rebind_receipt": rebind, "rebind_receipt_sha256": rebind_sha256})
                write_json(process_root / "CLONE.json", clone)
...
def self_check():
    with tempfile.TemporaryDirectory() as directory:
        ...
        if receipt.get("changed_fields") != 1 or receipt.get("all_other_tsv_fields_byte_identical") is not True or command.get("fixture_rebind_receipt_sha256") != receipt_sha256 or not (rebind_process / "REBIND.json").is_file():
            raise RuntimeError("attempt-2 clone directory rebind self-check failed")
        contract = load_json(METHOD_CONTRACT)
    ...
    result = {"schema": "phase4-g5-2-runner-self-check-v3-attempt-2", "status": "PASS", "checks": len(cases) + 10, "product_processes": 0, "product_rows": 0, "partial_process_evidence_retained": True, "directory_rebind_exact": True}
