...
def recompute(row):
    ...
    if row.get("schema") != "phase4-g5-2-harness-row-v2" or row.get("status") != "PASS": bad.append("row-schema/status")
    ...
    if stage == "preliminary":
        ...
        okay = isinstance(terminal, dict) and terminal.get("schema") == "phase4-g5-2-terminal-v2" and terminal.get("status") == "PASS" and terminal.get("complete_wall_ns", -1) <= terminal.get("limit_ns", -2) and terminal.get("lock_released") is True and terminal.get("terminal_fixture_roots") == 0
    ...
    return {"schema": "phase4-g5-2-independent-v2", "normalized": normalized, "normalized_sha256": hashlib.sha256(canonical(normalized).encode()).hexdigest()}

def main():
    rows = [json.loads(x) for x in pathlib.Path(sys.argv[1]).read_text().splitlines() if x.strip()]
    report = recompute(rows[0]) if len(rows) == 1 else {"schema": "phase4-g5-2-independent-v2", "normalized": {"status": "REVISE", "hard_failures": ["row-count"]}}
