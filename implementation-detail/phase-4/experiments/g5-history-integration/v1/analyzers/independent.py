...
def method(unfrozen=False):
    contract_file, reused_file = BASE / "method/METHOD-CONTRACT-v1.json", BASE / "REUSED-AUTHORITY-v1.json"
    ...
    schedule_file, expected_file, coverage_file = BASE / "method/SCHEDULE-v1.tsv", BASE / "method/EXPECTED-OUTCOMES-v1.tsv", BASE / "COVERAGE-MAP-v1.tsv"
    ...
    if not unfrozen:
        frozen = json.loads((BASE / "method/SOURCE-FREEZE-v1.json").read_text())
...
def main():
    ...
    report = {"schema": "phase4-g5-3-independent-v1", "normalized": normalized, "normalized_sha256": hashlib.sha256(canonical(normalized).encode()).hexdigest()}
