...
def authority(self_check=False):
    contract_path, reused_path = HERE / "method/METHOD-CONTRACT-v1.json", HERE / "REUSED-AUTHORITY-v1.json"
    ...
    schedule_path, expected_path, coverage_path = HERE / "method/SCHEDULE-v1.tsv", HERE / "method/EXPECTED-OUTCOMES-v1.tsv", HERE / "COVERAGE-MAP-v1.tsv"
    ...
    if not self_check:
        freeze = json.loads((HERE / "method/SOURCE-FREEZE-v1.json").read_text())
...
def main():
    ...
    report = {"schema": "phase4-g5-3-primary-v1", "normalized": normalized, "normalized_sha256": hashlib.sha256(compact(normalized).encode()).hexdigest()}
