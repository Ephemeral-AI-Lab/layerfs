...
def method(unfrozen=False):
    contract_file, reused_file = BASE / "method/METHOD-CONTRACT-v2.json", BASE / "REUSED-AUTHORITY-v2.json"
    ...
    schedule_file, expected_file, coverage_file = BASE / "method/SCHEDULE-v2.tsv", BASE / "method/EXPECTED-OUTCOMES-v2.tsv", BASE / "COVERAGE-MAP-v2.tsv"
    ...
    if not unfrozen:
        frozen = json.loads((BASE / "method/SOURCE-FREEZE-ATTEMPT-2-v2.json").read_text())
...
def audit(row, contract):
    ...
    return {"status": "PASS" if not bad else "REVISE", "phase": phase, "analysis_stage": stage, "hard_failures": sorted(bad), "product_processes": row.get("product_processes"), "history_revisions": 1 + len(fill) if type(fill) is list else None, "checkpoint_revisions": [point.get("revision") for point in points] if type(points) is list else None, "terminal_revision": end.get("revision"), "aba_root": aba.get("final_root"), "rss_bytes": row.get("maximum_resident_set_size"), "complete_wall_ns": elapsed}

def main():
    source, target = map(pathlib.Path, sys.argv[1:3])
    normalized = audit(json.loads(source.read_text()), method("--self-check-authority" in sys.argv[3:]))
    report = {"schema": "phase4-g5-3-independent-attempt-2-v2", "normalized": normalized, "normalized_sha256": hashlib.sha256(canonical(normalized).encode()).hexdigest()}
