...
def authority(self_check=False):
    contract_path, reused_path = HERE / "method/METHOD-CONTRACT-v2.json", HERE / "REUSED-AUTHORITY-v2.json"
    ...
    schedule_path, expected_path, coverage_path = HERE / "method/SCHEDULE-v2.tsv", HERE / "method/EXPECTED-OUTCOMES-v2.tsv", HERE / "COVERAGE-MAP-v2.tsv"
    ...
    if not self_check:
        freeze = json.loads((HERE / "method/SOURCE-FREEZE-ATTEMPT-2-v2.json").read_text())
...
def main():
    ...
    report = {"schema": "phase4-g5-3-primary-attempt-2-v2", "normalized": normalized, "normalized_sha256": hashlib.sha256(compact(normalized).encode()).hexdigest()}
