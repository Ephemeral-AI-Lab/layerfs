...
def authority(self_check=False):
    ...
    if not self_check:
        freeze = json.loads((HERE / "method/SOURCE-FREEZE-v2.json").read_text())
...
def main():
    ...
    report = {"schema": "phase4-g5-3-primary-v2", "normalized": normalized, "normalized_sha256": hashlib.sha256(compact(normalized).encode()).hexdigest()}
