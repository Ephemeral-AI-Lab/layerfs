...
def method(unfrozen=False):
    ...
    if not unfrozen:
        frozen = json.loads((BASE / "method/SOURCE-FREEZE-v2.json").read_text())
...
def main():
    ...
    report = {"schema": "phase4-g5-3-independent-v2", "normalized": normalized, "normalized_sha256": hashlib.sha256(canonical(normalized).encode()).hexdigest()}
