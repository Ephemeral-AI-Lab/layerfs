...
const G5_DEFAULT_CACHE_PAGES: i64 = 2_000;
const G5_CONCURRENCY_CACHE_PAGES: i64 = 1_280;
const G5_CONCURRENCY_CONNECTIONS: u64 = 3;
const G5_SQLITE_PAGE_SIZE_BYTES: i64 = 4_096;
const G5_CONCURRENCY_CACHE_REDUCTION_BYTES: u64 = 8_847_360;
...
struct G5ConcurrencyObservation {
    ...
    locked_errors: u64,
    q_high_water: u64,
    writer_cache_size_pages: i64,
    reader_one_cache_size_pages: i64,
    reader_two_cache_size_pages: i64,
    sqlite_page_size_bytes: i64,
    configured_cache_reduction_bytes: u64,
    simultaneous_connections_high_water: u64,
    simultaneous_connections_terminal: u64,
}
...
struct G5ConnectionLease {
    active: std::sync::Arc<std::sync::atomic::AtomicU64>,
}

impl G5ConnectionLease {
    fn observe(
        active: std::sync::Arc<std::sync::atomic::AtomicU64>,
        high_water: &std::sync::atomic::AtomicU64,
    ) -> Self {
        let current = active.fetch_add(1, std::sync::atomic::Ordering::SeqCst) + 1;
        high_water.fetch_max(current, std::sync::atomic::Ordering::SeqCst);
        Self { active }
    }
}

impl Drop for G5ConnectionLease {
    fn drop(&mut self) {
        let prior = self.active.fetch_sub(1, std::sync::atomic::Ordering::SeqCst);
        debug_assert!(prior > 0);
    }
}

fn g5_configure_concurrency_connection(store: &Store) -> AnyResult<(i64, i64)> {
    store.connection.pragma_update(
        None,
        "cache_size",
        G5_CONCURRENCY_CACHE_PAGES,
    )?;
    let cache_size_pages = store
        .connection
        .query_row("PRAGMA cache_size", [], |row| row.get::<_, i64>(0))?;
    let page_size_bytes = store
        .connection
        .query_row("PRAGMA page_size", [], |row| row.get::<_, i64>(0))?;
    if cache_size_pages != G5_CONCURRENCY_CACHE_PAGES
        || page_size_bytes != G5_SQLITE_PAGE_SIZE_BYTES
    {
        return Err(CoreError::ProfileMismatch.into());
    }
    Ok((cache_size_pages, page_size_bytes))
}
...
fn g5_reader_observation(
    ...
    connection_high_water: std::sync::Arc<std::sync::atomic::AtomicU64>,
) -> Result<(ObjectId, ObjectId, u64, i64), String> {
    let store = Store::open_existing_read_only(database, SELECTED_PROFILE)
        .map_err(|error| format!("reader open: {error:?}"))?;
    let lease = G5ConnectionLease::observe(active_connections, &connection_high_water);
    let (cache_size_pages, _) = g5_configure_concurrency_connection(&store)
        .map_err(|error| format!("reader cache profile: {error:?}"))?;
...
fn g5_concurrency_10m(
    ...
    let active_connections = std::sync::Arc::new(std::sync::atomic::AtomicU64::new(0));
    let connection_high_water = std::sync::Arc::new(std::sync::atomic::AtomicU64::new(0));
    let mut writer = Store::open(&database, SELECTED_PROFILE)?;
    let (writer_cache_size_pages, sqlite_page_size_bytes) =
        g5_configure_concurrency_connection(&writer)?;
    let writer_lease = G5ConnectionLease::observe(
        std::sync::Arc::clone(&active_connections),
    ...
    let active_one = std::sync::Arc::clone(&active_connections);
    let high_water_one = std::sync::Arc::clone(&connection_high_water);
    ...
    let active_two = std::sync::Arc::clone(&active_connections);
    let high_water_two = std::sync::Arc::clone(&connection_high_water);
    ...
    let simultaneous_connections_high_water =
        connection_high_water.load(std::sync::atomic::Ordering::SeqCst);
    if simultaneous_connections_high_water != G5_CONCURRENCY_CONNECTIONS
        || active_connections.load(std::sync::atomic::Ordering::SeqCst)
            != G5_CONCURRENCY_CONNECTIONS
    {
        return Err(CoreError::PublicationConflict.into());
    }
    ...
    if operation.transactions != 1
        ...
        || one.1 != next.root
        || two.1 != next.root
        || one.3 != G5_CONCURRENCY_CACHE_PAGES
        || two.3 != G5_CONCURRENCY_CACHE_PAGES
        || active_connections.load(std::sync::atomic::Ordering::SeqCst) != 1
    {
        return Err(CoreError::PublicationConflict.into());
    }
    drop(writer);
    drop(writer_lease);
    let simultaneous_connections_terminal =
        active_connections.load(std::sync::atomic::Ordering::SeqCst);
    if simultaneous_connections_terminal != 0 {
        return Err(CoreError::PublicationConflict.into());
    }
    remove_sqlite_image(&database)?;
    ...
    Ok(G5ConcurrencyObservation {
        ...
        writer_cache_size_pages,
        reader_one_cache_size_pages: one.3,
        reader_two_cache_size_pages: two.3,
        sqlite_page_size_bytes,
        configured_cache_reduction_bytes: G5_CONCURRENCY_CACHE_REDUCTION_BYTES,
        simultaneous_connections_high_water,
        simultaneous_connections_terminal,
    })
}

fn g5_release_closed_sqlite_memory() {
    // SAFETY: the history Store has been dropped, so this only releases
    // process-global SQLite memory that is no longer owned by a connection.
    unsafe {
    ...
    macro_rules! write_report {
        ($output:expr, $whole_q:expr) => {{
            ...
            write!(
                ...
                concurrency.q_high_water,
                concurrency.writer_cache_size_pages,
                concurrency.reader_one_cache_size_pages,
                concurrency.reader_two_cache_size_pages,
                concurrency.sqlite_page_size_bytes,
                concurrency.configured_cache_reduction_bytes,
                concurrency.simultaneous_connections_high_water,
                concurrency.simultaneous_connections_terminal,
            )?;
            write!(
                output,
...
mod g5_history_integration_tests {
    ...
    fn exact_checkpoint_range_and_revision_sources_are_bounded() {
        ...
        fs::set_permissions(&source, fs::Permissions::from_mode(0o444)).expect("sealed source");
        ...
        assert_eq!(fs::metadata(&source).unwrap().permissions().mode() & 0o7777, 0o444);
        for revision in [&revision_one, &revision_two] {
            assert_eq!(fs::metadata(revision).unwrap().permissions().mode() & 0o7777, 0o600);
            drop(OpenOptions::new().write(true).open(revision).expect("writable private copy"));
        }
    }

    #[test]
    fn completed_history_store_closes_before_independent_concurrency() {
        let source = include_str!("history.rs");
        let terminal = source.find("let terminal_snapshot =").expect("terminal snapshot");
        let source = &source[terminal..];
        let close = source.find("drop(store);").expect("history Store close");
        let release = source
            .find("g5_release_closed_sqlite_memory();")
            .expect("SQLite memory release");
        let concurrency = source
            .find("g5_concurrency_10m(work_root, concurrency_source)?")
            .expect("independent concurrency");
        assert!(close < release && release < concurrency);
    }

    #[test]
    fn concurrency_cache_subset_and_aggregate_equation_are_exact() {
        let (_root, _) = test_source();
        let database = _root.0.join("cache-subset.sqlite");
        let writer = Store::open(&database, SELECTED_PROFILE).expect("writer");
        let (writer_cache, page_size) =
            g5_configure_concurrency_connection(&writer).expect("writer cache");
        assert_eq!(writer_cache, G5_CONCURRENCY_CACHE_PAGES);
        assert_eq!(page_size, G5_SQLITE_PAGE_SIZE_BYTES);
        drop(writer);

        let reader = Store::open_existing_read_only(&database, SELECTED_PROFILE).expect("reader");
        let (reader_cache, reader_page_size) =
            g5_configure_concurrency_connection(&reader).expect("reader cache");
        assert_eq!(reader_cache, G5_CONCURRENCY_CACHE_PAGES);
        assert_eq!(reader_page_size, G5_SQLITE_PAGE_SIZE_BYTES);
        assert_eq!(
            G5_CONCURRENCY_CACHE_REDUCTION_BYTES,
            u64::try_from(G5_DEFAULT_CACHE_PAGES - G5_CONCURRENCY_CACHE_PAGES)
                .unwrap()
                * u64::try_from(G5_SQLITE_PAGE_SIZE_BYTES).unwrap()
                * G5_CONCURRENCY_CONNECTIONS
        );
        drop(reader);
