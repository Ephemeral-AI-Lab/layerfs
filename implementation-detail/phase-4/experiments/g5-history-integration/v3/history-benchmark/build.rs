...
fn main() {
    ...
    let history_g3_source = manifest.join("src/history_g3.rs");
    ...
    let g3 = fs::read_to_string(&g3_source).unwrap();
    let marker = "\n#[cfg(test)]\nmod tests {";
    let (implementation, tests) = g3.split_once(marker).unwrap();
    let g3 = format!(
        "{implementation}\n{}\n{marker}{tests}",
        fs::read_to_string(&history_g3_source).unwrap()
    );
    ...
    println!("cargo:rerun-if-changed={}", history_g3_source.display());
}
