...
CONTRACT=HERE/"method/METHOD-CONTRACT-v3.json"; SCHEDULE=HERE/"method/SCHEDULE-v3.tsv"; EXPECTED=HERE/"method/EXPECTED-OUTCOMES-v3.tsv"; COVERAGE=HERE/"COVERAGE-MAP-v3.tsv"; REUSED=HERE/"REUSED-AUTHORITY-v3.json"
PRIMARY=HERE/"analyzers/primary.py"; INDEPENDENT=HERE/"analyzers/independent.py"; READINESS=HERE/"PREMEASUREMENT-READINESS-v3.json"; INPUT_MANIFEST=HERE/"method/INPUT-ADOPTION-MANIFEST-v3.json"; PREPARATION_TERMINAL=HERE/"method/INPUT-ADOPTION-TERMINAL-v3.json"; FREEZE=HERE/"method/SOURCE-FREEZE-v3.json"; FORECAST=HERE/"ZERO-ROW-FORECAST-v3.json"
FOCUSED=HERE/"FOCUSED-TEST-ATTEMPTS-v3.json"; DISPOSITION=HERE/"PREMEASUREMENT-REVISE-v3.json"; FOCUSED_ATTEMPT=HERE/"evidence/focused-attempt-1"
STATIC_CLOSURE=HERE/"POST-SCREEN-STATIC-CLOSURE-v3.json"; STATIC_EVIDENCE=HERE/"evidence/post-screen-static-closure"
LOCK=TARGET/"phase4-g5-history-integration"/"BENCHMARK_LOCK"; RESULTS={"screen":"phase4-g5-history-integration-v3-screen","gate":"phase4-g5-history-integration-v3-gate"}
...
def verify_readiness_method(ready):
    files={"coverage_map":COVERAGE,"expected_outcomes":EXPECTED,"focused_ledger":FOCUSED,"independent":INDEPENDENT,"limitations":HERE/"LIMITATIONS-v3.md","method_contract":CONTRACT,"premeasurement_revise":DISPOSITION,"preregistration":HERE/"PREREGISTRATION-v3.md","primary":PRIMARY,"reused_authority":REUSED,"runner":pathlib.Path(__file__),"schedule":SCHEDULE}
...
def verify_reused_input_source(contract):
    root=owned_root(contract); v2_manifest_binding=contract["owned_input"]["v2_input_manifest"]; v2_terminal_binding=contract["owned_input"]["v2_preparation_terminal"]; v2_manifest_path=REPO/v2_manifest_binding["path"]; v2_terminal_path=REPO/v2_terminal_binding["path"]
    if sha(v2_manifest_path)!=v2_manifest_binding["sha256"] or sha(v2_terminal_path)!=v2_terminal_binding["sha256"] or load(v2_manifest_path).get("status")!="PASS" or load(v2_terminal_path).get("status")!="PASS": raise RuntimeError("v2 input preparation authority")
    root_status=root.lstat() if root.exists() else None; expected_names=set(contract["owned_input"]["files"].values())
    if root_status is None or not stat.S_ISDIR(root_status.st_mode) or stat.S_ISLNK(root_status.st_mode) or root_status.st_mode&0o7777!=contract["owned_input"]["root_mode"] or {path.name for path in root.iterdir()}!=expected_names: raise RuntimeError("reused input root authority")
    observed={}
    for name,expected in contract["expected_input_operands"].items():
        path=root/contract["owned_input"]["files"][name]; status=path.lstat()
        if not stat.S_ISREG(status.st_mode) or stat.S_ISLNK(status.st_mode) or status.st_mode&0o7777!=contract["owned_input"]["file_mode"] or status.st_size!=expected["bytes"] or sha(path)!=expected["sha256"]: raise RuntimeError(f"reused input {name} custody")
        if "data_rows" in expected and sum(1 for _ in path.open())-1!=expected["data_rows"]: raise RuntimeError("owned expected rows")
        observed[name]={"path":str(path),"bytes":status.st_size,"mode":status.st_mode&0o7777,"sha256":expected["sha256"]}
    return observed

def verify_owned_manifest(contract,executable):
    source=verify_reused_input_source(contract); manifest=load(INPUT_MANIFEST)
    if manifest.get("status")!="PASS" or manifest.get("classification")!="ReuseExistingV2SealedRootReopenRehashNoCopy" or manifest.get("input_reuse") is not True or manifest.get("v2_input_manifest_sha256")!=contract["owned_input"]["v2_input_manifest"]["sha256"] or manifest.get("v2_preparation_terminal_sha256")!=contract["owned_input"]["v2_preparation_terminal"]["sha256"] or manifest.get("executable_sha256")!=sha(executable) or manifest.get("operands")!=source: raise RuntimeError("v3 input adoption manifest authority")
    return source
...
def self_check():
    ...
    if not focused_raw.is_file(): raise RuntimeError("durable focused v3 receipt required")
    observed=load(focused_raw)
    mutations=["child","edit","range","policy-root-route-constant","reconstruction","aba","historical","concurrency","cache","terminal","rss","wall"]
    with tempfile.TemporaryDirectory() as directory:
        ...
        for label in ["valid","valid-final",*mutations]:
                ...
                elif label=="cache": row["product"]["concurrency"]["reader_two_cache_size_pages"]=2000
    ...
    result={"schema":"phase4-g5-3-runner-self-check-v3","status":"PASS","actual_receipt_sha256":sha(focused_raw),"mutation_cases":len(mutations),"analyzer_decisions":2*(2+len(mutations)),"product_processes":0,"product_rows":0}; print(compact(result)); return result

def adopt_inputs(executable):
    ready,contract,bound=require_ready("adopt_inputs"); executable=pathlib.Path(executable).resolve()
    if executable!=bound.resolve() or INPUT_MANIFEST.exists() or PREPARATION_TERMINAL.exists(): raise RuntimeError("one-shot v3 input adoption custody")
    started=time.monotonic_ns(); operands=verify_reused_input_source(contract)
    manifest={"schema":"phase4-g5-3-input-adoption-manifest-v3","status":"PASS","classification":"ReuseExistingV2SealedRootReopenRehashNoCopy","input_reuse":True,"owned_root":str(owned_root(contract)),"operands":operands,"v2_input_manifest_sha256":contract["owned_input"]["v2_input_manifest"]["sha256"],"v2_preparation_terminal_sha256":contract["owned_input"]["v2_preparation_terminal"]["sha256"],"executable_sha256":sha(executable)}; write(INPUT_MANIFEST,manifest)
    complete=time.monotonic_ns()-started; status="PASS" if complete<contract["limits"]["preparation_wall_ns"] else "REVISE"
    terminal={"schema":"phase4-g5-3-input-adoption-terminal-v3","status":status,"classification":"CompleteWallThroughReopenRehashModeAnd1001RowChecksAndManifestFsync","complete_wall_ns":complete,"target_ns":contract["limits"]["preparation_target_ns"],"hard_limit_ns":contract["limits"]["preparation_wall_ns"],"within_target":complete<contract["limits"]["preparation_target_ns"],"input_reuse":True,"product_processes":0,"files_copied":0}; write(PREPARATION_TERMINAL,terminal)
    if status!="PASS": raise RuntimeError("v3 input adoption wall exceeded")
...
def freeze(executable):
    ...
    owned=verify_owned_manifest(contract,executable)
    focused_files=sorted(path for path in FOCUSED_ATTEMPT.iterdir() if path.is_file()) if FOCUSED_ATTEMPT.is_dir() else []
    files=[pathlib.Path(__file__),PRIMARY,INDEPENDENT,CONTRACT,SCHEDULE,EXPECTED,COVERAGE,REUSED,READINESS,DISPOSITION,FOCUSED,*focused_files,INPUT_MANIFEST,PREPARATION_TERMINAL,HERE/"PREREGISTRATION-v3.md",HERE/"LIMITATIONS-v3.md",REPO/contract["owned_input"]["v2_input_manifest"]["path"],REPO/contract["owned_input"]["v2_preparation_terminal"]["path"],*(REPO/item["path"] for item in ready["product"]["source_files"]),*(pathlib.Path(item["path"]) for item in owned.values())]
    receipt={"schema":"phase4-g5-3-source-freeze-v3","status":"FROZEN_BEFORE_FORECAST","method_contract_sha256":sha(CONTRACT),"schedule_sha256":sha(SCHEDULE),"expected_outcomes_sha256":sha(EXPECTED),"coverage_map_sha256":sha(COVERAGE),"reused_authority_sha256":sha(REUSED),"input_manifest_sha256":sha(INPUT_MANIFEST),"preparation_terminal_sha256":sha(PREPARATION_TERMINAL),"input_reuse":True,"executable":str(executable),"executable_sha256":sha(executable),"authoritative_files":[{"path":str(path.relative_to(REPO)),"bytes":path.stat().st_size,"sha256":sha(path)} for path in files]}; write(FREEZE,receipt); return receipt

def verify_freeze():
    frozen=load(FREEZE)
    contract=load(CONTRACT); executable=pathlib.Path(frozen["executable"]); verify_owned_manifest(contract,executable)
    if frozen.get("status")!="FROZEN_BEFORE_FORECAST" or frozen.get("method_contract_sha256")!=sha(CONTRACT) or frozen.get("schedule_sha256")!=sha(SCHEDULE) or frozen.get("expected_outcomes_sha256")!=sha(EXPECTED) or frozen.get("coverage_map_sha256")!=sha(COVERAGE) or frozen.get("reused_authority_sha256")!=sha(REUSED) or frozen.get("input_manifest_sha256")!=sha(INPUT_MANIFEST) or frozen.get("preparation_terminal_sha256")!=sha(PREPARATION_TERMINAL) or frozen.get("input_reuse") is not True: raise RuntimeError("freeze changed")
...
def forecast():
    ...
    receipt={"schema":"phase4-g5-3-zero-row-forecast-v3","status":"PASS" if gate<=contract["limits"]["gate_wall_ns"] and screen<contract["limits"]["screen_wall_ns"] else "REVISE","product_processes":0,"product_rows":0,"screen_forecast_ns":screen,"gate_forecast_ns":gate,"screen_limit_ns":contract["limits"]["screen_wall_ns"],"gate_limit_ns":contract["limits"]["gate_wall_ns"],"gate_reserve_ns":contract["limits"]["gate_wall_ns"]-gate,"classification":"ProspectiveFeasibilityNotTimingEvidence"}; write(FORECAST,receipt); return receipt
...
def adopt_focused():
    ...
    adoption={"schema":"phase4-g5-3-focused-evidence-adoption-v3","status":status,"classification":"DurableDirectReceiptReopenedAndAnalyzedNoProductRerun","source_artifacts":source,"adopted_artifacts":{name:artifact(FOCUSED_ATTEMPT/name) for name in names.values()},"raw_envelope":artifact(FOCUSED_ATTEMPT/"RAW-ENVELOPE.json"),"primary":artifact(FOCUSED_ATTEMPT/"PRIMARY.json"),"independent":artifact(FOCUSED_ATTEMPT/"INDEPENDENT.json"),"analyzer_agreement":normalized[0]==normalized[1],"maximum_resident_set_size":observed_rss,"rss_limit_bytes":contract["limits"]["rss_bytes"],"product_processes_executed_by_adoption":0,"measured_campaign_rows":0}; write(FOCUSED_ATTEMPT/"ADOPTION.json",adoption)
    if status!="PASS": raise RuntimeError("focused v3 evidence REVISE")
    return adoption
...
def verify_screen_authority(contract,require_closure):
    ...
    paths={name:result/name for name in ("TERMINAL-v3.json","RAW-v3.json","PRIMARY-v3.json","INDEPENDENT-v3.json","LOCK-RELEASE-v3.json","FINAL-DECISION-v3.json","FINAL-ARTIFACT-MANIFEST-v3.tsv")}
    if any(not path.is_file() for path in paths.values()): raise RuntimeError("accepted screen artifacts absent")
    terminal,raw,primary,independent,release,decision=(load(paths[name]) for name in ("TERMINAL-v3.json","RAW-v3.json","PRIMARY-v3.json","INDEPENDENT-v3.json","LOCK-RELEASE-v3.json","FINAL-DECISION-v3.json"))
...
def screen_closure():
    ...
    if ledger.get("status")!="PASS" or len(tests)!=3 or any(test.get("result")!="PASS" for test in tests): raise RuntimeError("focused source-test authority")
...
def process_evidence(process,return_record,maximum,product):
    ...
    evidence={"schema":"phase4-g5-3-process-evidence-v3","status":"CAPTURED_BEFORE_EVALUATION","classification":"LosslessCommandStdoutStderrReturnRssAndParsedReceipt","artifacts":{name:artifact(process/name) if (process/name).is_file() else None for name in names},"return":return_record,"maximum_resident_set_size":maximum,"parsed_receipt":artifact(process/"PARSED-RECEIPT.json") if product is not None else None,"receipt_status":product.get("status") if product is not None else None}
    write(process/"PROCESS-EVIDENCE-v3.json",evidence); return evidence

def final_manifest(result):
    manifest=result/"FINAL-ARTIFACT-MANIFEST-v3.tsv"; rows=[]
...
def verify_final_manifest(result):
    manifest=result/"FINAL-ARTIFACT-MANIFEST-v3.tsv"
...
def campaign(phase):
    ...
    try:
        ...
        if result.exists(): write(result/"LOCK-RELEASE-v3.json",release)
    ...
    if product is not None:
        ...
        for script,name in ((PRIMARY,"PRIMARY-v3.json"),(INDEPENDENT,"INDEPENDENT-v3.json")):
            analyzed=analyzer(script,result/"RAW-v3.json",result/name)
    ...
    terminal={"schema":"phase4-g5-3-terminal-v3","status":"PASS" if failure is None else "REVISE","phase":phase,"complete_wall_ns":complete,"wall_scope":"ThroughPreliminaryAnalyzerPairCleanupAndOwnedLockReleaseFinalAnalyzerPairIsPostWallCustody","limit_ns":contract["campaigns"][phase]["complete_wall_hard_limit_ns"],"children_started":child_started,"children_reaped":child_reaped,"product_processes":child_started,"product_rows":1 if product else 0,"maximum_resident_set_size":maximum,"lock_released":not LOCK.exists(),"terminal_work_roots":0 if work is None or not work.exists() else 1}; write(result/"TERMINAL-v3.json",terminal)
    required=("TERMINAL-v3.json","RAW-v3.json","PRIMARY-v3.json","INDEPENDENT-v3.json","LOCK-RELEASE-v3.json")
    decision={"schema":"phase4-g5-3-final-decision-v3","status":"PASS" if failure is None else "REVISE","phase":phase,"classification":"TerminalRawTwoIndependentAnalyzersAndOwnedLockRelease","bindings":{name:artifact(result/name) for name in required if (result/name).is_file()},"process_evidence":artifact(process/"PROCESS-EVIDENCE-v3.json") if process is not None and (process/"PROCESS-EVIDENCE-v3.json").is_file() else None,"analyzer_agreement":len(normalized)==2 and normalized[0]==normalized[1],"hard_failures":normalized[0]["hard_failures"] if len(normalized)==2 and normalized[0]==normalized[1] else None}; write(result/"FINAL-DECISION-v3.json",decision)
    if failure: write(result/"FAILED-v3.json",{"status":"REVISE","error":f"{type(failure).__name__}: {failure}"})
...
def main():
    parser=argparse.ArgumentParser(); parser.add_argument("action",choices=("self-check","adopt-focused","adopt-inputs","freeze","forecast","screen","screen-closure","gate")); parser.add_argument("--executable"); args=parser.parse_args()
    if args.action=="self-check": self_check()
    elif args.action=="adopt-focused": print(compact(adopt_focused()))
    elif args.action=="adopt-inputs": print(compact(adopt_inputs(args.executable)))
