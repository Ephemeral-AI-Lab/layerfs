#!/usr/bin/env python3
"""Primary analyzer for the prospective G2-v5 protocol closure."""
...
def v4_history_failures():
    failures = [f"v4-custody:{path.name}" for path, expected in V4_HASHES.items() if not path.is_file() or sha256(path) != expected]
    if failures:
        return failures
    ...
    if dry.get("actual_rows") != 0 or dry.get("benchmark_children_invoked") != 0 or dry.get("base_proxy_plan", {}).get("database_symlink_target") != str(SEALED / "input-v1/base.sqlite") or (REPO / "target/phase4-g2-materialization-decomposition-20260822-v4").exists() or (REPO / "target/phase4-g2-materialization-decomposition-20260822-v4.lock").exists():
        failures.append("v4-rejected-preexec-shape")
    return failures
...
def edit_pair_failures(rows):
    ...
    if len(rows) != 2 or {row.get("arm") for row in rows} != {"A", "B"}:
        return ["v5-edit-pair-shape"]
    by_arm = {row["arm"]: row for row in rows}
    for arm, row in by_arm.items():
        prefix = f"v5:{arm}:{row.get('label', 'unlabeled')}"
        ...
        failures.extend(exact_failures(row, {**expected_schedule, "cli_operation": "edit-same", "kind": "measured", "workload": "v5-guard", "validation": "capture-only", "base_copy_method": "physical-byte-copy-identical-database-authority-expectations"}, prefix))
...
def artifact_failures(results, analyzer_label):
    ...
    if not all(path.is_file() for path in (custody_path, cleanup_path, plan_path, chronology_path, proxy_path)):
        return ["v5-artifact-set-missing"], [], {}, [], {}
    ...
    if len(custody) != 2 or {item.get("name") for item in custody} != set(expected_operands):
        failures.append("v5-operand-custody-shape")
    for item in custody:
        ...
        if not copied.is_file() or source != expected_source or not source.is_file() or sha256(copied) != digest or copied.stat().st_size != size or mode(copied) != "0500" or mode(source) != "0444" or item.get("sha256") != digest or item.get("size_bytes") != size or item.get("source_mode") != "0444" or item.get("copy_mode") != "0500" or item.get("execution_path") != "snapshot-only":
            failures.append(f"v5-operand-{item.get('name')}:bytes-mode")
            continue
    ...
    if not cleanup_expected:
        failures.append("v5-cleanup-contract")
    plan = json.loads(plan_path.read_text())
    expected = expected_plan(results)
    if plan != expected:
        failures.append("v5-chronology-plan")
    ...
    if observed != events[:prefix_length]:
        failures.append("v5-chronology-prefix")
...
def analyze(results):
    ...
    failures.extend(v4_history_failures())
    ...
    if not raw_v5.is_file():
        failures.append("v5-raw-missing")
        v5_rows = []
    else:
        v5_rows = [json.loads(line) for line in raw_v5.read_text().splitlines() if line]
    failures.extend(edit_pair_failures(v5_rows))
    campaign_failures, custody, cleanup, plan, proxy = artifact_failures(results, "primary-analysis")
    failures.extend(campaign_failures)
    ledger = normalized_ledger(results, failures, v1_rows, primary, observer, v5_rows, custody, cleanup, plan, proxy)
...
def main():
    ...
    output = {
        "schema": "phase4-g2-protocol-closure-primary-analysis-v5",
        "status": status,
    ...
    (args.results / "G2-V5-ANALYSIS.json").write_text(json.dumps(output, indent=2, sort_keys=True) + "\n")
