# Prospective G2-v5 protocol closure
...
G2-v3 is immutable historical `G2 REVISE`: its first prepare child failed
before any row with `ValidationAuthorityUnavailable` because the sealed base
authority was 0444 while `read_authority` requires an exact regular 0600 file.
Its raw JSONL is empty. V5 neither relabels nor edits v3.

G2-v4 is rejected pre-execution and is not a sealed campaign: its frozen
methodology and zero-row dry-run exist, but no v4 result root or lock exists.
The v4 proxy database was a symlink resolving to the sealed 0444 database;
Rust `fs::copy` preserves permissions, so the prepared row database would
remain 0444 before `Store::open` could mutate it. V5 binds v4's frozen files
and dry-run and does not execute or relabel v4.

G2-v5 changes one orchestration variable only: a transient base proxy supplies
the sealed database bytes through one private regular byte-identical 0600 copy
and the same authority bytes through one private 0600 copy. It otherwise composes the sealed v1 decomposition and
read-only guards with exactly one fresh `BA` same-middle pair. It changes no Rust source, binary, profile,
...
The v5 analyzers consume, without copying or modifying, these sealed v1 facts:
...
V5 also binds the sealed v3 payload manifest
`59e0bbb6d44da9ba02f8c9536a1b55fedfc48ed342a6068087bbd6aaf509a4c3`,
terminal `8befdf04037868e0bd2934dccb9e7d3be69b4dad38ba1059d41ea4a375e25f2a`,
terminal verification `85554b79ae15b5f72ccc2d11a84222e7d5aa34a2ce41d2088cc30034535809b3`,
empty raw hash `e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`,
and exact `Error: ValidationAuthorityUnavailable\n` preparation failure.

V5 also binds v4's preregistration
`31cd21ebc56d0d8ee18c1b1f9ca8813a87dd51e7e0cf59c737a3c91acfd2a3f3`,
runner `eb307f775d4aa6d0e1751e79b4838491956afee6a2d88a7fafc6b2dc01698450`,
primary analyzer `7b427aad341c250c4ef649fa1bcbe13889625758525205d9c3a08ff61311a547`,
independent analyzer `02a0da07e81a09a7de929eda531e005924266cf4905b9a9ece140f9cfd435b29`,
manifest `2101fa07cf66c17ab09261f02c51a8e187838d08f1deefe4ba9913f6de7238b1`,
and dry-run `6f12dfb18f1c68d3b22b79af5eb409b7ba188b103db9cd7861a4a8de513b4635`.
...
artifact. V5 does not reinterpret the two historical edit rows as new rows.
...
`target/phase4-g2-materialization-decomposition-20260822-v5`
...
Before any preparation or row, both sealed v3 retained operands are byte-copied into
`results-v5/operands-v5`, rehashed, and used only from those snapshots. The v1
recorded instrumented source and source-only diff bytes were reverted and are
not retained, so v5 explicitly does not claim they are byte-reverifiable.
...
The sealed fixture is referenced read-only. V5 byte-copies the sealed database
to regular `work-v5/base-proxy/base.sqlite`, chmods only that private copy to
0600, verifies its exact database hash and distinct device/inode, and creates
a private `base.sqlite.authority` copy with the same
`7855ea6096359925f639b91c8d6b9708cfe0bc0df4a3ffd97a280a8e9a9ded48`
hash and exact 0600 mode. Sealed database and authority modes remain untouched
at 0444.
The prepare implementation copies only database and authority, so no proxy
expectations copy is authorized. `LAYERFS_PREPARED_BASE_DATABASE` points to
the proxy. `BASE-PROXY-CUSTODY-v5.json` records target/source hashes, modes,
paths, device/inode distinction, contract, and post-cleanup absence.

`--fast-prepare` creates exactly
two distinct row database/authority/expectation copies under the single
declared `results-v5/rows-v5/work-v5` path. Apparent and allocated regular-file
usage includes the private 109-MiB base, private authority, both row databases,
sidecars, and any prepare-oracle files; it is sampled after proxy creation,
both preparations, and both measured rows. Each apparent and allocated peak
adds a conservative 109,314,048-byte prepare-oracle reserve even when no oracle
remains at a sampling boundary, and must be at most 512 MiB. Preparation and
hashing are outside row timing.
...
custody; sealed v3 failure anchors; rejected-pre-exec v4 anchors; base-proxy custody; chronology; cleanup;
and every resource ceiling. The runner compares
...
terminal/verification, sealed v3 terminal/verification/payload/empty raw,
rejected v4 manifest/dry-run, and
base-proxy custody. Any `REVISE` exits nonzero, and lock cleanup is
...
The v5 lock is removed and its absence proved before any authoritative PASS
...
unseals only v5, regenerates authoritative REVISE evidence, reseals it, and
exits nonzero; no failure path can leave authoritative PASS.

The private regular proxy database, proxy authority, two private row copies,
and any prepare-oracle files are transient.
Cleanup requires the exact work
path to exist, verifies both rows and the <=512-MiB peak, deletes exactly
`results-v5/rows-v5/work-v5`, and proves its absence before either analyzer.
Both analyzers gate cleanup PASS and its exact deletion set. The executable snapshots remain. This keeps
retained v5 evidence below 10 MiB without deleting any v1 evidence.
...
Even after G2-v5 PASS, G3 is ineligible until full workspace tests, Clippy
with `-D warnings`, formatting, and diff-check static closure all pass.

## Authorized commands before execute mode

After `METHODOLOGY-MANIFEST-v5.tsv` freezes this document and both analyzers:

```text
python3 analyze_g2_v5.py --self-test
python3 recompute_g2_v5.py --self-test
python3 -m py_compile analyze_g2_v5.py recompute_g2_v5.py run_g2_v5.py
G2_V5_METHODOLOGY_SHA256=<frozen-manifest-sha256> python3 run_g2_v5.py --dry-run
git diff --check -- implementation-detail/phase-4/experiments/g2-materialization-decomposition/v5
```
