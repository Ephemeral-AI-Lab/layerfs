#!/usr/bin/env python3
"""One-shot G2-v5 closure runner; dry-run is the only unauthorised mode."""
...
TARGET = REPO / "target/phase4-g2-materialization-decomposition-20260822-v5"
RESULTS = TARGET / "results-v5"
LOCK = REPO / "target/phase4-g2-materialization-decomposition-20260822-v5.lock"
MANIFEST = HERE / "METHODOLOGY-MANIFEST-v5.tsv"
DRY_RUN = HERE / "DRY-RUN-v5.json"
ANALYZER = HERE / "analyze_g2_v5.py"
RECOMPUTE = HERE / "recompute_g2_v5.py"
...
HASHES = {
    ...
    "v4_prereg": "31cd21ebc56d0d8ee18c1b1f9ca8813a87dd51e7e0cf59c737a3c91acfd2a3f3",
    "v4_runner": "eb307f775d4aa6d0e1751e79b4838491956afee6a2d88a7fafc6b2dc01698450",
    "v4_primary": "7b427aad341c250c4ef649fa1bcbe13889625758525205d9c3a08ff61311a547",
    "v4_independent": "02a0da07e81a09a7de929eda531e005924266cf4905b9a9ece140f9cfd435b29",
    "v4_manifest": "2101fa07cf66c17ab09261f02c51a8e187838d08f1deefe4ba9913f6de7238b1",
    "v4_dry_run": "6f12dfb18f1c68d3b22b79af5eb409b7ba188b103db9cd7861a4a8de513b4635",
}
...
AUTHORIZATION = "parent-authorized-exactly-one-fresh-same-middle-v5-ba"
...
def schedule():
    return [
        {"sequence": 1, "label": "01-measured-same-middle-pos1-B", "arm": "B", "position": 1, "order": "BA", "operation": "same-middle", "cli_operation": "edit-same", "iteration": 983001, "kind": "measured", "workload": "v5-guard", "warmup": False, "validation": "capture-only"},
        {"sequence": 2, "label": "02-measured-same-middle-pos2-A", "arm": "A", "position": 2, "order": "BA", "operation": "same-middle", "cli_operation": "edit-same", "iteration": 983002, "kind": "measured", "workload": "v5-guard", "warmup": False, "validation": "capture-only"},
    ]
...
def ensure_fresh():
    if TARGET.exists() or LOCK.exists():
        raise RuntimeError("G2-v5 result root or lock already exists")


def verify_methodology():
    expected = os.environ.get("G2_V5_METHODOLOGY_SHA256")
    if not expected or not MANIFEST.is_file() or sha256(MANIFEST) != expected:
        raise RuntimeError("G2-v5 methodology manifest custody mismatch")
...
def verify_v4_rejected_preexec():
    artifacts = {
        "preregistration": (V4_METHOD / "PROSPECTIVE-G2-MATERIALIZATION-DECOMPOSITION-v4.md", HASHES["v4_prereg"]),
        "runner": (V4_METHOD / "run_g2_v4.py", HASHES["v4_runner"]),
        "primary": (V4_METHOD / "analyze_g2_v4.py", HASHES["v4_primary"]),
        "independent": (V4_METHOD / "recompute_g2_v4.py", HASHES["v4_independent"]),
        "manifest": (V4_METHOD / "METHODOLOGY-MANIFEST-v4.tsv", HASHES["v4_manifest"]),
        "dry_run": (V4_METHOD / "DRY-RUN-v4.json", HASHES["v4_dry_run"]),
    }
    for path, digest in artifacts.values():
        verify(path, digest)
    dry = json.loads(artifacts["dry_run"][0].read_text())
    ...
    rejected_shape = dry.get("status") == "PASS" and dry.get("actual_rows") == 0 and dry.get("benchmark_children_invoked") == 0 and dry.get("database_copies_created") == 0 and dry.get("base_proxy_plan", {}).get("database_symlink_target") == str(BASE) and dry.get("base_proxy_plan", {}).get("database_source_mode") == "0444" and not target.exists() and not lock.exists()
    if not rejected_shape:
        raise RuntimeError("v4 rejected pre-exec custody mismatch")
    return {"classification": "REJECTED_PRE_EXEC_NOT_A_CAMPAIGN", "reason": "database proxy symlink resolves sealed 0444; fs::copy preserves permissions before Store::open", "actual_rows": 0, "benchmark_children_invoked": 0, "result_root_absent": True, "lock_absent": True, "preregistration_sha256": HASHES["v4_prereg"], "runner_sha256": HASHES["v4_runner"], "primary_sha256": HASHES["v4_primary"], "independent_sha256": HASHES["v4_independent"], "manifest_sha256": HASHES["v4_manifest"], "dry_run_sha256": HASHES["v4_dry_run"]}
...
def preflight():
    ...
    v4_history = verify_v4_rejected_preexec()
    return {"branch": branch, "head": head, "methodology_manifest_sha256": sha256(MANIFEST), "sealed_v1_raw_sha256": sha256(sealed_artifacts["v1_raw"]), "retained_g1_source_sha256": sha256(SOURCE), "fastcdc_source_sha256": sha256(CDC), "v1_root_read_only": True, "v1_lock_absent": True, "v1_instrumented_source_bytes": "not-retained-not-verified", "v1_source_diff_bytes": "not-retained-not-verified", "sealed_v1_payload": payload, "sealed_v3_history": v3_history, "v4_rejected_preexec": v4_history, "base_proxy_plan_verified": {**base_proxy_plan(), "source_database_exists": BASE.is_file(), "source_authority_exists": BASE_FILES["authority"].is_file(), "expectations_not_required_by_prepare": True}}


def dry_run(preflight_record):
    ensure_fresh()
    if DRY_RUN.exists():
        raise RuntimeError("G2-v5 dry-run already exists")
    rows = schedule()
    record = {
        "schema": "phase4-g2-protocol-closure-dry-run-v5",
        "status": "PASS",
        ...
        "transient_peak_ceiling_bytes": 512 * 1024 * 1024,
        "transient_paths_deleted_before_seal": ["results-v5/rows-v5/work-v5"],
...
def run_child(command, label, output_dir, env=None, timeout=CHILD_CEILING_SECONDS, allow_nonzero=False, started_ns=None):
    if started_ns is not None:
        ...
        if remaining <= 0:
            raise TimeoutError("G2-v5 global 59-second ceiling exhausted")
...
def snapshot_binaries():
    ...
    (RESULTS / "OPERAND-CUSTODY-v5.json").write_text(json.dumps(custody, indent=2, sort_keys=True) + "\n")
    return control, candidate


def copy_methodology():
    destination = RESULTS / "methodology-v5"
    destination.mkdir()
    with MANIFEST.open() as handle:
        for row in csv.DictReader(handle, delimiter="\t"):
...
def chronology(event, started_ns, **fields):
    ...
    with (RESULTS / "CHRONOLOGY-v5.jsonl").open("a") as handle:
...
def base_proxy_plan():
    proxy = RESULTS / "rows-v5/work-v5/base-proxy/base.sqlite"
    return {"database_proxy_path": str(proxy), "database_source_path": str(BASE), "database_sha256": HASHES["base_database"], "database_source_mode": "0444", "database_private_mode": "0600", "database_copy_kind": "private-regular-byte-identical", "authority_proxy_path": str(Path(str(proxy) + ".authority")), "authority_source_path": str(BASE_FILES["authority"]), "authority_sha256": HASHES["base_authority"], "authority_source_mode": "0444", "authority_private_mode": "0600", "expectations_copy": False, "prepare_source_contract": ["database", "authority"], "declared_cleanup": "rows-v5/work-v5", "transient_ceiling_bytes": 512 * 1024 * 1024}
...
def create_base_proxy():
    ...
    shutil.copyfile(BASE, proxy)
    proxy.chmod(0o600)
    shutil.copyfile(BASE_FILES["authority"], authority)
    authority.chmod(0o600)
    database_source_stat, database_copy_stat = BASE.stat(), proxy.stat()
    authority_source_stat, authority_copy_stat = BASE_FILES["authority"].stat(), authority.stat()
    custody = {**plan, "status": "READY", "database_is_regular": proxy.is_file() and not proxy.is_symlink(), "database_private_sha256": sha256(proxy), "database_private_mode_actual": mode(proxy), "database_source_device": database_source_stat.st_dev, "database_source_inode": database_source_stat.st_ino, "database_private_device": database_copy_stat.st_dev, "database_private_inode": database_copy_stat.st_ino, "database_distinct_device_inode": (database_source_stat.st_dev, database_source_stat.st_ino) != (database_copy_stat.st_dev, database_copy_stat.st_ino), "authority_private_sha256": sha256(authority), "authority_private_mode_actual": mode(authority), "authority_source_device": authority_source_stat.st_dev, "authority_source_inode": authority_source_stat.st_ino, "authority_private_device": authority_copy_stat.st_dev, "authority_private_inode": authority_copy_stat.st_ino, "authority_distinct_device_inode": (authority_source_stat.st_dev, authority_source_stat.st_ino) != (authority_copy_stat.st_dev, authority_copy_stat.st_ino), "work_path_absent_after_cleanup": False}
    exact = custody["database_is_regular"] and custody["database_private_sha256"] == HASHES["base_database"] and custody["database_private_mode_actual"] == "0600" and custody["database_distinct_device_inode"] and custody["authority_private_sha256"] == HASHES["base_authority"] and custody["authority_private_mode_actual"] == "0600" and custody["authority_distinct_device_inode"] and not Path(str(proxy) + ".expectations").exists()
    if not exact:
        raise RuntimeError("base proxy custody mismatch")
    (RESULTS / "BASE-PROXY-CUSTODY-v5.json").write_text(json.dumps(custody, indent=2, sort_keys=True) + "\n")
...
def prepare_rows(rows, started_ns, prepared_base):
    ...
    for spec, plan_entry in zip(rows, plans):
        row_root = RESULTS / "rows-v5/work-v5" / spec["label"]
        ...
        recorded_child(plan_entry, RESULTS / "preparation-v5", started_ns, env=env)
...
def acquire(spec, prepared, started_ns):
    ...
    completed = recorded_child(plan_entry, RESULTS / "rows-v5", started_ns, env=env)
    ...
    with (RESULTS / "rows-v5/G2-V5-RAW.jsonl").open("a") as handle:
        handle.write(json.dumps(row, separators=(",", ":"), sort_keys=True) + "\n")
    sample_transient(f"completed-{spec['label']}")


def transient_usage():
    root = RESULTS / "rows-v5/work-v5"
    ...
    prepare_oracle_reserve = 109314048
    return {"apparent_bytes": apparent, "allocated_bytes": allocated, "prepare_oracle_reserve_bytes": prepare_oracle_reserve, "accounted_apparent_bytes": apparent + prepare_oracle_reserve, "accounted_allocated_bytes": allocated + prepare_oracle_reserve}


def sample_transient(stage):
    path = RESULTS / "TRANSIENT-USAGE-v5.json"
    record = json.loads(path.read_text()) if path.is_file() else {"samples": [], "peak_apparent_bytes": 0, "peak_allocated_bytes": 0, "ceiling_bytes": 512 * 1024 * 1024}
    usage = {"stage": stage, **transient_usage()}
    record["samples"].append(usage)
    record["peak_apparent_bytes"] = max(record["peak_apparent_bytes"], usage["accounted_apparent_bytes"])
    record["peak_allocated_bytes"] = max(record["peak_allocated_bytes"], usage["accounted_allocated_bytes"])
...
def transient_report(prepared, complete):
    ...
    usage = json.loads((RESULTS / "TRANSIENT-USAGE-v5.json").read_text()) if (RESULTS / "TRANSIENT-USAGE-v5.json").is_file() else {"within_ceiling": False}
    proxy_custody = json.loads((RESULTS / "BASE-PROXY-CUSTODY-v5.json").read_text()) if (RESULTS / "BASE-PROXY-CUSTODY-v5.json").is_file() else {}
    work = RESULTS / "rows-v5/work-v5"
    report = {"schema": "phase4-g2-v5-transient-verification-v1", "status": "PENDING_DELETE", "rows_validated": complete and len(records) == 2 and exact, "base_proxy_ready": proxy_custody.get("status") == "READY" and proxy_custody.get("database_is_regular") is True and proxy_custody.get("database_private_mode_actual") == "0600" and proxy_custody.get("authority_private_mode_actual") == "0600" and proxy_custody.get("expectations_copy") is False, "records": records, "usage": usage, "declared_deletions": ["rows-v5/work-v5"], "work_path_existed": work.is_dir(), "deletion_complete": False, "work_path_absent": False}
    path = RESULTS / "TRANSIENT-VERIFICATION-v5.json"
...
def discard_transients(report_path, report):
    ...
    deleted = []
    for transient in (RESULTS / "rows-v5/work-v5",):
    ...
    report.update({"status": "PASS" if passed else "REVISE", "deletion_complete": True, "deleted": deleted, "work_path_absent": not (RESULTS / "rows-v5/work-v5").exists()})
    report_path.write_text(json.dumps(report, indent=2, sort_keys=True) + "\n")
    custody_path = RESULTS / "BASE-PROXY-CUSTODY-v5.json"
    if custody_path.is_file():
        custody = json.loads(custody_path.read_text())
        custody.update({"status": "PASS" if passed else "REVISE", "work_path_absent_after_cleanup": not (RESULTS / "rows-v5/work-v5").exists(), "deleted": deleted})
        custody_path.write_text(json.dumps(custody, indent=2, sort_keys=True) + "\n")


def payload_manifest():
    manifest = RESULTS / "PAYLOAD-MANIFEST-v5.tsv"
    excluded = {manifest, RESULTS / "TERMINAL-v5.json", RESULTS / "TERMINAL-VERIFICATION-v5.txt"}
...
def chronology_failures():
    records = [json.loads(line) for line in (RESULTS / "CHRONOLOGY-v5.jsonl").read_text().splitlines() if line]
...
def prepare_terminal(status, disposition, reason, started_ns):
    for _ in range(2):
        status_record = {"status": status, "disposition": disposition, "reason": reason, "fresh_rows": sum(1 for line in (RESULTS / "rows-v5/G2-V5-RAW.jsonl").read_text().splitlines() if line) if (RESULTS / "rows-v5/G2-V5-RAW.jsonl").is_file() else 0, "sealed_v1_rows_rerun": 0, "g3_eligible": False, "post_pass_static_closure_required": True}
        (RESULTS / "STATUS-v5.json").write_text(json.dumps(status_record, indent=2, sort_keys=True) + "\n")
        mode_policy = {"schema": "phase4-g2-v5-final-mode-policy-v1", "retained_files_mode": "0444", "retained_directories_mode": "0555", "symlinks": 0, "lock_absent_before_authoritative_terminal": True}
        (RESULTS / "FINAL-MODE-POLICY-v5.json").write_text(json.dumps(mode_policy, indent=2, sort_keys=True) + "\n")
        ...
        if status == "PASS" and issues:
            ...
            continue
        primary_path = RESULTS / "G2-V5-ANALYSIS.json"
        independent_path = RESULTS / "G2-V5-INDEPENDENT-RECOMPUTATION.json"
        normalized = None
        ...
        bound_paths = {"fresh_raw_sha256": RESULTS / "rows-v5/G2-V5-RAW.jsonl", "primary_analysis_sha256": primary_path, "independent_analysis_sha256": independent_path, "chronology_sha256": RESULTS / "CHRONOLOGY-v5.jsonl", "cleanup_sha256": RESULTS / "TRANSIENT-VERIFICATION-v5.json", "base_proxy_custody_sha256": RESULTS / "BASE-PROXY-CUSTODY-v5.json"}
        elapsed = time.monotonic_ns() - started_ns
        terminal = {"status": status, "disposition": disposition, "reason": reason, "payload_manifest_sha256": sha256(manifest), "payload_manifest_entries": len(rows), "payload_mismatches": len(mismatches), "retained_evidence_bytes": retained_bytes, "status_sha256": sha256(RESULTS / "STATUS-v5.json"), "methodology_manifest_sha256": sha256(MANIFEST), "dry_run_sha256": sha256(DRY_RUN), "normalized_ledger_sha256": normalized, "v1_terminal_sha256": sha256(SEALED / "TERMINAL-v1.json"), "v1_terminal_verification_sha256": sha256(SEALED / "TERMINAL-VERIFICATION-v1.txt"), "v3_terminal_sha256": HASHES["v3_terminal"], "v3_terminal_verification_sha256": HASHES["v3_terminal_verification"], "v3_payload_manifest_sha256": HASHES["v3_payload_manifest"], "v3_empty_raw_sha256": HASHES["v3_raw"], "v4_rejected_preexec_manifest_sha256": HASHES["v4_manifest"], "v4_rejected_preexec_dry_run_sha256": HASHES["v4_dry_run"], "v4_result_root_absent": not (REPO / "target/phase4-g2-materialization-decomposition-20260822-v4").exists(), "final_mode_policy_sha256": sha256(RESULTS / "FINAL-MODE-POLICY-v5.json"), "lock_absent": not LOCK.exists(), "global_elapsed_ns": elapsed, "global_ceiling_ns": CAMPAIGN_CEILING_NS, "global_within_ceiling": elapsed < CAMPAIGN_CEILING_NS, "g3_eligible": False, **{name: sha256(path) if path.is_file() else None for name, path in bound_paths.items()}}
        verification = {"status": "PASS" if not mismatches else "FAIL", "disposition": disposition, "payload_manifest_sha256": sha256(manifest), "payload_manifest_entries": len(rows), "payload_mismatches": len(mismatches), "status_sha256": sha256(RESULTS / "STATUS-v5.json"), "final_mode_policy_sha256": terminal["final_mode_policy_sha256"], "global_elapsed_ns": elapsed, "global_ceiling_ns": CAMPAIGN_CEILING_NS, "lock_absent": not LOCK.exists(), "final_mode_mismatches": 0}
    ...
    raise RuntimeError("unable to prepare G2-v5 terminal state")
...
def write_authoritative_terminal(terminal, verification, started_ns):
    ...
    terminal_path = RESULTS / "TERMINAL-v5.json"
    verification_path = RESULTS / "TERMINAL-VERIFICATION-v5.txt"
...
def release_lock():
    if LOCK.exists():
        if not LOCK.is_dir():
            raise RuntimeError("G2-v5 lock is not a directory")
        LOCK.rmdir()
    if LOCK.exists():
        raise RuntimeError("G2-v5 lock survived release")
...
def execute(preflight_record, started_ns):
    ensure_fresh()
    if os.environ.get("G2_V5_EXECUTE_AUTHORIZATION") != AUTHORIZATION:
        raise RuntimeError("parent execute authorization is absent")
    expected_dry = os.environ.get("G2_V5_DRY_RUN_SHA256")
    if not expected_dry or not DRY_RUN.is_file() or sha256(DRY_RUN) != expected_dry:
        raise RuntimeError("G2-v5 dry-run custody mismatch")
    LOCK.mkdir()
    failure = None
    prepared = {}
    cleanup_done = False
    ...
    try:
        ...
        (RESULTS / "rows-v5").mkdir()
        (RESULTS / "CHRONOLOGY-v5.jsonl").write_text("")
        chronology("campaign-start", started_ns, planned_rows=2, order="BA")
        copy_methodology()
        control_binary, candidate_binary = snapshot_binaries()
        (RESULTS / "G2-V5-SCHEDULE.json").write_text(json.dumps(schedule(), indent=2, sort_keys=True) + "\n")
        (RESULTS / "CHRONOLOGY-PLAN-v5.json").write_text(json.dumps(child_plan(), indent=2, sort_keys=True) + "\n")
        (RESULTS / "INPUT-BINDINGS-v5.json").write_text(json.dumps({"preflight": preflight_record, "hashes": HASHES, "methodology_manifest_sha256": sha256(MANIFEST), "dry_run_sha256": sha256(DRY_RUN)}, indent=2, sort_keys=True) + "\n")
        (RESULTS / "rows-v5/G2-V5-RAW.jsonl").write_text("")
        ...
        primary_result = json.loads((RESULTS / "G2-V5-ANALYSIS.json").read_text())
        independent_result = json.loads((RESULTS / "G2-V5-INDEPENDENT-RECOMPUTATION.json").read_text())
        if primary.returncode or independent.returncode or (primary_result["status"], primary_result["disposition"], sorted(primary_result["failures"]), primary_result["normalized_ledger"]) != (independent_result["status"], independent_result["disposition"], sorted(independent_result["failures"]), independent_result["normalized_ledger"]):
            raise RuntimeError("G2-v5 analyzers failed or disagreed")
        ...
        try:
            if RESULTS.exists():
                try:
                    if not cleanup_done and (RESULTS / "rows-v5/work-v5").exists():
...
if __name__ == "__main__":
    try:
        ...
        print(f"G2-v5 failure: {type(error).__name__}: {error}", file=sys.stderr)
        raise SystemExit(1)
