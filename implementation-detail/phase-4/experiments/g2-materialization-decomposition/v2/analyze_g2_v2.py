#!/usr/bin/env python3
"""Primary analyzer for the prospective G2-v2 protocol closure."""
...
def edit_pair_failures(rows):
    ...
    if len(rows) != 2 or {row.get("arm") for row in rows} != {"A", "B"}:
        return ["v2-edit-pair-shape"]
    by_arm = {row["arm"]: row for row in rows}
    for arm, row in by_arm.items():
        prefix = f"v2:{arm}:{row.get('label', 'unlabeled')}"
        ...
        failures.extend(exact_failures(row, {**expected_schedule, "cli_operation": "edit-same", "kind": "measured", "workload": "v2-guard", "validation": "capture-only", "base_copy_method": "physical-byte-copy-identical-database-authority-expectations"}, prefix))
    failures.extend(
        f"v2-edit-pair:parity:{key}"
...
def analyze(results):
    ...
    if not raw_v2.is_file():
        failures.append("v2-raw-missing")
        v2_rows = []
    else:
        v2_rows = [json.loads(line) for line in raw_v2.read_text().splitlines() if line]
    failures.extend(edit_pair_failures(v2_rows))
    ledger = {
        "v1_payload_entries": 178,
        "v1_payload_mismatches": len([item for item in failures if item.startswith("v1-payload-")]),
        "v1_rows": len(v1_rows),
        "v1_raw_sha256": sha256(V1_RAW),
        "v1_primary_rows": len([row for row in v1_rows if row.get("workload") == "primary"]),
        "v1_read_only_rows": len([row for row in v1_rows if row.get("operation") != "same-middle"]),
        "v1_historical_edit_rows": len([row for row in v1_rows if row.get("operation") == "same-middle"]),
        "v2_fresh_edit_rows": len(v2_rows),
        "v2_fresh_order": [row.get("arm") for row in sorted(v2_rows, key=lambda row: row.get("sequence", 0))],
        "v2_post_logical_store_bytes": EDIT["sqlite_post_logical_store_bytes"],
        "v2_post_apparent_store_bytes": EDIT["sqlite_post_apparent_store_bytes"],
        "v2_post_allocated_store_bytes": EDIT["sqlite_post_allocated_store_bytes"],
        "v2_allocated_store_delta_bytes": EDIT["allocated_store_delta_bytes"],
        "timing_claim": "none",
    }
...
def main():
    ...
    output = {
        "schema": "phase4-g2-protocol-closure-primary-analysis-v2",
        "status": status,
    ...
    (args.results / "G2-V2-ANALYSIS.json").write_text(json.dumps(output, indent=2, sort_keys=True) + "\n")
