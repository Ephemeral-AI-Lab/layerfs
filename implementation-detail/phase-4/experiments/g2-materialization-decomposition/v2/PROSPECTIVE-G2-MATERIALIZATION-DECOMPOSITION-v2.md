# Prospective G2-v2 protocol closure
...
G2-v2 changes one thing only: storage predicates are scoped by operation. It
...
The v2 analyzers consume, without copying or modifying, these sealed v1 facts:
...
artifact. V2 does not reinterpret the two historical edit rows as new rows.
...
`target/phase4-g2-materialization-decomposition-20260822-v2`

The schedule is exactly:

```text
01 measured same-middle B, v1 instrumented executable with decomposition disabled
02 measured same-middle A, exact G1 control executable
```

Before any preparation or row, both `/tmp` executables are byte-copied into
`results-v2/operands-v2`, rehashed, and used only from those snapshots. The v1
recorded instrumented source and source-only diff bytes were reverted and are
not retained, so v2 explicitly does not claim they are byte-reverifiable.
...
Every child receives a fresh physical copy of the sealed base. Preparation and
hashing are outside row timing. Acquisition refuses an existing result root or
...
budget. Chronology binds the exact row command and zero exit status.
...
guard pair, and full fresh edit custody/parity. Synthetic self-checks prove that a read-only allocation is
...
The copied 100-MiB fixture/base and two private row databases are declared
transient operands. Their hashes and endpoint custody are recorded, then only
`results-v2/input-v2` and `results-v2/rows-v2/work-v2` are deleted before the
payload manifest is frozen. The executable snapshots remain. This keeps
retained v2 evidence below 10 MiB without deleting any v1 evidence.
...
After `METHODOLOGY-MANIFEST-v2.tsv` freezes this document and both analyzers:

```text
python3 analyze_g2_v2.py --self-test
python3 recompute_g2_v2.py --self-test
python3 -m py_compile analyze_g2_v2.py recompute_g2_v2.py run_g2_v2.py
G2_V2_METHODOLOGY_SHA256=<frozen-manifest-sha256> python3 run_g2_v2.py --dry-run
git diff --check -- implementation-detail/phase-4/experiments/g2-materialization-decomposition/v2
```
