#!/usr/bin/env python3
"""One-shot G2-v3 closure runner; dry-run is the only unauthorised mode."""
...
TARGET = REPO / "target/phase4-g2-materialization-decomposition-20260822-v3"
RESULTS = TARGET / "results-v3"
LOCK = REPO / "target/phase4-g2-materialization-decomposition-20260822-v3.lock"
MANIFEST = HERE / "METHODOLOGY-MANIFEST-v3.tsv"
DRY_RUN = HERE / "DRY-RUN-v3.json"
ANALYZER = HERE / "analyze_g2_v3.py"
RECOMPUTE = HERE / "recompute_g2_v3.py"
...
AUTHORIZATION = "parent-authorized-exactly-one-fresh-same-middle-v3-ba"
...
def schedule():
    return [
        {"sequence": 1, "label": "01-measured-same-middle-pos1-B", "arm": "B", "position": 1, "order": "BA", "operation": "same-middle", "cli_operation": "edit-same", "iteration": 983001, "kind": "measured", "workload": "v3-guard", "warmup": False, "validation": "capture-only"},
        {"sequence": 2, "label": "02-measured-same-middle-pos2-A", "arm": "A", "position": 2, "order": "BA", "operation": "same-middle", "cli_operation": "edit-same", "iteration": 983002, "kind": "measured", "workload": "v3-guard", "warmup": False, "validation": "capture-only"},
    ]
...
def ensure_fresh():
    if TARGET.exists() or LOCK.exists():
        raise RuntimeError("G2-v3 result root or lock already exists")


def verify_methodology():
    expected = os.environ.get("G2_V3_METHODOLOGY_SHA256")
    if not expected or not MANIFEST.is_file() or sha256(MANIFEST) != expected:
        raise RuntimeError("G2-v3 methodology manifest custody mismatch")
...
def dry_run(preflight_record):
    ensure_fresh()
    if DRY_RUN.exists():
        raise RuntimeError("G2-v3 dry-run already exists")
    rows = schedule()
    record = {
        "schema": "phase4-g2-protocol-closure-dry-run-v3",
        "status": "PASS",
        ...
        "transient_paths_deleted_before_seal": ["results-v3/rows-v3/work-v3"],
...
def run_child(command, label, output_dir, env=None, timeout=CHILD_CEILING_SECONDS, allow_nonzero=False, started_ns=None):
    if started_ns is not None:
        ...
        if remaining <= 0:
            raise TimeoutError("G2-v3 global 59-second ceiling exhausted")
...
def snapshot_binaries():
    ...
    (RESULTS / "OPERAND-CUSTODY-v3.json").write_text(json.dumps(custody, indent=2, sort_keys=True) + "\n")
...
def prepare_rows(rows, started_ns):
    prepared = {}
    plans = child_plan()[:2]
    for spec, plan_entry in zip(rows, plans):
        row_root = RESULTS / "rows-v3/work-v3" / spec["label"]
        ...
        recorded_child(plan_entry, RESULTS / "preparation-v3", started_ns, env=env)
...
def acquire(spec, prepared, started_ns):
    ...
    completed = recorded_child(plan_entry, RESULTS / "rows-v3", started_ns, env=env)
    ...
    with (RESULTS / "rows-v3/G2-V3-RAW.jsonl").open("a") as handle:
        handle.write(json.dumps(row, separators=(",", ":"), sort_keys=True) + "\n")
    sample_transient(f"completed-{spec['label']}")


def transient_usage():
    root = RESULTS / "rows-v3/work-v3"
...
def sample_transient(stage):
    path = RESULTS / "TRANSIENT-USAGE-v3.json"
...
def transient_report(prepared, complete):
    ...
    usage = json.loads((RESULTS / "TRANSIENT-USAGE-v3.json").read_text()) if (RESULTS / "TRANSIENT-USAGE-v3.json").is_file() else {"within_ceiling": False}
    work = RESULTS / "rows-v3/work-v3"
    report = {"schema": "phase4-g2-v3-transient-verification-v1", "status": "PENDING_DELETE", "rows_validated": complete and len(records) == 2 and exact, "records": records, "usage": usage, "declared_deletions": ["rows-v3/work-v3"], "work_path_existed": work.is_dir(), "deletion_complete": False, "work_path_absent": False}
    path = RESULTS / "TRANSIENT-VERIFICATION-v3.json"
...
def discard_transients(report_path, report):
    ...
    deleted = []
    for transient in (RESULTS / "rows-v3/work-v3",):
    ...
    passed = report["rows_validated"] and report["usage"].get("within_ceiling") is True and report["work_path_existed"] and deleted == report["declared_deletions"]
    report.update({"status": "PASS" if passed else "REVISE", "deletion_complete": True, "deleted": deleted, "work_path_absent": not (RESULTS / "rows-v3/work-v3").exists()})
    report_path.write_text(json.dumps(report, indent=2, sort_keys=True) + "\n")


def payload_manifest():
    manifest = RESULTS / "PAYLOAD-MANIFEST-v3.tsv"
    excluded = {manifest, RESULTS / "TERMINAL-v3.json", RESULTS / "TERMINAL-VERIFICATION-v3.txt"}
...
def chronology_failures():
    records = [json.loads(line) for line in (RESULTS / "CHRONOLOGY-v3.jsonl").read_text().splitlines() if line]
...
def prepare_terminal(status, disposition, reason, started_ns):
    for _ in range(2):
        status_record = {"status": status, "disposition": disposition, "reason": reason, "fresh_rows": sum(1 for line in (RESULTS / "rows-v3/G2-V3-RAW.jsonl").read_text().splitlines() if line) if (RESULTS / "rows-v3/G2-V3-RAW.jsonl").is_file() else 0, "sealed_v1_rows_rerun": 0, "g3_eligible": False, "post_pass_static_closure_required": True}
        (RESULTS / "STATUS-v3.json").write_text(json.dumps(status_record, indent=2, sort_keys=True) + "\n")
        mode_policy = {"schema": "phase4-g2-v3-final-mode-policy-v1", "retained_files_mode": "0444", "retained_directories_mode": "0555", "symlinks": 0, "lock_absent_before_authoritative_terminal": True}
        (RESULTS / "FINAL-MODE-POLICY-v3.json").write_text(json.dumps(mode_policy, indent=2, sort_keys=True) + "\n")
        ...
        if status == "PASS" and issues:
            ...
            continue
        primary_path = RESULTS / "G2-V3-ANALYSIS.json"
        independent_path = RESULTS / "G2-V3-INDEPENDENT-RECOMPUTATION.json"
        normalized = None
        ...
        bound_paths = {"fresh_raw_sha256": RESULTS / "rows-v3/G2-V3-RAW.jsonl", "primary_analysis_sha256": primary_path, "independent_analysis_sha256": independent_path, "chronology_sha256": RESULTS / "CHRONOLOGY-v3.jsonl", "cleanup_sha256": RESULTS / "TRANSIENT-VERIFICATION-v3.json"}
        elapsed = time.monotonic_ns() - started_ns
        terminal = {"status": status, "disposition": disposition, "reason": reason, "payload_manifest_sha256": sha256(manifest), "payload_manifest_entries": len(rows), "payload_mismatches": len(mismatches), "retained_evidence_bytes": retained_bytes, "status_sha256": sha256(RESULTS / "STATUS-v3.json"), "methodology_manifest_sha256": sha256(MANIFEST), "dry_run_sha256": sha256(DRY_RUN), "normalized_ledger_sha256": normalized, "v1_terminal_sha256": sha256(SEALED / "TERMINAL-v1.json"), "v1_terminal_verification_sha256": sha256(SEALED / "TERMINAL-VERIFICATION-v1.txt"), "final_mode_policy_sha256": sha256(RESULTS / "FINAL-MODE-POLICY-v3.json"), "lock_absent": not LOCK.exists(), "global_elapsed_ns": elapsed, "global_ceiling_ns": CAMPAIGN_CEILING_NS, "global_within_ceiling": elapsed < CAMPAIGN_CEILING_NS, "g3_eligible": False, **{name: sha256(path) if path.is_file() else None for name, path in bound_paths.items()}}
        verification = {"status": "PASS" if not mismatches else "FAIL", "disposition": disposition, "payload_manifest_sha256": sha256(manifest), "payload_manifest_entries": len(rows), "payload_mismatches": len(mismatches), "status_sha256": sha256(RESULTS / "STATUS-v3.json"), "final_mode_policy_sha256": terminal["final_mode_policy_sha256"], "global_elapsed_ns": elapsed, "global_ceiling_ns": CAMPAIGN_CEILING_NS, "lock_absent": not LOCK.exists(), "final_mode_mismatches": 0}
    ...
    raise RuntimeError("unable to prepare G2-v3 terminal state")
...
def write_authoritative_terminal(terminal, verification, started_ns):
    ...
    terminal_path = RESULTS / "TERMINAL-v3.json"
    verification_path = RESULTS / "TERMINAL-VERIFICATION-v3.txt"
...
def release_lock():
    if LOCK.exists():
        if not LOCK.is_dir():
            raise RuntimeError("G2-v3 lock is not a directory")
        LOCK.rmdir()
    if LOCK.exists():
        raise RuntimeError("G2-v3 lock survived release")
...
def execute(preflight_record, started_ns):
    ensure_fresh()
    if os.environ.get("G2_V3_EXECUTE_AUTHORIZATION") != AUTHORIZATION:
        raise RuntimeError("parent execute authorization is absent")
    expected_dry = os.environ.get("G2_V3_DRY_RUN_SHA256")
    if not expected_dry or not DRY_RUN.is_file() or sha256(DRY_RUN) != expected_dry:
        raise RuntimeError("G2-v3 dry-run custody mismatch")
    LOCK.mkdir()
    failure = None
    prepared = {}
    cleanup_done = False
    ...
    try:
        ...
        (RESULTS / "rows-v3").mkdir()
        (RESULTS / "CHRONOLOGY-v3.jsonl").write_text("")
        chronology("campaign-start", started_ns, planned_rows=2, order="BA")
        copy_methodology()
        control_binary, candidate_binary = snapshot_binaries()
        (RESULTS / "G2-V3-SCHEDULE.json").write_text(json.dumps(schedule(), indent=2, sort_keys=True) + "\n")
        (RESULTS / "CHRONOLOGY-PLAN-v3.json").write_text(json.dumps(child_plan(), indent=2, sort_keys=True) + "\n")
        (RESULTS / "INPUT-BINDINGS-v3.json").write_text(json.dumps({"preflight": preflight_record, "hashes": HASHES, "methodology_manifest_sha256": sha256(MANIFEST), "dry_run_sha256": sha256(DRY_RUN)}, indent=2, sort_keys=True) + "\n")
        (RESULTS / "rows-v3/G2-V3-RAW.jsonl").write_text("")
        prepared = prepare_rows(schedule(), started_ns)
        for spec in schedule():
        ...
        primary_result = json.loads((RESULTS / "G2-V3-ANALYSIS.json").read_text())
        independent_result = json.loads((RESULTS / "G2-V3-INDEPENDENT-RECOMPUTATION.json").read_text())
        if primary.returncode or independent.returncode or (primary_result["status"], primary_result["disposition"], sorted(primary_result["failures"]), primary_result["normalized_ledger"]) != (independent_result["status"], independent_result["disposition"], sorted(independent_result["failures"]), independent_result["normalized_ledger"]):
            raise RuntimeError("G2-v3 analyzers failed or disagreed")
        ...
        try:
            if RESULTS.exists():
                try:
                    if not cleanup_done and (RESULTS / "rows-v3/work-v3").exists():
...
if __name__ == "__main__":
    try:
        ...
        print(f"G2-v3 failure: {type(error).__name__}: {error}", file=sys.stderr)
        raise SystemExit(1)
