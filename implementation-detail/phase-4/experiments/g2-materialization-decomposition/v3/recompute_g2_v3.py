#!/usr/bin/env python3
"""Independent recomputation for the prospective G2-v3 protocol closure."""
...
def artifact_failures(results):
    ...
    for item in custody:
        ...
        if not copied.is_file() or not source.is_file() or file_hash(copied) != digest or copied.stat().st_size != size or mode(copied) != "0500" or item.get("sha256") != digest or item.get("size_bytes") != size or item.get("copy_mode") != "0500" or item.get("execution_path") != "snapshot-only":
...
def normalized_ledger(failures, sealed_rows, primary, observer, fresh_rows, custody, cleanup, plan):
...
def recompute(results):
    ...
    fresh_rows = [json.loads(line) for line in raw_v3.read_text().splitlines() if line] if raw_v3.is_file() else []
    if len(fresh_rows) != 2 or {row.get("arm") for row in fresh_rows} != {"A", "B"}:
        ...
        for arm, row in pair.items():
            failures.extend(mismatch(row, {**expected_schedule[arm], "cli_operation": "edit-same", "kind": "measured", "workload": "v3-guard", "validation": "capture-only", "base_copy_method": "physical-byte-copy-identical-database-authority-expectations"}, f"fresh-edit:{arm}:schedule"))
            decomposition = row.get("g2_decomposition")
    ...
    campaign_failures, custody, cleanup, plan = artifact_failures(results)
    failures.extend(campaign_failures)
    ledger = normalized_ledger(failures, sealed_rows, primary_artifact, observer, fresh_rows, custody, cleanup, plan)
...
def main():
    ...
    result = {
        "schema": "phase4-g2-protocol-closure-independent-recomputation-v3",
        "status": status,
    ...
    (args.results / "G2-V3-INDEPENDENT-RECOMPUTATION.json").write_text(json.dumps(result, indent=2, sort_keys=True) + "\n")
