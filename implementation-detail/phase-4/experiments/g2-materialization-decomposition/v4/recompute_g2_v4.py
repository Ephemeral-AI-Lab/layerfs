#!/usr/bin/env python3
"""Independent recomputation for the prospective G2-v4 protocol closure."""
...
def recompute(results):
    ...
    fresh_rows = [json.loads(line) for line in raw_v4.read_text().splitlines() if line] if raw_v4.is_file() else []
    if len(fresh_rows) != 2 or {row.get("arm") for row in fresh_rows} != {"A", "B"}:
        ...
        for arm, row in pair.items():
            failures.extend(mismatch(row, {**expected_schedule[arm], "cli_operation": "edit-same", "kind": "measured", "workload": "v4-guard", "validation": "capture-only", "base_copy_method": "physical-byte-copy-identical-database-authority-expectations"}, f"fresh-edit:{arm}:schedule"))
            decomposition = row.get("g2_decomposition")
...
def main():
    ...
    result = {
        "schema": "phase4-g2-protocol-closure-independent-recomputation-v4",
        "status": status,
    ...
    (args.results / "G2-V4-INDEPENDENT-RECOMPUTATION.json").write_text(json.dumps(result, indent=2, sort_keys=True) + "\n")
