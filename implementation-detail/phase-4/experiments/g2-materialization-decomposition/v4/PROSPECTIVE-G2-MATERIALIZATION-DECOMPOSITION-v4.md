# Prospective G2-v4 protocol closure
...
Its raw JSONL is empty. V4 neither relabels nor edits v3.

G2-v4 changes one orchestration variable only: a transient base proxy supplies
the sealed database through a symlink and the same authority bytes through one
private 0600 copy. It otherwise composes the sealed v1 decomposition and
...
The v4 analyzers consume, without copying or modifying, these sealed v1 facts:
...
V4 also binds the sealed v3 payload manifest
...
artifact. V4 does not reinterpret the two historical edit rows as new rows.
...
`target/phase4-g2-materialization-decomposition-20260822-v4`
...
`results-v4/operands-v4`, rehashed, and used only from those snapshots. The v1
recorded instrumented source and source-only diff bytes were reverted and are
not retained, so v4 explicitly does not claim they are byte-reverifiable.
...
The sealed fixture and base database are referenced read-only. V4 creates
`work-v4/base-proxy/base.sqlite` as a symlink to the sealed database and a
private `base.sqlite.authority` copy with the same
`7855ea6096359925f639b91c8d6b9708cfe0bc0df4a3ffd97a280a8e9a9ded48`
hash and exact 0600 mode. Source authority mode remains untouched at 0444.
...
the proxy. `BASE-PROXY-CUSTODY-v4.json` records target/source hashes, modes,
...
declared `results-v4/rows-v4/work-v4` path. Apparent and allocated regular-file
...
custody; sealed v3 failure anchors; base-proxy custody; chronology; cleanup;
...
terminal/verification, sealed v3 terminal/verification/payload/empty raw, and
...
The v4 lock is removed and its absence proved before any authoritative PASS
...
unseals only v4, regenerates authoritative REVISE evidence, reseals it, and
exits nonzero; no failure path can leave authoritative PASS.

The private proxy authority plus the two private row copies are transient.
Cleanup requires the exact work
path to exist, verifies both rows and the <=300-MiB peak, deletes exactly
`results-v4/rows-v4/work-v4`, and proves its absence before either analyzer.
Both analyzers gate cleanup PASS and its exact deletion set. The executable snapshots remain. This keeps
retained v4 evidence below 10 MiB without deleting any v1 evidence.
...
Even after G2-v4 PASS, G3 is ineligible until full workspace tests, Clippy
with `-D warnings`, formatting, and diff-check static closure all pass.

## Authorized commands before execute mode

After `METHODOLOGY-MANIFEST-v4.tsv` freezes this document and both analyzers:

```text
python3 analyze_g2_v4.py --self-test
python3 recompute_g2_v4.py --self-test
python3 -m py_compile analyze_g2_v4.py recompute_g2_v4.py run_g2_v4.py
G2_V4_METHODOLOGY_SHA256=<frozen-manifest-sha256> python3 run_g2_v4.py --dry-run
git diff --check -- implementation-detail/phase-4/experiments/g2-materialization-decomposition/v4
```
