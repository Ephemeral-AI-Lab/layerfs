#!/usr/bin/env python3
"""Primary analyzer for the prospective G2-v4 protocol closure."""
...
def v3_history_failures():
    failures = []
    for path, expected in V3_HASHES.items():
        if not path.is_file() or sha256(path) != expected:
            failures.append(f"v3-custody:{path.name}")
    if failures:
        return failures
    status = json.loads((V3 / "STATUS-v3.json").read_text())
    terminal = json.loads((V3 / "TERMINAL-v3.json").read_text())
    if status != {"disposition": "G2 REVISE", "fresh_rows": 0, "g3_eligible": False, "post_pass_static_closure_required": True, "reason": "RuntimeError: child failed: prepare-01-measured-same-middle-pos1-B", "sealed_v1_rows_rerun": 0, "status": "REVISE"}:
        failures.append("v3-status-shape")
    if terminal.get("status") != "REVISE" or terminal.get("fresh_raw_sha256") != V3_HASHES[V3 / "rows-v3/G2-V3-RAW.jsonl"] or (V3 / "preparation-v3/prepare-01-measured-same-middle-pos1-B.stderr").read_text() != "Error: ValidationAuthorityUnavailable\n":
        failures.append("v3-pre-row-failure-shape")
    if (REPO / "target/phase4-g2-materialization-decomposition-20260822-v3.lock").exists() or any(item.stat().st_mode & 0o222 for item in (V3.parent, *V3.parent.rglob("*")) if not item.is_symlink()):
        failures.append("v3-seal-or-lock")
    return failures
...
def edit_pair_failures(rows):
    ...
    if len(rows) != 2 or {row.get("arm") for row in rows} != {"A", "B"}:
        return ["v4-edit-pair-shape"]
    by_arm = {row["arm"]: row for row in rows}
    for arm, row in by_arm.items():
        prefix = f"v4:{arm}:{row.get('label', 'unlabeled')}"
        ...
        failures.extend(exact_failures(row, {**expected_schedule, "cli_operation": "edit-same", "kind": "measured", "workload": "v4-guard", "validation": "capture-only", "base_copy_method": "physical-byte-copy-identical-database-authority-expectations"}, prefix))
...
def expected_plan(results):
    ...
    plan.extend((
        {"kind": "analyzer", "label": "primary-analysis", "command": [sys.executable, str(HERE / "analyze_g2_v4.py"), str(results)]},
        {"kind": "analyzer", "label": "independent-recomputation", "command": [sys.executable, str(HERE / "recompute_g2_v4.py"), str(results)]},
    ))
    return plan
...
def artifact_failures(results, analyzer_label):
    failures = []
    custody_path = results / "OPERAND-CUSTODY-v4.json"
    cleanup_path = results / "TRANSIENT-VERIFICATION-v4.json"
    plan_path = results / "CHRONOLOGY-PLAN-v4.json"
    chronology_path = results / "CHRONOLOGY-v4.jsonl"
    proxy_path = results / "BASE-PROXY-CUSTODY-v4.json"
    if not all(path.is_file() for path in (custody_path, cleanup_path, plan_path, chronology_path, proxy_path)):
        return ["v4-artifact-set-missing"], [], {}, [], {}
    ...
    if len(custody) != 2 or {item.get("name") for item in custody} != set(expected_operands):
        failures.append("v4-operand-custody-shape")
    for item in custody:
        ...
        if not copied.is_file() or source != expected_source or not source.is_file() or sha256(copied) != digest or copied.stat().st_size != size or mode(copied) != "0500" or mode(source) != "0444" or item.get("sha256") != digest or item.get("size_bytes") != size or item.get("source_mode") != "0444" or item.get("copy_mode") != "0500" or item.get("execution_path") != "snapshot-only":
            failures.append(f"v4-operand-{item.get('name')}:bytes-mode")
            continue
        if (source.stat().st_dev, source.stat().st_ino, copied.stat().st_dev, copied.stat().st_ino) != (item.get("source_device"), item.get("source_inode"), item.get("copy_device"), item.get("copy_inode")) or (source.stat().st_dev, source.stat().st_ino) == (copied.stat().st_dev, copied.stat().st_ino):
            failures.append(f"v4-operand-{item.get('name')}:inode")
    cleanup = json.loads(cleanup_path.read_text())
    proxy = json.loads(proxy_path.read_text())
    proxy_expected = proxy.get("status") == "PASS" and proxy.get("database_is_symlink") is True and proxy.get("database_resolved_path") == str((SEALED / "input-v1/base.sqlite").resolve()) and proxy.get("database_resolved_sha256") == "7db8d50de42b994546789cb67fc7a9b650e2e551dab118e15003e02106b19890" and proxy.get("database_resolved_mode") == "0444" and proxy.get("authority_private_sha256") == "7855ea6096359925f639b91c8d6b9708cfe0bc0df4a3ffd97a280a8e9a9ded48" and proxy.get("authority_private_mode_actual") == "0600" and proxy.get("authority_distinct_device_inode") is True and proxy.get("expectations_copy") is False and proxy.get("work_path_absent_after_cleanup") is True
    if not proxy_expected:
        failures.append("v4-base-proxy-custody")
    cleanup_expected = cleanup.get("status") == "PASS" and cleanup.get("base_proxy_ready") is True and cleanup.get("declared_deletions") == ["rows-v4/work-v4"] and cleanup.get("deleted") == ["rows-v4/work-v4"] and cleanup.get("work_path_existed") is True and cleanup.get("work_path_absent") is True and cleanup.get("rows_validated") is True and cleanup.get("usage", {}).get("within_ceiling") is True and not (results / "rows-v4/work-v4").exists()
    if not cleanup_expected:
        failures.append("v4-cleanup-contract")
    plan = json.loads(plan_path.read_text())
    expected = expected_plan(results)
    if plan != expected:
        failures.append("v4-chronology-plan")
    ...
    if observed != events[:prefix_length]:
        failures.append("v4-chronology-prefix")
...
def normalized_ledger(results, failures, v1_rows, primary, observer, fresh_rows, custody, cleanup, plan, proxy):
    ...
    gate_names = ("v1_custody_and_closure", "v3_historical_failure", "v1_observer", "v1_primary_pairs", "v1_positions", "v1_balanced_center", "v1_read_work", "v1_guard_work", "v1_operation_storage", "eligible_families", "fresh_semantics_and_edit", "fresh_storage", "fresh_durability", "fresh_q_rss_runtime", "operand_custody", "base_proxy_custody", "chronology", "cleanup", "retained_evidence")
    gates = {name: {"pass": not sorted_failures, "failure_ids": sorted_failures} for name in gate_names}
    return {
        "schema": "phase4-g2-v4-normalized-gate-ledger-v1",
...
def analyze(results):
    ...
    if failures:
        return failures, {}
    failures.extend(v3_history_failures())
    ...
    raw_v4 = results / "rows-v4/G2-V4-RAW.jsonl"
    if not raw_v4.is_file():
        failures.append("v4-raw-missing")
        v4_rows = []
    else:
        v4_rows = [json.loads(line) for line in raw_v4.read_text().splitlines() if line]
    failures.extend(edit_pair_failures(v4_rows))
    campaign_failures, custody, cleanup, plan, proxy = artifact_failures(results, "primary-analysis")
    failures.extend(campaign_failures)
    ledger = normalized_ledger(results, failures, v1_rows, primary, observer, v4_rows, custody, cleanup, plan, proxy)
...
def main():
    ...
    output = {
        "schema": "phase4-g2-protocol-closure-primary-analysis-v4",
        "status": status,
    ...
    (args.results / "G2-V4-ANALYSIS.json").write_text(json.dumps(output, indent=2, sort_keys=True) + "\n")
