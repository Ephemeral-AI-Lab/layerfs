...
impl Overlay {
    ...
    pub(crate) fn acquire(&self) -> Result<Arc<OverlayRoot>> {
        let current = self.current.lock().map_err(|_| {
            StoreError::Integrity("overlay installation lock")
        })?;
    ...
    pub(crate) fn prepare(
        ...
        let mut mutation = Mutation {
            index: &self.index,
            candidate: OverlayRoot {
                sequence: source.sequence.checked_add(1).ok_or(StoreError::InvalidInput(
                    "overlay sequence overflow",
                ))?,
        ...
        apply(&mut mutation)?;
        Ok(PreparedMutation { source, candidate: Arc::new(mutation.candidate) })
    }
    ...
    pub(crate) fn inode(&self, root: &OverlayRoot, inode: NodeId) -> Result<Option<Vec<u8>>> {
        Ok(self.index.get(&root.index, &ChangeKey::Inode(inode).encode()?)?)
    }

    /// None means consult immutable backing; Some(None) is a retained mask.
    pub(crate) fn binding(
        &self, root: &OverlayRoot, parent: NodeId, name: &[u8],
    ) -> Result<Option<Option<NodeId>>> {
        self.index.get(&root.index, &ChangeKey::Binding(parent, name.to_vec()).encode()?)?
            .map(|bytes| {
    ...
    pub(crate) fn changes(
        &self, root: &OverlayRoot, covered: u64, after: Option<&[u8]>,
        ...
        self.index.scan(&root.index, start, Bound::Excluded(&[CHANGE_SEQUENCE + 1]), CHANGE_BATCH)?
...
impl Mutation<'_> {
    pub(crate) fn allocate_inode(&mut self) -> Result<NodeId> {
        ...
        self.candidate.next_inode = id.checked_add(1)
    ...
    pub(crate) fn put_binding(
        &mut self, parent: NodeId, name: &[u8], inode: Option<NodeId>,
    ) -> Result<()> {
        ...
        self.candidate.index = self.index.set(
            &self.candidate.index, &key, &inode.map_or(0, |id| id.0).to_be_bytes(),
        )?;
        self.changed(&key)
    }
    ...
    fn changed(&mut self, key: &[u8]) -> Result<()> {
        ...
        self.candidate.index = self.index.set(
            &self.candidate.index, &by_key, &self.candidate.sequence.to_be_bytes(),
        )?;
        self.candidate.index = self.index.set(
            &self.candidate.index, &sequence_key(self.candidate.sequence, key), &[],
        )?;
        Ok(())
    }
    ...
    pub(crate) fn forget_covered(&mut self, key: &ChangeKey, sequence: u64) -> Result<()> {
        ...
        if self.index.get(&self.candidate.index, &by_key)?.as_deref()
            ...
            self.candidate.index = self.index.remove(
                &self.candidate.index, &sequence_key(sequence, &key),
            )?;
        }
        Ok(())
    }
...
impl ReplayWindow {
    pub(crate) fn new(session: [u8; 16], max_requests: usize, max_bytes: usize) -> Result<Self> {
        ...
        Ok(Self { session, watermark: 0, max_requests, max_bytes,
            reserved_bytes: 0, slots: BTreeMap::new() })
    }

    pub(crate) fn admit(
        &mut self, session: [u8; 16], sequence: u64, response_bytes: usize,
    ...
    pub(crate) fn acknowledge(&mut self, session: [u8; 16], through: u64) -> Result<()> {
        ...
        for sequence in self.watermark + 1..=through {
            if self.slots.get(&sequence).is_none_or(|slot| slot.result.is_none()) {
                return Err(StoreError::InvalidInput("unresolved overlay acknowledgment"));
            }
        }
        for sequence in self.watermark + 1..=through {
...
mod tests {
    ...
    fn overlay() -> Overlay {
        Overlay::temporary(&std::env::temp_dir(), ObjectId::for_bytes(b"base"), Limits {
            max_pages: 4096, max_roots: 128, max_key_bytes: 384, max_value_bytes: 1024,
        }).unwrap()
    }
    ...
    fn stale_disjoint_writer_reprepares_without_losing_updates_or_retained_roots() {
        ...
        let a = overlay.prepare(before.clone(), |m| m.put_inode(NodeId(2), b"A")).unwrap();
        let b = overlay.prepare(before.clone(), |m| m.put_inode(NodeId(3), b"B")).unwrap();
        assert!(overlay.install(a).unwrap());
        let captured = overlay.acquire().unwrap();
        assert!(!overlay.install(b).unwrap());
        let b = overlay.prepare(overlay.acquire().unwrap(), |m| m.put_inode(NodeId(3), b"B")).unwrap();
        assert!(overlay.install(b).unwrap());
        let current = overlay.acquire().unwrap();
        assert_eq!(overlay.inode(&current, NodeId(2)).unwrap().as_deref(), Some(&b"A"[..]));
        assert_eq!(overlay.inode(&current, NodeId(3)).unwrap().as_deref(), Some(&b"B"[..]));
        assert_eq!(overlay.inode(&captured, NodeId(3)).unwrap(), None);
        assert_eq!(overlay.inode(&before, NodeId(2)).unwrap(), None);
        assert_eq!(overlay.changes(&current, captured.sequence, None).unwrap()[0].2, ChangeKey::Inode(NodeId(3)));
    }

    #[test]
    fn tracking_replaces_old_sequence_and_cleanup_keeps_masks_and_newer_changes() {
        let overlay = overlay();
        let first = overlay.prepare(overlay.acquire().unwrap(), |m| {
        ...
        let second = overlay.prepare(snapshot.clone(), |m| m.put_inode(NodeId(2), b"B")).unwrap();
        assert!(overlay.install(second).unwrap());
        let cleanup = overlay.prepare(overlay.acquire().unwrap(), |m| {
        ...
        let current = overlay.acquire().unwrap();
        assert_eq!(overlay.binding(&current, NodeId(1), b"removed").unwrap(), Some(None));
        let changes = overlay.changes(&current, 0, None).unwrap();
        assert_eq!(changes.len(), 1);
        assert_eq!((changes[0].1, &changes[0].2), (2, &ChangeKey::Inode(NodeId(2))));
    ...
    fn failed_preparation_never_installs_partial_state() {
        let overlay = overlay();
        let before = overlay.acquire().unwrap();
        assert!(overlay.prepare(before.clone(), |m| {
    ...
    fn replay_retains_lost_reply_and_never_reapplies_stale_or_unknown_request() {
        ...
        assert!(matches!(replay.admit(session, 1, 8).unwrap(), ReplayAdmission::New));
        assert!(matches!(replay.admit(session, 1, 8).unwrap(), ReplayAdmission::Unknown));
        ...
        assert!(matches!(replay.admit([2; 16], 1, 8).unwrap(), ReplayAdmission::Unknown));
        ...
        assert!(matches!(replay.admit(session, 3, 1).unwrap(), ReplayAdmission::New));
        replay.finish(3, Arc::from(&b"x"[..])).unwrap();
        assert!(replay.acknowledge(session, 3).is_err());
        assert!(matches!(replay.admit(session, 2, 1).unwrap(), ReplayAdmission::New));
