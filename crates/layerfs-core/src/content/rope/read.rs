...
fn flush_read_batch<S: ObjectRead, W: Write>(
    ...
    let requests = selected
        .iter()
        .map(|(extent, overlap_start, overlap_end)| {
            let source_end = extent
                .source_offset
                .checked_add(extent.logical_length)
                .ok_or(CoreError::LengthOverflow)?;
            if source_end as usize > crate::cdc::MAXIMUM_CHUNK_BYTES {
                return Err(CoreError::ChunkLengthMismatch);
            }
            let start = u64::from(extent.source_offset)
                .checked_add(*overlap_start)
                .ok_or(CoreError::LengthOverflow)?;
            let end = u64::from(extent.source_offset)
                .checked_add(*overlap_end)
                .ok_or(CoreError::LengthOverflow)?;
            Ok((extent.payload_object_id, start..end))
        })
        .collect::<CoreResult<Vec<_>>>()?;
    let mut index = 0_usize;
    store.get_authenticated_payload_ranges_batch(
        &requests,
        crate::cdc::MAXIMUM_CHUNK_BYTES as u64,
        |id, payload| {
            let (extent, _, _) = selected
                .get(index)
                .copied()
            ...
            index += 1;
            sink.write_all(payload).map_err(|_| CoreError::Io)?;
            counters.payload_bytes_read = add(counters.payload_bytes_read, payload.len() as u64)?;
            Ok(())
        },
    )?;
