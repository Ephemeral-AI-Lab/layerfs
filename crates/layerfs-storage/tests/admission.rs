...
use layerfs_storage::{
    object_batches, CanonicalObject, MissingBitmap, StoreDb, StoreRole, OBJECT_BATCH_BYTES,
};
...
fn valid_oversize_singleton_is_admitted() {
    ...
    let db = StoreDb::create(root.join("store.sqlite"), StoreRole::Layer).unwrap();
    let object = CanonicalObject::new(
        layerfs_content::encode_bytes_object(&vec![7; OBJECT_BATCH_BYTES + 1]).unwrap(),
    )
    .unwrap();
