...
fn commit_busy_rolls_back_primary_before_reconciliation_and_engine_reuse() {
    ...
    assert!(matches!(
        error,
        EngineError::Sqlite {
            kind: layerfs_storage::SqliteErrorKind::Busy,
            ..
        }
    ));
...
fn initial_verified_scrub_serializes_against_trusted_writer() {
    ...
    assert!(matches!(
        ...
        Err(EngineError::Sqlite {
            kind: layerfs_storage::SqliteErrorKind::Busy | layerfs_storage::SqliteErrorKind::Locked,
            ..
        })
    ));
    drop(publication);
...
fn successful_live_scrub_clears_history_once_durably() {
    ...
    assert_eq!(first.integrity_statements, 11);
    assert_eq!(first.statements, 11);
    assert_eq!(first.primary_read_statements, 3);
    assert_eq!(first.live_verified_integrity_statements, 8);
    assert_eq!(
        Connection::open(&path)
            .unwrap()
            .query_row(
