use crate::admission::{
    admit_fact_rows, admit_object_rows, final_object_batch, finish_local_transaction,
    insert_commit, insert_layer, insert_stack, merge_local_objects, note_local_fact,
};
use crate::candidate::object_batches;
use crate::schema::require_full;
use crate::{
    AdmissionStats, BaseId, BranchId, BranchRecord, CanonicalObject, CommitId, CommitRecord,
    DatabaseReceipt, DeferredObjectStore, Fact, LayerHistoryRecord, LayerId, LayerRecord,
    LocalAdmissionReceipt, ObjectSource, Result, SchemaKind, SourceId, StackHistoryRecord, StackId,
    StackRecord, StorageError, StorageId, StorageReceipt, StoreDb, TransferIntent, TransferOutcome,
    FACT_BATCH_BYTES, FACT_BATCH_COUNT, ID_BATCH_COUNT,
};
...
impl StoreDb {
    ...
    pub fn commit_branch(
        ...
        let (last, mut receipt) = objects
            .map(|objects| final_object_batch(self, objects))
            .transpose()?
            .unwrap_or_default();
        let mut connection = self.connection()?;
        let transaction = connection.transaction()?;
        let final_objects = last
            .as_deref()
            .map(|objects| admit_object_rows(&transaction, objects))
            .transpose()?;
        let commit_inserted = insert_commit(&transaction, commit)?;
        ...
        if changed != 1 {
            ...
            receipt.database.rollback_transactions += 1;
            crate::transfer::record(StorageReceipt::Local(receipt));
            return Err(StorageError::CommitHeadMoved(crate::HeadMoved {
                expected: Some(expected),
                actual,
            }));
        }
        let commit_sync = std::time::Instant::now();
        transaction.commit()?;
        if let Some(objects) = final_objects {
            merge_local_objects(&mut receipt, objects);
        }
        note_local_fact(&mut receipt, Fact::Commit(commit), commit_inserted);
        finish_local_transaction(&mut receipt, last.is_some(), true, commit_sync.elapsed());
        crate::transfer::record(StorageReceipt::Local(receipt));
        Ok(())
    }
    ...
    pub fn provision_layer_history(
        ...
        require_full(self)?;
        let (last, mut receipt) = final_object_batch(self, objects)?;
        let mut connection = self.connection()?;
        let transaction = connection.transaction()?;
        let final_objects = last
            .as_deref()
        ...
        note_local_fact(&mut receipt, Fact::Layer(layer), true);
        note_local_fact(&mut receipt, Fact::LayerHistory(history), true);
        finish_local_transaction(&mut receipt, last.is_some(), true, commit_sync.elapsed());
        crate::transfer::record(StorageReceipt::Local(receipt));
        Ok(())
    }
    ...
    pub fn create_stack_history_record(
        ...
        note_local_fact(&mut receipt, Fact::Stack(stack), true);
        note_local_fact(&mut receipt, Fact::StackHistory(history), true);
        finish_local_transaction(&mut receipt, false, true, commit_sync.elapsed());
        crate::transfer::record(StorageReceipt::Local(receipt));
        Ok(())
    }
    ...
    pub fn add_layer_atomic(
        ...
        note_local_fact(
            &mut receipt,
            Fact::AddResult(crate::AddResultRecord {
                source_id: source,
                result_id: crate::ResultId::Layer(result),
            }),
            true,
        );
        finish_local_transaction(&mut receipt, last.is_some(), true, commit_sync.elapsed());
    ...
    pub fn add_stack_atomic(
        ...
        note_local_fact(
            &mut receipt,
            Fact::AddResult(crate::AddResultRecord {
                source_id: crate::SourceId::Branch(branch_id),
                result_id: crate::ResultId::Stack(result),
            }),
            true,
        );
        finish_local_transaction(&mut receipt, last.is_some(), true, commit_sync.elapsed());
    ...
    fn finish_transfer_with_authentication(
        ...
        if let Some(page) = final_object {
            admission
                .objects
                .merge(admit_object_rows(&transaction, page)?);
        }
        if let Some(page) = final_fact {
            admission
                .facts
                .entry(page[0].kind())
                .or_default()
                .merge(admit_fact_rows(&transaction, self.kind(), page)?);
        }
        ...
        if final_object.is_some() || final_fact.is_some() || changed {
            ...
            transaction.commit()?;
            admission.database.merge(DatabaseReceipt {
                write_transactions: 1,
                object_admission_transactions: u64::from(final_object.is_some()),
                fact_admission_transactions: u64::from(final_fact.is_some()),
                visibility_transactions: u64::from(changed),
