...
const SCHEMA_VERSION: i64 = schema::SCHEMA_VERSION;
pub const TRANSITION_FORMAT_VERSION: i64 = schema::TRANSITION_FORMAT_VERSION;
...
pub struct Engine {
    ...
    fetch_boundary_failure: AtomicBool,
...
impl Engine {
    ...
    fn copy_compaction_metadata(&self, candidate: &Connection) -> EngineResult<()> {
        ...
        for (table, _) in schema::PRODUCT_SCHEMAS {
            self.mark_compaction_sql(1)?;
            candidate
                .execute(
...
fn initial_verified_scrub(
    ...
    match product::verify_product_integrity(connection) {
...
fn admit_schema_counted(
    ...
    if state == SchemaState::Current {
        for (name, expected) in schema::PRODUCT_SCHEMAS {
            validate_table_shape(connection, statements, name, expected)?;
        }
    }
...
fn validate_authority(connection: &Connection, statements: &mut u64) -> EngineResult<()> {
    note_statement(statements)?;
...
fn create_current_schema(
    ...
    connection
        ...
        .map_err(map_sqlite_error)?;
    let result = (|| {
        for (_, sql) in schema::BASE_SCHEMAS {
            note_statement(statements)?;
            connection.execute_batch(sql).map_err(map_sqlite_error)?;
        }
        for (_, sql) in schema::PRODUCT_SCHEMAS {
            note_statement(statements)?;
            connection.execute_batch(sql).map_err(map_sqlite_error)?;
        }
        note_statement(statements)?;
