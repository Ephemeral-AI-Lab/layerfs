    ...
    let result = (|| {
        let chunk_count = {
            ...
            while remaining != 0 {
                ...
                if let Err(error) = stream.push(Ok(&buffers.source[..read]), control, &mut consumer)
                {
...
pub(crate) fn write_chunk_object<O: PreparedObjectSinkV1 + ?Sized>(
    ...
    let header = object_header(PhysicalObjectKindV1::Chunk, payload_len);
...
>(
    ...
    let header = object_header(PhysicalObjectKindV1::File, payload_len);
...
pub(crate) fn object_header(kind: PhysicalObjectKindV1, payload_len: u64) -> [u8; 52] {
    let mut header = [0_u8; 52];
    header[..8].copy_from_slice(b"ELSOBJ01");
    ...
    header[44..].copy_from_slice(&payload_len.to_be_bytes());
    header
}
