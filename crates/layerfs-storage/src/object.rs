...
use crate::identity::{
    ...
    FramedHasherV1, PhysicalChunkIdV1, PhysicalFileIdV1, PhysicalSymlinkIdV1, PhysicalTreeIdV1,
    PhysicalVersionRecordIdV1, ProfileId, VersionIdV1, COMPARISON_WINDOW_BYTES, TAG_PHYSICAL_CHUNK,
    TAG_PHYSICAL_FILE, TAG_PHYSICAL_SYMLINK, TAG_PHYSICAL_TREE, TAG_PHYSICAL_VERSION_RECORD,
};
...
const OBJECT_MAGIC: &[u8; 8] = b"ELSOBJ01";
const TREE_MIN_PAYLOAD_BYTES: u64 = 9;
const FILE_MIN_PAYLOAD_BYTES: u64 = 14;
const SYMLINK_MIN_PAYLOAD_BYTES: u64 = 5;
const MIN_TREE_LEAF_ENTRY_BYTES: u64 = 36;
const MIN_TREE_INDEX_ENTRY_BYTES: u64 = 42;
const MIN_FILE_EXTENT_BYTES: u64 = 9;
const CHUNK_REFERENCE_BYTES: u64 = 36;
const HOLE_MIN_BYTES: u64 = 65_536;
...
pub struct ValidatedPhysicalObjectV1<'a> {
    ...
    canonical_object: &'a [u8],
}

/// Random-readable canonical object bytes. Implementations must either fill
/// the complete destination or return an error. The decoder never requests a
/// range outside the length returned by `len`.
...
/// Validation result for an object decoded through bounded random reads.
/// Unlike [`ValidatedPhysicalObjectV1`], this value retains no source bytes.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct ValidatedPhysicalObjectFromPortV1 {
    header: PhysicalObjectHeaderV1,
    payload: PhysicalObjectPayloadV1,
    physical_id: TypedPhysicalObjectIdV1,
}
...
/// own independently bounded policy; this decoder itself retains no edges.
...
fn decode_envelope(bytes: &[u8]) -> CoreResult<(PhysicalObjectHeaderV1, &[u8])> {
    ...
    cursor.expect(OBJECT_MAGIC, CoreError::TypeDomain)?;
...
fn validate_payload_bound(kind: PhysicalObjectKindV1, payload_len: u64) -> CoreResult<()> {
    match kind {
