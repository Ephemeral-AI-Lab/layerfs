...
impl ByteBudget {
    ...
    fn try_reserve(self: &Arc<Self>, bytes: usize) -> Result<ByteReservation, MountedError> {
        if bytes > self.limit {
            return Err(MountedError::ResourceExhausted);
        }
        let mut state = self.state.lock().map_err(|_| MountedError::Indeterminate)?;
...
impl MountedWorkspace {
    ...
    pub fn rename(
        ...
        if target.is_some() {
            mutations.push(self.prepare_directory_entry(new_parent, new_name.clone(), None)?);
        }
        mutations.push(self.prepare_directory_entry(parent, name, None)?);
        mutations.push(self.prepare_directory_entry(new_parent, new_name, Some(source))?);
    ...
    pub fn rollback(&mut self, target: ObjectId) -> Result<RefState, MountedError> {
        ...
        let state = self.engine.move_ref(&self.accepted, target)?;
    ...
    fn load_canonical_node(
        ...
        if self.nodes.len() == MAX_MOUNTED_NODES
            || !self.by_inode.contains_key(&inode) && self.by_inode.len() == MAX_MOUNTED_NODES
        {
            return Err(MountedError::ResourceExhausted);
        }
    ...
    fn finalize_deleted_pending(&mut self, node: MountedNodeId) -> Result<(), MountedError> {
        ...
        let entry = self.nodes.get_mut(&node).ok_or(MountedError::Corrupt)?;
        let removed = match &mut entry.content {
            NodeContent::File { ranges, .. } => {
                let bytes = ranges.iter().map(|(start, range)| range.end - *start).sum();
                self.live_ranges = self.live_ranges.saturating_sub(ranges.len());
                ranges.clear();
                bytes
            }
            _ => 0,
