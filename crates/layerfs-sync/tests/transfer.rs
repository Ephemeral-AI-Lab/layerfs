...
use layerfs_storage::product::BranchId;
use layerfs_sync::client::{abort_fetch_transfer, fetch_objects, push_objects};
...
fn push_fetch_are_bounded_known_present_and_visibility_free() {
    ...
    assert!(
        durable
            .sync_custody_rows(
                layerfs_storage::product::RequestId::from_bytes([0x72; 32]),
                "push",
            )
            .unwrap()
            > 0
    ...
    assert!(matches!(
        ...
        Some((owner, direction, rows))
            if owner == layerfs_storage::product::RequestId::from_bytes([0x72; 32])
    ...
    assert_eq!(
        working_b
            .sync_custody_rows(
                layerfs_storage::product::RequestId::from_bytes(fetch_request),
                "fetch",
            )
            .unwrap(),
        0
...
fn transferred_new_and_incumbent_corruption_fail_closed() {
    ...
    durable
        .sync_accept_objects(
            layerfs_storage::product::RequestId::from_bytes([0x80; 32]),
            layerfs_storage::product::RequestId::from_bytes([0x80; 32]),
            "fetch",
            &[(id, canonical.clone())],
        )
