...
pub(crate) trait TypedId: Copy + Eq + Ord {
    fn as_slice(&self) -> &[u8];
    ...
    ($name:ident, $len:expr, $tag:expr) => {
        ...
        impl $name {
            pub fn from_bytes(bytes: [u8; $len]) -> Result<Self> {
                if bytes[0] != $tag {
                    return Err(StoreError::Integrity("typed ID tag"));
                }
                Ok(Self(bytes))
            }
        ...
        impl TypedId for $name {
            ...
            fn from_slice(bytes: &[u8]) -> Result<Self> {
                Self::from_bytes(
                    bytes
                        .try_into()
                        .map_err(|_| StoreError::Integrity("typed ID length"))?,
                )
            }
        }
...
impl CommitId {
    pub fn derive(root_id: ObjectId, parent_id: Option<Self>, base_layer_id: LayerId) -> Self {
        ...
        update_optional(&mut hasher, parent_id.as_ref().map(TypedId::as_slice));
...
impl LayerId {
    pub fn derive(
        ...
        update_optional(&mut hasher, parent_layer_id.as_ref().map(TypedId::as_slice));
        hasher.update(root_id.as_bytes());
...
fn parse_hex<const N: usize>(value: &str) -> Result<[u8; N]> {
    if value.len() != N * 2 {
        return Err(StoreError::Integrity("typed ID text"));
    }
    let mut bytes = [0; N];
...
fn from_hex(value: u8) -> Result<u8> {
    match value {
        ...
        b'A'..=b'F' => Ok(value - b'A' + 10),
        _ => Err(StoreError::Integrity("typed ID text")),
    }
}
...
mod tests {
    ...
    #[test]
    fn ids_remain_typed_uuidv7_and_v2_domain_separated() {
