use crate::tree::directory::codec::{
    decode_node_value, exact_value, node_count, node_subtree_bytes, node_subtree_count, ordered,
    validate_node_header, VERSION,
};
...
pub fn encode_inode_table_node(node: &InodeTableNodeV1) -> CoreResult<Vec<u8>> {
    match node {
        InodeTableNodeV1::Leaf(entries) => encode_inode_page(
            0,
            entries.len() as u64,
        ...
        InodeTableNodeV1::Branch {
            ...
            encode_inode_page(
                *level,
                *subtree_entry_count,
                children.iter().map(|(key, id)| (key, id.as_bytes())),
            )
        }
    }
}
...
pub(crate) fn encode_inode_page<'a>(
    ...
    if level == 0 && subtree_count != count as u64 {
        return Err(CoreError::InvalidRecord("inode leaf summary"));
    }
    ordered(entries.clone().map(|(key, _)| key.as_bytes().as_slice()))?;
    let bytes = subtree_count
        .checked_mul(64)
        .ok_or(CoreError::LengthOverflow)?;
    let mut canonical = super::super::directory::codec::canonical_node_header(
        b"LFS4INT\0",
        if level == 0 { 7 } else { 8 },
        level,
        count,
...
pub(crate) fn encode_inode_leaf_from_iter(
    count: usize,
    entries: impl Iterator<Item = CoreResult<(InodeId, ObjectId)>>,
) -> CoreResult<Vec<u8>> {
    if count > 127 {
        return Err(CoreError::NonCanonicalPagePartition);
    }
    ...
    for entry in entries {
        ...
        if actual >= count {
            return Err(CoreError::InvalidRecord("spilled inode leaf summary"));
        }
        canonical.extend_from_slice(key.as_bytes());
...
pub fn decode_inode_table_node(canonical: &[u8]) -> CoreResult<InodeTableNodeV1> {
    ...
    if value[11] == 0 {
        ...
        Ok(InodeTableNodeV1::Branch {
            level: value[11],
            subtree_entry_count: node_subtree_count(value),
            children: entries,
        })
    }
}
