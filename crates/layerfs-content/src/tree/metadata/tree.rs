use super::codec::{decode_metadata_node, encode_metadata_node, MetadataNodeV1};
use super::{MetadataCounters, MetadataEntryV1, MetadataKey};
...
pub(super) struct MetadataCursor {
...
impl MetadataCursor {
    pub(super) fn new(root: ObjectId) -> Self {
        Self {
            stack: vec![MetadataWalkItem {
                id: root,
                root: true,
    ...
    pub(super) fn next<S: ObjectRead>(&mut self, store: &S) -> CoreResult<Option<MetadataEntryV1>> {
        loop {
...
pub(crate) fn reconcile_metadata_roots<S: ObjectStore>(
    store: &mut S,
    base: ObjectId,
    source: ObjectId,
