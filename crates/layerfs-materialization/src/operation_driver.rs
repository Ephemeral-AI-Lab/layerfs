...
use crate::driver::{ProjectionDriver, ProjectionWorkspace};
...
std::thread_local! {
    static FAIL_START: std::cell::Cell<bool> = const { std::cell::Cell::new(false) };
    static FAIL_REFRESH: std::cell::Cell<bool> = const { std::cell::Cell::new(false) };
}

#[cfg(feature = "test-hooks")]
#[doc(hidden)]
pub fn inject_start_failure_for_test() {
    FAIL_START.set(true);
}

#[cfg(feature = "test-hooks")]
#[doc(hidden)]
pub fn inject_refresh_failure_for_test() {
...
impl<'a> MaterializationDriver<'a> {
    ...
    fn start_with_custody(
        ...
        #[cfg(feature = "test-hooks")]
        if FAIL_START.replace(false) {
            return Err(crate::VfsError::InvalidState);
        }
        let workspace = driver.open_workspace(&view, working.storage_id())?;
    ...
    pub fn refresh_to(&mut self, target: VersionRef) -> VfsResult<OperationCounters> {
        ...
        #[cfg(feature = "test-hooks")]
        if FAIL_REFRESH.replace(false) {
            self.poisoned = true;
            self.candidate = None;
            return Err(crate::VfsError::IncompleteDerived);
        }
    ...
    pub fn managed_replace_range(
        ...
        if self.exclusive_managed {
            let canonical = (|| {
                let mut writer = self.working.begin_candidate_write().map_err(|error| {
                    crate::VfsError::Io(std::io::Error::other(error.to_string()))
                })?;
                ...
                writer
                    .commit_trusted_operation_candidate(self.operation_id, candidate)
                    .map_err(|error| {
                        crate::VfsError::Io(std::io::Error::other(error.to_string()))
                    })?;
                Ok::<_, crate::VfsError>((root, logical))
            })();
            let (root, logical) = match canonical {
                Ok(value) => value,
                Err(error) => {
                    self.poisoned = true;
                    self.candidate = None;
            ...
            counters.add_rope(logical.rope)?;
            counters.add_namespace(logical.namespace)?;
