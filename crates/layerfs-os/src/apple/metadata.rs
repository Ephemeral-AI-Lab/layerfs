use layerfs_vfs::driver::{DriverError, NativeMetadata, Result, MAX_NATIVE_XATTR_BYTES};
use std::fs::File;
...
pub fn read(file: &File) -> Result<NativeMetadata> {
    ...
    for name in super::ffi::list_xattrs_file(file)? {
...
pub fn write(file: &File, metadata: &NativeMetadata) -> Result<()> {
    ...
    for name in current {
        ...
        if !metadata.xattrs.iter().any(|entry| entry.0 == name) {
...
fn preflight_inner(file: &File, metadata: &NativeMetadata) -> Result<Vec<Vec<u8>>> {
    ...
    let current = super::ffi::list_xattrs_file(file)?;
    for name in &current {
...
mod tests {
    ...
    fn provenance_value_is_not_canonical_input_and_exclusions_stay_fatal() {
        ...
        assert!(!after.xattrs.iter().any(|entry| is_environmental(&entry.0)));

        for name in EXCLUDED {
            let mut invalid = before.clone();
            invalid.xattrs.push((name.to_vec(), b"forbidden".to_vec()));
            assert!(matches!(
                write(&file, &invalid),
    ...
    fn provenance_is_filtered_while_supported_xattrs_round_trip() {
        ...
        assert!(!first
            .xattrs
            .iter()
            .any(|entry| entry.0 == b"com.apple.provenance"));
        assert!(first
            .xattrs
            .iter()
            .any(|entry| entry == &(supported.to_vec(), b"exact-value".to_vec())));
