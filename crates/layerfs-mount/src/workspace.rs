...
use layerfs_core::metadata::{
    build_metadata_tree, metadata_lookup, replace_metadata_entry, MetadataCounters,
    MetadataEntryV1, MetadataKey, PortableMetadataV1,
};
    ...
    fn checkpoint_inner(&mut self) -> Result<ObjectId, MountedError> {
        ...
        for id in &dirty_ids {
            ...
            let snapshot = CheckpointNode {
                ...
                metadata_root: node.record.map(|record| record.metadata_root),
            };
            ...
            let metadata_root = if snapshot.canonical.is_none() || snapshot.dirty_metadata {
                let (root, counters) = persist_metadata(&mut publication, &snapshot)?;
                self.counters.metadata_cow_nodes_read = self
                    .counters
                    .metadata_cow_nodes_read
                    .checked_add(counters.nodes_read)
                    .ok_or(MountedError::ResourceExhausted)?;
                self.counters.metadata_cow_nodes_created = self
                    .counters
                    .metadata_cow_nodes_created
                    .checked_add(counters.nodes_created)
                    .ok_or(MountedError::ResourceExhausted)?;
                root
            } else {
    ...
    fn preflight_dirty(
        ...
        for (index, node) in nodes.iter().enumerate() {
            if !self.nodes.contains_key(node) {
                return Err(MountedError::NotFound);
            }
...
fn persist_metadata(
    publication: &mut WorkingCandidateWrite<'_>,
    node: &CheckpointNode,
) -> Result<(ObjectId, MetadataCounters), MountedError> {
    ...
    let root = if let Some(root) = node.metadata_root {
        let root = replace_metadata_entry(publication, root, mode, &mut counters)?;
        replace_metadata_entry(publication, root, mtime, &mut counters)?
    } else {
        build_metadata_tree(publication, &[mode, mtime])?
    };
    Ok((root, counters))
}
...
mod product_tests {
    ...
    use layerfs_core::metadata::{
        build_metadata_tree, metadata_tree_entries, MetadataEntryV1, MetadataKey,
    };
    ...
    #[test]
    fn metadata_checkpoint_updates_only_cow_spines() {
        let directory = std::env::temp_dir().join(format!(
            "layerfs-mount-metadata-cow-{}-{}",
            std::process::id(),
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap()
        ...
        let store = directory.join("working");
        let working = WorkingStore::open(&store, IntegrityMode::TrustedLocalDev).unwrap();
        let mut writer = working.begin_candidate_write().unwrap();
        let (value, _) = build(&mut writer, Cursor::new(b"value")).unwrap();
        let mut entries = (0..298_u16)
            .map(|index| MetadataEntryV1 {
                ...
                .unwrap(),
                value_file_root: value.0,
            })
            .collect::<Vec<_>>();
        let (mode, _) = build(&mut writer, Cursor::new(0o755_u32.to_be_bytes())).unwrap();
        let (mtime, _) = build(&mut writer, Cursor::new([0_u8; 12])).unwrap();
        ...
        let metadata_root = build_metadata_tree(&mut writer, &entries).unwrap();
        let snapshot = CheckpointNode {
            canonical: Some(InodeId::allocate(working.storage_id(), 999)),
            record: Some(InodeRecordV1 {
                kind: InodeKind::Directory,
                namespace_ref_count: 1,
                content_root: ObjectId::for_bytes(b"unused content"),
                metadata_root,
            }),
            kind: MountedFileType::Directory,
            mode: 0o700,
        ...
        let (updated, counters) = persist_metadata(&mut writer, &snapshot).unwrap();
        assert!(counters.nodes_read > 0, "{counters:?}");
        assert!(counters.nodes_read < 20, "{counters:?}");
        assert!(
            counters.nodes_created <= counters.nodes_read,
            "{counters:?}"
        );
        assert_eq!(
            metadata_tree_entries(&writer, metadata_root).unwrap(),
            entries
        );
        assert_eq!(metadata_tree_entries(&writer, updated).unwrap().len(), 300);
        assert_eq!(
            metadata_lookup(
                &writer,
                updated,
                &MetadataKey::new("apple.xattr".to_owned(), b"user.attribute-0173".to_vec())
                    .unwrap(),
            )
            .unwrap()
            .unwrap()
    ...
    #[test]
    fn metadata_after_a_hard_link_checkpoint_remains_checkpointable() {
        let directory = std::env::temp_dir().join(format!(
            "layerfs-mount-hard-link-metadata-{}-{}",
            std::process::id(),
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap()
        ...
        let stack = working
            ...
            .unwrap();
        let head = working
            .create_top_level_branch(BranchId::from_bytes([0x33; 32]), Some("main"), stack)
            .unwrap();
        ...
        .unwrap();
        let nested = mounted.mkdir(ROOT_NODE, b"nested", 0o755).unwrap();
        let (file, handle) = mounted.create_file(nested.node, b"file", 0o644).unwrap();
        mounted.write(file.node, handle, 0, b"data").unwrap();
        mounted.link(file.node, nested.node, b"alias").unwrap();
        mounted.fsync().unwrap();
        mounted.release(handle).unwrap();
        mounted.chmod(file.node, 0o640).unwrap();
        mounted.set_mtime(file.node, 1_700_000_000, 0).unwrap();
        let (orphan, orphan_handle) = mounted
            .create_file(ROOT_NODE, b"unlink-open", 0o644)
            .unwrap();
        mounted
            .write(orphan.node, orphan_handle, 0, b"open")
            .unwrap();
        mounted.unlink(ROOT_NODE, b"unlink-open").unwrap();
        assert_eq!(
            mounted.read(orphan.node, orphan_handle, 0, 4).unwrap(),
            b"open"
        );
    ...
    fn fsync_is_private_until_exact_operation_commit() {
        ...
        assert_eq!(locality.candidate_shallow_bindings, 1, "{locality:?}");
