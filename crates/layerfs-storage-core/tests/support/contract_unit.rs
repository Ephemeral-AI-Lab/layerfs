use crate::{
    referenced_objects, BaseId, BranchId, BranchRecord, CanonicalObject, CommitId, Fact, FactKind,
    LayerHistoryId, LayerId, ObjectSource, Result, StorageError, StoreEndpoint, TransferExchange,
    TransferPipeline,
};
use layerfs_core::{
...
impl StoreEndpoint for Destination {
    ...
    fn transfer_exchange_unlocked(
        ...
        for object in objects {
            ...
            admission.inserted_ids += 1;
            admission.inserted_bytes += object.bytes.len() as u64;
        }
        admission.inserted_ids += facts.len() as u64;
        admission.inserted_bytes += facts
            .iter()
            .map(|fact| fact.encoded_size() as u64)
            .sum::<u64>();
        admission.transactions =
            (crate::object_batches(objects)?.len() + crate::fact_batches(facts)?.len()) as u64;
...
fn known_descendant_prunes_before_source_read_and_turns_are_p_plus_one() {
    ...
    let stats = transfer.test_stats().unwrap();
    assert_eq!(source.reads.borrow().get(&root), Some(&1));
    assert_eq!(source.reads.borrow().get(&child), None);
    assert!(stats.wire_turns <= stats.object_pages + stats.typed_pages + 1);
}
...
fn deep_closure_drains_bounded_object_pages() {
    ...
    assert!(stats.payload_batches > 1);
    assert_eq!(stats.payload_frames, stats.sent_ids + stats.payload_batches);
    assert_eq!(stats.transactions, stats.payload_batches);
    assert!(stats.peak_buffer_bytes <= crate::OBJECT_BATCH_BYTES as u64);
    assert_eq!(stats.wire_turns, stats.object_pages + 1);
}
...
fn root_membership_is_deduplicated_in_512_id_pages() {
    ...
    let stats = transfer.test_stats().unwrap();
    assert_eq!(stats.object_pages, 3);
    assert_eq!(
        *source.pages.borrow(),
    ...
    transfer
        ...
        .unwrap();
    let stats = transfer.test_stats().unwrap();
    assert_eq!(stats.object_pages, 1);
    assert_eq!(*source.pages.borrow(), [1]);
}

#[test]
fn fact_membership_is_512_wide_while_admission_stays_128_wide() {
    ...
    let stats = transfer.test_stats().unwrap();
    assert_eq!(stats.typed_pages, 3);
    assert_eq!(stats.transactions, 9);
    assert_eq!(stats.wire_turns, 4);
}
