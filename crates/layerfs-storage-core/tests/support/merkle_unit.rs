...
fn deferred_objects_are_memory_first_and_spill_after_eight_mib() {
    ...
    small
        .stage(CanonicalObject::new(layerfs_core::encode_bytes_object(b"small").unwrap()).unwrap())
        .unwrap();
    ...
    for index in 0..9_u8 {
        ...
        large
            .stage(
                CanonicalObject::new(layerfs_core::encode_bytes_object(&bytes).unwrap()).unwrap(),
            )
            .unwrap();
    }
