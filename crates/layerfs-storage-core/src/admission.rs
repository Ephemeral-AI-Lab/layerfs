use crate::{
    fact_batches, referenced_objects, AdmissionStats, BranchRecord, CanonicalObject, CommitId,
    Fact, FactKind, MissingBitmap, ObjectSource, Result, StackId, StorageError, StorageId, StoreDb,
    StoreEndpoint, TransferExchange, TransferStats, FACT_BATCH_BYTES, FACT_BATCH_COUNT,
    ID_BATCH_COUNT, OBJECT_BATCH_BYTES, OBJECT_BATCH_COUNT,
};
use layerfs_core::{authenticate_identity, ObjectId};
...
impl crate::StoreDb {
    ...
    fn admit_objects_with_authentication(
        ...
        let mut stats = admit_object_rows(&transaction, objects)?;
        transaction.commit()?;
    ...
    pub(crate) fn admit_received_facts(&self, facts: &[Fact]) -> Result<AdmissionStats> {
        ...
        let mut connection = self.connection()?;
        let transaction = connection.transaction()?;
        let mut stats = admit_fact_rows(&transaction, self.kind(), facts)?;
        transaction.commit()?;
        stats.transactions = 1;
        Ok(stats)
    }
...
pub(crate) fn insert_commit(
    ...
    let changed = transaction.execute(
        "INSERT INTO commits(commit_id,root_id,parent_id,merge_parent_id) VALUES(?1,?2,?3,?4)
         ON CONFLICT(commit_id) DO UPDATE SET commit_id=excluded.commit_id
         WHERE root_id=excluded.root_id AND parent_id IS excluded.parent_id AND merge_parent_id IS excluded.merge_parent_id",
        params![commit.id.as_slice(), commit.root_id.as_bytes().as_slice(), commit.parent_id.map(|id| id.to_bytes().to_vec()), commit.merge_parent_id.map(|id| id.to_bytes().to_vec())],
    )?;
    if changed == 1 {
        Ok(())
    } else {
...
impl<'a> TransferPipeline<'a> {
    ...
    pub fn snapshots(
        ...
        if let Some((root, missing)) = self.preannounced.take() {
            if roots.remove(&root) {
                if missing.is_missing(0)? {
                    ...
                    crate::merkle::transfer_closure(source, object, self)?;
                }
            } else {
                self.preannounced = Some((root, missing));
            }
        ...
        for page in roots.chunks(ID_BATCH_COUNT) {
            self.stats.announced_ids += page.len() as u64;
            self.stats.object_pages += 1;
            let missing = self.exchange(page, None, true)?.1;
    ...
    pub fn announce_fact_ids(&mut self, kind: FactKind, ids: &[Vec<u8>]) -> Result<MissingBitmap> {
        self.stats.typed_pages += 1;
        self.stats.announced_ids += ids.len() as u64;
        let (_, _, missing) = self.exchange(&[], Some((kind, ids)), true)?;
    ...
    fn stage_fact_page(&mut self, facts: &[Fact], publication: bool) -> Result<()> {
        ...
        self.stats.missing_ids += facts.len() as u64;
        self.stats.missing_bytes += bytes;
        self.stats.sent_ids += facts.len() as u64;
        self.stats.sent_bytes += bytes;
        Ok(())
    }

    pub fn finish(self) -> Result<()> {
        self.finish_stats().map(drop)
    }

    fn finish_stats(self) -> Result<TransferStats> {
        let (stats, outcome) = self.finish_with(crate::TransferIntent::None)?;
    ...
    #[cfg(test)]
    pub(crate) fn test_stats(self) -> Result<TransferStats> {
        self.finish_stats()
    }
    ...
    pub fn finish_branch(
        ...
        let (_, outcome) = self.finish_with(crate::TransferIntent::Branch { branch, expected })?;
        match outcome {
    ...
    pub fn finish_stack(self, push: crate::StackPush) -> Result<crate::RefOutcome<StackId>> {
        let (_, outcome) = self.finish_with(crate::TransferIntent::Stack(push))?;
        match outcome {
    ...
    pub fn finish_layer_history(
        ...
        let (_, outcome) = self.finish_with(crate::TransferIntent::ObserveLayer(history))?;
        match outcome {
    ...
    pub(crate) fn announce_objects(&mut self, ids: &[ObjectId]) -> Result<crate::MissingBitmap> {
        self.stats.object_pages += 1;
        self.stats.announced_ids += ids.len() as u64;
        let (_, missing, _) = self.exchange(ids, None, true)?;
        missing.validate_tail(ids.len())?;
        Ok(missing)
    }
    ...
    pub(crate) fn stage_object(&mut self, object: CanonicalObject) -> Result<()> {
        ...
        self.stats.announced_bytes += bytes;
        self.stats.missing_ids += 1;
        self.stats.missing_bytes += bytes;
        self.stats.sent_ids += 1;
        self.stats.sent_bytes += bytes;
    ...
    fn exchange(
        ...
        let payload_frames = self.pending_objects.len() as u64 + 1;
        ...
        if reply_turn {
            ...
            self.stats.payload_batches += 1;
            self.stats.payload_frames += payload_frames;
        }
        let (admission, objects, facts) = reply.into_parts();
        self.stats.merge_admission(admission);
        Ok((admission, objects, facts))
    }

    fn finish_with(
        mut self,
        intent: crate::TransferIntent,
    ) -> Result<(TransferStats, crate::TransferOutcome)> {
        if self.preannounced.is_some() {
            return Err(StorageError::Integrity("unused preflight root"));
        }
        if !self.pending_objects.is_empty() {
            self.stats.payload_batches += 1;
            self.stats.payload_frames += self.pending_objects.len() as u64 + 1;
        }
        let (exchange, outcome) =
            self.target
                .finish(&self.pending_objects, &self.pending_facts, intent)?;
        self.stats.wire_turns += 1;
        self.stats.merge_admission(exchange.into_parts().0);
        self.stats.validate()?;
        Ok((self.stats, outcome))
    }
}
...
impl StoreDb {
    ...
    pub fn transfer_exchange(
        ...
        for batch in crate::merkle::object_batches(objects)? {
