...
try:
    ...
    prior=OUT.parent/'container-frontier-10-s1-r3'
    ...
    (OUT/'input-identity.json').write_text(json.dumps(dict(source=str(prior),fixture=fixture,store_sha256=hashlib.sha256((OUT/'delete-prepared/store.sqlite').read_bytes()).hexdigest(),benchmark_sha256=hashlib.sha256((OUT/'fs-benchmark-pro').read_bytes()).hexdigest()),indent=2)+'\n')
    for kind in ['delete']:
