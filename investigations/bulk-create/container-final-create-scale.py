...
OUT=ROOT/'investigations/bulk-create/evidence/container-final-create-500-s1'
...
try:
    ...
    for kind in ['create']:
      for treatment,batch in [('final-refs','1')]:
        arm=kind+'-'+treatment
        ...
        run(arm+'-run',['docker','exec','-e','LAYERFS_EXPERIMENT_FINAL_NEW_REFS='+batch,active,'fs-benchmark-pro','workspace-run','/data/sample','/unused-input','tiny-bulk-'+kind+'-500','1','performance','diagnostic-host-fuse'])
        run(arm+'-after',['docker','exec',active,'sh','-c',stats])
        run(arm+'-verify',['docker','exec',active,'fs-benchmark-pro','workspace-colocated-verify-existing','/data/sample','tiny-bulk-create-500','1'],1800)
        run(arm+'-after-verification',['docker','exec',active,'sh','-c',stats])
