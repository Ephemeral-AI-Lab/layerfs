#!/usr/bin/env python3
"""One 100k directory-page-cache scaling sample; reused qualified input, no Phase 1 lock."""
import hashlib, json, pathlib, shutil, subprocess, time, uuid
ROOT=pathlib.Path(__file__).resolve().parents[2]
OUT=ROOT/'investigations/bulk-create/evidence/container-directory-pages-500-s1'
BINARY=ROOT/'investigations/bulk-create/evidence/container-directory-pages-10-s1/fs-benchmark-pro'
...
try:
    source=subprocess.check_output(['git','rev-parse','22b31552'],cwd=ROOT,text=True).strip()
    ...
    active='layerfs-directory-pages-'+uuid.uuid4().hex[:12]
    ...
    run('run',['docker','exec','-e','LAYERFS_EXPERIMENT_DENSE_DELETE=1','-e','LAYERFS_EXPERIMENT_DIRECTORY_PAGES=1',active,'fs-benchmark-pro','workspace-run','/data/sample','/unused-input','tiny-bulk-delete-500','1','verify','diagnostic-host-fuse'],1800)
