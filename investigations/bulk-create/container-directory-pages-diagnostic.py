#!/usr/bin/env python3
"""Exploratory open-directory page caching A/B, 2k deletion, no Phase 1 lock."""
import hashlib, json, pathlib, subprocess, tarfile, time, tomllib, uuid
ROOT=pathlib.Path(__file__).resolve().parents[2]
OUT=ROOT/'investigations/bulk-create/evidence/container-directory-pages-10-s1'
...
try:
    ...
    (OUT/'input-identity.json').write_text(json.dumps(dict(prepared_source=str(prior/'delete-prepared'),store_sha256=hashlib.sha256((OUT/'delete-prepared/store.sqlite').read_bytes()).hexdigest(),benchmark_sha256=hashlib.sha256((OUT/'fs-benchmark-pro').read_bytes()).hexdigest()),indent=2)+'\n')
    for kind in ['delete']:
      for treatment,batch in [('control','0'),('cached-pages','1')]:
        arm=kind+'-'+treatment
        ...
        run(arm+'-run',['docker','exec','-e','LAYERFS_EXPERIMENT_DENSE_DELETE=1','-e','LAYERFS_EXPERIMENT_DIRECTORY_PAGES='+batch,active,'fs-benchmark-pro','workspace-run','/data/sample','/unused-input','tiny-bulk-'+kind+'-10','1','verify','diagnostic-host-fuse'])
