...
for (const packageInfo of publishablePackages) {
  ...
  try {
    await access(staleSentinel);
    throw new Error(
      `${packageInfo.manifest.name} clean build retained its stale-output sentinel`,
    );
  } catch (error) {
  ...
  const sourceFiles = (
    await filesBelow(path.join(packageInfo.directory, "src"))
  ).filter(
    (name) =>
  ...
  if (!sameFiles(actualDistFiles, expectedDistFiles)) {
    throw new Error(
      `${packageInfo.manifest.name} clean dist differs from source (unexpected: ${listDifference(actualDistFiles, expectedDistFiles).join(", ") || "none"}; missing: ${listDifference(expectedDistFiles, actualDistFiles).join(", ") || "none"})`,
    );
  }
...
`,
  ...
  console.log(
    `exports: ${publishablePackages.length} clean packages (${totalDistFiles} dist files) match source/API snapshots; stale sentinels removed; ${publishablePackages.length} tarballs (${totalPackedFiles} files) pass clean runtime/type consumers and core deep-import denials`,
  );
} finally {
