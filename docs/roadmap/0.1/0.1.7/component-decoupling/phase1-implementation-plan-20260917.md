...
| P1-7 | fuse `compare_replacements` into the split descent (comparing sink; **Equal verdict preserved bit-for-bit**) | `O((⌈L/64K⌉+R)·h)` → `O(h)` (D27 9 → strictly lower) | unchanged | identical / identical | V2 baselines; `edit_noop` pins the verdict. Risks: error-ordering moves (a bad source now fails after the split starts — pin it deliberately); the `edit.compare` child disappears from the chunked route's top-level timing tree (new pin, flagged) |
