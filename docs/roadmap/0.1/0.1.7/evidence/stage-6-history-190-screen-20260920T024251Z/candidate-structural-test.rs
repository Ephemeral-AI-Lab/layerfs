...
fn mixed_new_and_existing_counts_preserve_roots_and_expose_provider_work() {
    for interior in [false, true] {
        ...
        for batch in [1, 32] {
            ...
            .expect("mixed update");
            assert_eq!(result.counters.base_records_read, 4);
            // Captured before the candidate: identical fixtures and resources.
            // Demand filtering must reduce real provider work, not only the
            // zero-count serial counter; retained output pins each base root.
            let (baseline_pages, baseline_waves) = match (interior, batch) {
                (false, 1) => (418, 415),
                (false, 32) => (283, 278),
                (true, 1) => (813, 806),
                (true, 32) => (556, 546),
            ...
            assert!(provider.demands() < baseline_pages);
            assert!(provider.waves().len() < baseline_waves);
