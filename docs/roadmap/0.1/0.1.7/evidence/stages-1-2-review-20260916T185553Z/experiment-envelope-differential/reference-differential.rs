//! Differential corpus: the reference decoder, same inputs as the candidate run.
...
fn category(r: Result<&[u8], layerfs_content_ref::CoreError>) -> String {
    use layerfs_content_ref::CoreError::*;
    match r {
        Ok(v) => format!("ok:{}", v.len()),
        Err(ObjectLimitExceeded) => "limit".into(),
        Err(UnexpectedEof) => "eof".into(),
        Err(Unsupported) => "framing".into(),
        Err(WrongLogicalRole) => "role".into(),
        Err(TrailingBytes) => "trailing".into(),
        Err(LengthOverflow) => "overflow".into(),
