...
fn main() {
    ...
    // `cleanup_failed` reports a removal that actually failed; this client's
    // backing refuses before delegating, and the runs' own drops already removed
    // every file, which `files_bytes_after_return` above asserts.
