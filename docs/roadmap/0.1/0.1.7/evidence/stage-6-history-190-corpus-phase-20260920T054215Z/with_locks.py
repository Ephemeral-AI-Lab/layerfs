...
This is the #190 global lock protocol (the wrapper is the retained campaign's, with
this round's own record directory): `flock(LOCK_EX|LOCK_NB)` on the resolved
...
def main() -> int:
    argv = sys.argv[1:]
    label = argv[0]
    record = {"schema": "h190-corpus-phase-lock-v1", "label": label, "command": argv[1:],
