...
fn catalogue_preparation_preserves_current_rows_parameters_and_errors() {
    ...
    // Baseline expectation, archived before the prepared-statement candidate.
    ...
    assert_eq!(before_invalid, 5);
    println!("catalogue SELECT preparation events={before_invalid} for five helper queries");
}
