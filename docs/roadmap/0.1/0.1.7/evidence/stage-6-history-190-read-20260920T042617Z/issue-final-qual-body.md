...
> **Status: five improvements merged; the read-path attribution round is complete and its one treatment is retained. Qualification is the next round, and its first two items are owner decisions.** Latest product checkpoint: [`4049e28b6`](https://github.com/Ephemeral-AI-Lab/layerfs/commit/4049e28b6) with the LOC record in `3cec2f8da`, [PR #202](https://github.com/Ephemeral-AI-Lab/layerfs/pull/202). Timing results are single-sample diagnostics, not release admission. Keep this issue open.
...
**[Next-agent handoff: admission qualification for the retained-history lane](https://github.com/Ephemeral-AI-Lab/layerfs/blob/main/docs/roadmap/0.1/0.1.7/issue190-qualification-handoff.md).** It carries the diagnosis of `admission INELIGIBLE` and `g1.o3-pinned-counters = INCOMPLETE` with exact sources, names the three structural blockers in severity order, separates the two **owner decisions** from the engineering, and fixes what must not be done to make the labels move.

## Qualification status — what is still missing, and who can supply it

Neither label is a product defect, neither is caused by any retained change, and both were present in every earlier round.

| Gap | What it actually is | What closes it |
|---|---|---|
| `g1.o3-pinned-counters = INCOMPLETE` | The harness's verdict that **no frozen oracle exists** for the `history.*` rows: `tests/golden/expected.tsv` pins 217 case ids, and `src/families/history.rs:1-9` places this group deliberately outside the 217, `--smoke` and `--lane full`. Not drift, not a miss. | An owner decision that these rows carry a frozen oracle, a qualifying baseline run through the runner, an invariance argument per pinned counter, and regeneration via `shared/pin_expected.py`. |
| `admission INELIGIBLE` | A **declared stance**, not a gate — no `INELIGIBLE` record exists in any trace. The rows declare `CacheState::CreatedInSample`, and the only enforced cache contract (`shared/cold.py`) applies solely to `init_namespace`/`namespace-100000`. | An owner decision on admission mode and budget for these lanes, plus a declared and *verified* cache stance using the existing `residency`/`disk_read_bytes` primitives. |

Three blockers, in severity order: **(1)** there is no admission path for these lanes — `runner.py:136-147` grants `ADMISSION_MODE` only to `--lane full`, and the `history.*` lanes are not in `full`, so a history invocation is always sampled and `INCOMPLETE` whatever else is fixed; **(2)** the measured complete commands (38.1 s stride10, 73.1 s stride3) exceed even the 25 s declared exception ceiling; **(3)** the cache stance is a modelling question, because the workload is a same-process write-then-read replay whose Store is created in-sample, so the open question is whether `CreatedInSample` can be *verified*, not whether a cold claim can be manufactured.

Explicitly not to be done: adding the history rows to `registry::cases()`/`FROZEN_CARDINALITY`, changing `HISTORY_LANES` membership in `full` without the owner, hand-editing a pin, inventing a cold claim, moving reads into setup, shrinking a selection to fit, or promoting the 120/240 s diagnostic caps to admission rows.
...
Done in the read-path round (PR #202, ledger L42):

- [x] Attribute pack acquisition/copy, complete directory parsing, physical/value-group decoding and reconstruction, with named residuals and real read counters.
- [x] Select one justified bounded treatment from that attribution. Selected-group BLOB reads were **not** selected, for the measured reason recorded in the campaign.
- [x] One matched stride10 pair, a stride3 confirmation, separate identity-matched verification, an independent re-derivation and exact LOC.

Open, in priority order:

- [ ] **Qualification, and it starts with two owner decisions.** See the [qualification handoff](https://github.com/Ephemeral-AI-Lab/layerfs/blob/main/docs/roadmap/0.1/0.1.7/issue190-qualification-handoff.md): whether the `history.*` rows carry a frozen oracle at all, and whether admission mode plus a longer frozen budget extend to these lanes.
- [ ] Review within-operation reuse separately (an operation-scoped pooled pack cache is a real 2.1–3.9 s target but needs an append-invalidation contract). Do not move a cache hit before required validation without a precise authenticated lifetime argument.
