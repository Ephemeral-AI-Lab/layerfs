...
def main() -> int:
    ...
    if (before["competing_resource_processes"] or before["last_cpu_idle_percent"] is None
        ...
        return 3
    document = {
        "schema": "h190-pooled-scope-diagnostic-v1",
        "arm": args.arm,
        "case": args.case,
        "command": command,
        "cwd": str(repo),
        ...
        "harness_clean": git(repo, "status", "--porcelain", "--",
        ...
        "purpose": ("diagnostic: one scope change (the pooled metadata reader's lifetime) "
                    "against the retained tree, matched arms, identical instrumentation"),
