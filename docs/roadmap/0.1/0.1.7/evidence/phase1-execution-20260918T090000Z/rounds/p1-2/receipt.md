...
> `d2c6dfb7f`) under [`../../CONTRACT.md`](../../CONTRACT.md). The **before** arm is
...
| 5 | `cargo +1.85.1 test --manifest-path core/Cargo.toml --workspace --locked --no-fail-fast` | 0 (**439 passed, 0 failed**) |
...
`P1-2 | +70..+130` was the plan's estimate; the actual is **+54**
(`cas/provider.rs` 73 → 100, `cas/read.rs` 74 → 101).
`core` **18,805 → 18,859 (delta +54)**; `crates/` reference 65,417 → 65,417;
combined 84,222 → 84,276. Method: `tools/production_loc.py` over the first parent
