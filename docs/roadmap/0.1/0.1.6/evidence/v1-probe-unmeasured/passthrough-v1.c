/* v1-probe-unmeasured / FUSE_PASSTHROUGH probe (mechanism 1), VERSION 1.
 *
 * PRESERVED FIRST VERSION. Raw output: run-passthrough.log (+ .json).
 * Two measurement defects in this version are annotated in the code and fixed
 * in passthrough.c (run-passthrough2.log):
 *   (1) the descriptor baseline (fds_before) was taken AFTER opening the four
 *       backing files, so PAS-4 measured a delta of 0;
 *   (2) PAS-5a never asked the server to reply a *cached* OPEN for the first
 *       handle, so it compared two passthrough opens instead of testing the
 *       cached-then-passthrough order.
 *
 * Original header follows.
 *
 * == FUSE_PASSTHROUGH probe (mechanism 1).
 *
 * Questions:
...
static int main_experiments(void) {
    ...
    int fds_before = count_open_fds(); /* DEFECT 1 (v1): counted after the backing files were opened */
    ...
    /* ---------------- PAS-4: descriptor cost ------------------------------ */
    {
        ...
        printf("PAS-4 daemon_fds before_registration=%d after_registering_%d_backing_files=%d "
               "after_%d_passthrough_clients_opened_the_mapping=%d (one daemon fd per inode plus "
               "the kernel's per-open backing_file)\n",
               fds_before, NFILES, fds_after_reg, 1, fds_now);
        printf("RESULT PAS4 daemon_fds_per_passthrough_inode=%d client_passthrough_opens=%lu\n",
               fds_now - fds_before >= NFILES ? 1 : 0, pas_open_replies);
    }
    ...
    /* ---------------- PAS-5: cached/passthrough exclusivity --------------- */
    {
        ...
        /* DEFECT 2 (v1): cached_reply_mask was not set here, so the server
         * replied a passthrough OPEN and this compared passthrough to
         * passthrough. */
        int h_cached = open(p2, O_RDWR | O_CREAT, 0644);
