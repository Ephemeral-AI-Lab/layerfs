...
root=pathlib.Path('/Users/yifanxu/Ephemeral-AI-Lab/layerfs');out=root/'docs/roadmap/0.1/0.1.6/evidence/phase3-host-operations';name='mounted-host-attempt02';receipt=out/(name+'.json');assert not receipt.exists()
native=root/'target/debug/deps/layerfs_workspace-6b336fb7a81e951e';helper=root/'target/aarch64-unknown-linux-musl/debug/layerfs-fuse'
expected={native:'1f84e1f00c529a5796029a5fdfe644ac0faca06c4534353e84843f0993994325',helper:'29f56dcd24aad94149510d8f0474a2d9895b96ea7904465404aa39172a33e513'}
...
r={'identity':'host_runtime::tests::mounted_host_owner_preserves_sdk_mapping_handles_and_owned_commit_input','source':'../step1-publication/source-before.json','linux_source':'../step1-publication/source-before.json','binaries':{str(p.relative_to(root)):h for p,h in expected.items()},'image':image,'status':'NOT_RUN','performance':'N/A correctness component, no benchmark case','scope':'macOS Store/HostOverlay/SDK/coordinator/spool; Linux actual FUSE/helper/workload; explicit owned input supplied to construction, V1 acquisition remains OPEN'}
...
with lockpath.open('a') as lock:
 ...
 try:
  ...
  if container:
   ...
   verified=subprocess.run(['docker','inspect',container],text=True,capture_output=True);r['container_cleanup']['verified_absent']=verified.returncode!=0 and 'no such object' in verified.stderr.lower()
