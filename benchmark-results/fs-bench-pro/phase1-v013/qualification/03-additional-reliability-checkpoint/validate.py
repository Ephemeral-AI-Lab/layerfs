"""Exactly eight new03 reliability verdicts; prior373timings/52proofs are reused."""
...
SOURCE=ROOT/'benchmark/fs-bench-pro/generate-workspace-report.py';validator_revision='5fa1a39923252a38ac7ec50367971e16ab255241'
source_bytes=SOURCE.read_bytes()
if source_bytes!=subprocess.check_output(['git','show',f'{validator_revision}:benchmark/fs-bench-pro/generate-workspace-report.py'],cwd=ROOT):raise ValueError('validator differs from specified committed5fa source')
spec=importlib.util.spec_from_file_location('additional03_report',SOURCE);report=importlib.util.module_from_spec(spec);spec.loader.exec_module(report)
...
build=report.read(CAMPAIGN/'assets-03d4914e/evidence/build.json');actual_source='03d4914ee36da6d303ab268e9102519d1755a8e4'
if build.get('status')!='pass' or build['revision']!=actual_source:raise ValueError('target03 proofbuild mismatch')
...
selected=[('candidate-failure-retry','433a618b695c'),('live-execution-busy','edffc95dfa10'),('open-writer-busy','831cb3177e1f'),('admission-batch-failure-retry','8cae9a34c131'),('final-publication-failure-retry','404be5b9c3ee'),('published-presentation-failure','e11cafe84bba'),('dirty-end-discard','67bd58c6b4d9'),('dirty-net-zero','ff7ee03d68df')]
rows=[];manifest_hashes={};validations={};joins=[]
for operation,suffix in selected:
    scenario=f'workspace-{operation}-proof'
    matches=[row for row in ledger.values() if row.get('scenario_id')==scenario and row.get('seed')==1 and row.get('mode')=='verify' and row.get('source_revision')==actual_source and row.get('evidence_path','').endswith(suffix)]
    ...
    value=report.validate_attempt(outcome,{},case,build)
...
if len(rows)!=8 or len({row['environment_identity'] for row in rows})!=1 or len({row['input_identity'] for row in rows})!=1:raise ValueError('additional cohort exactcount/input/environment mismatch')
if report.custody.sha(SOURCE)!=validator_sha:raise ValueError('report changed during scoped validations')
report.custody.write_json(OUT/'incremental-full-rows.json',dict(schema='phase1-incremental-full-verification-rows-v1',rows=rows,source_revision=actual_source,report_generator_sha256=validator_sha,attempt_manifest_sha256=manifest_hashes,prior_report_sha256=prior_sha))
report.custody.write_json(OUT/'checkpoint.json',dict(schema='phase1-additional-reliability-checkpoint-v1',status='PASS',recorded_at_utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),new_full_proofs_validated=8,actual_source_revision=actual_source,validator_source_revision=validator_revision,report_generator_sha256=validator_sha,prior_report=dict(path=str(prior_path),sha256=prior_sha,published_commit=published,git_blob=expected_blob),reused_without_revalidation=dict(performance=373,full_verdicts=52),explicit_exclusions=['already qualified lease proof4264','failed short-spool proof65ca','new repaired short-spool/deferred-NoSpace proofs owned by coordinator'],attempt_manifest_sha256=manifest_hashes,validations=validations,joins=joins,scope='Eight actual03 standalone full-proof successes only. New product repair compatibility is a separate binder gate. No globalterminal claim or successor performance-cost assertion.',product_executions=0,full_report_reruns=0))
report.custody.seal(OUT)
print(json.dumps(dict(status='PASS',new_full_proofs=8,issues=0,violations=0,incremental_rows=str(OUT/'incremental-full-rows.json'),validator_sha256=validator_sha)))
