"""Exactly4 new e24 cross-file fast verdicts; all prior qualified results excluded."""
...
SHA='329fa5da5cf54d959cb5298ee0683fc95714be9bcf0a6cb9159d05885c21b932';REV='e24a3b34b943e1f0a7f5ccf7fadf80217b6f1fb0'
...
prior=r.read(C/'qualification/78-additional-reliability-checkpoint/checkpoint.json')['prior_report'];build=r.read(C/'assets-e24a3b34/evidence/build.json');ledger=r.read(C/'slots.json');assert build['revision']==REV and build['status']=='pass'
...
assert len(rows)==4 and len({x['environment_identity'] for x in rows})==1
if r.custody.sha(SOURCE)!=SHA:raise ValueError('report changed during validation')
r.custody.write_json(OUT/'pending-fast-rows.json',dict(schema='phase1-incremental-fast-verification-rows-v1',rows=rows,source_revision=REV,report_generator_sha256=SHA,attempt_manifest_sha256=seals,prior_report_sha256=prior['sha256']))
