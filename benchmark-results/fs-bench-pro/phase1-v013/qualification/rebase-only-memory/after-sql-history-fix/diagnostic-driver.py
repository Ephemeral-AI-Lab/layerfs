...
root=Path('benchmark-results/fs-bench-pro/phase1-v013/qualification/rebase-only-memory/after-sql-history-fix').resolve();root.mkdir(exist_ok=False)
...
(root/'identity.json').write_text(json.dumps({'source_sha256':hashlib.sha256(source.read_bytes()).hexdigest(),'schema_sha256':hashlib.sha256(Path('crates/layerfs-layerstack-store/src/schema.rs').read_bytes()).hexdigest(),'build_command':cmd,'runtime_deadline_seconds':180,'source_store':'workspace-dense-rewrite-500-s2-performance-d648880f3f73','memory_tools':'heap -s --noContent and vmmap -summary at after-load/after-rebase only'},indent=2)+'\n')
