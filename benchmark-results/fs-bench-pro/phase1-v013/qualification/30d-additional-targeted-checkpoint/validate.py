"""Eight newly completed targeted full proofs, each validated once."""
...
SHA='dbfabc637656e558e286a3f1fa276eb5bf96d3c16b4b7eeb4f401a101d9f47a3';REV='30d13deeec72b46ff7bc411f1ec08a46990541e1'
if hashlib.sha256(SOURCE.read_bytes()).hexdigest()!=SHA:raise ValueError('validator differs from frozen source')
spec=importlib.util.spec_from_file_location('targeted30d_report',SOURCE);r=importlib.util.module_from_spec(spec);spec.loader.exec_module(r)
prior=r.read(C/'qualification/78-additional-reliability-checkpoint/checkpoint.json')['prior_report'];build=r.read(C/'assets-30d13dee/evidence/build.json');ledger=r.read(C/'slots.json')
...
selected=[('symlink-semantics','f2906a7c0573'),('open-rename-unlink','0b40002fed62'),('metadata-chmod','c4ead7d1e513'),('metadata-mtime','5c51e08abffc'),('metadata-xattr','782772958106'),('exec-500','7f87fba267ba'),('repeat-publication','e8f414e3d9ad'),('boundaries','e7dc59a43d26')]
...
assert len(rows)==8 and len({x['environment_identity'] for x in rows})==1
...
r.custody.write_json(OUT/'checkpoint.json',dict(schema='phase1-additional-targeted-checkpoint-v1',status='PASS',recorded_at_utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),new_full_proofs_validated=8,new_reliability_proofs=7,new_cdc_boundary_proofs=1,actual_source_revision=REV,report_generator_sha256=SHA,prior_report=prior,attempt_manifest_sha256=manifest_hashes,validations=validations,joins=joins,reused_without_revalidation='All prior performance and qualified full-proof verdicts',explicit_exclusions=['Failed hardlink-alias af050e843eb8 remains failed, no passing credit','Already qualified repaired shared-path-contention278b3a754568'],scope='Eight actual30d standalone full proofs only. No aggregate terminal claim; hardlink correction remains required.',product_executions=0,full_report_reruns=0))
r.custody.seal(OUT)
print(json.dumps(dict(status='PASS',new_full_proofs=8,issues=0,violations=0,receipt=str(OUT/'incremental-full-rows.json'))))
