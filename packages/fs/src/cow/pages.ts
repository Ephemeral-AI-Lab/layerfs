...
export function overlayCowPages(
  ...
  for (const page of pages) {
    ...
    seen.add(page.index);
    if (page.bytes.byteLength > pageBytes)
      throw new RangeError("COW page exceeds persisted page size");
    const start = page.index * pageBytes;
    if (start >= logicalSize && page.bytes.byteLength)
      throw new RangeError("COW page begins beyond logical EOF");
    result.set(
      page.bytes.subarray(0, Math.min(page.bytes.byteLength, logicalSize - start)),
      start,
    );
  }
  return result;
