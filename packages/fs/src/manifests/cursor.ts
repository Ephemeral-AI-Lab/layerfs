import { decodeManifestNode, decodeManifestRoot, type ManifestChild, type ManifestEntry, type ManifestInternal, type ManifestLeaf } from "./codec.js";
...
export interface ManifestCursorEntry { readonly entry: ManifestEntry; readonly offset: number }

interface CursorFrame { readonly node: ManifestInternal; childIndex: number; childOffset: number }
...
export class ManifestSequentialCursor {
  ...
  constructor(rootBytes: Uint8Array, offset: number, reader: ManifestNodeReader, expectedRootHash?: Uint8Array, maxDepth = 8) {
    ...
    if (!Number.isSafeInteger(maxDepth) || maxDepth <= 0) throw new RangeError("maxDepth must be a positive safe integer");
    this.#reader = reader; this.#maxDepth = maxDepth;
    if (offset === root.fileSize) { this.#entryOffset = root.fileSize; return; }
    const rootNode = this.#load(root.rootNodeHash, 1);
    if (rootNode.span !== root.fileSize || rootNode.entryCount !== root.entryCount) throw new Error("manifest root totals mismatch");
  ...
  get nodesRead(): number { return this.#nodesRead; }
  get retainedNodeCount(): number { return this.#stack.length + (this.#leaf ? 1 : 0); }

  #load(hash: Uint8Array, depth: number, expected?: ManifestChild): ManifestLeaf | ManifestInternal {
    if (depth > this.#maxDepth) throw new Error("manifest depth exceeds configured maximum");
    const encoded = this.#reader.get(hash); if (!encoded) throw new Error("missing manifest node");
    const node = decodeManifestNode(encoded, hash); this.#nodesRead += 1;
    if (expected && (node.span !== expected.span || node.entryCount !== expected.entryCount)) throw new Error("manifest child totals mismatch");
    return node;
  }

  #descend(initial: ManifestLeaf | ManifestInternal, nodeOffset: number, relative: number, depth: number): void {
    let node = initial; let start = nodeOffset; let remaining = relative; let currentDepth = depth;
    while (node.kind === "internal") {
      let childOffset = start; let selected = -1;
      for (let index = 0; index < node.children.length; index += 1) {
        const child = node.children[index]!;
        if (remaining < child.span) { selected = index; break; }
        remaining -= child.span; childOffset += child.span;
      }
      if (selected < 0) throw new Error("manifest internal span does not contain requested offset");
      this.#stack.push({ node, childIndex: selected, childOffset });
      const child = node.children[selected]!;
      currentDepth += 1; node = this.#load(child.hash, currentDepth, child); start = childOffset;
    }
    let entryOffset = start;
    for (let index = 0; index < node.entries.length; index += 1) {
      const entry = node.entries[index]!;
      if (remaining < entry.length) { this.#leaf = node; this.#entryIndex = index; this.#entryOffset = entryOffset; return; }
      remaining -= entry.length; entryOffset += entry.length;
    }
    throw new Error("manifest leaf span does not contain requested offset");
  }

  #advanceLeaf(): void {
    this.#leaf = undefined; this.#entryIndex = 0;
    while (this.#stack.length > 0) {
      const frame = this.#stack.at(-1)!;
      const completed = frame.node.children[frame.childIndex]!;
      const nextIndex = frame.childIndex + 1;
      if (nextIndex >= frame.node.children.length) { this.#stack.pop(); continue; }
      frame.childIndex = nextIndex; frame.childOffset += completed.span;
...
export function lookupManifest(rootBytes: Uint8Array, offset: number, reader: ManifestNodeReader, expectedRootHash?: Uint8Array, maxDepth = 8): ManifestLookup {
  const cursor = new ManifestSequentialCursor(rootBytes, offset, reader, expectedRootHash, maxDepth);
  const selected = cursor.peek();
  return { entry: selected?.entry ?? null, entryOffset: selected?.offset ?? offset, nodesRead: cursor.nodesRead };
}
