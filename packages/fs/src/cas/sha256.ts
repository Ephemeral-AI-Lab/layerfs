import { bytesToHex, equalBytes, freezeBytes } from "../utils/bytes.js";
...
export class IncrementalSha256 {
  readonly #state = new Uint32Array([0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19]);
  ...
  update(input: Uint8Array): this {
    ...
    if (this.#bytesHashed + input.byteLength > Number.MAX_SAFE_INTEGER) throw new RangeError("SHA-256 input is too large");
...
export interface CasObject { readonly id: CasObjectId; readonly bytes: Uint8Array }
...
export function verifyCasObject(expectedDigest: Uint8Array | string, bytes: Uint8Array): void {
  const actual = sha256(bytes);
  const valid = typeof expectedDigest === "string" ? bytesToHex(actual) === expectedDigest : equalBytes(actual, expectedDigest);
