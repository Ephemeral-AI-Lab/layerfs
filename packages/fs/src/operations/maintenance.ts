...
import type { GcRunRow, OperationsStorage, StorageTransactionPorts } from "./storage-ports.js";

const COMPLETE = 4; const ABANDONED = 5;

export class MaintenanceManager implements FilesystemMaintenance {
  readonly #port: OperationsStorage; readonly #storage: StorageLimits; readonly #runtime: RuntimeLimits; readonly #clock: () => number;
  constructor(port: OperationsStorage, storage: StorageLimits, runtime: RuntimeLimits, clock: () => number) { this.#port = port; this.#storage = storage; this.#runtime = runtime; this.#clock = clock; }

  async collectGarbage(options: GarbageCollectionOptions = {}): Promise<GarbageCollectionResult> {
    if (this.#port.readOnly) throw fsError("EROFS", "collectGarbage", undefined, "garbage collection requires writable storage"); if (options.signal?.aborted) throw abortError();
    ...
    const now = this.#now(); const expiryLimit = Math.max(1, Math.min(this.#storage.maxGcBatchSize, this.#storage.maxQueryBatchSize, this.#storage.maxFinalTransactionRows - 8));
    while (this.#write((tx) => tx.staging(this.#storage).expireBatch(now, expiryLimit)) === expiryLimit) {}
    this.#write((tx) => { tx.maintenance().beginRun(runId, now); });
    let batches = 0;
    try {
      while (batches < maxBatches) {
        if (options.signal?.aborted) throw abortError(); const state = this.#read((tx) => tx.maintenance().run(runId)?.state); if (state === undefined) throw fsError("ENOENT", "collectGarbage", undefined, "collection run does not exist"); if (state === COMPLETE || state === ABANDONED) break;
        if (state === 0) this.#markBatch(runId); else this.#sweepBatch(runId, state); batches += 1;
      }
    } catch (error) { if (!(error instanceof DOMException && error.name === "AbortError")) { try { this.#write((tx) => tx.maintenance().abandonRun(runId, COMPLETE, ABANDONED)); } catch {} } throw error; }
    const row = this.#read((tx) => tx.maintenance().run(runId)); if (!row) throw fsError("ENOENT", "collectGarbage", undefined, "collection run disappeared");
  ...
  async snapshotStorage(): Promise<StorageSnapshot> {
    const row = this.#read((tx) => tx.maintenance().snapshot()); if (!row) throw new Error("ECORRUPT: usage metadata is missing");
    const physical = this.#read((tx) => { const value = tx.maintenance().physical(); return Object.freeze({ mainFileBytes: value.pageCount * value.pageSize, freelistBytes: value.freePages * value.pageSize }); });
  ...
  async verify(options: VerificationOptions = {}): Promise<VerificationResult> {
    ...
    const generation = this.#read((tx) => {
      const maintenance = tx.maintenance(); const result = maintenance.generation(); const repo = tx.content(this.#storage);
  ...
  #markBatch(runId: string): void { this.#write((tx) => { const maintenance = tx.maintenance(); const perMark = 129; const limit = Math.max(1, Math.min(this.#storage.maxGcBatchSize, Math.floor(this.#storage.maxFinalTransactionRows / perMark))); const rows = maintenance.pendingMarks(runId, limit, this.#runtime.maxQueryBatchBytes); const repo = tx.content(this.#storage); let roots = 0; let nodes = 0; let objects = 0;
  ...
  #sweepBatch(runId: string, state: number): void { this.#write((tx) => { const maintenance = tx.maintenance(); const run = maintenance.run(runId)!; const rows = maintenance.sweepCandidates(runId, state, run.high_water, this.#storage.maxGcBatchSize, this.#runtime.maxQueryBatchBytes); maintenance.applySweep(runId, state, rows, COMPLETE);
    }); }
  #read<T>(callback: (tx: StorageTransactionPorts) => T): T { return this.#port.transaction("read", { maxRows: this.#storage.maxFinalTransactionRows, maxBytes: this.#storage.maxFinalTransactionBytes }, callback); }
  #write<T>(callback: (tx: StorageTransactionPorts) => T): T { return this.#port.transaction("write", { maxRows: this.#storage.maxFinalTransactionRows, maxBytes: this.#storage.maxFinalTransactionBytes }, callback); }
