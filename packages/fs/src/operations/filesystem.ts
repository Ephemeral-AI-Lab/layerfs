...
import type { ClosureCertificate, InodeRow, NamespaceStore, OperationsStorage, ResolvedPath, StorageTransactionMode, StorageTransactionPorts } from "./storage-ports.js";
...
export class EphemeralFS implements EphemeralFilesystem {
  ...
  readonly #storagePort: OperationsStorage; readonly #clock: () => number; readonly #observer: FilesystemObserver | undefined; readonly #ownsDatabase: boolean;
  ...
  private constructor(options: OpenFilesystemOptions, capabilities: FilesystemCapabilities, storagePort: OperationsStorage) {
    this.#storagePort = storagePort; this.#clock = options.clock ?? Date.now; this.#observer = options.observer; this.#ownsDatabase = options.ownsDatabase ?? false;
    ...
    this.branches = new BranchManager(this.#storagePort, this.#filesystemLimits, this.#storageLimits, this.#runtimeLimits, this.#branchLimits, this.#clock, this.#admission, this.#cache);
    this.maintenance = new MaintenanceManager(this.#storagePort, this.#storageLimits, this.#runtimeLimits, this.#clock);
  }

  static async open(options: OpenFilesystemOptions, storagePort: OperationsStorage): Promise<EphemeralFS> {
    ...
    const metadata = storagePort.initialize({ ...(options.format?.cowPageBytes === undefined ? {} : { cowPageBytes: options.format.cowPageBytes }), now: (options.clock ?? Date.now)() });
    ...
    return new EphemeralFS(options, Object.freeze({ adapter: storagePort.capabilities, filesystem, storage, branch, runtime, format, effectiveLimits, readOnly: storagePort.readOnly }), storagePort);
  }
  ...
  readFile(path: string, options?: ReadTextOptions): Promise<Uint8Array | string> {
    return this.#operation("readFile", path, undefined, async () => {
      ...
      const bytes = this.#transaction("read", (tx) => {
        const inode = this.#requireFile(tx.namespace(this.#filesystemLimits, "readFile").resolve(canonical, true), "readFile");
        ...
        return readManifestRange(tx.content(this.#storageLimits, this.#cache), inode.manifest_hash!, 0, inode.size!);
      });
  ...
  readRange(path: string, options: ReadRangeOptions): Promise<Uint8Array> {
    return this.#operation("readRange", path, undefined, async () => {
      ...
      const canonical = canonicalizePath(path, this.#filesystemLimits, "readRange");
       return this.#transaction("read", (tx) => { const inode = this.#requireFile(tx.namespace(this.#filesystemLimits, "readRange").resolve(canonical, true), "readRange"); return readManifestRange(tx.content(this.#storageLimits, this.#cache), inode.manifest_hash!, options.offset, options.length); });
    });
  }
  ...
  readStream(path: string, options: ReadStreamOptions = {}): Promise<ReadableStream<Uint8Array>> {
    return this.#operation("readStream", path, options.signal, async () => {
      if (this.#storagePort.readOnly) throw fsError("EROFS", "readStream", path, "durable stream leases require writable storage");
      ...
      const selected = this.#transaction("read", (tx) => { const inode = this.#requireFile(tx.namespace(this.#filesystemLimits, "readStream").resolve(canonical, true), "readStream"); return { manifestHash: inode.manifest_hash!.slice(), size: inode.size! }; });
      ...
      this.#transaction("write", (tx) => {
        const expires = this.#now() + this.#storageLimits.readLeaseMs; tx.staging(this.#storageLimits).acquireReadLease(leaseId, owner, selected.manifestHash, expires);
      });
      ...
      const release = async (): Promise<void> => {
        ...
        try { this.#transaction("write", (tx) => { tx.staging(this.#storageLimits).releaseReadLease(leaseId, owner); }); } catch (error) { if (!this.#closing) throw error; }
      };
      const stream = new ReadableStream<Uint8Array>({
        ...
        pull: async (controller) => {
          ...
          const length = Math.min(this.#filesystemLimits.preferredStreamChunkBytes, end - position); const free = this.#admission.reserve(length);
           try { const bytes = this.#transaction("read", (tx) => readManifestRange(tx.content(this.#storageLimits, this.#cache), selected.manifestHash, position, length)); position += bytes.byteLength; controller.enqueue(bytes); queuedRelease = free; } catch (error) { free(); await release(); controller.error(error); }
        },
  ...
  writeFile(path: string, content: FileContent, options: WriteFileOptions = {}): Promise<void> {
    ...
    return this.#operation("writeFile", path, options.signal, async () => {
      ...
      if (options.exclusive) { const exists = this.#transaction("read", (tx) => tx.namespace(this.#filesystemLimits, "writeFile").resolveOptional(canonical, false)); if (exists) throw fsError("EEXIST", "writeFile", canonical.value, "destination exists"); }
      const prepared = await prepareContent(this.#storagePort, frozen, this.#storageLimits, this.#runtimeLimits, this.#admission, options.signal, this.#cache, this.#clock);
      try { this.#transaction("write", (tx) => {
        const ns = tx.namespace(this.#filesystemLimits, "writeFile"); const raw = ns.resolveOptional(canonical, false); let existing = raw;
  ...
  mkdir(path: string, options: MkdirOptions = {}): Promise<void> {
    return this.#operation("mkdir", path, undefined, async () => {
      ...
      this.#transaction("write", (tx) => {
        const ns = tx.namespace(this.#filesystemLimits, "mkdir"); const existing = ns.resolveOptional(canonical, false);
  ...
  readdir(path: string, options: ReaddirOptions = {}): Promise<DirectoryEntry[]> {
    return this.#operation("readdir", path, undefined, async () => {
      ...
      const canonical = canonicalizePath(path, this.#filesystemLimits, "readdir"); return this.#transaction("read", (tx) => {
        const ns = tx.namespace(this.#filesystemLimits, "readdir"); const selected = ns.resolve(canonical, true); if (selected.inode.type !== 1) throw fsError("ENOTDIR", "readdir", canonical.value, "path is not a directory");
  ...
  chmod(path: string, mode: number): Promise<void> {
    return this.#operation("chmod", path, undefined, async () => {
      ...
      this.#transaction("write", (tx) => { const ns = tx.namespace(this.#filesystemLimits, "chmod"); const selected = ns.resolve(canonical, true); if (selected.inode.mode === normalized) return; const now = Math.max(this.#now(), selected.inode.ctime_ms); const revision = ns.nextRevision(now, 1); ns.setMode(selected.inode.id, normalized, now, revision); ns.recordInode(revision, selected.inode.id); });
    });
  }
  ...
  rename(oldPath: string, newPath: string): Promise<void> {
    return this.#operation("rename", oldPath, undefined, async () => {
      ...
      this.#transaction("write", (tx) => { const ns = tx.namespace(this.#filesystemLimits, "rename"); const source = ns.resolve(sourcePath, false); const destination = ns.resolveOptional(destinationPath, false); const parent = ns.resolveParent(destinationPath);
  ...
  close(): Promise<void> {
    ...
    this.#closePromise = (async () => {
      for (const stream of this.#streams.values()) stream.error(); await Promise.allSettled([...this.#streams.values()].map((stream) => stream.release())); await Promise.allSettled([...this.#pending]);
       try { this.#cache.clear(); if (this.#ownsDatabase) await this.#storagePort.close(); } finally { this.#closed = true; }
    })(); return this.#closePromise;
  }

  async [Symbol.asyncDispose](): Promise<void> { await this.close(); }

  #stat(path: string, followFinal: boolean, syscall: string): Promise<FileStat> { return this.#operation(syscall, path, undefined, async () => { const canonical = canonicalizePath(path, this.#filesystemLimits, syscall); return this.#transaction("read", (tx) => { const selected = tx.namespace(this.#filesystemLimits, syscall).resolve(canonical, followFinal); return fileStat(selected.inode, canonical.segments.at(-1) ?? ""); }); }); }
  #readCompleteForMutation(path: string, syscall: string): { bytes: Uint8Array; token: number } { return this.#transaction("read", (tx) => { const selected = tx.namespace(this.#filesystemLimits, syscall).resolve(path, true); const inode = this.#requireFile(selected, syscall); if (inode.size! > this.#runtimeLimits.maxManagedResidentBytes) throw fsError("EFBIG", syscall, path, "file exceeds bounded mutation memory"); return { bytes: readManifestRange(tx.content(this.#storageLimits, this.#cache), inode.manifest_hash!, 0, inode.size!), token: inode.token }; }); }
  async #replaceExisting(path: string, bytes: Uint8Array, expectedToken: number, syscall: string): Promise<void> { const prepared = await prepareContent(this.#storagePort, bytes, this.#storageLimits, this.#runtimeLimits, this.#admission, undefined, this.#cache, this.#clock); try { this.#transaction("write", (tx) => { const ns = tx.namespace(this.#filesystemLimits, syscall); const selected = ns.resolve(path, true); const inode = this.#requireFile(selected, syscall); if (inode.token !== expectedToken) throw fsError("EAGAIN", syscall, path, "file changed while content was prepared"); const now = Math.max(this.#now(), inode.mtime_ms, inode.ctime_ms); this.#validatePrepared(tx, prepared.certificate, now); const revision = ns.nextRevision(now, 1); if (ns.setFileContent(inode.id, prepared.size, prepared.hash, now, now, revision, expectedToken) !== 1) throw fsError("EAGAIN", syscall, path, "file changed while content was prepared"); ns.recordInode(revision, inode.id); this.#releasePrepared(tx, prepared.certificate); }); } catch (error) { this.#abandonPrepared(prepared.certificate); throw error; } }
  ...
  #transaction<T>(mode: StorageTransactionMode, callback: (tx: StorageTransactionPorts) => T): T { return this.#storagePort.transaction(mode, { maxRows: this.#storageLimits.maxFinalTransactionRows, maxBytes: this.#storageLimits.maxFinalTransactionBytes }, callback); }
