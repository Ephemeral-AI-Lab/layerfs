  ...
  putManifestNodesBatch(
    ...
    if (insert.length) {
      ...
      if (allocationConflicts.length > 0)
        throw new Error(
          `ECORRUPT: manifest allocation sequence collision at ${sequence}..${sequence + insert.length - 1} (existing ${allocationConflicts.map((row) => row.allocation_sequence).join(",")})`,
        );
      for (let index = 0; index < insert.length; index += 1) {
