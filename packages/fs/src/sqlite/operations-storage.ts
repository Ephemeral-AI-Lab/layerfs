...
import type { OperationsStorage, StorageTransactionPorts } from "../operations/storage-ports.js";
...
export function createSqliteOperationsStorage(driver: FilesystemSQLiteDriver): OperationsStorage {
  return Object.freeze({
    readOnly: driver.readOnly,
    capabilities: driver.capabilities,
    initialize: (options = {}) => initializeOrValidateSchema(driver, options),
    transaction: <T>(mode: "read" | "write" | "exclusive", budget: { readonly maxRows: number; readonly maxBytes: number }, callback: (ports: StorageTransactionPorts) => T): T => runUnitOfWork(driver, mode, budget, (tx) => {
      const ports: StorageTransactionPorts = Object.freeze({
        content: (limits: StorageLimits, cache?: ContentCache) => new ContentRepository(tx, limits, cache),
        namespace: (limits: FilesystemLimits, syscall: string) => new NamespaceRepository(tx, limits, syscall),
        ...
        overlay: (limits: StorageLimits, pageBytes: CowPageBytes) => new OverlayRepository(tx, limits, pageBytes),
      });
