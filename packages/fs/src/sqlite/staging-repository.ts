...
interface ReconciliationRow extends SqliteRow { owner_nonce: Uint8Array; manifest_hash: Uint8Array; next_sequence: number; object_count: number; object_bytes: number; node_count: number; node_bytes: number; membership_count: number; complete: number }
interface QueueRow extends SqliteRow { kind: number; hash: Uint8Array; sequence: number; declared_size: number; declared_span: number | null; declared_entry_count: number | null; edge_cursor: number }
interface BackingRow extends SqliteRow { stored_size: number; membership_size: number; encoded?: Uint8Array }
interface LeaseChargeRow extends SqliteRow { state: number; owner_nonce: Uint8Array; staged_bytes: number }
interface ExpiredLeaseRow extends LeaseChargeRow { id: string }
interface UsageRow extends SqliteRow { object_bytes: number; manifest_root_bytes: number; manifest_node_bytes: number; page_bytes: number; patch_bytes: number; staging_bytes: number; result_bytes: number; maintenance_bytes: number }

export interface ReconciliationProgress { readonly processed: number; readonly complete: boolean }
...
export class StagingRepository {
  ...
  release(leaseId: string, ownerNonce: Uint8Array, requireSealed: boolean): boolean {
    ...
    if (result.changes) { this.#releaseStagingBytes(charge.staged_bytes); this.bumpRoot(6, leaseId); }
  ...
  delete(leaseId: string, ownerNonce: Uint8Array): boolean {
    ...
    if (result.changes) { if (charge.state === 0 || charge.state === 1) this.#releaseStagingBytes(charge.staged_bytes); this.bumpRoot(6, leaseId); }
    return result.changes === 1;
  }
  ...
  expireBatch(now: number, limit: number): number {
    counters([now, limit]);
    if (limit <= 0 || limit > this.#limits.maxGcBatchSize || limit > this.#limits.maxQueryBatchSize) throw new RangeError("invalid expired-lease batch limit");
    const rows = this.#tx.all<ExpiredLeaseRow>("SELECT l.id,l.state,l.owner_nonce,COALESCE(c.object_bytes+c.node_bytes,0) staged_bytes FROM efs_leases l LEFT JOIN efs_staging_certificates c ON c.lease_id=l.id WHERE l.expires_at_ms<? OR l.state=2 ORDER BY l.id LIMIT ?", [now, limit], { maxRows: limit, maxBytes: Math.max(1024, limit * 256) });
    let releasedBytes = 0; let deleted = 0;
    for (const row of rows) {
      const result = this.#tx.run("DELETE FROM efs_leases WHERE id=? AND (expires_at_ms<? OR state=2)", [row.id, now]);
      if (result.changes) { deleted += 1; if (row.state === 0 || row.state === 1) releasedBytes = checkedAdd(releasedBytes, row.staged_bytes); }
    }
  ...
  appendBatch(leaseId: string, ownerNonce: Uint8Array, members: readonly StagingMember[]): ClosureCertificate {
    ...
    if (members.length > this.#limits.maxQueryBatchSize) throw new RangeError("staging membership batch exceeds configured query/binding limit");
    ...
    if (this.#tx.all("SELECT lease_id FROM efs_staging_reconciliations WHERE lease_id=?", [leaseId], { maxRows: 1, maxBytes: 128 }).length) throw new Error("ECORRUPT: staged closure cannot change after reconciliation begins");
    const seen = new Set<string>(); let chain = row.chain_digest; let sequence = row.next_sequence;
    let objectCount = row.object_count; let objectBytes = row.object_bytes; let nodeCount = row.node_count; let nodeBytes = row.node_bytes; let stagedDelta = 0;
    for (const member of members) {
      ...
      if (member.kind === "object") {
        ...
        if (!inserted.changes) continue; objectCount += 1; objectBytes = checkedAdd(objectBytes, member.size);
      } else {
        ...
        if (!inserted.changes) continue; nodeCount += 1; nodeBytes = checkedAdd(nodeBytes, member.size);
      }
  ...
  beginReconciliation(leaseId: string, ownerNonce: Uint8Array, manifestHash: Uint8Array): void {
    if (manifestHash.byteLength !== 32) throw new RangeError("manifest root hash must be 32 bytes");
    const certificate = this.#row(leaseId);
    if (!equalBytes(certificate.owner_nonce, ownerNonce) || certificate.sealed !== 0) throw new Error("ECORRUPT: staging owner mismatch or certificate already sealed");
    const existing = this.#reconciliation(leaseId);
    if (existing) {
      if (!equalBytes(existing.owner_nonce, ownerNonce) || !equalBytes(existing.manifest_hash, manifestHash)) throw new Error("ECORRUPT: reconciliation identity mismatch");
      return;
    }
    this.#tx.run("INSERT INTO efs_staging_reconciliations(lease_id,owner_nonce,manifest_hash,next_sequence,object_count,object_bytes,node_count,node_bytes,membership_count,complete) VALUES(?,?,?,0,0,0,0,0,0,0)", [leaseId, ownerNonce, manifestHash]);
  ...
  reconcileBatch(leaseId: string, ownerNonce: Uint8Array, workLimit: number): ReconciliationProgress {
    if (!Number.isSafeInteger(workLimit) || workLimit <= 0 || workLimit > this.#limits.maxQueryBatchSize) throw new RangeError("invalid reconciliation work limit");
    const state = this.#reconciliation(leaseId);
    if (!state || !equalBytes(state.owner_nonce, ownerNonce)) throw new Error("ECORRUPT: reconciliation owner mismatch");
    if (state.complete === 1) return Object.freeze({ processed: 0, complete: true });
    const queue = this.#tx.all<QueueRow>("SELECT kind,hash,sequence,declared_size,declared_span,declared_entry_count,edge_cursor FROM efs_staging_reconciliation_queue WHERE lease_id=? AND processed=0 ORDER BY sequence LIMIT ?", [leaseId, workLimit], { maxRows: workLimit, maxBytes: Math.max(1024, workLimit * 320) });
    let remaining = workLimit; let processed = 0;
    const objects = queue.filter((item) => item.kind === 0);
    if (objects.length) {
      this.#tx.run("UPDATE efs_staging_reconciliation_queue SET processed=1 WHERE lease_id=? AND kind=0 AND processed=0 AND sequence>=? AND sequence<=?", [leaseId, objects[0]!.sequence, objects.at(-1)!.sequence]);
      remaining -= objects.length; processed += objects.length;
    }
    for (const item of queue) {
      if (remaining <= 0) break;
      if (item.kind === 0) continue;
      const backing = this.#manifestBacking(leaseId, item.kind, item.hash);
      if (!backing.encoded || backing.stored_size !== item.declared_size) throw new Error("ECORRUPT: reconciled manifest size changed");
      if (item.kind === 1) {
        const root = decodeManifestRoot(backing.encoded, item.hash);
        this.#enqueueVerified(leaseId, 2, root.rootNodeHash, root.fileSize, root.entryCount);
        this.#tx.run("UPDATE efs_staging_reconciliation_queue SET edge_cursor=1,processed=1 WHERE lease_id=? AND kind=1 AND hash=? AND processed=0", [leaseId, item.hash]);
        remaining -= 1; processed += 1; continue;
      }
      const node = decodeManifestNode(backing.encoded, item.hash);
      if (item.declared_span !== node.span || item.declared_entry_count !== node.entryCount) throw new Error("ECORRUPT: manifest child declaration mismatch");
      const edgeCount = node.kind === "leaf" ? node.entries.length : node.children.length;
      const end = Math.min(edgeCount, item.edge_cursor + remaining);
      if (node.kind === "leaf") {
        for (let index = item.edge_cursor; index < end; index += 1) { const edge = node.entries[index]!; this.#enqueueVerified(leaseId, 0, edge.hash, edge.length, 1); remaining -= 1; processed += 1; }
      } else {
        for (let index = item.edge_cursor; index < end; index += 1) { const edge = node.children[index]!; this.#enqueueVerified(leaseId, 2, edge.hash, edge.span, edge.entryCount); remaining -= 1; processed += 1; }
      }
      this.#tx.run("UPDATE efs_staging_reconciliation_queue SET edge_cursor=?,processed=? WHERE lease_id=? AND kind=2 AND hash=? AND processed=0", [end, end === edgeCount ? 1 : 0, leaseId, item.hash]);
    }
    const pending = this.#tx.all("SELECT sequence FROM efs_staging_reconciliation_queue WHERE lease_id=? AND processed=0 LIMIT 1", [leaseId], { maxRows: 1, maxBytes: 128 }).length !== 0;
    if (!pending) {
      const reconciled = this.#reconciliation(leaseId)!; const certificate = this.#row(leaseId);
      if (reconciled.object_count !== certificate.object_count || reconciled.object_bytes !== certificate.object_bytes || reconciled.node_count !== certificate.node_count || reconciled.node_bytes !== certificate.node_bytes || reconciled.membership_count !== certificate.membership_count || reconciled.next_sequence !== certificate.membership_count) throw new Error("ECORRUPT: complete manifest closure differs from staged membership");
      this.#tx.run("UPDATE efs_staging_reconciliations SET complete=1 WHERE lease_id=? AND complete=0", [leaseId]);
    }
  ...
  seal(certificate: ClosureCertificate): void {
    ...
    if (!reconciliation || reconciliation.complete !== 1 || !equalBytes(reconciliation.owner_nonce, certificate.ownerNonce) || !equalBytes(reconciliation.manifest_hash, certificate.manifestHash) || reconciliation.object_count !== certificate.objectCount || reconciliation.object_bytes !== certificate.objectBytes || reconciliation.node_count !== certificate.nodeCount || reconciliation.node_bytes !== certificate.nodeBytes || reconciliation.membership_count !== certificate.membershipCount || reconciliation.next_sequence !== certificate.membershipCount) throw new Error("ECORRUPT: staged closure reconciliation is incomplete or mismatched");
  ...
  #verifyMemberBacking(_leaseId: string, member: StagingMember): void {
    const row = member.kind === "object"
      ? this.#tx.all<{ stored_size: number } & SqliteRow>("SELECT size stored_size FROM efs_cas_objects WHERE hash=?", [member.hash], { maxRows: 1, maxBytes: 128 })[0]
      : member.kind === "manifest-root"
        ? this.#tx.all<{ stored_size: number } & SqliteRow>("SELECT length(encoded) stored_size FROM efs_manifest_roots WHERE hash=?", [member.hash], { maxRows: 1, maxBytes: 128 })[0]
        : this.#tx.all<{ stored_size: number } & SqliteRow>("SELECT length(encoded) stored_size FROM efs_manifest_nodes WHERE hash=?", [member.hash], { maxRows: 1, maxBytes: 128 })[0];
    if (!row || row.stored_size !== member.size) throw new Error("ECORRUPT: staged membership does not match immutable content");
  }

  #manifestBacking(leaseId: string, kind: number, hash: Uint8Array): BackingRow {
    const row = kind === 1
      ? this.#tx.all<BackingRow>("SELECT length(r.encoded) stored_size,m.size membership_size,r.encoded FROM efs_manifest_roots r JOIN efs_lease_staged_manifests m ON m.lease_id=? AND m.kind=0 AND m.manifest_hash=r.hash WHERE r.hash=?", [leaseId, hash], { maxRows: 1, maxBytes: this.#limits.maxManifestNodeBytes + 256 })[0]
      : this.#tx.all<BackingRow>("SELECT length(n.encoded) stored_size,m.size membership_size,n.encoded FROM efs_manifest_nodes n JOIN efs_lease_staged_manifests m ON m.lease_id=? AND m.kind=1 AND m.manifest_hash=n.hash WHERE n.hash=?", [leaseId, hash], { maxRows: 1, maxBytes: this.#limits.maxManifestNodeBytes + 256 })[0];
    if (!row || row.stored_size !== row.membership_size) throw new Error("ECORRUPT: manifest closure member is absent or has a mismatched size");
    return row;
  }

  #enqueueVerified(leaseId: string, kind: number, hash: Uint8Array, declaredSpan: number | undefined, declaredEntryCount: number | undefined): void {
    let size: number;
    if (kind === 0) {
      const row = this.#tx.all<BackingRow>("SELECT o.size stored_size,m.size membership_size FROM efs_cas_objects o JOIN efs_lease_objects m ON m.lease_id=? AND m.object_hash=o.hash WHERE o.hash=?", [leaseId, hash], { maxRows: 1, maxBytes: 256 })[0];
      if (!row || row.stored_size !== row.membership_size || row.stored_size !== declaredSpan) throw new Error("ECORRUPT: object closure member is absent or has a mismatched size");
      size = row.stored_size;
    } else {
      const row = this.#manifestBacking(leaseId, kind, hash); size = row.stored_size;
      if (kind === 1) decodeManifestRoot(row.encoded!, hash);
      else {
        const node = decodeManifestNode(row.encoded!, hash);
        if (node.span !== declaredSpan || node.entryCount !== declaredEntryCount) throw new Error("ECORRUPT: manifest child declaration mismatch");
      }
    }
    const state = this.#reconciliation(leaseId); if (!state) throw new Error("ECORRUPT: missing staging reconciliation");
    const inserted = this.#tx.run("INSERT OR IGNORE INTO efs_staging_reconciliation_queue(lease_id,kind,hash,sequence,declared_size,declared_span,declared_entry_count,edge_cursor,processed) VALUES(?,?,?,?,?,?,?,0,0)", [leaseId, kind, hash, state.next_sequence, size, declaredSpan ?? null, declaredEntryCount ?? null]);
    if (!inserted.changes) {
      const existing = this.#tx.all<QueueRow>("SELECT kind,hash,sequence,declared_size,declared_span,declared_entry_count,edge_cursor FROM efs_staging_reconciliation_queue WHERE lease_id=? AND kind=? AND hash=?", [leaseId, kind, hash], { maxRows: 1, maxBytes: 256 })[0];
      if (!existing || existing.declared_size !== size || existing.declared_span !== (declaredSpan ?? null) || existing.declared_entry_count !== (declaredEntryCount ?? null)) throw new Error("ECORRUPT: repeated manifest closure edge disagrees");
      return;
    }
    const object = kind === 0 ? 1 : 0; const node = kind === 0 ? 0 : 1;
    this.#tx.run("UPDATE efs_staging_reconciliations SET next_sequence=next_sequence+1,object_count=object_count+?,object_bytes=object_bytes+?,node_count=node_count+?,node_bytes=node_bytes+?,membership_count=membership_count+1 WHERE lease_id=? AND complete=0", [object, object ? size : 0, node, node ? size : 0, leaseId]);
  }

  #reconciliation(leaseId: string): ReconciliationRow | undefined {
    return this.#tx.all<ReconciliationRow>("SELECT owner_nonce,manifest_hash,next_sequence,object_count,object_bytes,node_count,node_bytes,membership_count,complete FROM efs_staging_reconciliations WHERE lease_id=?", [leaseId], { maxRows: 1, maxBytes: 512 })[0];
  }

  #leaseCharge(leaseId: string): LeaseChargeRow | undefined {
    return this.#tx.all<LeaseChargeRow>("SELECT l.state,l.owner_nonce,COALESCE(c.object_bytes+c.node_bytes,0) staged_bytes FROM efs_leases l LEFT JOIN efs_staging_certificates c ON c.lease_id=l.id WHERE l.id=?", [leaseId], { maxRows: 1, maxBytes: 256 })[0];
  }

  #admitStagingBytes(bytes: number): void {
    if (bytes === 0) return;
    const usage = this.#tx.all<UsageRow>("SELECT object_bytes,manifest_root_bytes,manifest_node_bytes,page_bytes,patch_bytes,staging_bytes,result_bytes,maintenance_bytes FROM efs_usage WHERE singleton=1", [], { maxRows: 1, maxBytes: 512 })[0];
    if (!usage) throw new Error("ECORRUPT: missing usage singleton");
    const normal = usage.object_bytes + usage.manifest_root_bytes + usage.manifest_node_bytes + usage.page_bytes + usage.patch_bytes + usage.staging_bytes + usage.result_bytes;
    if (usage.staging_bytes + bytes > this.#limits.maxStagingPayloadBytes || normal + bytes > this.#limits.maxManagedPayloadBytes - this.#limits.maintenanceReserveBytes) throw new Error("ENOSPC: staging payload quota exceeded");
    this.#tx.run("UPDATE efs_usage SET staging_bytes=staging_bytes+? WHERE singleton=1", [bytes]);
  }

  #releaseStagingBytes(bytes: number): void {
    if (bytes === 0) return;
    const result = this.#tx.run("UPDATE efs_usage SET staging_bytes=staging_bytes-? WHERE singleton=1 AND staging_bytes>=?", [bytes, bytes]);
    if (result.changes !== 1) throw new Error("ECORRUPT: staging payload usage underflow");
  }
