...
export interface ChildRow extends SqliteRow { name: string; name_sort: Uint8Array; inode_id: string; token: number; type: number }
...
export class NamespaceRepository {
  ...
  nextRevision(now: number, changeCount: number, writer = "filesystem"): number {
    ...
    this.#tx.run("INSERT INTO efs_root_journal(generation,kind,root_id) VALUES(?,0,?)", [generation, encodeUtf8(String(revision))]);
    return revision;
  }
  recordInode(revision: number, inodeId: string, tombstone = false): void { const inode = tombstone ? undefined : this.inode(inodeId); const encoded = inode ? encodeUtf8(JSON.stringify({ ...inode, manifest_hash: inode.manifest_hash ? bytesToHex(inode.manifest_hash) : null })) : null; this.#tx.run("INSERT INTO efs_inode_revisions(revision,inode_id,tombstone,encoded) VALUES(?,?,?,?) ON CONFLICT(revision,inode_id) DO UPDATE SET tombstone=excluded.tombstone,encoded=excluded.encoded", [revision, inodeId, tombstone ? 1 : 0, encoded]); this.#tx.run("DELETE FROM efs_revision_manifest_roots WHERE revision=? AND inode_id=?", [revision, inodeId]); if (inode?.manifest_hash) this.#tx.run("INSERT INTO efs_revision_manifest_roots(revision,inode_id,manifest_hash) VALUES(?,?,?)", [revision, inodeId, inode.manifest_hash]); }
  recordEntry(revision: number, parentInode: string, nameSort: Uint8Array, tombstone = false): void { const entry = tombstone ? undefined : this.entry(parentInode, nameSort); const encoded = entry ? encodeUtf8(JSON.stringify({ ...entry, name_sort: bytesToHex(entry.name_sort) })) : null; this.#tx.run("INSERT INTO efs_entry_revisions(revision,parent_inode,name_sort,tombstone,encoded) VALUES(?,?,?,?,?) ON CONFLICT(revision,parent_inode,name_sort) DO UPDATE SET tombstone=excluded.tombstone,encoded=excluded.encoded", [revision, parentInode, nameSort, tombstone ? 1 : 0, encoded]); }
  ...
  children(parentInode: string, limit: number, maxBytes: number, startAfter?: Uint8Array): readonly ChildRow[] {
    return startAfter
      ? this.#tx.all<ChildRow>("SELECT e.name,e.name_sort,e.inode_id,e.token,i.type FROM efs_entries e JOIN efs_inodes i ON i.id=e.inode_id WHERE e.parent_inode=? AND e.inode_id IS NOT NULL AND e.name_sort>? ORDER BY e.name_sort LIMIT ?", [parentInode, startAfter, limit], { maxRows: limit, maxBytes })
      : this.#tx.all<ChildRow>("SELECT e.name,e.name_sort,e.inode_id,e.token,i.type FROM efs_entries e JOIN efs_inodes i ON i.id=e.inode_id WHERE e.parent_inode=? AND e.inode_id IS NOT NULL ORDER BY e.name_sort LIMIT ?", [parentInode, limit], { maxRows: limit, maxBytes });
  }
  childCount(parentInode: string): number { return this.#tx.all<{ count: number } & SqliteRow>("SELECT count(*) count FROM efs_entries WHERE parent_inode=? AND inode_id IS NOT NULL", [parentInode], { maxRows: 1, maxBytes: 1024 })[0]?.count ?? 0; }
  linkCount(inodeId: string): number { return this.#tx.all<{ count: number } & SqliteRow>("SELECT count(*) count FROM efs_entries WHERE inode_id=?", [inodeId], { maxRows: 1, maxBytes: 1024 })[0]?.count ?? 0; }
  createInode(value: { readonly id: string; readonly type: number; readonly mode: number; readonly now: number; readonly revision: number; readonly size?: number | null; readonly manifestHash?: Uint8Array | null; readonly symlinkTarget?: string | null }): void {
    this.#tx.run("INSERT INTO efs_inodes(id,type,mode,birthtime_ms,mtime_ms,ctime_ms,nlink,size,manifest_hash,symlink_target,token) VALUES(?,?,?,?,?,?,1,?,?,?,?)", [value.id, value.type, value.mode, value.now, value.now, value.now, value.size ?? null, value.manifestHash ?? null, value.symlinkTarget ?? null, value.revision]);
  }
  ...
  setFileContent(id: string, size: number, manifestHash: Uint8Array, mtime: number, ctime: number, token: number, expectedToken?: number): number {
    const result = expectedToken === undefined
      ? this.#tx.run("UPDATE efs_inodes SET size=?,manifest_hash=?,mtime_ms=max(mtime_ms,?),ctime_ms=max(ctime_ms,?),token=? WHERE id=?", [size, manifestHash, mtime, ctime, token, id])
      : this.#tx.run("UPDATE efs_inodes SET size=?,manifest_hash=?,mtime_ms=?,ctime_ms=?,token=? WHERE id=? AND token=?", [size, manifestHash, mtime, ctime, token, id, expectedToken]);
    return result.changes;
  }
  setMode(id: string, mode: number, ctime: number, token: number): void { this.#tx.run("UPDATE efs_inodes SET mode=?,ctime_ms=max(ctime_ms,?),token=? WHERE id=?", [mode, ctime, token, id]); }
  incrementLinks(id: string, ctime: number, token: number): void { this.#tx.run("UPDATE efs_inodes SET nlink=nlink+1,ctime_ms=max(ctime_ms,?),token=? WHERE id=?", [ctime, token, id]); }
  decrementLinks(id: string, ctime: number, token: number): void { this.#tx.run("UPDATE efs_inodes SET nlink=nlink-1,ctime_ms=max(ctime_ms,?),token=? WHERE id=?", [ctime, token, id]); }
  setLinks(id: string, count: number, ctime: number, token: number): void { this.#tx.run("UPDATE efs_inodes SET nlink=?,ctime_ms=max(ctime_ms,?),token=? WHERE id=?", [count, ctime, token, id]); }
  touch(id: string, mtime: number, ctime: number, token: number): void { this.#tx.run("UPDATE efs_inodes SET mtime_ms=max(mtime_ms,?),ctime_ms=max(ctime_ms,?),token=? WHERE id=?", [mtime, ctime, token, id]); }
  deleteEntriesUnder(parentInode: string, tombstonesOnly = false): void { this.#tx.run(tombstonesOnly ? "DELETE FROM efs_entries WHERE parent_inode=? AND inode_id IS NULL" : "DELETE FROM efs_entries WHERE parent_inode=?", [parentInode]); }
  deleteInode(id: string): void { this.#tx.run("DELETE FROM efs_inodes WHERE id=?", [id]); }
  bumpRoot(kind: number, id: string): void {
    const generation = this.meta().root_mutation_generation + 1;
    this.#tx.run("UPDATE efs_meta SET root_mutation_generation=? WHERE singleton=1", [generation]);
