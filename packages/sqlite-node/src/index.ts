...
export interface OpenNodeSqliteOptions {
  readonly filename: string; readonly readOnly?: boolean; readonly create?: boolean; readonly busyTimeoutMs?: number;
  readonly durability?: "acknowledged" | "relaxed-test"; readonly cacheTargetBytes?: number; readonly mmapLimitBytes?: number;
  readonly maxPhysicalDatabaseBytes?: number; readonly maxJournalBytes?: number;
}

function binding(value: SqliteValue, capabilities: SQLiteDriverCapabilities): null | string | number | Uint8Array {
  if (typeof value === "number" && !Number.isSafeInteger(value)) throw new RangeError("SQLite numbers must be safe integers");
  if (value instanceof Uint8Array) { if (value.byteLength > capabilities.maxBlobBytes) throw new RangeError("SQLite BLOB exceeds adapter limit"); return value.slice(); }
  return value;
}

function output(value: SQLOutputValue): SqliteValue {
  if (typeof value === "bigint") { if (value < BigInt(Number.MIN_SAFE_INTEGER) || value > BigInt(Number.MAX_SAFE_INTEGER)) throw new RangeError("SQLite returned an unsafe integer"); return Number(value); }
...
function rowBytes(row: SqliteRow): number {
  let bytes = 32;
  for (const [name, value] of Object.entries(row)) bytes += name.length * 2 + (value instanceof Uint8Array ? value.byteLength : typeof value === "string" ? value.length * 2 : 8);
  return bytes;
}
...
export class NodeSQLiteDriver implements FilesystemSQLiteDriver {
  ...
  constructor(options: OpenNodeSqliteOptions) {
    ...
    if (!options.readOnly && options.create === false && options.filename !== ":memory:" && !existsSync(options.filename)) throw new Error("SQLite database does not exist and create is false");
    this.readOnly = options.readOnly ?? false;
    const cacheTargetBytes = options.cacheTargetBytes ?? 16 * 1024 * 1024; const mmapLimitBytes = options.mmapLimitBytes ?? 0;
    const maxPhysicalDatabaseBytes = options.maxPhysicalDatabaseBytes ?? 10 * 1024 ** 3; const maxJournalBytes = options.maxJournalBytes ?? 1024 ** 3;
    this.capabilities = Object.freeze({ maxBlobBytes: 64 * 1024 * 1024, maxBindings: 32_766, durability: options.durability ?? "acknowledged", journalMode: this.readOnly ? "wal" : "wal", memoryPolicy: "configured", cacheTargetBytes, mmapLimitBytes, maxPhysicalDatabaseBytes, maxJournalBytes, physicalQuotaPolicy: "driver-enforced" });
    this.#database = new DatabaseSync(options.filename, { readOnly: this.readOnly, timeout: options.busyTimeoutMs ?? 5_000, readBigInts: true, enableForeignKeyConstraints: true, enableDoubleQuotedStringLiterals: false, allowExtension: false });
    if (!this.readOnly) {
      this.#database.exec(`PRAGMA journal_mode=WAL; PRAGMA synchronous=${this.capabilities.durability === "acknowledged" ? "FULL" : "NORMAL"}; PRAGMA cache_size=-${Math.max(1, Math.floor(cacheTargetBytes / 1024))}; PRAGMA mmap_size=${mmapLimitBytes}; PRAGMA journal_size_limit=${maxJournalBytes};`);
      const pageSize = Number(this.#database.prepare("PRAGMA page_size").get()?.page_size ?? 4096);
      this.#database.exec(`PRAGMA max_page_count=${Math.max(1, Math.floor(maxPhysicalDatabaseBytes / pageSize))}`);
    }
  }
  transaction<T>(mode: TransactionMode, callback: (tx: FilesystemSQLiteTransaction) => T): T {
    ...
    const tx: FilesystemSQLiteTransaction = Object.freeze({
      ...
      all: <Row extends SqliteRow = SqliteRow>(sql: string, bindings: SqliteBindings, budget: QueryBudget): readonly Row[] => {
        ...
        for (const raw of this.#database.prepare(sql).iterate(...bindings.map((value) => binding(value, this.capabilities)))) {
          if (result.length >= budget.maxRows) throw new RangeError("SQLite result row budget exceeded");
          const normalized = Object.fromEntries(Object.entries(raw).map(([name, value]) => [name, output(value)])) as Row;
          bytes += rowBytes(normalized); if (bytes > budget.maxBytes) throw new RangeError("SQLite result byte budget exceeded"); result.push(Object.freeze(normalized));
        }
    ...
    try {
      ...
      if (result && typeof result === "object" && "then" in result) throw new TypeError("SQLite transaction callbacks must be synchronous");
