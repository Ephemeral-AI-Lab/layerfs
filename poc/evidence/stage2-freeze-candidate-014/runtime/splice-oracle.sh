...
volume="layerfs_stage2_final014_splice_store"
script_dir="$(cd "$(dirname "$0")" && pwd)"
evidence="$(cd "$script_dir/.." && pwd)/splice"
names=(
  layerfs-stage2-final014-splice-baseline
  layerfs-stage2-final014-splice-control
  layerfs-stage2-final014-splice-remount
)
...
if [[ -e "$evidence" ]]; then
  echo "evidence already exists: $evidence" >&2
  exit 1
fi
mkdir "$evidence"
cp "$script_dir/splice-request.txt" "$evidence/splice-request.txt"
...
docker run -d --name "${names[1]}" --platform linux/arm64 --init \
  --cpus 1 --memory 3g --pids-limit 512 \
  --device /dev/fuse:rwm --cap-add SYS_ADMIN --network none \
  --tmpfs /tmp:rw,nosuid,nodev,size=1g,mode=1777 \
