...
PRODUCT_SOURCE = "7e82abcd7320f6a214be336d82488ba0527b6025"
PRODUCT_TREE = "df13d88eb7e7d2471971b0c58ca6425bb81b0b03"
...
def launch_runtime(image: str, owner: str, phase: str, output: Path) -> tuple[str, str, list[str]]:
    container = f"layerfs-stage2-015-{owner}-{phase}"
    volume = f"layerfs_stage2_015_{owner.replace('-', '_')}_{phase.replace('-', '_')}_store"
...
def authoritative_phase(image: str, owner: str, evidence_root: Path, control: str) -> None:
    ...
    successful = False
    try:
        actual_image = run(["docker", "image", "inspect", image, "--format", "{{.Id}}"]).stdout.decode().strip()
        if actual_image != IMAGE_ID:
            raise RuntimeError(f"image identity mismatch: {actual_image}")
        ...
        json_new(
            output / "plan.json",
            {
                "schema": "layerfs-stage2-015-authoritative-plan-v1",
                "image": image,
                "container": container,
                "volume": volume,
                "daemon_argv": launch,
        ...
        json_new(
            output / "capture.json",
            {
                "schema": "layerfs-stage2-015-authoritative-capture-v1",
...
def in_container_scenario_cpu(output: Path, daemon_executable: str) -> None:
    ...
    for index, (name, command, prep) in enumerate(SCENARIOS, 1):
        ...
        artifact = {
            "schema": "layerfs-stage2-015-scenario-cpu-raw-v1",
            "config": {
                "reps": 1,
                "warmup": 0,
                "randomizeTargets": 0,
    ...
    json_new(
        ...
        {
            "schema": "layerfs-stage2-015-scenario-cpu-collection-v1",
...
def scenario_cpu_phase(image: str, owner: str, evidence_root: Path) -> None:
    ...
    try:
        ...
        json_new(
            output / "plan.json",
            {
                "schema": "layerfs-stage2-015-scenario-cpu-plan-v1",
                "image": image,
                "container": container,
                "volume": volume,
                "daemon_argv": launch,
...
def self_check() -> None:
    ...
    assert validate_owner("audit-015") == "audit-015"
