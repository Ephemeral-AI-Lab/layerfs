...
import { closeSync, existsSync, fsyncSync, mkdtempSync, openSync, rmSync } from "node:fs";
...
async function freshNodeVerify(dbFile, expectedHash) {
  const child = spawn(
    process.execPath,
    ["--experimental-sqlite", SCRIPT, "--verify-db", dbFile, expectedHash],
    {
      env: process.env,
  ...
  if (code !== 0) throw new Error(`fresh DB verifier exited ${code}: ${stderr}`);
  return JSON.parse(stdout);
}
...
function assertEnvelope(container) {
...
async function main() {
  ...
  const stateDir = mkdtempSync(resolve(tmpdir(), "cloudflare-local-durable-"));
  const dbFile = resolve(stateDir, "authoritative.sqlite");
  ...
  const receipt = {
    ...
    cleanup: { containerAbsent: false, stateAbsent: false },
  };
  ...
  try {
    ...
    const { stdout: cidOut } = await docker(
      ...
      "--mount",
      `type=bind,src=${stateDir},dst=/durable-state`,
      "--mount",
    ...
    const freshDb = await freshNodeVerify(dbFile, expectedHash);
    if (freshDb.pid === pulled.pid) throw new Error("DB verification reused the writer process");
    receipt.freshDbProcess = freshDb;
    ...
    receipt.cleanup = {
      ...
      stateAbsent: !readable(dbFile),
    };
    if (!receipt.cleanup.containerAbsent || !receipt.cleanup.stateAbsent) {
    ...
    try {
      ...
      // Temporary state may already be absent.
    }
    try {
    ...
    receipt.cleanup.stateAbsent = !readable(dbFile);
  }
