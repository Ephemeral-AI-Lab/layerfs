...
cargo fmt --all -- --check
cargo check -p layerfs-vfs -p layerfs-sdk --all-targets
cargo clippy -p layerfs-vfs -p layerfs-sdk --all-targets -- -D warnings
cargo test -p layerfs-vfs -- --test-threads=1
cargo test -p layerfs-sdk --all-targets -- --test-threads=1
