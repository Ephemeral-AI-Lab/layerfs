use layerfs_sdk::{CompactionDiagnostics, Diagnostics, LayerFs, RootId, VfsError};
...
fn run_workflow(base: &Path) -> Result<PocReceipt, Box<dyn std::error::Error>> {
    ...
    let mut initial_large = vec![0_u8; 3 * 1024 * 1024];
    ...
    let root_a = source.capture_quiescent()?;
    assert_one_commit(before_capture, opened.fs.diagnostics()?)?;
    ...
    let cold = opened.fs.materialize_external(root_a, &base.join("cold"))?;
    let mut warm = opened.fs.materialize_external(root_a, cold.path())?;
    ...
    managed.replace("nested/managed.bin", 4096, 4096, &vec![0xa5; 4096])?;
    let root_s3 = managed.capture()?;
    stages.push(stage_receipt("S3", root_s3, &opened.fs)?);
    let mut managed = opened.fs.materialize_managed(root_s3)?;
    managed.replace("nested/managed.bin", 8192, 0, &vec![0x3c; 8192])?;
    let root_s4 = managed.capture()?;
    stages.push(stage_receipt("S4", root_s4, &opened.fs)?);
    let mut managed = opened.fs.materialize_managed(root_s4)?;
    managed.replace("nested/managed.bin", 16_384, 4096, &[])?;
    let root_s5 = managed.capture()?;
    stages.push(stage_receipt("S5", root_s5, &opened.fs)?);
    let mut managed = opened.fs.materialize_managed(root_s5)?;
    managed.replace("nested/managed.bin", 1_048_576, 4096, &[])?;
    managed.rename("relative-link", "managed-link")?;
    let root_s6 = managed.capture()?;
    ...
    let before_capture = opened.fs.diagnostics()?;
    let root_final = external.capture_quiescent()?;
    ...
    assert_eq!(
        read_range(&historical.path().join("nested/large.bin"), 4096, 4096)?,
    ...
    diverged.replace("nested/managed.bin", 0, 1, b"D")?;
    let root_diverged = diverged.capture()?;
