...
//!
//! Relocated from `backing/metadata_index.rs` by slice 3.0 of the phase-3
//! plan: a relocation only, with no page format, algorithm or public path
//! change. The old module path stays a reexport of this one, and keeps the
//! crate-wide charged `vector` helper until stage two moves it with the page
//! store.
use crate::{
    backing::{
