...
def bootstrap(service_dir, port, client_key, server_public, content, wide):
    ...
    try:
        manifest = (route.manifest_entry(0, b'', 2, 0o755, 1700000000, 0)
                    + route.manifest_entry(0, b'data.bin', 1, 0o644, 1700000001, 1, content)
        ...
        return {'branch': snapshot['branch']['branch'].hex(), 'root': committed['root'].hex(),
                'root_serial': created['root_serial'], 'file_serial': serial,
...
def execute(args, report, started):
    ...
    try:
        ...
        report['fixture'] = bootstrap(service_dir, port, client_key, server_public, bytes.fromhex(fixture['file_root']), args.case == 'frontier')
...
def main():
    ...
    report = {'status': 'FAIL', 'mode': MODE, 'case': args.case,
              ...
              'commit_deadline_seconds': (25 if args.case == 'frontier' else 10) if MODE in ('functional-workspace-composite-commit', 'functional-workspace-resize') else (10 if MODE == 'functional-workspace-commit-staged' else None), 'performance_claim': False, 'cache_claim': None,
              'not_run': NOT_RUN}
