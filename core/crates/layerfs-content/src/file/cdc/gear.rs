//! Frozen two-byte rolling GEAR content-defined chunking.
//!
//! The profile (minimum/target/maximum chunk sizes, normalization shift, seed,
//! masks and gear table) is frozen: changing any of it changes every partition
//! and every chunk identity. Scanning is streaming and bounded - one owned
//! chunk buffer of exactly the maximum chunk size, no whole-input retention.

use std::io::Read;

use crate::error::{ContentError, ContentResult};

/// Smallest emitted chunk payload.
pub const MINIMUM_CHUNK_BYTES: usize = 8_192;
/// Boundary-search target below which the small mask applies.
pub const TARGET_CHUNK_BYTES: usize = 16_384;
/// Largest emitted chunk payload.
pub const MAXIMUM_CHUNK_BYTES: usize = 32_768;
/// Rolling-hash normalization shift.
pub const NORMALIZATION_SHIFT: u32 = 2;
/// Frozen rolling-hash seed.
...
/// Frozen profile identity: a digest of every profile parameter and gear value.
pub fn profile_id() -> [u8; 32] {
    static PROFILE_ID: std::sync::OnceLock<[u8; 32]> = std::sync::OnceLock::new();
...
pub struct CdcCounters {
    ...
    /// Chunks handed to the callback.
    pub chunks_emitted: u64,
}

/// Frozen streaming content-defined chunker.
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct FastCdc;
...
impl FastCdc {
    ...
    /// Streams `reader`, handing each bounded chunk slice to `on_chunk`.
    ///
    /// The callback runs before the scanner reuses its buffer, so the slice is
    /// valid only for the duration of the call. A callback error stops the scan
    /// and is returned unchanged.
    pub fn scan<R: Read, F: FnMut(&[u8]) -> ContentResult<()>>(
        self,
        mut reader: R,
        mut on_chunk: F,
    ) -> ContentResult<CdcCounters> {
        let mut scanner = Scanner::new();
        let mut input = [0_u8; MAXIMUM_CHUNK_BYTES];
        let mut counters = CdcCounters::default();
        loop {
            let read = reader.read(&mut input).map_err(|_| ContentError::Io)?;
            if read == 0 {
...
impl Scanner {
    ...
    fn process_pending_pair<F: FnMut(&[u8]) -> ContentResult<()>>(
        &mut self,
        first: u8,
        second: u8,
        on_chunk: &mut F,
    ...
    fn emit<F: FnMut(&[u8]) -> ContentResult<()>>(
        &mut self,
        on_chunk: &mut F,
        counters: &mut CdcCounters,
    ) -> ContentResult<()> {
