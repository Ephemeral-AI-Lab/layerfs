...
impl EditStream {
    ...
    pub fn new(base_len: u64, edits: Vec<Edit>) -> ContentResult<Self> {
        ...
        for edit in &edits {
            ...
            // Edits are applied in order and never overlap. Adjacent edits are
            // allowed and stay separate: merging them would change chunk
            // boundaries and therefore the exact result root.
    ...
    /// True when the stream carries no edit at all.
...
/// In-memory replacement source used by callers that already hold the bytes.
...
impl EditSource for Replacements {
    fn replacement_len(&self, index: usize) -> u64 {
