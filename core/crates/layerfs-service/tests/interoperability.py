...
try:
    with tempfile.TemporaryDirectory(prefix='layerfs-interop-proof-')as temp:
        ...
        print(json.dumps({'status':'PASS','reference_role':'software-provider interoperability peer only; never fallback','reference_daemon_sha256':digest,'current_service_sha256':current['binaries'][str(BIN/'layerfs-service')],'runtime_sha256':current['runtime_sha256'],'bytes_each_direction':len(payload),'root':root.hex(),'input_sha256':hashlib.sha256(payload).hexdigest()}))
finally:
