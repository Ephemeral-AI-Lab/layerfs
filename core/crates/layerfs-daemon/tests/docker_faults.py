    ...
    try:
      ...
      with tempfile.TemporaryDirectory(prefix="layerfs-issue192-faults-") as temp:
        ...
        if args.selection=="errors":
            ...
            progress={"phase":"upload","input_bytes_submitted":0};evidence["large_progress"]=progress
            def stream_input():
                try:
                    ...
                    for index in range(size//16384):
                        ...
                        expected.update(data);child.stdin.write(frame(3,1,data));progress["input_bytes_submitted"]+=len(data)
                    child.stdin.write(frame(4,1,struct.pack(">Q",size)));child.stdin.flush();progress["phase"]="finish-result"
            ...
            # The sources are held before END_INPUT. Do not open an external
            # SQLite reader during product writes: its lock can affect the
            # zero-busy-timeout transaction outcome under test.
            ...
            passed("overlapping-identical-writers-reverse-finish",{"root":a.hex(),"input_sha256":hashlib.sha256(payload).hexdigest(),"bytes":len(payload),"both_sources_held_before_end_input":True,"duplicate_locator_identities":duplicate,"B_read_before_A_finished":True,"equal_session_request_ids":2,"third_operation":"Capacity"})
            ...
            # A 2 MiB undrained result is absorbed by Docker attach buffering, so no genuine
            # end-to-end block can be built inside the 50 s command budget (a payload large
            # enough to block costs the whole budget and more). The relay hazard itself is
            # proven deterministically in layerfs-bridge/tests/pipe_deadline.rs (a full pipe
            # fails at its deadline instead of blocking). Here the end-to-end claim is
            # termination: a daemon wedged in that write cannot observe its closed input.
            time.sleep(.3)
            state=json.loads(subprocess.check_output(["docker","inspect",names[-1]],text=True))[0]["State"];stopped_on_its_own=not state["Running"]
            if stopped_on_its_own:end=time.monotonic()
            else:
                child.stdin.close();end=time.monotonic()+2
            while True:
                state=json.loads(subprocess.check_output(["docker","inspect",names[-1]],text=True))[0]["State"]
                if not state["Running"]:break
                if time.monotonic()>=end:raise TimeoutError("daemon stayed wedged after its input closed")
                time.sleep(.05)
            child.stdout.close()
            if not stopped_on_its_own:child.stdin.close()
            child.wait(timeout=2);passed("blocked-result-consumer",{"requested_deadline_ms":250,"container_exit":state["ExitCode"],"stopped_on_its_own":stopped_on_its_own,"partial_read":"provisional; no terminal success consumed","undrained_result_bytes":len(payload)})
            ...
            while True:
                ...
                time.sleep(.05)
            diagnostics.append(Diagnostics(child.diagnostics_stream,framed=True));child.wait(timeout=2)
            passed("blocked-diagnostic-consumer",{"successful_product_calls":20,"container_exit":state["ExitCode"],"daemon_client_exit":child.returncode,"sample":"reduced from the registered 200-call form for the slow selection; the same probe keeps 200 under --selection blocked-stderr"})
        elif args.selection=="blocked-stderr":
            child=spawn(telemetry="forward",drain=False)
            ...
            raise ValueError("unhandled selection: "+args.selection)
