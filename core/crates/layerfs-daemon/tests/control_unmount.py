#!/usr/bin/env python3
"""Shared actual daemon lifecycle setup; default selection is authenticated Unmount."""
import argparse
import fcntl
import hashlib
import json
...
MODE = 'functional-authenticated-unmount'
...
def begin(client, identity, workspace=b'read', incarnation=b'\x71' * 32, budget=1000, end=True, opcode=10):
    body = struct_header(budget, opcode) + route.blob(workspace) + incarnation
...
def struct_header(budget, opcode):
    import struct
...
def execute(args, report):
    ...
    keys = {name: os.urandom(32).hex() for name in dict.fromkeys(['service','daemon'] + [entry[1] for entry in CONTROL_PEERS])}
    ...
    try:
        ...
        peers = ';'.join(f'{selector},{public[key]},{expiry if key=="expiry" else int(time.time())+3600},{bits}'
            for selector, key, bits in CONTROL_PEERS)
        environment = os.environ.copy()
        ...
        if args.case == 'success':
            ...
            assert unmount(client, 2)['outcome'] == 'Unmounted'
            before = current(client, 3, mounted=False)
            result = close_clean(client, 4); assert result['outcome'] == 'Closed', result
            kind, after = status.query(client, 5)
            assert kind == 6 and after['closed'] and not after['mounted']
            assert all(after[field] == 0 for field in ('active_operations','nodes','handles','cookies')), after
            assert after['consumer_accounted_bytes'] < before['consumer_accounted_bytes']
            mount.checked(['docker','exec',name,'test','!','-e','/layerfs/workspace/read'])
            assert close_clean(client, 6)['outcome'] == 'Closed'
            report['result'] = result; report['before'] = before; report['after'] = after
        elif args.case == 'close_mounted':
            reader = held_file(name)
            result = close_clean(client, 2)
            assert result['outcome'] == 'Retained' and result['code'] == 13, result
            observation = current(client, 3)
            assert observation['handles'] == 1 and not observation['stopping']
            release(reader); reader = None
            report['retained'] = result; report['during'] = observation
            assert unmount(client, 4)['outcome'] == 'Unmounted'
            assert close_clean(client, 5)['outcome'] == 'Closed'
        elif args.case == 'close_authority':
            assert unmount(client, 2)['outcome'] == 'Unmounted'
            close(client, name); client = None
            for label, key, selector, options in [
                ('old-lifecycle-grant','old_lifecycle',2,{}), ('no-authority','none',4,{}),
                ('wrong-target','good',1,{'workspace':b'other'}),
                ('wrong-incarnation','good',1,{'incarnation':b'\x72'*32})]:
                client = controller(key, selector); result = close_clean(client, 1, **options)
                assert result == {'kind':'failure','code':3,'unknown':0,'cleanup':0}, result
                close(client, name, 1); client = controller(); current(client, 1, mounted=False); close(client, name); client = None
                report.setdefault('refusals', []).append({'case':label,'result':result})
            client = controller('close', 3); kind, refusal = status.query(client, 1)
            assert kind == 7 and refusal['code'] == 3 and not refusal['unknown'], refusal
            close(client, name, 1); client = controller('close', 3)
            refused = unmount(client, 1)
            assert refused == {'kind':'failure','code':3,'unknown':0,'cleanup':0}, refused
            close(client, name, 1); client = None
            service_client = status.controller(port, keys['daemon'], public['service'])
            result = close_clean(service_client, 1)
            assert result == {'kind':'failure','code':2,'unknown':0,'cleanup':0}, result
            service_client.stdin.close(); assert service_client.wait(timeout=6) == 1
            report['service_refusal'] = result
            client = controller('close', 3); result = close_clean(client, 1)
            assert result['outcome'] == 'Closed', result
            close(client, name); client = controller(); kind, after = status.query(client, 1)
            assert kind == 6 and after['closed'], after
            report['after'] = after
        elif args.case == 'close_cleanup_failure':
            assert unmount(client, 2)['outcome'] == 'Unmounted'
            before = current(client, 3, mounted=False)
            mount.checked(['docker','exec',name,'python3','-c',
                "from pathlib import Path; Path('/layerfs/workspace/read/owned-stray').write_bytes(b'owned-test')"])
            result = close_clean(client, 4)
            assert result['outcome'] == 'Retained' and result['code'] == 10, result
            during = current(client, 5, mounted=False, stopping=True)
            assert during['nodes'] == before['nodes'] and during['consumer_accounted_bytes'] == before['consumer_accounted_bytes']
            mount.checked(['docker','exec',name,'python3','-c',
                "from pathlib import Path; p=Path('/layerfs/workspace/read/owned-stray'); assert p.read_bytes()==b'owned-test'; p.unlink()"])
            assert close_clean(client, 6)['outcome'] == 'Closed'
            kind, after = status.query(client, 7); assert kind == 6 and after['closed'], after
            report['retained'] = result; report['during'] = during; report['after'] = after
        elif args.case == 'close_loss':
            assert unmount(client, 2)['outcome'] == 'Unmounted'
            close(client, name); client = None
            proxy = lost_result_proxy(control_port)
            client = controller(endpoint=proxy[0]); result = close_clean(client, 1)
            assert result == {'kind':'failure','code':12,'unknown':1,'cleanup':0}, result
            client.stdin.close(); assert client.wait(timeout=6) == 1; client = None
            proxy[1].join(6)
            for worker in proxy[2]: worker.join(6)
            assert not proxy[1].is_alive() and 'error' not in proxy[3] and proxy[3].get('withheld_ciphertext_bytes',0)>0, proxy[3]
            status.control_threads(name, 1)
            client = controller(); kind, after = status.query(client, 1)
...
def main():
    ...
    try:
        ...
        for path in (args.fixture,args.binaries,args.linux_daemon,args.output):space.assert_owned(path,'lifecycle proof input/output')
