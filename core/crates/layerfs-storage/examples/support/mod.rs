//! In-memory [`EditSequence`] and [`EditSource`] fixtures for the C2 examples.
//!
//! C1 ships no resident-first convenience sequence, so an example that already
//! holds its replacement bytes supplies the two trait implementations itself.
//! This is example support, not product code.
#![allow(dead_code)]
