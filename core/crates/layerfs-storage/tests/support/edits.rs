...
//! the two traits. These two types are that caller, and they keep the validation
//! the retired convenience type performed so the malformed-stream assertions in
//! `edit_pipeline` and `delta_payload` still exercise a real check.
