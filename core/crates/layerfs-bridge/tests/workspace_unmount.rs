...
fn value(outcome: WorkspaceUnmountOutcome) -> WorkspaceUnmountWire {
    WorkspaceUnmountWire {
        workspace: b"mounted".to_vec(),
        incarnation: [4; 32],
        outcome,
    }
}
fn response(outcome: WorkspaceUnmountOutcome) -> Response {
...
fn entered_outcomes_have_exact_identity_bounds_and_a_closed_retained_code_set() {
    for outcome in [
        ...
        WorkspaceUnmountOutcome::Retained(Code::Deadline),
        WorkspaceUnmountOutcome::Retained(Code::Io),
        WorkspaceUnmountOutcome::Retained(Code::Busy),
        WorkspaceUnmountOutcome::Retained(Code::Unsupported),
    ] {
        ...
        assert_eq!(
            ...
            WORKSPACE_UNMOUNT_RESULT_BYTES
                - usize::from(outcome == WorkspaceUnmountOutcome::Unmounted)
        );
        assert_eq!(
    ...
    let base = encode_response(&response(WorkspaceUnmountOutcome::Retained(Code::Io))).unwrap();
    ...
    for code in [
        ...
        let invalid = value(WorkspaceUnmountOutcome::Retained(code));
        assert!(invalid.validate().is_err());
        assert!(encode_response(&Response::WorkspaceUnmount(Box::new(invalid))).is_err());
    }
    let mut invalid = value(WorkspaceUnmountOutcome::Unmounted);
    invalid.incarnation = [0; 32];
    assert!(invalid.validate().is_err());
    invalid.incarnation = [4; 32];
    invalid.workspace = b"bad/name".to_vec();
    assert!(invalid.validate().is_err());
    let mut bytes = encode_response(&response(WorkspaceUnmountOutcome::Unmounted)).unwrap();
    *bytes.last_mut().unwrap() = 2;
    assert!(decode_response(&bytes).is_err());
    bytes = encode_response(&response(WorkspaceUnmountOutcome::Unmounted)).unwrap();
    bytes[3 + b"mounted".len()..3 + b"mounted".len() + 32].fill(0);
    assert!(decode_response(&bytes).is_err());
    let mut trailing = encode_response(&response(WorkspaceUnmountOutcome::Unmounted)).unwrap();
    trailing.push(0);
    assert_eq!(
        decode_response(&trailing).unwrap_err().code,
        Code::InvalidInput
    );
    let mut widest = value(WorkspaceUnmountOutcome::Retained(Code::Deadline));
...
fn authenticated_entered_attempts_and_pre_admission_failures_remain_distinct() {
    for expected in [
        Ok(response(WorkspaceUnmountOutcome::Unmounted)),
        Ok(response(WorkspaceUnmountOutcome::Retained(Code::Deadline))),
        Ok(response(WorkspaceUnmountOutcome::Retained(Code::Io))),
        Ok(response(WorkspaceUnmountOutcome::Retained(Code::Busy))),
        Ok(response(WorkspaceUnmountOutcome::Retained(
            Code::Unsupported,
        ))),
    ...
    for fault in [
        ...
        std::thread::scope(|threads| {
            let server = threads.spawn(|| {
                ...
                let mut result = value(WorkspaceUnmountOutcome::Unmounted);
                ...
                match fault {
                    ...
                    Fault::RetainedCode => {
                        bytes =
                            encode_response(&response(WorkspaceUnmountOutcome::Retained(Code::Io)))
                                .unwrap();
