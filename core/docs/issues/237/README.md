...
| [Service layout and ImportBatch integration](service-layout-and-import-batch.md) | Product source reorganized under `server/`, `read/`, `save/`, `save/import/batch/` | **1.110 s / 270.189 MB/s** one-shot diagnostic | 0/27,503 resident source payload pages; separate full reopened readback PASS. Public row INCOMPLETE from telemetry loss, metadata cache unqualified. Earlier matched prototype gain remains distinct. |
...
The accepted C1/C2 source changes are integrated on `main`; the complete #229
sparse-history space/readback gate remains open after both attempted arms
