...
The later [#252 cutover](../252/REPORT.md) replaced the wire and Service input
route discussed below. The analysis remains a record of its pinned source;
the deleted `edit_stream.rs` link points to that source revision.
...
| Server | [`edit_stream::read`](https://github.com/Ephemeral-AI-Lab/layerfs/blob/74ac30e28bcfaef180f405c9bc9045970f1678eb/core/crates/layerfs-server/src/service/save/edit_stream.rs) retains the raw descriptor block, parsed `edits`, and replacement `starts`, then spools replacement bytes above 64 KiB. | Several `O(R)` resident allocations remain, although the replacement spool has a fixed application buffer. |
