...
assert source_commit == '741a82cbc7a486dadaa3a7b15510879599bbafce'
image = 'sha256:ee46256a7b4b0f67d60c1f4fa2ea87b6c4945178fd16203cccaaa43c55c17668'
assert subprocess.check_output(['docker', 'image', 'inspect', image, '--format', '{{.Id}}'], text=True).strip() == image
run = secrets.randbits(128) or 1
output = Path(tempfile.mkdtemp(prefix='sdk-route-reuse-final-', dir=repo/'core/target/sdk-proof/evidence'))
