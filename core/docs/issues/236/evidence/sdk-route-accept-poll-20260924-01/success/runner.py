...
assert source_commit == 'af04f21ae8aef3193f38691065005b85a8338827'
image = 'sha256:bda2f50ebda7700b064ee10f014969f158192568f95d488ae7629c2fce63d1f5'
assert subprocess.check_output(['docker', 'image', 'inspect', image, '--format', '{{.Id}}'], text=True).strip() == image
run = secrets.randbits(128) or 1
output = Path(tempfile.mkdtemp(prefix='sdk-route-accept-poll-exec-', dir=repo/'core/target/sdk-proof/evidence'))
command = ['cargo', '+1.85.1', 'test', '--manifest-path', 'core/Cargo.toml', '--locked', '-p', 'layerfs-sdk', '--test', 'agent_route', '--', '--nocapture']
test_binary = repo/'core/target/debug/deps/agent_route-0d5b02b8877e22c0'
