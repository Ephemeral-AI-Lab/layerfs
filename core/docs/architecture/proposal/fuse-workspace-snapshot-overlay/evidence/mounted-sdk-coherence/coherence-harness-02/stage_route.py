...
def main():
    ...
    report = {'status': 'FAIL', 'mode': MODE, 'case': args.case,
              ...
              'commit_deadline_seconds': (25 if args.case == 'frontier' else 10) if MODE in ('functional-workspace-composite-commit', 'functional-workspace-resize', 'functional-workspace-write') else (10 if MODE in ('functional-workspace-commit-staged', 'functional-workspace-maintenance', 'functional-mounted-sdk-coherence') else None), 'performance_claim': False, 'cache_claim': None,
              'not_run': NOT_RUN}
