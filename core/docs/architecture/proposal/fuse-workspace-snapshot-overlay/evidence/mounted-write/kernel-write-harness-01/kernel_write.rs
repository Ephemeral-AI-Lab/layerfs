...
mod linux {
    ...
    fn kernel_write_origin() {
        ...
        println!(
            ...
            std::mem::size_of::<ProjectionWritePermit>(),
            f.workspace.status().unwrap().accounted_bytes - before_mount
        );
        assert!(matches!(
            f.workspace.begin_projection_write(deadline()),
        ...
        drop(observe);
        let before = f.workspace.status().unwrap().revision;
        let mut permit = f.workspace.begin_projection_write(deadline()).unwrap();
        assert!(matches!(
        ...
        assert!(matches!(
            f.workspace
                .begin_projection_write(Instant::now() - Duration::from_secs(1)),
