...
mod linux {
    ...
    fn mount_failure_admission() {
        ...
        let foreign = f.host.attach(options, deadline()).unwrap();
        let failure = failed(layerfs_fuse::mount(&foreign, deadline()));
        assert_eq!(failure.phase, MountPhase::Admission);
        assert!(matches!(&failure.cause, MountError::Unsupported));
        assert!(failure.retained.is_none());
        for workspace in [&f.workspace, &ro, &foreign] {
