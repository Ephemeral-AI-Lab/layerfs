...
with tempfile.TemporaryDirectory(prefix='layerfs192-native-image-')as directory:
 ...
 try:
  command=['docker','run','--name',name,'--label=io.layerfs.task=issue192','--log-driver=none','--cpus=1','--memory=32m','--memory-swap=32m','--pids-limit=16','--cap-drop=ALL',image,'--nocapture'];subprocess.run(command,check=True,timeout=10)
  item=json.loads(subprocess.check_output(['docker','inspect','--size',name],text=True))[0]
  record={'status':'PASS','image':image,'test_binary_sha256':sha,'command':command,'scope':'external Linux native telemetry tests, including output lifetime and retention faults; optional local configuration','container_id':item['Id'],'mounts':item['Mounts'],'labels':item['Config']['Labels'],'memory_limit':item['HostConfig']['Memory'],'exit':item['State']['ExitCode'],'logical_writable_layer_bytes':item.get('SizeRw'),'diff':subprocess.check_output(['docker','diff',name],text=True),'phase_memory_peak':'unavailable; not claimed'}
  assert not item['Mounts'];(proof/'review-linux-native-identities-20260920.json').write_text(json.dumps(record,indent=2)+'\n');print(json.dumps(record))
